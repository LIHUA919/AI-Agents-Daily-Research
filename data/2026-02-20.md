...
TL;DR: Proposes Macro-Micro Cross-Attention Transformer (MMCAformer) using CV data to integrate micro driving behavior features with macro traffic features for improved speed prediction and uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Accurate speed prediction is vital for proactive traffic management. Existing studies rely on aggregated macro data, but traffic dynamics are also influenced by individual micro driving behaviors. Recent Connected Vehicle data provide opportunities to incorporate these behavioral insights.

Method: Develops MMCAformer: uses self-attention for macro traffic dependencies, cross-attention for spatiotemporal interplay between macro traffic status and micro driving behavior, optimized with Student-t negative log-likelihood loss for point-wise prediction and uncertainty estimation.

Result: Experiments on four Florida freeways show superior performance. Introducing micro features improves accuracy (RMSE, MAE, MAPE reduced by 9.0%, 6.9%, 10.2%) and shrinks uncertainty (predictive intervals decreased by 10.1-24.0%). Hard braking and acceleration frequencies are most influential, especially under congested conditions.

Conclusion: MMCAformer effectively integrates macro and micro features for enhanced speed prediction, reducing errors and uncertainty, with greater benefits in low-speed traffic, highlighting the importance of driving behavior in traffic modeling.
...
TL;DR: This paper proposes a few-shot LLM framework for predicting electricity price spikes using system state features as natural-language prompts, achieving competitive performance with supervised models in data-scarce settings.


<details>
  <summary>Details</summary>
Motivation: There's a need for effective prediction of electricity price spikes, especially in scenarios with limited historical data, where traditional supervised machine learning models may underperform due to data scarcity.

Method: The framework aggregates system state information (demand, renewable generation, weather forecasts, recent prices) into statistical features, formats them as natural-language prompts, and feeds them along with instructions to a Large Language Model for classification with confidence scores.

Result: Using Texas electricity market data, the few-shot LLM approach shows performance comparable to SVM and XGBoost, and outperforms them when historical data is limited, demonstrating its efficacy.

Conclusion: LLMs offer potential as a data-efficient tool for classifying electricity price spikes in data-scarce environments, bridging gaps where traditional models falter.

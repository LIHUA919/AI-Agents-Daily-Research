...
Method: Integrates bearing vibration frequency analysis with multi-agent generation, serializing data into natural language for LLM processing and leveraging semantic search for procedural knowledge.
...
Method: GeoFlow provides detailed tool-calling objectives to agents for runtime geospatial API invocation.
...
<details>
  <summary>Details</summary>
Motivation: To address the data dependency of Q&A benchmarks and evaluate LLMs' strategic reasoning and adaptability in adversarial environments.

Method: Proposes Qi Town, a platform with 5 board games and 20 LLM players, using Elo ratings, PLG, and PSS for quantitative and qualitative evaluation in a round-robin tournament.

Result: LLMs demonstrate optimism and adaptability in adversarial settings but show instability in skill play, as revealed by PLG analysis.

Conclusion: The framework effectively evaluates LLMs' adversarial performance, highlighting their adaptability and areas needing further exploration, such as skill instability.
...
<details>
  <summary>Details</summary>
Motivation: Current cloud-based solutions for AWebGIS raise privacy and scalability concerns due to centralized processing and internet dependency.
...
Conclusion: Client-side SLMs are feasible for AWebGIS, offering accuracy, privacy, and scalability advantages over cloud-based methods.
...
<details>
  <summary>Details</summary>
Motivation: Static AI agents in healthcare lack strategic planning skills, limiting their efficacy in complex tasks.
...
Result: HealthFlow significantly outperforms state-of-the-art AI frameworks in complex healthcare tasks.

Conclusion: This work advances AI from tool-users to self-evolving task-managers, enhancing autonomy and effectiveness in scientific discovery.
...
Method: Fine-tuned three LLMs and an LVLM using RL (policy-gradient algorithm) and tested on eight datasets across three non-ideal scenarios.

Result: RL fine-tuning boosts performance in ideal settings but fails in non-ideal scenarios, exposing reasoning deficits.

Conclusion: Current methods overstate reasoning abilities; non-ideal scenario evaluation is crucial for realistic assessment.
...
<details>
  <summary>Details</summary>
Motivation: Web navigation is complex for AGI, requiring integration of offline and online learning paradigms.

Method: Derives a dual-process framework (System 1 for fast, intuitive actions; System 2 for slow, deliberate planning) and implements it in CogniWeb.

Result: Achieves 43.96% success rate on WebArena with 75% reduction in token usage.

Conclusion: CogniWeb bridges offline and online learning, offering efficient and effective web navigation.
...
<details>
  <summary>Details</summary>
Motivation: To explore how generative AI can reshape knowledge generation and validation in higher education, focusing on metacognitive engagement and avoiding de-skilling.

Method: A controlled experiment with 65 pre-service teachers in Germany, comparing a Socratic AI Tutor to an uninstructed chatbot.

Result: Students using the Socratic Tutor reported greater support for critical, independent, and reflective thinking.

Conclusion: The study provides empirical evidence and a conceptual roadmap for hybrid learning ecosystems integrating human-AI co-agency and pedagogical alignment.
...
Result: Achieves ~10% better binding mode prediction than state-of-the-art methods.
...
<details>
  <summary>Details</summary>
Motivation: Traditional rule-based and machine learning methods for spatial data integration are limited. LLMs offer a potential solution but need evaluation.

Method: Analyzed LLMs' spatial reasoning, tested their performance with relevant features, and adapted a review-and-refine method.
...
<details>
  <summary>Details</summary>
Motivation: Medical AI needs efficient updates for evolving knowledge without full retraining, but lacks benchmarks for multimodal knowledge editing.
...
Result: Experiments reveal limitations in current editing approaches, emphasizing the need for specialized strategies in medicine.
...
Method: Combines finetuned Qwen3-8B with heuristic search (DE and PSO) and dynamic loss functions based on Ease of Attainability (EOA).
...
<details>
  <summary>Details</summary>
Motivation: Event logs often have missing data due to manual recording or collection issues, and existing methods either rely on process models or simpler ML/DL models, which may not fully capture complex trace structures.

Method: Develops an HGNN model to handle incomplete events, leveraging graph-based representations for richer encodings, and evaluates it against autoencoder-based approaches on synthetic and real logs.

Result: The HGNN model shows superior performance in reconstructing all event attributes compared to existing model-free methods, which often focus only on subsets of attributes.

Conclusion: HGNNs offer a more effective solution for trace reconstruction in Process Mining, addressing limitations of current approaches by utilizing graph-based learning.
...
<details>
  <summary>Details</summary>
Motivation: Existing RAG methods retrieve from text or images separately, limiting their ability to handle complex queries requiring multi-hop reasoning or up-to-date knowledge.
...
Conclusion: QA-Dragon effectively addresses complex VQA tasks by enabling multimodal, multi-turn, and multi-hop reasoning, significantly improving performance.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods fail to address challenges like information overload and slow response times in complex organizational communication.
...
Conclusion: The proposed framework enhances trust and efficiency in organizational communication.
...
<details>
  <summary>Details</summary>
Motivation: To integrate symbolic reasoning (decision trees) with generative LLMs for robust, interpretable, and extensible neuro-symbolic reasoning.

Method: Embeds decision trees and random forests as callable oracles in a multi-agent framework, with LLMs handling abductive reasoning and planning.

Result: Achieves +7.2% entailment consistency on ProofWriter, +5.3% accuracy on GSM8k, and +6.0% abstraction accuracy on ARC.

Conclusion: The architecture provides a versatile solution for general-purpose neuro-symbolic reasoning, validated by strong benchmark performance and practical applications.
...
Method: Historical analysis and contemporary usage patterns inform a framework defining agent requirements and characterizing systems along multiple dimensions.

Result: A proposed framework provides precise vocabulary for describing AI agents while preserving their multifaceted nature.
...
<details>
  <summary>Details</summary>
Motivation: To empirically understand LLM behavior in open-ended, multi-agent settings involving legal and ethical deliberation.

Method: NomicLaw, a structured simulation where LLMs collaborate on law-making, propose rules, justify them, and vote on peer proposals. Trust and reciprocity are measured via voting patterns, and strategic language use is qualitatively assessed.

Result: Experiments show LLMs form alliances, betray trust, and adapt rhetoric to influence decisions. Results highlight latent social reasoning and persuasive capabilities in ten open-source LLMs.

Conclusion: The study provides insights for designing AI systems capable of autonomous negotiation, coordination, and legal drafting.
...
<details>
  <summary>Details</summary>
Motivation: Existing models struggle with partial correctness in multi-question tasks due to coarse binary rewards.

Method: StructVRM uses a model-based verifier for fine-grained feedback, assessing semantic and mathematical equivalence.

Result: Achieves state-of-the-art on 6/12 benchmarks and a new STEM-Bench.

Conclusion: Structured, verifiable rewards effectively enhance multimodal reasoning in complex domains.
...
<details>
  <summary>Details</summary>
Motivation: To enable proactive maintenance in railway systems by predicting faults in real-time with explainable outcomes, improving safety and reducing costs.

Method: An online processing pipeline with sample pre-processing (statistical/frequency features), incremental ML classification, and explainability modules.

Result: Achieves 98% F-measure and 99% accuracy, robust to class imbalance and noise, with effective explanations.

Conclusion: The pipeline is methodologically sound and practically applicable for real-world railway maintenance, enabling swift, informed decisions.
...
Method: DeepPHY uses simulated environments with varying difficulty levels and fine-grained metrics to assess VLMs' physical reasoning.
...
<details>
  <summary>Details</summary>
Motivation: To explore how LLMs are revolutionizing organic synthesis by automating tasks and enhancing efficiency, while addressing limitations and ensuring human oversight.

Method: Coupling LLMs with graph neural networks, quantum calculations, and real-time spectroscopy to optimize synthesis and discovery cycles.

Result: LLMs enable faster, greener, and data-driven chemistry, though issues like biased datasets and opaque reasoning persist.

Conclusion: Community initiatives and technological integration can democratize AI-driven molecular innovation while maintaining human control.
...
<details>
  <summary>Details</summary>
Motivation: To address the underexplored issue of geographic variability in AI alignment, ensuring AI systems respect regional norms and realities.

Method: Reviews key geographic research problems, suggests future topics, and outlines methods for assessing alignment sensitivity.

Result: Identifies the challenge of globally inconsistent AI outputs and the need for spatio-temporally aware alignment.

Conclusion: Urges context-sensitive AI alignment to manage geographic variability effectively.
...
<details>
  <summary>Details</summary>
Motivation: The need to address the gap between current AI benchmarks and regulatory requirements under the EU AI Act, focusing on systemic risks.
...
Conclusion: Highlights the urgent need for policymakers and developers to refine evaluation tools to address systemic risks and ensure safer AI.
...
<details>
  <summary>Details</summary>
Motivation: ERC datasets are scarce and biased, and LLMs are expensive for ERC tasks. The goal is to create diverse, synthetic datasets to enhance ERC classification.

Method: A small, general-purpose LLM is used to synthesize six novel ERC datasets, supplementing three benchmarks. These datasets are evaluated for classification and label imbalance effects.

Result: Generated datasets improve ERC classifier robustness and performance, achieving significant gains on existing benchmarks.

Conclusion: Synthetic datasets from a resource-efficient LLM effectively address ERC data scarcity and bias, enhancing model performance.
...
<details>
  <summary>Details</summary>
Motivation: Current post-training methods for LLMs are resource-intensive and lack scalability due to heuristic data curation.

Method: InfiAlign integrates SFT and DPO with a data selection pipeline using multidimensional quality metrics.

Result: Achieves performance parity with 12% of training data, 3.89% improvement on AIME benchmarks, and strong generalization.

Conclusion: Combining principled data selection with post-training offers a scalable, data-efficient solution for aligning LLMs.
...
Method: GRAIL uses LLM-guided exploration, path filtering, and a two-stage training process to optimize graph retrieval.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation methods for foundation models as agents overlook step-by-step reasoning and are limited to narrow domains, necessitating a more generalizable framework.
...
Result: The framework achieves 4.76% and 10.52% higher alignment accuracy with human evaluations on GAIA and BigCodeBench benchmarks compared to GPT-4o-based LLM-as-a-Judge.

Conclusion: The proposed framework demonstrates potential as a general-purpose evaluation method for agent task completion.
...
<details>
  <summary>Details</summary>
Motivation: LORs are time-consuming to review manually, and leadership skills are increasingly important in STEM admissions.

Method: Uses natural language processing with RoBERTa and LLAMA to detect leadership attributes like teamwork and communication.

Result: Achieves a weighted F1 score of 91.6%, precision of 92.4%, and recall of 91.6%.

Conclusion: LORI streamlines admissions and ensures comprehensive evaluation of leadership skills in applicants.
...
<details>
  <summary>Details</summary>
Motivation: Social media's multimodal nature makes identifying harmful intent (e.g., sarcasm, hate speech) challenging due to cross-modal contradictions and cultural shifts.

Method: MV-Debate uses four debate agents (surface analyst, deep reasoner, modality contrast, social contextualist) with dynamic reflection gating for iterative refinement.

Result: Outperforms single-model and multi-agent baselines on three benchmark datasets.

Conclusion: Multi-agent debate shows promise for reliable harmful intent detection in safety-critical online contexts.
...
Result: AIF offers a unified Bayesian objective for balancing exploration and exploitation, enabling efficient learning from self-generated data.

Conclusion: AIF combined with generative models provides a scalable path to autonomous AI aligned with human values and constraints.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods fail to capture learning dynamics or provide explainability. LearnerAgent addresses this by simulating realistic teaching environments with diverse learner profiles.

Method: Uses a multi-agent framework (LearnerAgent) with LLMs, incorporating weekly knowledge acquisition, monthly strategic choices, tests, and peer interaction to track learning progress over a year.

Result: 1) Deep Learner shows sustained cognitive growth; Surface Learner's shallow knowledge is diagnosed. 2) Behavior aligns with psychological profiles. 3) Self-concept evolves realistically. 4) Base LLM defaults to a brittle Surface Learner.

Conclusion: LearnerAgent effectively simulates human learning, providing insights into LLM behavior and cognitive processes, aligning with real-world scenarios.
...
Conclusion: NAEx enhances interpretability and trust in network alignment models through its plug-and-play, inductive framework.
...
Result: LumiGen outperforms baselines on the LongBench-T2I Benchmark with a score of 3.08, excelling in text rendering and pose expression.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the fragmented and limited tools for simulating missing data, which often overlook heterogeneous tabular data.

Method: Developed MissMecha, an open-source toolkit with visual diagnostics, MCAR testing, and type-aware imputation metrics.

Result: A unified platform for simulating and evaluating missing data, supporting mixed-type datasets.

Conclusion: MissMecha aids data quality research, benchmarking, and education by providing a comprehensive tool for handling incomplete data.
...
<details>
  <summary>Details</summary>
Motivation: The intensive computational demands of diffusion models for inference on edge devices and privacy risks in cloud-based solutions motivate the need for an efficient, scalable, and privacy-preserving framework for personalized content generation.

Method: The framework uses Low-Rank Adaptation (LoRA) for parameter-efficient local fine-tuning, clusters clients by task similarity, performs intra-cluster aggregation for personalization, and enables inter-cluster knowledge interaction for hybrid content generation. Federated learning is employed to train personalized and global models.
...
Conclusion: The proposed framework effectively balances efficiency, personalization, and privacy in edge-AIGC scenarios, demonstrating its potential for scalable deployment.
...
<details>
  <summary>Details</summary>
Motivation: FSL is critical in fields with limited data (e.g., biomedical, environmental), but existing methods lack generalization. The study aims to improve FSL using multi-modal models.

Method: Developed M3FD, a diverse few-shot dataset, and M3F, a modular LMMM framework. Fine-tuned M3F on M3FD to enhance performance.

Result: M3F outperforms conventional meta-learning methods, demonstrating improved generalization for FSL tasks.

Conclusion: The M3F framework and M3FD dataset provide a scalable, unified solution for FSL in data-scarce scientific domains, lowering deployment barriers.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM-based methods for molecular property prediction rely on human-crafted prompts and verbose reasoning, lacking relevance.
...
Conclusion: AttriLens-Mol effectively extracts relevant molecular attributes, enhancing interpretability and prediction performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods overlook textual perturbations in multimodal time series data, leading to degraded performance under noise.

Method: PA-RNet uses a perturbation-aware projection module and cross-modal attention to filter noise while preserving meaningful textual representations.

Result: PA-RNet outperforms state-of-the-art baselines across diverse domains and temporal settings, with theoretical guarantees of stability.
...
Method: InfoQ quantizes layers at varying bit-widths, measures mutual information changes in subsequent layers via a single forward pass, and solves bit-width allocation as an integer linear programming problem.

Result: Achieves up to 1% accuracy improvement for MobileNetV2 and ResNet18 on ImageNet at high compression rates (14X and 10.66X), with significantly less data usage.

Conclusion: InfoQ provides a superior, efficient, and accurate solution for MPQ by leveraging global information flow sensitivity, outperforming state-of-the-art methods.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate LLMs for dynamic insulin dosing in Type 1 diabetes, comparing their zero-shot performance against trained RL agents and identifying limitations.

Method: Open-source LLMs were tested in a diabetes simulator using zero-shot prompts, comparing results with trained small neural network RL agents.

Result: Smaller LLMs achieved comparable or better performance than RL agents in stable cases but showed issues like aggressive dosing and reasoning errors.

Conclusion: LLMs can integrate into clinical workflows but require prompt engineering, validation, and hybrid approaches for safe and effective use.
...
Method: EPOPR combines Equity-Conformalized Quantile Regression for uncertainty-aware prediction and Spatial-Temporal Attentional RL for equitable decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Existing federated recommendation methods struggle with non-stationary data, while continual learning methods assume centralized data access, creating a gap for privacy-preserving, adaptive solutions.

Method: Proposes F3CRec, featuring Adaptive Replay Memory (client-side) for selective retention of past preferences and Item-wise Temporal Mean (server-side) for integrating new knowledge while preserving prior information.

Result: F3CRec outperforms existing methods in maintaining recommendation quality over time in federated settings.

Conclusion: F3CRec effectively bridges the gap between federated and continual learning, offering a robust solution for privacy-preserving, adaptive recommendations.
...
<details>
  <summary>Details</summary>
Motivation: Existing autoregressive models for event sequences suffer from inefficiency and error accumulation in long-range predictions.

Method: Proposes a unified flow matching framework using continuous and discrete flow matching to jointly model inter-event times and event types.

Result: Outperforms autoregressive and diffusion-based baselines in accuracy and efficiency on six real-world benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: The CAN protocol lacks built-in security, making it vulnerable to cyber-attacks, necessitating advanced intrusion detection methods.

Method: Uses Variational Graph Autoencoder (VGAE) for anomaly detection and Knowledge-Distilled Graph Attention Network (KD-GAT) for attack classification, encoding CAN traffic as graph sequences.

Result: Achieves 96% parameter reduction and 16.2% average F1-score improvement, excelling on imbalanced datasets (up to 55% F1-score gains).

Conclusion: The framework enhances CAN security with efficient, high-performance intrusion detection, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Despite OPTQ's widespread use in post-training quantization for deep neural networks, it lacks rigorous theoretical guarantees. This work aims to fill that gap.

Method: The authors analyze OPTQ's iterative procedure, derive non-asymptotic error bounds, and extend the analysis to Qronos, including deterministic and stochastic variants.

Result: The paper presents explicit error bounds for OPTQ and Qronos, justifying heuristic choices like feature ordering and regularization parameter selection.

Conclusion: The theoretical analysis enhances understanding of OPTQ and Qronos, providing insights for practical applications and future improvements.
...
<details>
  <summary>Details</summary>
Motivation: Low-resource programming languages lack sufficient pre-training data and require extensive per-language engineering for post-training, hindering LLM performance.

Method: Agnostics rewrites unit tests into I/O format, uses a configurable verifier for compilation/execution, and applies RLVR in a robust execution environment.

Result: Improves Qwen-3 4B to rival larger models, scales to diverse model families, and achieves SOTA pass@1 on MultiPL-E and LiveCodeBench.

Conclusion: Agnostics simplifies RL post-training for any language via reusable datasets and configurations, democratizing access to low-resource language support.
...
<details>
  <summary>Details</summary>
Motivation: Current neural operators like FNO rely on Fourier transforms, which assume periodicity and lack phase-sensitive features. HNO aims to overcome these limitations by incorporating signal processing insights.

Method: HNO maps input signals to their analytic representation via the Hilbert transform, making phase and amplitude explicit. It then applies spectral convolutions to this representation.

Result: HNO is hypothesized to better model operators for causal, phase-sensitive, and non-stationary systems due to its design.

Conclusion: HNO offers a theoretically motivated alternative to FNO, with potential advantages in handling non-periodic and phase-sensitive problems.
...
Result: GM layers achieve comparable test performance to two-layer fully connected networks and exhibit distinct behavior.
...
<details>
  <summary>Details</summary>
Motivation: Traditional physics-based models fail to capture surface ozone trends for human-health impacts, and existing deep learning models lack interpretability for policy decisions.
...
Result: The model effectively quantifies uncertainty and identifies optimal ground stations for bias correction, with land-use information impacting ozone residual modeling.

Conclusion: The approach enhances interpretability and decision-making for air pollution policy and public health measures.
...
Method: Uses a 2D CNN to estimate surface ozone model residuals (bias) from MOMO-Chem, comparing it to traditional ML and incorporating satellite land use data.

Result: The CNN outperforms traditional ML in capturing model residuals, and satellite data enhances urban-scale ozone bias understanding.

Conclusion: The approach improves ozone modeling and scientific insights, aiding better environmental policy decisions.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the gap in applying deep learning for hydrology, especially in generalizing across diverse datasets, to enhance water level forecasting for ecosystem management.
...
Result: RAF significantly improves forecasting accuracy in real-world Everglades data, demonstrating its potential for environmental hydrology.

Conclusion: RAF shows promise for adaptive AI in ecosystem management, encouraging broader adoption by domain experts.
...
TL;DR: The paper analyzes replicability in transfer learning, introducing a measure called selection sensitivity (ΔQ) to quantify trade-offs between adaptation effectiveness and consistency. It shows that replicability failure increases with sensitivity and decreases with sample size, validated through experiments on the MultiNLI corpus.
...
Method: Theoretical framework quantifying selection sensitivity (ΔQ) and empirical analysis using six adaptive selection strategies on the MultiNLI corpus.

Result: Highly adaptive strategies (e.g., gradient-based) improve performance but increase replicability failure rates, while less adaptive methods keep failure rates below 7%. Source domain pretraining mitigates failure rates by up to 30%.
...
<details>
  <summary>Details</summary>
Motivation: The lack of a unified platform for fair and systematic evaluation of DAL algorithms under diverse real-world challenges (e.g., distribution shifts, data imbalance) motivated the creation of ALScope.

Method: ALScope integrates 10 datasets from CV and NLP, and 21 DAL algorithms, supporting flexible configuration of experimental factors like OOD sample ratio and class imbalance.

Result: Experiments reveal DAL performance varies by domain and task, with room for improvement in non-standard scenarios (imbalanced, open-set) and trade-offs in selection time.
...
TL;DR: REINA is a novel loss function for Simultaneous Speech Translation (SimulST) that optimizes the tradeoff between translation quality and latency by waiting for more input only when it provides useful information. It achieves SOTA results on multiple languages.
...
Result: Achieves SOTA streaming results, improving the latency/quality tradeoff by up to 21% compared to prior methods.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods like Bagging, Boosting, and NCL lack precise control over the accuracy-diversity trade-off, prompting the need for a more flexible and theoretically sound approach.

Method: SEA decomposes ensemble errors into individual performance and diversity terms, introducing an adjustable parameter in the loss function for fine-tuned control.
...
Conclusion: SEA provides a flexible, high-performing solution for ensemble learning with superior control over accuracy and diversity.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenges of nonconvex stochastic optimization in decentralized settings, combining momentum for fast convergence and compression for reduced communication, while theoretically proving their effectiveness.

Method: Two algorithms: (1) a compressed decentralized adaptive method for bounded gradients, and (2) a compressed decentralized heavy-ball method with gradient tracking for data heterogeneity. Both use momentum and compression techniques.

Result: Both algorithms achieve optimal convergence rates, linear speed-up, and topology-independent parameters. Superior performance in training DNNs and Transformers compared to state-of-the-art methods.

Conclusion: The proposed algorithms effectively combine momentum and compression, addressing key challenges in decentralized optimization and demonstrating strong theoretical and empirical performance.
...
Result: MENDR achieves near state-of-the-art performance in clinical EEG tasks with fewer parameters and enhances interpretability through geometric embeddings.
...
Method: Integrates RC for data-driven modeling and UKF for Bayesian estimation, using RC as a surrogate model in UKF.
...
Result: Experiments show MMCI suppresses biases and improves performance on standard MSA datasets and OOD test sets.

Conclusion: MMCI effectively addresses spurious correlations in MSA, enhancing generalization and stability under distribution shifts.
...
Method: R-Zero uses two co-evolving models (Challenger and Solver) that interact to create a self-improving curriculum, generating tasks and solutions without external labels.
...
Conclusion: R-Zero demonstrates a scalable, autonomous approach to advancing LLMs beyond human-curated limitations, enhancing reasoning performance.
...
<details>
  <summary>Details</summary>
Motivation: Current RL-based fine-tuning for LLMs is resource-intensive, and heuristic-driven methods lack scalability and generalizability.

Method: SPaRFT uses cluster-based data reduction and a multi-armed bandit to select and allocate training samples adaptively.

Result: SPaRFT matches or outperforms baselines using up to 100x fewer samples, with ablation studies confirming its effectiveness.

Conclusion: Performance-driven training curricula can enhance LLM reasoning efficiently with minimal resources.
...
TL;DR: The paper explores how AR systems in CPR training can compromise situational awareness (SA) due to cognitive tunneling. It introduces an AR app for real-time CPR feedback, evaluates SA using eye tracking, and proposes a graph neural network (FixGraphPool) to predict SA with high accuracy.


<details>
  <summary>Details</summary>
Motivation: AR systems improve task performance but may reduce SA in safety-critical scenarios like CPR. The study aims to understand and mitigate this risk by analyzing SA in AR-guided CPR.

Method: Developed an AR app for CPR feedback, conducted a user study with simulated incidents, and collected SA metrics via observation, questionnaires, and eye tracking. Proposed FixGraphPool, a graph neural network, to model gaze data for SA prediction.

Result: Higher SA correlated with specific eye movement patterns (e.g., larger saccades, fewer fixations on virtual content). FixGraphPool achieved 83.0% accuracy in SA prediction, outperforming other models.

Conclusion: Eye tracking is effective for SA modeling in AR. The findings can guide AR system design to enhance user safety and SA.
...
Method: Uses structured forgetting and meta-learning (KNOWN hyper-model) to predict enhanced weights.
...
Method: Uses a Lyapunov function for energy-reducing dynamics and a tangential component learned via message passing, decomposing feature evolution into orthogonal flows.

Result: Achieves strong performance in node/graph classification and regression benchmarks, mitigating oversquashing.
...
<details>
  <summary>Details</summary>
Motivation: To address limitations of existing activation functions like ReLU and Mish by proposing non-monotonic, piecewise alternatives that treat positive and negative inputs differently.

Method: ULU is defined piecewise for positive and negative inputs, while AULU uses learnable parameters for adaptive responses. The LIB metric quantifies inductive bias.

Result: ULU and AULU significantly outperform ReLU and Mish in image classification and object detection tasks.

Conclusion: ULU and AULU offer superior performance and adaptability, with the LIB metric providing a tool for analyzing model inductive bias.
...
<details>
  <summary>Details</summary>
Motivation: To understand why multimodal policies outperform unimodal ones by analyzing their theoretical foundations and connecting empirical successes to learning theory.

Method: Analyzes multimodal policies using statistical learning theory, focusing on generalization bounds and optimization landscapes. Reviews frameworks like Rademacher complexity, PAC learning, and information theory.

Result: Multimodal policies achieve tighter generalization bounds and better optimization landscapes compared to unimodal ones.

Conclusion: Properly integrated multimodal architectures (e.g., PerAct, CLIPort) leverage theoretical advantages for superior performance in imitation learning.
...
TL;DR: Integrated Influence is a new data attribution method that addresses limitations of LOO-based methods by incorporating a baseline approach, providing more reliable attributions.


<details>
  <summary>Details</summary>
Motivation: Prevailing LOO-based methods lack collective influence analysis and counterfactual explanations, limiting transparency and flexibility.

Method: Proposes Integrated Influence, which uses a baseline dataset and a degeneration process to accumulate sample influence, with a theoretical framework.

Result: Outperforms existing methods in data attribution and mislabeled example identification tasks.

Conclusion: Integrated Influence offers a more comprehensive and reliable approach to data attribution, enhancing model transparency.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the cold-start problem in preference learning, where initial labeled data is scarce or noisy, especially in computational social systems and economic analysis.

Method: Combines self-supervised pre-training (PCA for pseudo-labels) with an active learning loop involving a simulated noisy oracle.

Result: Outperforms standard active learning, achieving higher accuracy with fewer labeled pairs across diverse datasets.

Conclusion: The framework effectively mitigates the cold-start problem, improving sample efficiency and applicability in data-constrained settings.
...
### [75] [Exploring Superior Function Calls via Reinforcement Learning](https://arxiv.org/abs/2508.05118)
*Bingguang Hao,Maolin Wang,Zengzhuang Xu,Yicheng Chen,Cunyin Peng,Jinjie GU,Chenyi Zhuang*

Main category: cs.LG

TL;DR: A novel reinforcement learning framework improves function calling in LLMs by enhancing exploration and structured reasoning, achieving 86.02% accuracy on complex tasks.


<details>
  <summary>Details</summary>
Motivation: Current training methods for LLMs in function calling lack robustness, relying on superficial patterns or struggling with complex action spaces.

Method: A two-stage data pipeline with strategic entropy-based exploration and abstract syntax tree validation, tailored for function calling tasks.

Result: Achieves state-of-the-art 86.02% accuracy, outperforming standard methods by up to 6% in complex scenarios, especially benefiting code-pretrained models.

Conclusion: The framework advances LLM function calling, with plans to release code, models, and datasets to the community.
...
### [76] [Learning from Similarity-Confidence and Confidence-Difference](https://arxiv.org/abs/2508.05108)
*Tomoya Tate,Kosuke Sugiyama,Masato Uchida*

Main category: cs.LG

TL;DR: A novel WSL framework leverages multiple weak supervision signals (similarity-confidence and confidence-difference) for improved performance with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of accurate labeling in machine learning by utilizing complementary weak supervision signals when labeled data is scarce.

Method: Introduces SconfConfDiff Classification, integrating two weak labels, and derives unbiased risk estimators (convex combination and interaction modeling). Includes risk correction for overfitting.

Result: Proposed method achieves optimal convergence rates and outperforms baselines in experiments.

Conclusion: The framework effectively combines multiple weak supervision signals, proving robust and superior in performance.
...
### [77] [Domain-driven Metrics for Reinforcement Learning: A Case Study on Epidemic Control using Agent-based Simulation](https://arxiv.org/abs/2508.05154)
*Rishabh Gaur,Gaurav Deshkar,Jayanta Kshirsagar,Harshal Hayatnagarkar,Janani Venugopalan*

Main category: cs.LG

TL;DR: The paper introduces domain-driven metrics for evaluating RL-based agent-based models, demonstrated through a pandemic case study.


<details>
  <summary>Details</summary>
Motivation: Assessing RL-based ABMs is challenging due to system complexity and lack of standardized metrics.

Method: Develop domain-driven RL metrics, combining them with traditional metrics, and apply them to a rational ABM for pandemic modeling.

Result: Domain-driven rewards, alongside existing metrics, effectively evaluate RL performance in scenarios like mask availability.

Conclusion: Domain-driven metrics enhance the assessment of RL in ABMs, providing more relevant evaluations for complex systems.
...
TL;DR: HIA (Heuristic-Guided Inference-time Alignment) is a tuning-free, black-box-compatible method that balances alignment quality and computational cost using heuristic reward models and two-stage filtering. It outperforms existing baselines under the same inference budget.
...
Method: HIA uses a lightweight prompt optimizer, heuristic reward models, and two-stage filtering to reduce inference calls while preserving alignment quality.

Result: HIA outperforms best-of-N sampling, beam search, and greedy search baselines on real-world datasets (HelpSteer and ComPRed) under the same inference budget. It works even with low budgets (1-2 queries).

Conclusion: HIA offers a practical, scalable solution for personalized LLM deployment by balancing alignment quality and computational efficiency.
...
TL;DR: The paper introduces HFedDG, a hierarchical federated learning method addressing domain shift, and proposes HFedATM for improved performance and efficiency.
...
Method: Proposes HFedATM, using Filter-wise Optimal Transport Alignment and Shrinkage-aware Regularized Mean Aggregation.

Result: HFedATM outperforms FedDG baselines, maintains efficiency, and shows tighter generalization bounds.
...
### [80] [FAITH: A Framework for Assessing Intrinsic Tabular Hallucinations in finance](https://arxiv.org/abs/2508.05201)
*Mengao Zhang,Jiayu Fu,Tanya Warrier,Yuwen Wang,Tianhui Tan,Ke-wei Huang*

Main category: cs.LG

TL;DR: A framework for evaluating intrinsic hallucinations in financial LLMs using context-aware masked span prediction on real-world financial data.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of hallucinations in LLMs for finance, where precise numerical extraction is critical for reliable analysis and compliance.
...
Conclusion: Provides a robust methodology for evaluating financial LLMs, advancing trustworthy financial Generative AI systems.
...
### [81] [Deep Neural Networks with General Activations: Super-Convergence in Sobolev Norms](https://arxiv.org/abs/2508.05141)
...
<details>
  <summary>Details</summary>
Motivation: To address the gap in error-estimation theory for neural-network-based PDE approaches and demonstrate their superior accuracy.

Method: Analysis of deep fully-connected networks in Sobolev spaces, measuring errors in $W^{m,p}$-norm for $m < n$.

Result: Deep networks surpass classical methods in approximation rates, termed super-convergence.

Conclusion: Provides a unified theoretical foundation for neural networks in scientific computing, closing a significant theoretical gap.
...
### [82] [Advanced Hybrid Transformer LSTM Technique with Attention and TS Mixer for Drilling Rate of Penetration Prediction](https://arxiv.org/abs/2508.05210)
*Saddam Hussain Khan*

Main category: cs.LG

TL;DR: A hybrid deep learning model combining LSTM, Transformer encoders, TS-Mixer, and attention mechanisms outperforms traditional methods for ROP prediction, achieving high accuracy and interpretability.


<details>
  <summary>Details</summary>
Motivation: Accurate ROP prediction is challenging due to complex drilling data; existing models fail to capture temporal and contextual relationships.

Method: Proposes a hybrid architecture integrating LSTM, Transformer encoders, TS-Mixer, and attention mechanisms.

Result: Achieves R-squared 0.9988 and MAPE 1.447%, outperforming benchmarks.
...
### [83] [PSEO: Optimizing Post-hoc Stacking Ensemble Through Hyperparameter Tuning](https://arxiv.org/abs/2508.05144)
...
<details>
  <summary>Details</summary>
Motivation: Current CASH methods in AutoML use fixed strategies for ensembles, failing to adapt to task-specific needs. PSEO addresses this gap.

Method: PSEO selects base models via binary quadratic programming (balancing diversity and performance) and optimizes multi-layer stacking with two mechanisms. It searches for the best ensemble strategy in a hyperparameter space.

Result: PSEO achieved the best average test rank (2.96) on 80 datasets, surpassing other AutoML and ensemble methods.

Conclusion: PSEO effectively optimizes post-hoc ensembles, adapting to task characteristics and outperforming existing methods.
...
### [84] [Marine Chlorophyll Prediction and Driver Analysis based on LSTM-RF Hybrid Models](https://arxiv.org/abs/2508.05260)
...
TL;DR: The paper proposes an LSTM-RF hybrid model for predicting marine chlorophyll concentration, outperforming standalone LSTM and RF models in accuracy.
...
Method: A hybrid LSTM-RF model is developed, trained with multi-source ocean data, and enhanced with standardized treatment and sliding window techniques.

Result: The LSTM-RF model achieved R^2=0.5386, MSE=0.005806, and MAE=0.057147, significantly better than standalone models.

Conclusion: The hybrid model provides an innovative solution for high-frequency marine ecological variable prediction.
...
### [85] [FlowState: Sampling Rate Invariant Time Series Forecasting](https://arxiv.org/abs/2508.05287)
*Lars Graf,Thomas Ortner,Stanisław Woźniak,Angeliki Pantazi*

Main category: cs.LG

TL;DR: FlowState is a novel time series foundation model (TSFM) using a state space model encoder and functional basis decoder, addressing generalization, adaptability, and efficiency issues in existing TSFMs. It outperforms state-of-the-art models on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing TSFMs struggle with generalization across varying context/target lengths, adaptability to sampling rates, and computational inefficiency. FlowState aims to solve these challenges.

Method: FlowState combines a state space model (SSM) encoder and a functional basis decoder for continuous-time modeling and dynamic time-scale adjustment. It includes an efficient pretraining strategy.

Result: FlowState outperforms other models on GIFT-ZS and Chronos-ZS benchmarks, despite being smaller. It adapts online to varying input sampling rates.

Conclusion: FlowState is a state-of-the-art TSFM with superior generalization, adaptability, and efficiency, validated by benchmarks and ablation studies.
...
### [86] [pFedDSH: Enabling Knowledge Transfer in Personalized Federated Learning through Data-free Sub-Hypernetwork](https://arxiv.org/abs/2508.05157)
...
<details>
  <summary>Details</summary>
Motivation: Existing pFL methods assume static client participation, but real-world scenarios involve dynamic onboarding of new clients, posing challenges like performance preservation and knowledge transfer.

Method: Proposes pFedDSH, a hypernetwork-based framework with batch-specific masks and data-free replay for backward transfer.
...
Conclusion: pFedDSH achieves robust performance stability for existing clients and efficient adaptation for new clients while preserving privacy.
...
### [87] [ASkDAgger: Active Skill-level Data Aggregation for Interactive Imitation Learning](https://arxiv.org/abs/2508.05310)
...
<details>
  <summary>Details</summary>
Motivation: Human teaching effort is a bottleneck in interactive imitation learning. Existing methods waste novice plans' valuable information during queries.

Method: Introduces ASkDAgger with S-Aware Gating (SAG), Foresight Interactive Experience Replay (FIER), and Prioritized Interactive Experience Replay (PIER) to utilize novice plans and teacher feedback.

Result: Balances query frequency with failure incidence, reduces demonstration annotations, improves generalization, and speeds domain adaptation. Validated in language-conditioned manipulation tasks.

Conclusion: ASkDAgger effectively reduces human teaching effort and enhances learning efficiency in interactive imitation learning.
...
### [88] [S$^2$M-Former: Spiking Symmetric Mixing Branchformer for Brain Auditory Attention Detection](https://arxiv.org/abs/2508.05164)
...
### [89] [Optimal Corpus Aware Training for Neural Machine Translation](https://arxiv.org/abs/2508.05364)
...
Result: OCAT achieves +3.6 and +1.8 chrF improvements in WMT23 translation tasks, outperforming vanilla training and matching other fine-tuning techniques.

Conclusion: OCAT is lightweight, effective, and less sensitive to hyperparameters, making it a superior alternative to CAT.
...
### [90] [Echo: Decoupling Inference and Training for Large-Scale RL Alignment on Heterogeneous Swarms](https://arxiv.org/abs/2508.05387)
*Jie Xiao,Shaoduo Gan,Changyuan Fan,Qingnan Ren,Alfred Long,Yuchen Zhang,Rymon Yu,Eric Yang,Lynn Ai*

Main category: cs.LG

TL;DR: Echo is a system that decouples inference and training phases in RL-based post-training for LLMs, improving efficiency with heterogeneous hardware.


<details>
  <summary>Details</summary>
Motivation: Current RL systems co-locate trajectory sampling and policy optimization, violating SPMD assumptions and reducing efficiency.

Method: Echo decouples phases using inference and training swarms, with lightweight sync protocols: sequential pull and async push-pull modes.

Result: Echo matches co-located baselines in convergence and reward while using decentralized hardware.

Conclusion: Echo enables efficient large-scale RL for LLMs with heterogeneous resources.
...
### [91] [Near Optimal Inference for the Best-Performing Algorithm](https://arxiv.org/abs/2508.05173)
*Amichai Painsky*

Main category: cs.LG

TL;DR: The paper introduces a framework for selecting the best-performing algorithm from a benchmark by formulating it as a subset selection problem for multinomial distributions.


<details>
  <summary>Details</summary>
Motivation: To identify the most likely top-performing algorithm on future datasets, especially when performance differences are marginal.

Method: A novel framework for subset selection, with asymptotic and finite-sample schemes, and matching lower bounds.

Result: Proposed schemes significantly outperform existing methods, with theoretical guarantees.

Conclusion: The framework effectively addresses the subset selection problem, providing improved performance and confidence.
...
### [92] [Tail-Risk-Safe Monte Carlo Tree Search under PAC-Level Guarantees](https://arxiv.org/abs/2508.05441)
*Zuyuan Zhang,Arnob Ghosh,Tian Lan*

Main category: cs.LG

TL;DR: The paper introduces CVaR-MCTS and W-MCTS to address tail-risk in MCTS, providing rigorous safety guarantees and improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing safety-aware MCTS methods lack rigorous tail-risk guarantees, which can lead to serious consequences in high-stake scenarios.

Method: 1. CVaR-MCTS embeds Conditional Value-at-Risk for tail-risk control. 2. W-MCTS uses a Wasserstein ambiguity set to address estimation bias in tail-risk.

Result: Both methods outperform baselines, achieving robust tail-risk guarantees with better rewards and stability.

Conclusion: The proposed CVaR-MCTS and W-MCTS effectively address tail-risk in MCTS, providing safety guarantees and improved performance.
...
### [93] [Human Activity Recognition from Smartphone Sensor Data for Clinical Trials](https://arxiv.org/abs/2508.05175)
...
TL;DR: A ResNet-based HAR model detects gait and everyday activities with high accuracy and robustness across smartphone wear locations, outperforming a state-of-the-art model.


<details>
  <summary>Details</summary>
Motivation: To develop a lightweight HAR model for detecting gait and everyday activities, applicable to healthy individuals and people with multiple sclerosis (PwMS).

Method: The model uses ResNet architecture, trained and evaluated on smartphone sensor data from healthy controls and PwMS, including datasets from GaitLab, Roche, and public sources.

Result: Achieved 98.4%-99.6% accuracy for gait detection and 96.2% for everyday activities, outperforming the state-of-the-art model. Robust across 9 wear locations.

Conclusion: The proposed HAR model is accurate and practical for real-world applications, especially for diverse wear locations.
...
### [94] [EnergyPatchTST: Multi-scale Time Series Transformers with Uncertainty Estimation for Energy Forecasting](https://arxiv.org/abs/2508.05454)
*Wei Li,Zixin Wang,Qizheng Sun,Qixiang Gao,Fenglei Yang*

Main category: cs.LG

TL;DR: EnergyPatchTST improves energy time series prediction with multi-scale feature extraction, probability prediction, and integration of future variables, reducing errors by 7-12%.
...
Method: Proposes EnergyPatchTST, featuring multi-scale feature extraction, Monte Carlo-based uncertainty estimation, and integration of future variables.
...
### [95] [Physics-Informed Time-Integrated DeepONet: Temporal Tangent Space Operator Learning for High-Accuracy Inference](https://arxiv.org/abs/2508.05190)
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods (FR and AR) fail in long-term accuracy due to poor generalization and error accumulation, limiting reliability.

Method: PITI-DeepONet uses a dual-output architecture trained with physics-informed or hybrid objectives, integrating time-derivative operators via classical schemes.

Result: Reduced mean relative errors by 84% (vs. FR) and 79% (vs. AR) for heat equation, 87% (vs. FR) and 98% (vs. AR) for Burgers equation, and 42% (vs. FR) and 89% (vs. AR) for Allen-Cahn equation.
...
### [96] [Task complexity shapes internal representations and robustness in neural networks](https://arxiv.org/abs/2508.05463)
*Robert Jankowski,Filippo Radicchi,M. Ángeles Serrano,Marián Boguñá,Santo Fortunato*

Main category: cs.LG

TL;DR: The paper explores how task difficulty affects neural network representations using data-agnostic probes like pruning and noise injection, revealing insights into model robustness and topology.
...
Method: Introduces five data-agnostic probes (pruning, binarization, noise injection, sign flipping, bipartite network randomization) applied to MLPs on MNIST and Fashion-MNIST datasets.

Result: Binarizing weights in hard-task models collapses accuracy, while easy-task models remain robust. Noise injection can enhance accuracy, and preserving sign structure suffices for high accuracy.

Conclusion: Task complexity can be measured by the performance gap between full-precision and binarized/shuffled models, highlighting the role of signed bipartite topology in representations and suggesting practical model compression strategies.
...
### [97] [Bidding-Aware Retrieval for Multi-Stage Consistency in Online Advertising](https://arxiv.org/abs/2508.05206)
*Bin Liu,Yunfei Liu,Ziru Xu,Zhaoyu Zhou,Zhi Kou,Yeqiu Yang,Han Zhu,Jian Xu,Bo Zheng*

Main category: cs.LG

TL;DR: The paper proposes Bidding-Aware Retrieval (BAR) to address inconsistency between retrieval and ranking stages in online advertising, improving revenue and advertiser outcomes by incorporating bid values into retrieval.


<details>
  <summary>Details</summary>
Motivation: The inconsistency between retrieval and ranking stages in online advertising, exacerbated by auto-bidding, leads to sub-optimal revenue and advertiser results.

Method: BAR integrates bid values into retrieval scoring via Bidding-Aware Modeling (monotonicity-constrained learning, multi-task distillation) and Asynchronous Near-Line Inference for real-time updates. A Task-Attentive Refinement module enhances feature interactions.

Result: BAR increased platform revenue by 4.32% and impressions by 22.2% for positively-operated ads.

Conclusion: BAR effectively resolves multi-stage inconsistency, enhancing both platform revenue and advertiser outcomes.
...
### [98] [Tractable Sharpness-Aware Learning of Probabilistic Circuits](https://arxiv.org/abs/2508.05537)
*Hrithik Suresh,Sahil Sidheekh,Vishnu Shreeram M. P,Sriraam Natarajan,Narayanan C. Krishnan*

Main category: cs.LG

TL;DR: The paper addresses overfitting in Probabilistic Circuits (PCs) by proposing a Hessian-based regularizer, inspired by sharpness aware minimization, to guide PCs toward flatter minima and improve generalization.


<details>
  <summary>Details</summary>
Motivation: Overfitting in PCs, especially with limited data, is often caused by convergence to sharp optima that generalize poorly.

Method: A Hessian-based regularizer is introduced, leveraging the tractable computation of the Hessian trace in PCs, to minimize sharpness and improve generalization.

Result: Experiments show the method effectively guides PCs to flatter minima and enhances generalization performance.

Conclusion: The proposed Hessian-based regularizer is a practical solution to mitigate overfitting in PCs, improving their robustness and performance.
...
### [99] [Adapting Vision-Language Models Without Labels: A Comprehensive Survey](https://arxiv.org/abs/2508.05547)
...
Method: Proposes a taxonomy of four paradigms based on unlabeled data availability, analyzing core methodologies and strategies for each.

Result: Provides a structured overview of unsupervised VLM adaptation, including benchmarks and open challenges.

Conclusion: Bridges the gap in literature by offering a systematic survey and identifying future research directions.
...
### [100] [DFW: A Novel Weighting Scheme for Covariate Balancing and Treatment Effect Estimation](https://arxiv.org/abs/2508.05215)
*Ahmad Saeed Khan,Erik Schaffernicht,Johannes Andreas Stork*

Main category: cs.LG

TL;DR: Proposes Deconfounding Factor Weighting (DFW) to improve causal effect estimation by stabilizing sample weights and better approximating RCT conditions.


<details>
  <summary>Details</summary>
Motivation: Addresses limitations of propensity score-based methods like IPW, which suffer from unstable weights and poor covariate balance due to high variance in propensity scores.

Method: DFW leverages a deconfounding factor to construct stable weights, prioritizing less confounded samples and bounding weights to reduce variance.
...
Conclusion: DFW is a robust alternative to existing methods, effective for binary and multi-treatment settings, improving causal inference from observational data.
...
TL;DR: LLMs can generate interpretable computable phenotypes (CPs) for clinical decision support, approaching state-of-the-art ML performance with fewer examples.


<details>
  <summary>Details</summary>
Motivation: Explore LLMs' potential for generating accurate CPs to improve hypertension care.

Method: Propose a synthesize, execute, debug, instruct strategy for iterative CP refinement.

Result: LLMs with iterative learning produce reasonably accurate, interpretable CPs.

Conclusion: LLMs show promise for scalable clinical decision support with minimal training data.
...
### [102] [ML-based Short Physical Performance Battery future score prediction based on questionnaire data](https://arxiv.org/abs/2508.05222)
*Marcin Kolakowski,Seif Ben Bader*

Main category: cs.LG

TL;DR: Predicting older adults' SPPB scores at a 4-year horizon using ML models, with XGBoost performing best (MAE: 0.79). Feature subsets improved results (MAE: 0.82).


<details>
  <summary>Details</summary>
Motivation: Early intervention to slow physical deterioration in older adults requires predicting SPPB scores.
...
Result: XGBoost achieved the best performance (MAE: 0.79). Feature subsets yielded slightly higher MAE (0.82).

Conclusion: XGBoost is effective for predicting SPPB scores, and feature selection maintains performance with fewer inputs.
...
### [103] [Shuffle-R1: Efficient RL framework for Multimodal Large Language Models via Data-centric Dynamic Shuffle](https://arxiv.org/abs/2508.05612)
*Linghao Zhu,Yiran Guan,Dingkang Liang,Jianzhong Ju,Zhenbo Luo,Bin Qin,Jian Luan,Yuliang Liu,Xiang Bai*

Main category: cs.LG

TL;DR: Shuffle-R1 improves RL fine-tuning efficiency for MLLMs by addressing Advantage Collapsing and Rollout Silencing through dynamic trajectory sampling and batch reshuffling.
...
### [104] [Don't Reach for the Stars: Rethinking Topology for Resilient Federated Learning](https://arxiv.org/abs/2508.05224)
...
<details>
  <summary>Details</summary>
Motivation: Addresses limitations of centralized FL (single point of failure, poor personalization, vulnerability to adversarial clients) by leveraging P2P topology.

Method: Uses agreement scores on local validation sets to select trustworthy updates and aggregates with regularization for stability.
...
### [105] [TrajEvo: Trajectory Prediction Heuristics Design via LLM-driven Evolution](https://arxiv.org/abs/2508.05616)
...
<details>
  <summary>Details</summary>
Motivation: Improving accuracy, generalizability, and explainability in trajectory prediction for safety-critical applications like robotics and autonomous vehicles.
...
Conclusion: TrajEvo advances automated, fast, explainable, and generalizable trajectory prediction, with open-source code for future research.
...
### [106] [Cross-LoRA: A Data-Free LoRA Transfer Framework across Heterogeneous LLMs](https://arxiv.org/abs/2508.05232)
*Feifan Xia,Mingyang Liao,Yuyang Fang,Defang Li,Yantong Xie,Weikang Li,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.LG

TL;DR: Cross-LoRA is a data-free framework for transferring LoRA modules between diverse LLMs, using LoRA-Align and LoRA-Shift for subspace alignment and projection, achieving up to 5.26% performance gains.
...
Method: Cross-LoRA employs LoRA-Align (subspace alignment via SVD and linear transformation) and LoRA-Shift (projection of aligned subspaces) to transfer LoRA modules without additional data or training.

Result: Achieves up to 5.26% relative gains over base models on benchmarks like ARCs, OBOA, and HellaSwag, while maintaining performance comparable to directly trained LoRA adapters.

Conclusion: Cross-LoRA provides a lightweight, data-free solution for transferring LoRA modules across diverse LLMs, enhancing applicability without compromising performance.
...
### [107] [MoBE: Mixture-of-Basis-Experts for Compressing MoE-based LLMs](https://arxiv.org/abs/2508.05257)
...
<details>
  <summary>Details</summary>
Motivation: Addressing the memory challenges of deploying large MoE-based LLMs by reducing parameter counts without significant accuracy drops.

Method: Decomposes expert matrices into shared basis matrices and unique components, minimizing reconstruction error.

Result: Achieves 24%-30% parameter reduction with only 1%-2% accuracy drop, outperforming prior methods.

Conclusion: MoBE offers an effective solution for compressing MoE models while preserving performance.
...
TL;DR: The paper proposes using RLHF to align LLM-based CRS with user preferences by optimizing implicit feedback signals like dwell time and engagement, outperforming traditional methods in accuracy and satisfaction.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised fine-tuning fails to capture implicit user feedback, necessitating a method to align CRS with user preferences more effectively.

Method: The solution uses RLHF with a reward model trained on weakly-labeled engagement data and optimizes the LLM via PPO, modeling conversational state transitions for item suggestions.
...
TL;DR: Optimal batch-size and learning-rate schedules improve training efficiency and convergence in deep learning, validated by theory and experiments.


<details>
  <summary>Details</summary>
Motivation: Addressing computational bottlenecks in deep learning by optimizing batch-size and learning-rate scheduling to balance efficiency and generalization.

Method: Theoretical derivation of optimal growth schedules for batch size and learning rate based on stochastic first-order oracle (SFO) complexity, validated through experiments.

Result: Derived schedules reduce SFO complexity, offering scalable and efficient large-batch training.

Conclusion: Provides theoretical and practical guidelines for efficient deep learning training through optimized hyperparameter scheduling.
...
<details>
  <summary>Details</summary>
Motivation: Mini-batch SGD's performance depends heavily on batch size and learning rate settings. The study aims to optimize these parameters for faster convergence.
...
Result: Experiments show the adaptive joint scheduler outperforms existing schedulers in convergence speed.

Conclusion: Adaptive scheduling based on critical batch size theory effectively accelerates SGD convergence.
...
TL;DR: USP is a framework for semi-supervised continual learning (SSCL) that improves learning plasticity, unlabeled learning, and memory stability through feature space reservation, divide-and-conquer pseudo-labeling, and class-mean-anchored distillation.
...
Result: USP outperforms prior SSCL methods, achieving up to 5.94% higher accuracy in evaluations.

Conclusion: USP effectively synergizes the three key aspects of SSCL, demonstrating superior performance and practical utility.
...
<details>
  <summary>Details</summary>
Motivation: Learning from scratch is costly for personalization tasks with limited decision points. Latent bandits reduce exploration time but require accurate joint distributions, which may not exist.

Method: A posterior-sampling algorithm is introduced, relaxing latent bandit assumptions to focus on preference ordering rather than exact reward distributions.

Result: The algorithm performs competitively with full-knowledge latent bandits when well-specified and outperforms them when reward scales differ.

Conclusion: Relaxing assumptions to preference ordering improves adaptability and performance in scenarios with varying reward scales.
...
<details>
  <summary>Details</summary>
Motivation: DNNs are vulnerable to backdoor attacks, necessitating robust defense mechanisms.

Method: NT-ML involves retraining with non-target labels and mutual learning between teacher and student models.

Result: Effectively defends against 6 backdoor attacks and outperforms 5 state-of-the-art defenses.

Conclusion: NT-ML is a promising solution for mitigating backdoor attacks with minimal clean data.
...
TL;DR: The paper revisits a 2017 adaptive learning rate method, identifies inconsistencies in its application to Adam, proposes a corrected variant, and benchmarks it against SGD and Adam to evaluate practical benefits.


<details>
  <summary>Details</summary>
Motivation: To investigate the utility of adaptive learning rate mechanisms in deep learning, particularly for dynamically adjusting step sizes based on the loss landscape.

Method: Revisits a cumulative path-based adaptation scheme, corrects its inconsistency for Adam, and benchmarks SGD and Adam with and without adaptation against a recent alternative.

Result: The study clarifies when and why adaptive learning rate strategies, like the proposed corrected variant, offer practical benefits in training.

Conclusion: The corrected adaptive learning rate variant for Adam provides more consistent and practical benefits, as demonstrated by benchmarking against other methods.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of ensuring high-quality, diverse molecular generation from text while maintaining fast inference, which existing methods struggle with.

Method: Proposes a Causality-Aware Transformer (CAT) for joint encoding of molecular graphs and text with causal dependencies, and a Variational Mean Flow (VMF) framework for efficient one-step inference using a mixture of Gaussians.

Result: Outperforms state-of-the-art baselines with higher novelty (up to 74.5%), diversity (up to 70.3%), and 100% validity, while requiring fewer function evaluations for generation.

Conclusion: The framework achieves superior performance in molecular generation, balancing quality, diversity, and computational efficiency.
...
TL;DR: The paper explores Echo State Networks (ESNs) for forecasting cryptocurrency prices, showing their robustness during chaotic market periods and outperforming other methods.


<details>
  <summary>Details</summary>
Motivation: Forecasting volatile and non-stationary stock and cryptocurrency prices is challenging, and ESNs show promise for capturing nonlinear patterns in such data.

Method: The study uses ESNs for cryptocurrency forecasting, conducts chaos analysis via the Lyapunov exponent, and compares performance with Boosting and Naïve methods.
...
Conclusion: ESNs are effective for cryptocurrency forecasting, particularly in high-volatility scenarios, and demonstrate robustness in chaotic conditions.
...
<details>
  <summary>Details</summary>
Motivation: Current VAEs fail to capture the variability in neural spike trains, and Poisson-VAE's rigid constraints are biologically unrealistic.
...
TL;DR: CR-FMOL introduces a federated multi-objective learning framework with client-wise Pareto optimality via preference-cone constraints, improving fairness over FedAvg.


<details>
  <summary>Details</summary>
Motivation: Address the under-serving of minority clients in federated learning (FL) by enforcing client-wise Pareto optimality, unlike existing methods like FMOL.

Method: Uses preference-cone constraints after local FMGDA/FSMGDA steps; server solves cone-constrained Pareto-MTL sub-problem for Pareto-stationary directions.

Result: Enhances client fairness on non-IID benchmarks, with early-stage performance slightly below FedAvg but expected to match accuracy over time.

Conclusion: CR-FMOL is a promising framework for fair FL, balancing client-wise Pareto optimality and eventual performance parity with FedAvg.
...
TL;DR: The paper introduces Group Causal Policy Optimization (GCPO), a method that improves upon GRPO by incorporating causal dependencies among candidate responses, leading to better performance in reasoning tasks.
...
Method: GCPO uses a Structural Causal Model (SCM) to reveal hidden dependencies, projects responses onto a causally informed subspace, and introduces a causally informed reward adjustment and KL regularization.

Result: GCPO outperforms GRPO and other methods across multiple reasoning benchmarks.

Conclusion: Incorporating causal structure into policy optimization enhances prediction quality and performance in specialized tasks.
...
<details>
  <summary>Details</summary>
Motivation: Deep reinforcement learning lacks interpretability, hindering trust and real-world deployment. MLES aims to address this by providing transparent and efficient policy discovery.

Method: MLES uses multimodal large language models as policy generators, integrates evolutionary search for optimization, and employs visual feedback for behavior analysis and targeted improvements.

Result: MLES matches PPO's performance in control tasks while providing interpretable policies and scalable, adaptable solutions.
...
TL;DR: Treating competing risks as censoring in survival analysis introduces bias, overestimates risk, and amplifies disparities. This paper quantifies the error and its fairness implications.


<details>
  <summary>Details</summary>
Motivation: To address the overlooked consequences of treating competing risks as censoring in survival analysis, which leads to biased estimates and exacerbates disparities.

Method: The authors formalize the problem, quantify the error in survival estimates, and analyze its impact on predictive performance and fairness, supported by empirical analysis of cardiovascular data.

Result: Ignoring competing risks disproportionately affects high-risk individuals and worsens disparities. The framework quantifies the error and its fairness implications.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of evaluating AI systems in the absence of ground truth and resist adversarial manipulation.

Method: The paper leverages the connection between gaming resistance and output quality, using f-mutual information measures under natural conditions.

Result: The mechanisms achieve perfect discrimination between faithful and strategic agents, with 10-100x better robustness to adversarial manipulation. Performance peaks at a 10:1 compression ratio.

Conclusion: The proposed mechanisms are highly effective for AI evaluation, particularly in scenarios with optimal information diversity.
...
TL;DR: The paper proposes a multi-task recurrent neural network to model clinical presence (inter-observation time and missingness) alongside survival outcomes, improving prediction model transportability.


<details>
  <summary>Details</summary>
Motivation: Clinical presence impacts observed outcomes in electronic health records, but standard models overlook this, limiting performance and transportability.
...
Conclusion: Incorporating clinical presence enhances prediction model performance and transportability.
...
Method: The proposed algorithms, AMVFCM-U and AAMVFCM-U, replace fuzzification parameters with entropy regularization and SNR-based feature weighting. AAMVFCM-U adds hierarchical dimensionality reduction via adaptive thresholding.
...
Conclusion: The unified parameter-free framework effectively addresses multi-view clustering challenges, offering adaptive, efficient, and accurate solutions.
...
TL;DR: X-VFL is a new Vertical Federated Learning framework addressing non-aligned data and enabling local inference via Cross Completion and Decision Subspace Alignment modules.


<details>
  <summary>Details</summary>
Motivation: Current VFL frameworks require perfectly aligned data and joint inference, limiting practicality. X-VFL aims to handle missing features and support local inference.

Method: Proposes X-VFL with Cross Completion (XCom) for feature reconstruction and Decision Subspace Alignment (DS-Align) for local inference. Provides convergence theorems for training algorithms.

Result: X-VFL outperforms existing methods, achieving 15% higher accuracy on CIFAR-10 and 43% on MIMIC-III.

Conclusion: X-VFL effectively addresses VFL challenges, proving superior in handling missing features and enabling local inference.
...
TL;DR: Fairy±i introduces a 2-bit quantization framework for complex-valued LLMs, surpassing the accuracy ceiling of full-precision models by leveraging complex domain advantages and enabling multiplication-free inference.


<details>
  <summary>Details</summary>
Motivation: Current QAT methods focus on minimizing quantization error without surpassing the full-precision model's accuracy ceiling. This work aims to break this ceiling by improving full-precision accuracy and enabling efficient 2-bit quantization.
...
TL;DR: The paper introduces an extension for PyKEEN to integrate advanced negative sampling strategies for knowledge graph embedding, enhancing performance and compatibility.
...
Method: Developed a PyKEEN extension with modular architecture for static and dynamic negative sampling strategies.

Result: Empirical study shows improved performance in link prediction tasks and insights for better strategy design.

Conclusion: The extension enhances PyKEEN, improves embedding methods, and aids in developing more effective negative sampling strategies.
...
<details>
  <summary>Details</summary>
Motivation: Addressing IoT security concerns due to increasing cyberattacks by exploring KANs as a novel intrusion detection method.
...
Result: KANs outperform MLPs and achieve competitive accuracy with state-of-the-art models while providing better interpretability.
...
TL;DR: The paper introduces the 'one-poison hypothesis,' showing that a single poison sample with limited background knowledge can inject a backdoor into a model without affecting benign task performance, proven for linear models.


<details>
  <summary>Details</summary>
Motivation: To address the open question of how little poison data is needed for a successful backdoor attack, challenging prior assumptions.
...
Result: Demonstrates that a single poison sample can achieve zero backdooring-error and minimal impact on benign task performance.

Conclusion: The one-poison hypothesis holds for linear models, revealing vulnerabilities even with minimal adversarial effort.
...
TL;DR: Dynamic Fine-Tuning (DFT) improves SFT for LLMs by dynamically rescaling gradients, outperforming standard SFT and showing competitive RL results.


<details>
  <summary>Details</summary>
Motivation: Address limited generalization of SFT compared to RL by analyzing problematic reward structures in SFT gradients.

Method: Propose DFT, dynamically rescaling the objective function with token probabilities to stabilize gradient updates.

Result: DFT significantly outperforms SFT in benchmarks, improving generalization and showing competitive RL results.
...
TL;DR: BTPG-max improves upon Bidirectional TPG (BTPG) by finding more bidirectional pairs, enhancing efficiency and robustness in Multi-Agent Path Finding (MAPF) under delays.
...
<details>
  <summary>Details</summary>
Motivation: To enhance navigation efficiency in high-density environments with autonomous agents by mitigating local congestion through global flow optimization.

Method: Proposes CMPP, which uses flow-based penalties on graph vertices to model congestion. Two solvers are developed: an exact solver for small instances and a scalable A-CMTS algorithm for large-scale problems.

Result: CMPP significantly reduces local congestion and boosts system throughput in both discrete- and continuous-space scenarios.

Conclusion: CMPP improves multi-agent system performance, making it viable for real-world applications like logistics and autonomous vehicles.

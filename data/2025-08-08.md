...
Motivation: To enhance industrial machinery maintenance by moving beyond anomaly detection to offer intelligent, actionable recommendations.

Method: Integrates bearing vibration frequency analysis with multi-agentic generation, serializing data into natural language for LLM processing. Classifies faults and retrieves procedural knowledge for recommendations.
...
<details>
  <summary>Details</summary>
Motivation: Current AI agents in healthcare rely on static strategies, limiting their ability to improve strategic planning, which is crucial for complex domains like healthcare.

Method: HealthFlow uses a meta-level evolution mechanism to autonomously refine high-level problem-solving policies by learning from successes and failures, and is evaluated on the EHRFlowBench benchmark.

Result: HealthFlow significantly outperforms state-of-the-art agent frameworks in complex health data analysis tasks.

Conclusion: This work advances AI in healthcare by shifting focus from tool-users to self-evolving task-managers, enabling more autonomous and effective scientific discovery.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of AGI in web navigation by combining offline and online learning paradigms, inspired by human cognitive processes.

Method: Derived a decomposition into System 1 (fast, intuitive) and System 2 (slow, deliberative) processes, implemented in the modular CogniWeb architecture.

Result: Achieved a 43.96% success rate on WebArena with a 75% reduction in token usage.

Conclusion: CogniWeb effectively bridges offline and online learning, demonstrating competitive and efficient performance in web navigation tasks.
...
Method: GeoFlow provides agents with detailed tool-calling objectives for runtime API invocation.

Result: Increases agentic success by 6.8% and reduces token usage by up to fourfold.

Conclusion: GeoFlow outperforms state-of-the-art approaches in efficiency and effectiveness.
...
<details>
  <summary>Details</summary>
Motivation: To address the evolving role of generative AI in education and counter narratives of de-skilling by demonstrating how pedagogically aligned AI can enhance learning.

Method: A controlled experiment with 65 pre-service teachers in Germany, comparing a Socratic AI Tutor to an uninstructed AI chatbot.
...
Conclusion: The study provides empirical support for hybrid learning ecosystems with human-AI co-agency and proposes a conceptual framework for orchestrated MAS in education.
...
<details>
  <summary>Details</summary>
Motivation: To address the data dependency limitation of Q&A benchmarks and comprehensively assess LLMs' performance in adversarial environments.

Method: Proposes Qi Town, a platform with 5 board games and 20 LLM players, using Elo ratings, PLG for technical evaluation, and PSS for mental fitness. Conducts a round-robin tournament for systematic comparison.

Result: LLMs show adaptability to adversarial environments but exhibit instability in skill play, as revealed by PLG analysis.

Conclusion: The framework effectively evaluates LLMs in adversarial settings, revealing both strengths (adaptability) and weaknesses (skill instability), suggesting further research is needed.
...
<details>
  <summary>Details</summary>
Motivation: Current cloud-based AWebGIS solutions raise privacy and scalability concerns due to reliance on internet access and centralized servers.

Method: Three approaches were compared: (1) cloud-based LLMs, (2) offline classical ML classifiers, and (3) client-side fine-tuned SLMs (T5-small).

Result: The SLM approach achieved the highest accuracy (0.93 exact matching, 0.99 Levenshtein similarity, 0.98 ROUGE scores) and reduced server load.

Conclusion: Client-side SLMs are feasible for AWebGIS, offering accuracy, privacy, and scalability advantages over cloud-based and classical methods.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate LLM reasoning in realistic, non-ideal scenarios, inspired by human reliability under imperfect inputs.

Method: Fine-tune three LLMs and an LVLM using RL (policy-gradient), test on eight datasets across three non-ideal scenarios.

Result: RL improves reasoning in ideal settings but declines in non-ideal scenarios, exposing limitations. Remediation methods are insufficient.

Conclusion: LLM reasoning is overstated; non-ideal scenario evaluation is crucial. Code and data will be released.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-task learning models perform poorly in ligand docking due to structural complexities, creating a need for better methods.
...
Result: LoopPlay achieves ~10% better binding mode prediction accuracy on benchmark datasets.
...
<details>
  <summary>Details</summary>
Motivation: Traditional rule-based and machine learning methods for spatial data integration are limited. LLMs offer a potential solution but their spatial reasoning capabilities need evaluation.
...
<details>
  <summary>Details</summary>
Motivation: To enable efficient updates of outdated medical knowledge in MLLMs without retraining, given the lack of systematic benchmarks for multimodal medical knowledge editing.

Method: Built on a medical visual QA dataset, MedMKEB includes tasks like counterfactual correction and adversarial robustness, validated by human experts.
...
Conclusion: MedMKEB sets a standard for developing trustworthy and efficient medical knowledge editing algorithms.
...
<details>
  <summary>Details</summary>
Motivation: Analog circuit design is complex and resource-intensive, with existing AI methods lacking portability and efficiency.

Method: Combines finetuned Qwen3-8B with dynamic task-specific loss functions and heuristic search (DE and PSO) for gate sizing.
...
Conclusion: EasySize reduces human expertise and computational needs, simplifying analog design, and will be open-sourced.
...
<details>
  <summary>Details</summary>
Motivation: Event logs in Process Mining often have missing data due to manual activities or collection issues, requiring effective reconstruction methods.

Method: Develops an HGNN model to reconstruct missing event attributes in traces, leveraging graph-based representations for richer encodings.

Result: The HGNN model shows superior performance in reconstructing all event attributes compared to autoencoder-based approaches, tested on synthetic and real logs.

Conclusion: HGNNs offer a promising solution for comprehensive event log reconstruction, addressing limitations of existing model-free methods.
...
Method: QA-Dragon uses a domain router for subject identification and a search router for dynamic retrieval strategy selection, combining text and image search agents for hybrid reasoning.
...
Conclusion: QA-Dragon effectively addresses complex VQA tasks by integrating dynamic retrieval and multimodal reasoning, enhancing performance over traditional RAG methods.
...
Method: Uses RDF graph databases and LLMs for natural language query processing, with a planning-orchestration architecture for transparent reasoning.

Result: Enables intuitive querying of concepts like equipment and personnel, delivering explainable results and improving communication efficiency.

Conclusion: The proposed framework enhances trust and efficiency in organizational communication by leveraging advanced query processing and reasoning.
...
<details>
  <summary>Details</summary>
Motivation: To integrate symbolic reasoning (decision trees) with neural capabilities (LLMs) for robust, interpretable, and extensible general-purpose reasoning.

Method: Embeds decision trees as callable oracles in a multi-agent framework, with LLMs handling abductive reasoning and planning, and an orchestrator managing consistency.

Result: Achieves +7.2% entailment consistency on ProofWriter, +5.3% accuracy on GSM8k, and +6.0% abstraction accuracy on ARC.

Conclusion: The hybrid architecture provides a scalable and interpretable solution for neuro-symbolic reasoning, demonstrated in clinical and scientific applications.
...
Method: Historical analysis and contemporary usage patterns inform a framework defining minimum agent requirements and classifying systems along multiple dimensions.
...
Conclusion: The framework improves research clarity, reproducibility, and policy effectiveness, with recommendations for standardization and adoption.
...
Method: NomicLaw simulates collaborative law-making with LLMs proposing rules, justifying them, and voting. Trust and reciprocity are measured via voting patterns, and strategic language is analyzed.

Result: LLMs form alliances, betray trust, and adapt rhetoric to influence decisions, showcasing social reasoning and persuasive skills.

Conclusion: The study highlights LLMs' potential for autonomous negotiation and legislation drafting, informing future AI system design.
...
<details>
  <summary>Details</summary>
Motivation: Existing models struggle with partial correctness in multi-question reasoning, needing better reward mechanisms.
...
Result: Achieves state-of-the-art on 6/12 benchmarks and a new STEM-Bench, showing effectiveness.

Conclusion: Structured, verifiable rewards significantly enhance multimodal reasoning in complex domains.
...
<details>
  <summary>Details</summary>
Motivation: To enhance railway predictive maintenance by enabling real-time fault prediction with explainability, improving service reliability and safety.
...
Result: Achieved 99% accuracy and 98% F-measure, robust to class imbalance and noise, with effective explanations.
...
<details>
  <summary>Details</summary>
Motivation: VLMs lack attention to detail and precise action planning in complex environments, necessitating a systematic evaluation of their physical reasoning.
...
Conclusion: DeepPHY highlights the limitations of VLMs in physical reasoning, suggesting a need for improved models in this domain.
...
<details>
  <summary>Details</summary>
Motivation: To explore how LLMs are revolutionizing organic synthesis by automating tasks and accelerating discovery, while addressing limitations and ensuring human oversight.
...
Result: LLMs, combined with other technologies, shorten discovery cycles and enable greener, data-driven chemistry.

Conclusion: Community efforts like open benchmarks and explainable interfaces aim to democratize AI-driven molecular innovation while maintaining human control.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to address the underexplored issue of geographic variability in AI alignment, as current approaches often ignore regional differences in norms and realities.
...
Conclusion: The conclusion underscores the urgency of context-sensitive AI alignment to ensure globally acceptable and regionally appropriate AI behavior.
...
<details>
  <summary>Details</summary>
Motivation: Address the urgent need to align AI evaluation practices with emerging regulatory requirements, focusing on systemic risks.

Method: Introduces Bench-2-CoP, a framework using LLM-as-judge analysis to map benchmark questions against the EU AI Act's taxonomy.

Result: Reveals significant gaps, with benchmarks focusing narrowly on behavioral propensities (e.g., hallucination, bias) while neglecting critical functional capabilities (e.g., loss-of-control scenarios).

Conclusion: Highlights the need for policymakers and developers to refine evaluation tools to ensure safer, compliant AI.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the scarcity and bias in ERC datasets by leveraging LLMs for synthetic data generation.

Method: Employing a resource-efficient LLM to synthesize six novel ERC datasets, tailored to supplement three widely used benchmarks.

Result: Generated datasets enhance ERC classifier robustness and yield statistically significant performance improvements.

Conclusion: Synthetic data generation via LLMs effectively addresses ERC dataset limitations and improves model performance.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for improving LLM reasoning are resource-heavy and lack scalability. InfiAlign aims to address this by optimizing data usage and alignment.

Method: InfiAlign integrates SFT with DPO and uses a data selection pipeline to curate high-quality alignment data from open-source datasets.

Result: The SFT model matches DeepSeek-R1-Distill-Qwen-7B performance with 12% of the data. DPO further improves results, especially in math tasks (3.89% gain on AIME benchmarks).
...
<details>
  <summary>Details</summary>
Motivation: Existing RAG methods struggle with structured knowledge like graphs, leading to precision issues and reasoning gaps.

Method: GRAIL combines LLM-guided random exploration, path filtering, and a two-stage training process to optimize graph retrieval and reasoning.

Result: GRAIL improves accuracy by 21.01% and F1 by 22.43% on knowledge graph QA datasets.

Conclusion: GRAIL effectively bridges the gap in handling structured knowledge for retrieval-augmented reasoning, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current evaluation methods overlook detailed reasoning and lack generalizability across domains.
...
Conclusion: The proposed framework offers a general-purpose solution for robust agent evaluation.
...
<details>
  <summary>Details</summary>
Motivation: Reviewing text-heavy LORs is time-consuming; LORI aims to streamline admissions and provide feedback for professional growth.

Method: Uses natural language processing (NLP) and large language models (RoBERTa, LLAMA) to detect leadership attributes like teamwork and communication.
...
Result: AIF offers a unified Bayesian framework for autonomous learning, balancing exploration and exploitation without external rewards.

Conclusion: AIF provides a scalable and principled approach to developing autonomous AI agents that align with human values and constraints.
...
Method: Uses a multi-agent framework with LLMs, including Deep, Surface, Lazy, and General Learner profiles. Tracks learning via weekly knowledge acquisition, monthly strategic choices, tests, and peer interaction over a year.

Result: 1) Deep Learner shows sustained cognitive growth; Surface Learner's shallow knowledge is diagnosed. 2) Behavioral patterns align with psychological profiles. 3) Self-concept evolves realistically. 4) Base LLM defaults to a diligent but brittle Surface Learner.

Conclusion: LearnerAgent effectively simulates human learning, providing insights into LLM behavior and cognitive processes, aligning well with real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Limited interpretability of network alignment models hinders trust and understanding, especially in high-stakes domains.
...
Conclusion: NAEx enhances interpretability of network alignment, addressing trust and understanding challenges.
...
<details>
  <summary>Details</summary>
Motivation: Existing T2I models struggle with complex instructions, fine-grained control, and semantic consistency. LVLMs' cross-modal capabilities inspire the integration for better performance.

Method: LumiGen uses an Intelligent Prompt Parsing & Augmentation (IPPA) module for prompt enhancement and an Iterative Visual Feedback & Refinement (IVFR) module for image correction.

Result: LumiGen achieves a superior score of 3.08 on LongBench-T2I, excelling in text rendering and pose expression.

Conclusion: LVLM integration in LumiGen enhances T2I generation, proving effective for controllable, high-quality image synthesis.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the fragmented and limited tools for simulating missing data, especially for heterogeneous tabular data.

Method: Developed MissMecha, an open-source toolkit with visual diagnostics, MCAR testing, and type-aware imputation metrics.

Result: A unified platform for mechanism-aware missing data studies, supporting mixed-type datasets.

Conclusion: MissMecha aids data quality research, benchmarking, and education by providing comprehensive missing data simulation and analysis tools.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to overcome the computational and privacy challenges of diffusion models in edge-AIGC applications, ensuring efficient and scalable personalized content generation.

Method: The method involves clustering clients based on task similarity, intra-cluster aggregation for personalization, and inter-cluster knowledge interaction. It uses LoRA for fine-tuning and FL for collaboration, with encoded prompts for privacy.
...
Conclusion: The proposed framework effectively balances personalization, efficiency, and privacy in edge-AIGC scenarios, demonstrating its potential for real-world deployment.
...
<details>
  <summary>Details</summary>
Motivation: FSL is critical in fields with limited data, but existing methods struggle with generalization. The study aims to improve FSL using multi-modal models.

Method: Developed M3FD (a diverse few-shot dataset) and M3F (a modular LMMM framework), fine-tuned for FSL tasks.

Result: M3F outperforms conventional meta-learning models, demonstrating feasibility for real-world FSL applications.

Conclusion: The M3FD dataset and M3F framework provide a scalable, unified solution for FSL in data-scarce scientific domains.
...
<details>
  <summary>Details</summary>
Motivation: Existing LLM-based methods for molecular property prediction rely on human-crafted prompts and verbose reasoning, lacking relevance and efficiency.

Method: AttriLens-Mol uses three rewards: format, count, and rationality, to guide LLMs in generating structured, relevant molecular attributes.

Result: The method outperforms supervised fine-tuning and advanced models, improving interpretability and performance.

Conclusion: AttriLens-Mol effectively elicits relevant molecular attributes, enhancing both prediction accuracy and interpretability.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods often ignore textual noise, which degrades performance. PA-RNet addresses this gap by focusing on perturbation-aware modeling.
...
Result: The model shows strong performance across diverse domains, with theoretical guarantees of stability under noise.

Conclusion: PA-RNet effectively mitigates textual noise, enhancing robustness in multimodal time series forecasting.
...
Result: Achieves superior search-time/accuracy trade-off (2 orders of magnitude less data than LIMPQ) and up to 1% accuracy improvement for MobileNetV2/ResNet18 on ImageNet at high compression (14X, 10.66X).

Conclusion: InfoQ offers an efficient, training-free MPQ solution with global sensitivity awareness, outperforming state-of-the-art methods in accuracy and compression.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate if LLMs can replace or complement RL-based DTRs in clinical settings, leveraging their implicit knowledge and zero-shot capabilities.

Method: Compared zero-shot performance of LLMs (e.g., Qwen2.5-7B) against trained small RL agents (SRAs) in a Type 1 diabetes simulator, testing prompts and reasoning approaches.

Result: LLMs matched or outperformed SRAs in stable cases but showed issues like aggressive dosing and reasoning errors (e.g., arithmetic hallucination).

Conclusion: LLMs can integrate into clinical workflows with caution, needing hybrid approaches combining linguistic reasoning and physiological modeling for safety.
...
<details>
  <summary>Details</summary>
Motivation: Current power restoration methods are inequitable, disadvantaging communities with fewer restoration requests.

Method: EPOPR combines Equity-Conformalized Quantile Regression and Spatial-Temporal Attentional RL for equitable decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Existing Federated Recommendation methods struggle with evolving user preferences, while Continual Learning methods assume centralized data access, creating a gap in privacy-preserving, adaptive recommendation systems.

Method: Proposes F3CRec, featuring Adaptive Replay Memory (client-side) and Item-wise Temporal Mean (server-side) to balance knowledge retention and adaptation.

Result: F3CRec outperforms existing methods in maintaining recommendation quality over time in federated settings.

Conclusion: F3CRec successfully bridges the gap between privacy preservation and adaptive learning in recommendation systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing systems prioritize operator revenue, often neglecting passenger and driver satisfaction. HCRide aims to address this imbalance.

Method: Uses a novel multi-agent reinforcement learning algorithm (Habic) with a competition mechanism, dynamic Actor network, and Bi-Critic network.

Result: HCRide improves system efficiency by 2.02%, fairness by 5.39%, and driver preference by 10.21% over baselines.

Conclusion: HCRide successfully balances efficiency, fairness, and driver preference, demonstrating its effectiveness in real-world datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing autoregressive models for event sequences are inefficient and error-prone for long-range predictions.
...
Result: Outperforms autoregressive and diffusion-based baselines in accuracy and efficiency on six benchmarks.
...
Method: Uses a Variational Graph Autoencoder (VGAE) for anomaly detection and a Knowledge-Distilled Graph Attention Network (KD-GAT) for attack classification, encoding CAN bus activity as graph sequences.

Result: Achieves 96% parameter reduction and 16.2% average F1-score improvement, excelling on imbalanced datasets (up to 55% improvement).

Conclusion: The framework enhances CAN security with high accuracy and efficiency, outperforming existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Despite OPTQ's widespread use in reducing memory and compute costs for deep neural networks, it lacks rigorous theoretical guarantees. This paper aims to fill that gap by analyzing quantization error and deriving error bounds.

Method: The authors analyze OPTQ's iterative procedure to quantify error, deriving non-asymptotic 2-norm bounds for deterministic and infinity-norm bounds for stochastic variants. They also extend this analysis to Qronos.

Result: The paper establishes explicit error bounds dependent on calibration data and regularization parameters, validating practical heuristics like feature ordering by norm and providing parameter selection guidance.

Conclusion: The theoretical analysis justifies OPTQ's empirical success and extends to Qronos, offering insights into their performance and practical use.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with low-resource programming languages due to data scarcity and per-language post-training requirements. Agnostics aims to simplify and generalize post-training.

Method: Uses an LLM to rewrite unit tests into I/O format, configures a verifier for compilation/execution, and applies RL with verifiable rewards (RLVR).

Result: Improves Qwen-3 4B to rival larger models, scales to diverse families, and sets new state-of-the-art results on MultiPL-E and LiveCodeBench.

Conclusion: Agnostics simplifies RL post-training for any language, with released datasets and configurations for broader adoption.
...
<details>
  <summary>Details</summary>
Motivation: Current neural operators like FNO rely on Fourier transforms, which assume periodicity and limit their effectiveness for non-stationary or phase-sensitive systems. HNO addresses these limitations by incorporating signal processing insights.
...
Result: HNO is hypothesized to better model operators for causal, phase-sensitive, and non-stationary systems compared to Fourier-based methods.

Conclusion: HNO offers a promising alternative to Fourier-based neural operators, with potential advantages for specific PDE problems due to its signal-processing-inspired design.
...
Conclusion: The GM layer offers a novel approach with unique dynamics, validated through experiments, expanding the toolkit for neural network architectures.
...
<details>
  <summary>Details</summary>
Motivation: Air pollution, particularly surface ozone, is a global hazard, but traditional models fail to capture human-health-relevant scales. Deep learning offers potential but lacks interpretability for decision-making.

Method: An uncertainty-aware U-Net architecture is used with Bayesian and quantile regression to predict MOMO-Chem model's surface ozone residuals, tested in North America and Europe for June 2019.

Result: The study demonstrates regional bias estimation, evaluates uncertainty quantification methods, identifies optimal ground stations for bias correction, and assesses land-use impact on ozone modeling.

Conclusion: The proposed deep learning approach improves surface ozone modeling with interpretability and uncertainty awareness, supporting better policy and public health measures.
...
<details>
  <summary>Details</summary>
Motivation: Air pollution, especially surface ozone, is a major health risk, but current models struggle to predict it accurately at human-relevant scales, limiting policy effectiveness.

Method: A 2D Convolutional Neural Network (CNN) is used to estimate and correct biases in the MOMO-Chem model, incorporating high-resolution satellite land use data.

Result: The CNN-based approach outperforms traditional machine learning in capturing model residuals, with satellite data further enhancing accuracy in North America and Europe.

Conclusion: The technique improves understanding of ozone bias drivers at urban scales, aiding better environmental policy decisions.
...
<details>
  <summary>Details</summary>
Motivation: Accurate water level forecasting is essential for ecosystem management, but current deep learning models struggle with generalization and adaptation in hydrology.

Method: Proposes RAF, a framework that retrieves historically analogous hydrological episodes to enrich model input, comparing similarity-based and mutual information-based methods.

Result: RAF significantly improves forecasting accuracy in real-world Everglades data.

Conclusion: RAF shows promise for hydrology and ecosystem management, encouraging broader use of adaptive AI methods.
...
TL;DR: The paper analyzes replicability in transfer learning, introducing a measure called selection sensitivity ($\Delta_Q$) to quantify trade-offs between adaptation effectiveness and consistency. It shows that replicability failure increases with sensitivity and decreases with sample size, validated through experiments on the MultiNLI corpus. Highly adaptive strategies perform better but are less replicable, while pretraining mitigates failures.


<details>
  <summary>Details</summary>
Motivation: To understand the reliability of transfer learning adaptations, especially with adaptive data selection strategies, and to quantify the trade-off between performance and replicability.

Method: Theoretical framework with selection sensitivity ($\Delta_Q$) and empirical analysis on MultiNLI corpus using six adaptive selection strategies.

Result: Highly adaptive strategies improve performance but increase replicability failure rates (up to 30% reduction with pretraining). Less adaptive methods keep failure rates below 7%.

Conclusion: Provides guidelines for balancing performance and replicability in transfer learning, emphasizing replicability-aware design.
...
<details>
  <summary>Details</summary>
Motivation: The lack of a unified platform for evaluating DAL algorithms under diverse real-world challenges (e.g., distribution shifts, data imbalance) motivated the creation of ALScope.
...
Result: Experiments reveal DAL performance varies by domain and setting, with room for improvement in non-standard scenarios, and trade-offs between performance and selection time.

Conclusion: ALScope provides a comprehensive evaluation platform, highlighting the need for further research to improve DAL algorithms for complex real-world challenges.
...
TL;DR: The paper introduces REINA, a novel loss function for Simultaneous Speech Translation (SimulST) to optimize the tradeoff between translation quality and latency by waiting for more input only when it provides useful information.


<details>
  <summary>Details</summary>
Motivation: SimulST systems struggle with balancing translation quality and latency. The goal is to improve this tradeoff by dynamically adapting to input.

Method: Proposes Regularized Entropy INformation Adaptation (REINA), a loss function derived from information theory, to train an adaptive policy using a non-streaming translation model.

Result: Achieves state-of-the-art streaming results on French, Spanish, and German, with a 21% improvement in latency/quality tradeoff compared to prior methods.
...
TL;DR: SEA is a novel ensemble learning framework that decomposes errors into individual performance and diversity terms, offering precise control over the accuracy-diversity trade-off, outperforming traditional methods like NCL.
...
Result: SEA outperforms baseline methods, provides broader adjustment range, tighter theoretical bounds, and more consistent diversity changes.

Conclusion: SEA offers flexible adjustment, superior performance, and validated tighter bounds, making it a robust framework for ensemble learning.
...
TL;DR: Two compressed decentralized algorithms for nonconvex stochastic optimization are proposed, combining momentum and message-compression for fast convergence and reduced communication costs. One handles bounded gradients, the other data heterogeneity, both achieving optimal convergence rates and superior empirical performance.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of combining momentum acceleration and compressed communication in decentralized optimization while controlling consensus, compression, and momentum gradient errors.

Method: Two algorithms: (1) compressed decentralized adaptive method for bounded gradients, (2) compressed decentralized heavy-ball method with gradient tracking for data heterogeneity. Both use momentum and compression techniques.

Result: Both methods achieve optimal convergence rates, linear speedup, and topology-independent parameters within error tolerance. Outperform state-of-the-art in training DNNs and Transformers.
...
TL;DR: MENDR is a transparent and interpretable EEG foundation model using Riemannian Manifold Transformer and wavelet transforms, achieving near state-of-the-art performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Current EEG foundation models lack transparency and interpretability, hindering clinical integration. MENDR addresses this by leveraging wavelet-based representations and Riemannian geometry.

Method: MENDR uses a Riemannian Manifold Transformer to learn symmetric positive definite matrix embeddings from wavelet-decomposed EEG signals, pretrained on 4,000+ hours of data.

Result: MENDR achieves near state-of-the-art performance in clinical EEG tasks with fewer parameters and enhances interpretability through geometric visualization and signal reconstruction.

Conclusion: MENDR offers an efficient, interpretable, and clinically viable solution for EEG analysis, bridging the gap between performance and transparency.
...
<details>
  <summary>Details</summary>
Motivation: Accurate modeling of complex systems is challenging, requiring innovative approaches to integrate data-driven and Bayesian methods.

Method: RCUKF integrates reservoir computing for data-driven modeling and unscented Kalman filtering for Bayesian estimation, combining prediction and real-time correction.

Result: Demonstrated effectiveness on benchmark problems and real-time vehicle trajectory estimation in simulations.

Conclusion: RCUKF provides a robust solution for modeling complex systems by leveraging data-driven and Bayesian techniques.
...
TL;DR: The paper proposes MMCI, a causal intervention model for multimodal sentiment analysis, addressing spurious correlations by disentangling causal and shortcut features using backdoor adjustment.
...
Result: Experiments on standard datasets and OOD test sets show MMCI suppresses biases and improves performance.

Conclusion: MMCI effectively mitigates spurious correlations, enhancing generalization in multimodal sentiment analysis.
...
TL;DR: R-Zero is an autonomous framework for self-evolving LLMs, eliminating the need for human-curated data by using two models (Challenger and Solver) that co-evolve, improving reasoning capabilities.
...
Result: R-Zero boosts reasoning, e.g., +6.49 on math and +7.54 on general-domain benchmarks for Qwen3-4B-Base.

Conclusion: R-Zero demonstrates scalable, autonomous LLM improvement, advancing toward super-intelligence without human intervention.
...
Method: SPaRFT uses cluster-based data reduction and multi-armed bandits to optimize training data selection dynamically.

Result: SPaRFT matches or outperforms baselines using up to 100x fewer samples.

Conclusion: Performance-driven curricula can unlock LLM reasoning with minimal resources.
...
<details>
  <summary>Details</summary>
Motivation: AR systems can enhance task performance but risk reducing SA in safety-critical scenarios like CPR. The study aims to understand and mitigate this issue.

Method: Developed an AR app for CPR feedback, conducted a user study with simulated incidents, and used eye tracking to analyze SA. Proposed FixGraphPool, a graph neural network, to model SA from gaze data.
...
Conclusion: Eye tracking is valuable for SA modeling in AR, and FixGraphPool shows promise for designing safer AR systems by maintaining user awareness.
...
Method: Uses structured forgetting and meta-learning (via the KNOWN hyper-model) to predict knowledge-enriched weights by reversing controlled forgetting.

Result: KNOW prediction consistently outperforms naive fine-tuning and simple weight prediction, enhancing downstream task performance.

Conclusion: The work reinterprets forgetting dynamics to advance knowledge transfer in deep learning.
...
TL;DR: TANGO is a graph representation learning framework inspired by dynamical systems, using a learned Lyapunov function for stable node feature evolution and incorporating tangential dynamics for flexibility.
...
Method: Uses a learnable Lyapunov function for energy-reducing node embeddings and adds a tangential component via message passing to maintain flexibility.

Result: Achieves strong performance in node and graph classification/regression benchmarks, mitigating oversquashing.

Conclusion: TANGO demonstrates the effectiveness of jointly learning energy functions and tangential flows for graph neural networks.
...
TL;DR: ULU is a new non-monotonic, piecewise activation function that outperforms ReLU and Mish in image tasks. Its adaptive variant, AULU, uses learnable parameters for better flexibility. The LIB metric measures model inductive bias.


<details>
  <summary>Details</summary>
Motivation: To improve activation functions by addressing the limitations of ReLU and Mish, especially for handling positive and negative inputs differently.

Method: Proposes ULU, a piecewise activation function with distinct behaviors for positive and negative inputs. Introduces AULU, an adaptive variant with learnable parameters, and the LIB metric for inductive bias measurement.

Result: ULU significantly outperforms ReLU and Mish in image classification and object detection tasks. AULU further enhances adaptability.

Conclusion: ULU and AULU offer superior performance and flexibility, with the LIB metric providing a quantitative tool for analyzing inductive bias.
...
TL;DR: The paper explores how multimodal imitation learning benefits from statistical learning theory, showing tighter generalization bounds and better optimization landscapes for multimodal policies compared to unimodal ones.


<details>
  <summary>Details</summary>
Motivation: To understand why multimodal architectures (e.g., PerAct, CLIPort) outperform unimodal ones by analyzing their theoretical foundations.

Method: Analyzes multimodal imitation learning using statistical learning theory, focusing on RGB-D, proprioception, and language modalities. Connects empirical results to Rademacher complexity, PAC learning, and information theory.

Result: Properly integrated multimodal policies achieve superior performance with tighter generalization bounds and more favorable optimization landscapes.

Conclusion: Multimodal learning, when properly integrated, offers theoretical and practical advantages over unimodal approaches in imitation learning.
...
TL;DR: The paper introduces Integrated Influence, a data attribution method that addresses limitations of LOO-based methods by incorporating a baseline approach and collective sample influence.


<details>
  <summary>Details</summary>
Motivation: Existing LOO-based data attribution methods lack collective influence analysis and counterfactual explanations, limiting their flexibility and reliability.

Method: Proposes Integrated Influence, which uses a baseline dataset and a degeneration process to accumulate sample influences, theoretically unifying existing methods like influence functions.

Result: Integrated Influence outperforms existing methods in data attribution and mislabeled example identification tasks.

Conclusion: The method enhances transparency and reliability in data attribution by addressing collective influence and baseline integration.
...
<details>
  <summary>Details</summary>
Motivation: The cold-start problem in preference learning, especially in data-scarce domains like computational social systems and economic analysis, motivates the need for an efficient solution.

Method: The method combines PCA-based self-supervised pre-training to generate pseudo-labels and an active learning loop with a noisy oracle for refinement.
...
Conclusion: The proposed framework effectively mitigates the cold-start problem, improving preference learning in data-constrained settings.
...
TL;DR: A novel Weakly Supervised Learning (WSL) framework leverages multiple weak supervision signals (similarity-confidence and confidence-difference) to improve classification when labeled data is limited. It introduces unbiased risk estimators and a risk correction approach, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Accurate labeling is often impractical, and existing WSL methods rely on single weak supervision signals, limiting their effectiveness.

Method: Proposes SconfConfDiff Classification, integrating two weak labels (similarity-confidence and confidence-difference) and deriving unbiased risk estimators. Includes a risk correction approach to prevent overfitting.

Result: The method achieves optimal convergence rates and outperforms baselines in experiments, showing robustness against label noise and inaccurate class priors.

Conclusion: The framework effectively combines multiple weak supervision signals, offering a practical solution for limited labeled data scenarios.
...
TL;DR: A novel reinforcement learning framework improves function calling in LLMs by addressing exploration, reasoning, and parameter extraction challenges, achieving 86.02% accuracy.
...
Method: The framework uses strategic entropy-based exploration and a two-stage data preparation pipeline (LLM evaluation and AST validation) to enhance group relative policy optimization.

Result: Achieves 86.02% overall accuracy on the Berkeley Function Calling Leaderboard, outperforming standard GRPO by 6% in complex tasks, especially with code-pretrained models.

Conclusion: The method significantly improves function calling performance, suggesting structured language generation as a strong foundation for reinforcement learning in such tasks. Code and models will be released.
...
TL;DR: HFedATM, a hierarchical aggregation method, addresses domain shift in Hierarchical Federated Learning (HFL) by aligning models and merging them efficiently, improving performance and convergence.


<details>
  <summary>Details</summary>
Motivation: Conventional FL and HFL face scalability and domain shift issues, reducing model performance on unseen data. FedDG methods exist but lack integration into HFL.
...
Conclusion: HFedATM effectively tackles domain shift in HFL, enhancing performance and stability.
...
<details>
  <summary>Details</summary>
Motivation: To demonstrate the superior approximation capabilities of deep networks over traditional numerical methods for PDEs.

Method: Analyzes deep fully-connected networks with general activation functions in Sobolev spaces, comparing errors in $W^{m,p}$-norm for $m < n$.

Result: Deep networks achieve super-convergence, surpassing finite element and spectral methods in accuracy.

Conclusion: This work provides a unified theoretical foundation for neural-network-based PDE solutions, closing a gap in error-estimation theory.
...
### [79] [Domain-driven Metrics for Reinforcement Learning: A Case Study on Epidemic Control using Agent-based Simulation](https://arxiv.org/abs/2508.05154)
...
<details>
  <summary>Details</summary>
Motivation: The complexity and stochasticity of ABMs/RABMs, along with the absence of standardized metrics, make performance evaluation of RL-based models difficult.

Method: Develop domain-driven RL metrics, building on existing state-of-the-art metrics, and demonstrate them using policy optimization in a rational ABM disease modeling case study.

Result: Domain-driven rewards, combined with traditional and advanced metrics, are shown effective in various simulation scenarios, such as differential mask availability.

Conclusion: Domain-driven metrics enhance the evaluation of RL-based ABMs/RABMs, providing a more tailored and effective approach for complex, stochastic systems.
...
### [80] [PSEO: Optimizing Post-hoc Stacking Ensemble Through Hyperparameter Tuning](https://arxiv.org/abs/2508.05144)
*Beicheng Xu,Wei Liu,Keyao Ding,Yupeng Lu,Bin Cui*

Main category: cs.LG

TL;DR: PSEO is a framework for optimizing post-hoc stacking ensembles in AutoML, outperforming existing methods by adapting to task characteristics and leveraging multi-layer stacking.


<details>
  <summary>Details</summary>
Motivation: Current CASH methods in AutoML use fixed strategies for ensembles, limiting adaptability. PSEO aims to improve this by optimizing ensemble strategies.

Method: PSEO selects base models via binary quadratic programming (balancing diversity and performance) and optimizes multi-layer stacking with two new mechanisms. It searches for the optimal ensemble strategy in a hyperparameter space.

Result: PSEO achieved the best average test rank (2.96) on 80 datasets, surpassing 16 other methods, including state-of-the-art AutoML and ensemble techniques.

Conclusion: PSEO effectively addresses the limitations of fixed ensemble strategies in AutoML, demonstrating superior performance through adaptive optimization.
...
TL;DR: HIA (Heuristic-Guided Inference-time Alignment) is a tuning-free, black-box-compatible method that balances alignment quality and computational cost by using lightweight prompt optimization and heuristic reward models.


<details>
  <summary>Details</summary>
Motivation: Aligning LLMs with user preferences is costly in terms of fine-tuning or inference, creating a trade-off between alignment quality and computational expense. Existing methods overlook this balance.
...
### [82] [FAITH: A Framework for Assessing Intrinsic Tabular Hallucinations in finance](https://arxiv.org/abs/2508.05201)
*Mengao Zhang,Jiayu Fu,Tanya Warrier,Yuwen Wang,Tianhui Tan,Ke-wei Huang*

Main category: cs.LG

TL;DR: A framework for evaluating intrinsic hallucinations in financial LLMs, using masked span prediction on real-world financial data, with contributions including dataset creation, evaluation, and analysis.


<details>
  <summary>Details</summary>
Motivation: Addressing the critical challenge of hallucination in LLMs for finance, where minor numerical errors can impact decision-making and compliance.

Method: Developed a context-aware masked span prediction task using real-world financial documents, including automated dataset creation and evaluation on S&P 500 reports.

Result: A robust methodology for evaluating LLMs in finance, identifying intrinsic hallucination patterns in state-of-the-art models.

Conclusion: The framework advances trustworthy financial Generative AI by providing scalable evaluation tools and insights into hallucination issues.
...
### [83] [pFedDSH: Enabling Knowledge Transfer in Personalized Federated Learning through Data-free Sub-Hypernetwork](https://arxiv.org/abs/2508.05157)
...
<details>
  <summary>Details</summary>
Motivation: Addressing the gap in Personalized Federated Learning (pFL) where existing methods assume static client participation, ignoring real-world dynamic scenarios with new clients joining incrementally.

Method: Proposes pFedDSH, leveraging a central hypernetwork to generate personalized models via embeddings, batch-specific masks for knowledge stability, and data-free replay for backward transfer.

Result: pFedDSH outperforms state-of-the-art pFL and Federated Continual Learning baselines on CIFAR-10, CIFAR-100, and Tiny-ImageNet, showing robust performance stability and adaptation.

Conclusion: pFedDSH effectively handles dynamic client onboarding, preserving privacy and performance while enabling efficient knowledge transfer.
...
### [84] [Advanced Hybrid Transformer LSTM Technique with Attention and TS Mixer for Drilling Rate of Penetration Prediction](https://arxiv.org/abs/2508.05210)
*Saddam Hussain Khan*

Main category: cs.LG

TL;DR: A hybrid deep learning model combining LSTM, Transformer encoders, TS-Mixer, and attention mechanisms outperforms benchmarks for accurate ROP prediction in drilling operations.
...
Result: Achieves an R-squared score of 0.9988 and MAPE of 1.447%, outperforming standalone models.
...
### [85] [S$^2$M-Former: Spiking Symmetric Mixing Branchformer for Brain Auditory Attention Detection](https://arxiv.org/abs/2508.05164)
...
TL;DR: A hybrid LSTM-RF model improves marine chlorophyll concentration prediction by combining time-series and nonlinear feature strengths, outperforming standalone LSTM and RF models.


<details>
  <summary>Details</summary>
Motivation: Accurate prediction of marine chlorophyll concentration is vital for ecosystem health monitoring and red tide warnings, addressing limitations of single-model approaches.

Method: The LSTM-RF hybrid model leverages multi-source ocean data (temperature, salinity, dissolved oxygen) with standardized treatment and sliding window techniques for enhanced accuracy.

Result: The model achieved R^2=0.5386, MSE=0.005806, and MAE=0.057147, significantly outperforming standalone LSTM (R^2=0.0208) and RF (R^2=0.4934).

Conclusion: The LSTM-RF hybrid model offers an innovative solution for high-frequency marine ecological variable prediction, improving accuracy and reliability.
...
### [87] [FlowState: Sampling Rate Invariant Time Series Forecasting](https://arxiv.org/abs/2508.05287)
*Lars Graf,Thomas Ortner,Stanisław Woźniak,Angeliki Pantazi*

Main category: cs.LG

TL;DR: FlowState is a novel time series foundation model (TSFM) using state space model (SSM) and functional basis decoder, outperforming existing models in generalization, adaptability, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing TSFMs struggle with generalization, adaptability to sampling rates, and computational inefficiency.

Method: FlowState combines an SSM-based encoder and functional basis decoder for continuous-time modeling and dynamic time-scale adjustment.

Result: FlowState achieves state-of-the-art performance on GIFT-ZS and Chronos-ZS benchmarks, with smaller models and reduced data needs.

Conclusion: FlowState's design and pretraining strategy enable superior generalization, adaptability, and efficiency in time series forecasting.
...
### [88] [Near Optimal Inference for the Best-Performing Algorithm](https://arxiv.org/abs/2508.05173)
*Amichai Painsky*

Main category: cs.LG

TL;DR: The paper introduces a framework for selecting the best-performing machine learning algorithm from a benchmark, addressing it as a subset selection problem for multinomial distributions.


<details>
  <summary>Details</summary>
Motivation: To identify the most likely top-performing algorithm on unseen datasets, especially when performance differences are marginal.

Method: Formulates the problem as subset selection for multinomial distributions, proposing asymptotic and finite-sample schemes.

Result: The proposed schemes significantly outperform existing methods, with matching lower bounds confirming their effectiveness.

Conclusion: The framework provides a robust solution for selecting top-performing algorithms with high confidence.
...
### [89] [ASkDAgger: Active Skill-level Data Aggregation for Interactive Imitation Learning](https://arxiv.org/abs/2508.05310)
...
Result: ASkDAgger reduces query frequency, annotation needs, improves generalization, and speeds domain adaptation, validated in simulation and real-world tasks.

Conclusion: ASkDAgger effectively balances query efficiency and learning performance, advancing interactive imitation learning.
...
### [90] [Human Activity Recognition from Smartphone Sensor Data for Clinical Trials](https://arxiv.org/abs/2508.05175)
...
TL;DR: A ResNet-based HAR model detects gait and everyday activities with high accuracy and robustness across various smartphone wear locations, outperforming state-of-the-art models.


<details>
  <summary>Details</summary>
Motivation: To develop a lightweight HAR model for detecting gait and everyday activities in healthy individuals and people with multiple sclerosis (PwMS), ensuring practical applicability across different smartphone wear locations.
...
Conclusion: The HAR model is accurate, robust, and practical for real-world use in activity recognition.
...
### [91] [Optimal Corpus Aware Training for Neural Machine Translation](https://arxiv.org/abs/2508.05364)
...
Result: +3.6 and +1.8 chrF improvement in WMT23 tasks; on-par or better than other fine-tuning techniques.

Conclusion: OCAT is lightweight, effective, and less sensitive to hyperparameters.
...
### [92] [Physics-Informed Time-Integrated DeepONet: Temporal Tangent Space Operator Learning for High-Accuracy Inference](https://arxiv.org/abs/2508.05190)
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods (FR and AR) fail in long-term PDE modeling due to poor generalization and error accumulation.

Method: PITI-DeepONet uses a dual-output architecture trained with physics-informed objectives, integrating time-derivative operators for stable evolution.
...
Conclusion: PITI-DeepONet offers more reliable long-term PDE integration, outperforming traditional approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current RL systems violate SPMD assumptions by mixing inference and training workloads on the same GPU cluster, leading to inefficiencies.
...
Result: Echo matches co-located baselines in convergence and reward while using decentralized edge hardware.
...
### [94] [Tail-Risk-Safe Monte Carlo Tree Search under PAC-Level Guarantees](https://arxiv.org/abs/2508.05441)
*Zuyuan Zhang,Arnob Ghosh,Tian Lan*

Main category: cs.LG

TL;DR: The paper introduces two safety-aware MCTS methods, CVaR-MCTS and W-MCTS, to address tail-risk in decision-making, providing rigorous guarantees and outperforming baselines.
...
Method: Proposes CVaR-MCTS for tail-risk control using Conditional Value-at-Risk and W-MCTS for addressing estimation bias with Wasserstein ambiguity sets.

Result: Both methods demonstrate robust tail-risk guarantees, improved rewards, and stability in simulations.

Conclusion: The proposed methods effectively address tail-risk in MCTS, offering safety guarantees and performance improvements.
...
### [95] [Bidding-Aware Retrieval for Multi-Stage Consistency in Online Advertising](https://arxiv.org/abs/2508.05206)
...
Method: BAR uses Bidding-Aware Modeling (monotonicity-constrained learning and multi-task distillation) and Asynchronous Near-Line Inference for real-time updates. A Task-Attentive Refinement module enhances feature interactions.

Result: BAR increased platform revenue by 4.32% and impressions by 22.2% for positively-operated ads.

Conclusion: BAR effectively addresses multi-stage inconsistency in ad systems, improving both revenue and advertiser outcomes.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with multi-scale dynamics and irregular data in energy forecasting.

Method: Proposes EnergyPatchTST, featuring multi-scale feature extraction, probability prediction, future variable integration, and pre-training/fine-tuning.

Result: Outperforms other methods, reducing prediction errors by 7-12% and providing reliable uncertainty estimates.
...
### [97] [Task complexity shapes internal representations and robustness in neural networks](https://arxiv.org/abs/2508.05463)
*Robert Jankowski,Filippo Radicchi,M. Ángeles Serrano,Marián Boguñá,Santo Fortunato*

Main category: cs.LG

TL;DR: The paper introduces five data-agnostic probes to study how task difficulty affects the topology and robustness of neural network representations, revealing insights into model compression and interpretability.


<details>
  <summary>Details</summary>
Motivation: Understanding how neural networks' internal representations are shaped by input data complexity and task difficulty, given their black-box nature.

Method: Uses five probes (pruning, binarization, noise injection, sign flipping, bipartite network randomization) on MLPs represented as bipartite graphs, tested on MNIST and Fashion-MNIST datasets.

Result: Hard-task models collapse under binarization, while easy-task models remain robust. Pruning reveals phase transitions, noise injection can improve accuracy, and sign structure preservation maintains performance.

Conclusion: The study defines a task-complexity measure and highlights the importance of signed bipartite topology, offering practical strategies for model compression and interpretability.
...
### [98] [DFW: A Novel Weighting Scheme for Covariate Balancing and Treatment Effect Estimation](https://arxiv.org/abs/2508.05215)
*Ahmad Saeed Khan,Erik Schaffernicht,Johannes Andreas Stork*

Main category: cs.LG

TL;DR: Proposes Deconfounding Factor Weighting (DFW), a novel method to improve causal effect estimation by stabilizing weights and enhancing covariate balance.


<details>
  <summary>Details</summary>
Motivation: Addresses instability in propensity score-based weighting methods like IPW, which suffer from high variance and degraded performance due to data or model issues.

Method: DFW leverages a deconfounding factor to construct stable weights, prioritizing less confounded samples and bounding weights to improve balance.

Result: DFW outperforms IPW and CBPS in covariate balancing and treatment effect estimation on benchmark and synthetic datasets.

Conclusion: DFW provides a robust alternative to traditional propensity score methods, extending naturally to multi-treatment settings.
...
TL;DR: Predicting older adults' SPPB scores at a 4-year horizon using ML models, with XGBoost achieving the best performance (MAE: 0.79). Feature selection improved results (MAE: 0.82).


<details>
  <summary>Details</summary>
Motivation: Early intervention is crucial to slow physical deterioration in older adults. Predicting SPPB scores can help identify at-risk individuals.
...
Result: XGBoost performed best (MAE: 0.79). Feature selection reduced error slightly (MAE: 0.82).

Conclusion: XGBoost is effective for predicting SPPB scores, and feature selection can optimize performance.
...
### [100] [Tractable Sharpness-Aware Learning of Probabilistic Circuits](https://arxiv.org/abs/2508.05537)
*Hrithik Suresh,Sahil Sidheekh,Vishnu Shreeram M. P,Sriraam Natarajan,Narayanan C. Krishnan*

Main category: cs.LG

TL;DR: The paper addresses overfitting in Probabilistic Circuits (PCs) by proposing a Hessian-based regularizer, improving generalization by guiding PCs to flatter minima.


<details>
  <summary>Details</summary>
Motivation: Overfitting in PCs due to sharp optima in log-likelihood landscapes motivates the need for a regularization method.

Method: A Hessian-based regularizer is introduced, leveraging the efficient computation of the Hessian trace in PCs to guide training toward flatter minima.

Result: Experiments show the method improves generalization by avoiding sharp optima.
...
### [101] [Don't Reach for the Stars: Rethinking Topology for Resilient Federated Learning](https://arxiv.org/abs/2508.05224)
...
Method: Uses agreement scores on local validation sets to select trustworthy updates, with regularization for stability.

Result: Outperforms centralized and existing P2P FL methods, especially in adversarial and heterogeneous settings.

Conclusion: LIGHTYEAR offers a robust, personalized solution for decentralized FL, improving client-level performance.
...
### [102] [Adapting Vision-Language Models Without Labels: A Comprehensive Survey](https://arxiv.org/abs/2508.05547)
...
Method: Proposes a taxonomy categorizing approaches into four paradigms: Data-Free Transfer, Unsupervised Domain Transfer, Episodic Test-Time Adaptation, and Online Test-Time Adaptation. Analyzes methodologies and benchmarks.

Result: A structured overview of unsupervised VLM adaptation, highlighting methodologies, benchmarks, and open challenges.
...
### [103] [Cross-LoRA: A Data-Free LoRA Transfer Framework across Heterogeneous LLMs](https://arxiv.org/abs/2508.05232)
*Feifan Xia,Mingyang Liao,Yuyang Fang,Defang Li,Yantong Xie,Weikang Li,Yang Li,Deguo Xia,Jizhou Huang*

Main category: cs.LG

TL;DR: Cross-LoRA is a data-free framework for transferring LoRA modules between diverse LLMs, using LoRA-Align and LoRA-Shift to ensure compatibility and performance.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitation of traditional PEFT methods like LoRA, which are tightly coupled with base model architectures, restricting their use across heterogeneous LLMs.

Method: Cross-LoRA uses LoRA-Align (subspace alignment via SVD and linear transformation) and LoRA-Shift (projecting aligned subspaces). Both are data-free and training-free.
...
### [104] [Iterative Learning of Computable Phenotypes for Treatment Resistant Hypertension using Large Language Models](https://arxiv.org/abs/2508.05581)
...
<details>
  <summary>Details</summary>
Motivation: To explore LLMs' potential for generating accurate and concise CPs for clinical phenotypes, aiming to improve hypertension care.
...
Conclusion: LLMs are promising for scalable CP generation, offering interpretability and efficiency in clinical decision support.
...
### [105] [MoBE: Mixture-of-Basis-Experts for Compressing MoE-based LLMs](https://arxiv.org/abs/2508.05257)
...
<details>
  <summary>Details</summary>
Motivation: Large MoE-based LLMs face deployment challenges due to high memory requirements, and current compression methods lead to significant accuracy drops.

Method: MoBE decomposes expert matrices via rank decomposition, re-parameterizing larger matrices as shared basis combinations, minimizing reconstruction error.

Result: MoBE reduces parameters by 24%-30% with only 1%-2% accuracy drop, significantly better than prior methods.

Conclusion: MoBE offers an effective solution for compressing MoE models with minimal performance degradation.
...
<details>
  <summary>Details</summary>
Motivation: Current RL pipelines for MLLMs suffer from inefficiencies due to Advantage Collapsing and Rollout Silencing, hindering learning.
...
TL;DR: TrajEvo is a framework using LLMs and evolutionary algorithms to design trajectory prediction heuristics, outperforming traditional and deep learning methods, especially in OOD scenarios.
...
Result: TrajEvo outperforms existing heuristic and deep learning methods, particularly in generalizing to unseen OOD datasets.

Conclusion: TrajEvo advances automated design of fast, explainable, and generalizable trajectory prediction heuristics.
...
TL;DR: The paper proposes a reinforcement learning fine-tuning method (RLHF) for conversational recommender systems (CRS) using LLMs, leveraging implicit user feedback to improve recommendation accuracy and user satisfaction.


<details>
  <summary>Details</summary>
Motivation: Traditional supervised fine-tuning fails to capture implicit user feedback signals like dwell time or sentiment, limiting the effectiveness of CRS.

Method: Uses RLHF with a reward model trained on weakly-labeled engagement data and optimizes the LLM via proximal policy optimization (PPO) to model conversational state transitions.

Result: RLHF-fine-tuned models outperform others in top-k recommendation accuracy, coherence, and user satisfaction on synthetic and real-world datasets.
...
TL;DR: Optimal batch-size and learning-rate schedules improve training efficiency and convergence in deep learning, balancing computational cost and performance.


<details>
  <summary>Details</summary>
Motivation: Addressing computational bottlenecks in deep learning by optimizing batch-size and learning-rate scheduling to enhance training efficiency without compromising generalization.

Method: Theoretical analysis using stochastic first-order oracle (SFO) complexity to derive optimal growth schedules for batch size and learning rate, validated through experiments.

Result: Derived optimal schedules reduce SFO complexity, improving scalability and efficiency in large-batch training.

Conclusion: Provides theoretical and practical guidelines for efficient large-batch training in deep learning.
...
<details>
  <summary>Details</summary>
Motivation: Mini-batch SGD's convergence is sensitive to batch size and learning rate settings, prompting a need for adaptive strategies.

Method: An adaptive scheduling strategy adjusts batch size and learning rate using observed gradient norm decay during training.

Result: Experiments showed the adaptive joint scheduler outperformed existing schedulers in convergence speed.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenges of SSCL by balancing unlabeled learning, memory stability, and learning plasticity, which previous methods tackled in isolation.
...
Conclusion: USP effectively addresses SSCL challenges by synergistically enhancing key aspects, validated by superior performance.
...
TL;DR: The paper proposes relaxing the assumptions of latent bandits to focus on preference ordering of actions in each latent state, improving performance when reward scales vary.


<details>
  <summary>Details</summary>
Motivation: Learning from scratch is costly for personalization tasks with limited decision points, and existing latent bandit models may not account for varying reward scales among individuals.

Method: A posterior-sampling algorithm is introduced, requiring only a model of action preference orderings in latent states, not full reward distributions.

Result: The algorithm performs competitively with full-knowledge latent bandits when reward distributions are well-specified and outperforms them when reward scales differ.

Conclusion: Relaxing latent bandit assumptions to focus on preference orderings enhances adaptability and performance in scenarios with varying reward scales.
...
TL;DR: NT-ML, a novel defense mechanism, combats backdoor attacks in DNNs by combining non-target label training and mutual learning, outperforming existing defenses.
...
Method: NT-ML involves retraining models with non-target labels and mutual learning between teacher and student models to purify predictions.

Result: NT-ML effectively defends against 6 backdoor attacks with minimal clean samples and surpasses 5 state-of-the-art defenses.

Conclusion: NT-ML is a promising solution for mitigating backdoor attacks in DNNs, demonstrating superior performance and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To investigate the utility of adaptive learning rate mechanisms in deep learning, especially for dynamically adjusting step sizes based on the loss landscape.

Method: Revisits a cumulative path-based adaptation scheme, identifies flaws in its application to Adam, proposes a corrected variant, and benchmarks it against SGD, Adam, and a recent alternative method.

Result: The corrected variant better reflects Adam's update dynamics, and benchmarking clarifies when adaptive strategies are beneficial.

Conclusion: The study provides insights into the practical benefits of adaptive learning rate mechanisms, particularly for Adam, and offers a corrected approach for improved performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for text-conditioned molecular generation struggle with balancing quality, diversity, and fast inference.

Method: Proposes a Causality-Aware Transformer (CAT) for joint encoding and a Variational Mean Flow (VMF) framework for efficient, high-quality generation.

Result: Achieves higher novelty (74.5%), diversity (70.3%), and 100% validity, with computational efficiency (1-5 NFEs).

Conclusion: The framework advances molecular generation by ensuring quality, diversity, and efficiency simultaneously.
...
TL;DR: The paper explores Echo State Networks (ESNs) for forecasting cryptocurrency prices, especially during high volatility, outperforming other methods.


<details>
  <summary>Details</summary>
Motivation: Forecasting volatile and non-stationary stock and cryptocurrency prices is challenging, and ESNs show promise for capturing nonlinear patterns.

Method: The study uses ESNs for cryptocurrency forecasting and includes chaos analysis via the Lyapunov exponent during extreme volatility.

Result: ESNs outperform existing machine learning methods, particularly during chaotic periods, aligning with Lyapunov exponent analysis.

Conclusion: ESNs are robust and effective for cryptocurrency forecasting in high-volatility scenarios, surpassing traditional methods.
...
TL;DR: NegBio-VAE extends VAEs by using a negative binomial distribution for spike counts, improving neural activity modeling with better dispersion control and reconstruction fidelity.
...
Method: Introduces NegBio-VAE with negative binomial distribution, two ELBO optimization schemes, and differentiable reparameterization strategies for dispersion control.

Result: Empirical results show significant gains in reconstruction fidelity by modeling overdispersion.

Conclusion: NegBio-VAE's minor yet impactful change underscores the importance of accurate dispersion modeling in neural representations.
...
<details>
  <summary>Details</summary>
Motivation: Address the under-serving of minority clients in FL and the lack of client-wise Pareto optimality in existing methods like FMOL.

Method: Uses local federated multi-gradient descent averaging (FMGDA/FSMGDA) and a cone-constrained Pareto-MTL sub-problem to enforce client-wise Pareto optimality.
...
TL;DR: The paper introduces Group Causal Policy Optimization (GCPO), a method that improves upon GRPO by incorporating causal analysis to address overlooked semantic interactions among candidate responses.
...
Method: The authors propose GCPO, which uses a Structural Causal Model (SCM) to reveal hidden dependencies and integrates causal insights into optimization via reward adjustment and KL regularization.

Result: GCPO outperforms GRPO and other methods across multiple reasoning benchmarks.

Conclusion: Incorporating causal structure into policy optimization enhances performance in specialized domains.
...
TL;DR: MLES introduces a programmatic policy discovery method using multimodal LLMs and evolutionary search, achieving PPO-level performance with interpretability.


<details>
  <summary>Details</summary>
Motivation: Address the dual challenges of high performance and interpretability in control policies, which are critical for safety-critical tasks but often lack transparency in deep reinforcement learning.

Method: Combines multimodal large language models as policy generators with evolutionary mechanisms and visual feedback-driven behavior analysis for policy optimization.

Result: MLES matches PPO in performance and efficiency across control tasks while providing transparent logic and traceable design.

Conclusion: MLES is a scalable, interpretable approach for control policy discovery, overcoming domain-specific language limitations and enabling knowledge transfer.
...
TL;DR: The paper highlights the bias and fairness issues caused by treating competing risks as censoring in survival analysis, demonstrating its impact on accuracy and disparities.
...
Method: The authors formalize the problem, quantify the error in survival estimates, and develop a framework to assess its impact on predictive performance and fairness. They also analyze group-specific errors due to differing risk profiles.

Result: Empirical analysis shows that ignoring competing risks disproportionately affects high-risk individuals, worsening inequities. The study quantifies the error and its fairness implications.

Conclusion: Practitioners must account for competing risks in survival models to improve accuracy, reduce disparities, and inform better decisions.
...
Method: Leverages f-mutual information measures, proving their uniqueness under natural conditions, and evaluates them empirically across diverse domains.

Result: Achieves perfect discrimination between faithful and strategic agents, with 10-100x better robustness to adversarial manipulation than current methods. Performance peaks at a 10:1 compression ratio.

Conclusion: The proposed mechanisms are highly effective for AI evaluation without ground truth, offering superior robustness and performance under optimal conditions.
...
Method: A multi-task recurrent neural network jointly models inter-observation time, missingness, and survival outcomes, addressing clinical presence shift in new settings.

Result: The method improves performance and transportability in a mortality prediction task using the MIMIC-III dataset, outperforming models ignoring clinical presence.

Conclusion: Incorporating clinical presence enhances prediction model performance and transportability, crucial for deploying models in diverse healthcare settings.
...
Method: The proposed algorithms replace fuzzification parameters with entropy regularization and SNR-based feature weighting ($\delta_j^h = \frac{\bar{x}_j^h}{(\sigma_j^h)^2}$). AAMVFCM-U adds hierarchical dimensionality reduction via adaptive thresholding ($\theta^{h^{(t)}} = \frac{d_h^{(t)}}{n}$).

Result: Evaluation shows superiority over 15 benchmarks, with AAMVFCM-U achieving 97% computational efficiency, reducing dimensionality to 0.45% of original size, and identifying optimal view combinations.

Conclusion: The proposed framework effectively addresses multi-view clustering challenges, offering adaptive, parameter-free solutions with strong performance and efficiency.
...
TL;DR: X-VFL is a new Vertical Federated Learning framework addressing non-aligned data and missing features, enabling local inference with novel modules (XCom and DS-Align) and showing superior performance.


<details>
  <summary>Details</summary>
Motivation: Current VFL requires perfectly aligned data and joint inference, limiting practicality. X-VFL aims to overcome these challenges.

Method: Proposes X-VFL with Cross Completion (XCom) for missing features and Decision Subspace Alignment (DS-Align) for local inference. Provides convergence theorems for training algorithms.

Result: Achieves 15% accuracy improvement on CIFAR-10 and 43% on MIMIC-III, outperforming existing methods.

Conclusion: X-VFL is effective for scenarios with missing features and local inference, validated by experiments.
...
<details>
  <summary>Details</summary>
Motivation: Current QAT methods focus on minimizing quantization error without surpassing the full-precision accuracy ceiling. This work aims to break this ceiling by improving full-precision accuracy and then quantizing efficiently.
...
TL;DR: The paper introduces an extension for PyKEEN, integrating advanced negative samplers to improve knowledge graph embedding performance.


<details>
  <summary>Details</summary>
Motivation: Existing KGE libraries lack advanced negative sampling strategies, limiting performance and customization.

Method: Developed a modular extension for PyKEEN with static and dynamic negative samplers.

Result: Enhanced PyKEEN's capabilities and provided empirical insights for better negative sampling strategies.

Conclusion: The extension improves embedding methods and offers flexibility for future KGE research.
...
<details>
  <summary>Details</summary>
Motivation: The rise in IoT-related cyberattacks necessitates improved intrusion detection methods.
...
Result: KANs achieve competitive accuracy and superior interpretability for IoT intrusion detection.

Conclusion: KANs are a promising alternative for secure and interpretable IoT intrusion detection.
...
TL;DR: The paper explores the 'one-poison hypothesis,' showing that a single poison sample with limited background knowledge can inject a backdoor into machine learning models without affecting benign task performance.
...
Method: Theoretical proof for linear regression and classification, leveraging unused data distribution directions for poison samples, and experimental validation on benchmark datasets.

Result: Demonstrates zero backdooring-error and minimal impact on benign task performance, validating the one-poison hypothesis.

Conclusion: The one-poison hypothesis is viable, with implications for understanding and mitigating backdoor attacks in machine learning.
...
TL;DR: Dynamic Fine-Tuning (DFT) improves SFT for LLMs by dynamically rescaling gradients, enhancing generalization and outperforming standard SFT and RL in benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard SFT's limited generalization compared to RL due to problematic reward structures in gradients.

Method: Proposes DFT, dynamically rescaling the objective function with token probabilities to stabilize gradient updates.
...
TL;DR: BTPG-max improves upon Bidirectional TPG (BTPG) by finding more bidirectional pairs, enhancing robustness to delays in Multi-Agent Path Finding (MAPF).
...
Method: Proposes BTPG-max, a locally optimal algorithm that maximizes bidirectional pairs in Temporal Plan Graphs (TPG).

Result: BTPG-max constructs BTPGs with more bidirectional edges, better anytime behavior, and improved delay robustness.

Conclusion: BTPG-max advances MAPF solutions by optimizing bidirectional dependencies for real-world execution.
...
TL;DR: The paper introduces congestion mitigation path planning (CMPP) to reduce local congestion in high-density multi-agent systems by embedding congestion into the cost function. Two solvers are developed, and empirical results show improved throughput.


<details>
  <summary>Details</summary>
Motivation: To address congestion in high-density environments where autonomous agents move simultaneously, ensuring efficient navigation.

Method: CMPP embeds congestion into the cost function using flow-based penalties. Two solvers are developed: an exact solver for small instances and a scalable two-layer search algorithm (A-CMTS) for large-scale instances.

Result: CMPP significantly reduces local congestion and enhances system throughput in both discrete- and continuous-space scenarios.

Conclusion: CMPP improves multi-agent system performance, making it suitable for real-world applications like logistics and autonomous-vehicle operations.

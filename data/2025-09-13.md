...
Method: Created an interval type-2 version of Bayes Theorem with a conservative method to prevent input inconsistencies, and developed a novel algorithm for encoding SME-provided intervals into interval type-2 fuzzy membership functions.

Result: Extended Bayes Theorem to handle interval uncertainty while maintaining validity of output results, providing a more flexible approach for encoding expert interval estimates into formal mathematical representations.

Conclusion: The proposed interval type-2 Bayesian framework better accommodates real-world uncertainty in expert-provided probability estimates and generalizes previous work on interval encoding for Computing with Words applications.
...
<details>
  <summary>Details</summary>
Motivation: To streamline the transition from game design to implementation by automating the creation of functional Unity game prototypes from design documentation, addressing gaps in AI-assisted game development.

Method: End-to-end system combining fine-tuned LLaMA-3 model specialized for Unity code generation with custom Unity integration package to parse GDDs, extract specifications, and synthesize Unity-compatible C# code.
...
Conclusion: The system effectively positions LLMs as valuable tools for automating game development workflow from design to implementation, demonstrating practical utility in generating functional game templates.
...
<details>
  <summary>Details</summary>
Motivation: Natural language descriptions of optimization problems are difficult to translate into correct MiniZinc models due to the need for both logical reasoning and constraint programming expertise, creating a barrier for non-experts.

Method: Multiple specialized LLM agents each focus on detecting and generating code for specific global constraint types, with a final assembler agent integrating all constraint snippets into a complete MiniZinc model.

Result: Initial experiments with various LLMs demonstrate better performance compared to baseline approaches like one-shot prompting and chain-of-thought prompting.

Conclusion: The agentic approach effectively decomposes complex modeling tasks into simpler sub-tasks, reducing overall complexity and showing promise for future enhancements in automated constraint modeling.
...
Method: Identifies model overconfidence as key driver of collapse, proposes confidence-aware loss function (TCE) that downweights high-confidence predictions, provides model-agnostic framework linking loss design to collapse mitigation.

Result: TCE significantly delays model collapse, extends model's fidelity interval before collapse by more than 2.3x, and generalizes across different modalities.
...
<details>
  <summary>Details</summary>
Motivation: While XAI is well-studied in some areas, uncertainty explanations and global explanations are often overlooked. The research aims to address this gap by developing guidelines for XAI schemes and testing algorithms that provide intuitive visual understanding despite complexity.

Method: The researchers selected an algorithm that simultaneously covers multiple XAI concepts (uncertainty, robustness, global XAI) and tested its ability to calibrate user trust. They evaluated whether complex algorithms providing intuitive visual explanations can enhance user satisfaction and human interpretability.

Result: The paper presents derived general guidelines for XAI schemes and examines the effectiveness of algorithms that offer visual explanations for improving trust calibration and user satisfaction.

Conclusion: Algorithms that provide intuitive visual explanations, even when complex, can effectively enhance user satisfaction and human interpretability in XAI systems, particularly for uncertainty and global explanations.
...
Method: Proposes P(u, Ds) → R̂ formulation where u is cold-start user profile, Ds is curated support set. Uses transformer-based LLMs (BioGPT, LLaMA-2, GPT-4) with token-level alignments, embedding space regularization, and attention-controlled instruction structuring.

Result: Significant improvements in precision@k and NDCG scores for cold-start recommendation tasks, demonstrating that optimal prompt composition controls attention scales and decoder behavior during inference.
...
Method: Compared humans (N=216), LLMs (GPT-4o, Gemini 1.5 Pro), and Bayesian agents in identical dynamic negotiation settings, capturing both outcomes and behavioral dynamics.
...
Result: Achieved mean AUROC of 0.961 with standard deviation of 0.005 on a dataset of 195,789 customer IDs, securing second place in the competition.

Conclusion: The proposed comprehensive ML pipeline demonstrates robust performance in AML risk detection and provides explainable insights through XAI modules, making it suitable for real-world financial applications.
...
Conclusion: The work outlines a neuroscience-grounded roadmap for developing more advanced spatial reasoning capabilities in AI systems, with potential to generalize across dynamic or unstructured environments, benefiting the research community with a structured pathway forward.
...
Method: Uses dynamic heterogeneous graph-based scenario modeling to explicitly capture evolving social interactions, with a factorized architecture to process spatio-temporal dependencies and progressively eliminate uncertainty through multi-scale decoding.
...
Conclusion: The proposed ProgD method effectively addresses the limitation of evolving interactions in multi-agent prediction and demonstrates superior performance through progressive modeling and multi-scale decoding.
...
<details>
  <summary>Details</summary>
Motivation: Address governance and accountability challenges posed by unpredictable behaviors and heterogeneous capabilities of LLM-powered autonomous agents in various domains.

Method: Propose a three-layer architecture (agent layer, blockchain data layer, regulatory application layer) with three key modules: behavior tracing/arbitration, dynamic reputation evaluation, and malicious behavior forecasting.
...
Conclusion: The framework provides a foundation for future research on blockchain-enabled regulatory frameworks in multi-agent systems, addressing critical governance challenges in autonomous agent collaboration.
...
Method: Created a scalable pipeline to extract high-quality tool-based data analysis tasks from Jupyter notebooks, built the NbQA dataset, and developed Jupiter framework using Monte Carlo Tree Search (MCTS) for multi-step reasoning and value model learning.
...
Conclusion: The proposed approach significantly enhances LLMs' multi-step reasoning and tool-use capabilities for data science, demonstrating improved generalization and strong performance across diverse reasoning tasks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional RAG approaches struggle with thematic and holistic understanding of complex texts, requiring deeper text and context analysis that knowledge graphs can provide.
...
Conclusion: Discussion of strengths/limitations of each method and insights for future improvements in knowledge graph-based question answering systems.
...
Method: Propose staged GRPO training paradigm using partially revealed MCTS rollouts, introducing tree-structured advantage estimation and prefix-conditioned reward signals
...
Method: Proposes LightAgent framework with core functionalities including Memory (mem0), Tools, and Tree of Thought (ToT) while maintaining an extremely lightweight structure and seamless integration with mainstream chat platforms.

Result: Developed a fully open-source solution that enables developers to easily build self-learning agents, available at https://github.com/wxai-space/LightAgent.

Conclusion: LightAgent effectively resolves the flexibility-simplicity trade-off in multi-agent systems and provides a practical solution for agent deployment across various application scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Tournaments model pairwise dominance relationships, and there's a need for formal explainable AI approaches to justify why certain candidates win under different tournament solution concepts.
...
Result: They determine the size of smallest minimal supports for each rule and provide efficient algorithms for all rules except the weighted uncovered set, for which the problem is NP-complete.

Conclusion: Minimal supports serve as effective certified explanations that are compact and intuitive, contributing to formal explainable AI in tournament-based decision making.
...
<details>
  <summary>Details</summary>
Motivation: Teams in emergency response, military, and law enforcement must coordinate physical movements without visual cues or extensive communication, yet most research focuses on co-located or knowledge work coordination.
...
Method: Uses browser automation and LLM-based task acquisition to collect ML challenges from platforms; employs leaderboard-driven difficulty modeling using participant counts and score dispersion; implements multi-dimensional evaluation framework.

Result: Created 150 curated AutoML tasks organized into three benchmark subsets (Lite, Medium, Full) with balanced coverage across modalities and difficulty levels.

Conclusion: TAM Bench provides a diverse, realistic, and structured evaluation framework for LLM-based agents on end-to-end ML tasks, addressing limitations of existing benchmarks.
...
Method: Integration of Vision-Language Model common-sense through layered reward function, modeling VLM query as dedicated action for strategic external guidance, combined with curriculum learning for robust training.
...
<details>
  <summary>Details</summary>
Motivation: Existing few-shot prompting approaches limit LLMs' inherent reasoning abilities and require costly task-specific prompt construction with potential inconsistencies.

Method: Template-Oriented Reasoning (TORSO) elicits models to generate proper responses using internal reasoning without manually crafted few-shot examples.

Result: TORSO achieves strong performance on diverse LLM benchmarks with reasonable rationales.

Conclusion: TORSO provides an effective alternative to few-shot prompting that better utilizes LLMs' internal reasoning capabilities across various tasks.
...
Method: Technical analysis of hallucination causes and manifestations, evaluation of RAG mitigation strategy effectiveness, and exploration of ethical/regulatory implications.

Result: RAG strategy has limitations; holistic optimizations needed; human oversight is irreplaceable; consultative AI paradigm prioritizing veracity and traceability is required.

Conclusion: Solution lies not in incremental generative model improvements but in adopting consultative AI that amplifies rather than replaces professional judgment, with emphasis on verifiability and human oversight.
...
Method: SEDM integrates verifiable write admission using reproducible replay, a self-scheduling memory controller that dynamically ranks and consolidates entries based on utility, and cross-domain knowledge diffusion that abstracts reusable insights for transfer across heterogeneous tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current AI tools lack compositional generalization capabilities that humans possess. Previous tensor-based semantic approaches failed, but quantum models' increased training efficiency suggests potential for better performance.
...
Conclusion: Quantum approaches show potential for improving compositional generalization in vision-language models, though further refinement is needed for more complex encodings like CLIP vectors.
...
Method: Auras disaggregates perception and generation modules and provides controlled pipeline parallelism. It establishes a public context for perception and generation to share to address data staleness problems when increasing parallelism.
...
Conclusion: Auras effectively overcomes the constraints of sequential computation and provides high throughput for embodied AI agents while maintaining accuracy through its co-designed algorithm-system approach.
...
<details>
  <summary>Details</summary>
Motivation: To understand why LLMs fail at simple tasks when made longer despite excelling at complex reasoning, and to investigate whether model scaling yields diminishing returns for long-horizon task completion.

Method: Isolate execution capability by providing explicit knowledge and plans for long tasks, test models on multi-turn execution, analyze error patterns and self-conditioning effects, and benchmark frontier thinking models on single-turn task length.

Result: Larger models execute significantly more turns even when small models have 100% single-turn accuracy. Per-step accuracy degrades with more steps due to self-conditioning (models become error-prone when context contains prior mistakes). Thinking models avoid self-conditioning and execute longer tasks.

Conclusion: Scaling model size and sequential test-time compute provides massive benefits for long-horizon tasks. Execution capability, not reasoning inability, causes LLM failures on longer tasks. Self-conditioning is a key limitation not solved by scaling alone, while thinking models offer superior performance.
...
<details>
  <summary>Details</summary>
Motivation: Traditional additive decomposition of predictive uncertainty into epistemic and aleatoric components has been questioned, and there's a need for more intuitive uncertainty quantification methods for high-risk applications.
...
Result: The framework provides an alternative approach to uncertainty decomposition and helps discuss the existence of diversity collapse in committee machines.

Conclusion: The proposed signal-to-noise ratio based framework offers a more intuitive approach to uncertainty estimation and decomposition compared to traditional additive methods.
...
<details>
  <summary>Details</summary>
Motivation: The standard MMWU algorithm achieves minimax-optimal regret O(√(T log d)) for matrix LEA, but this bound may not be tight for specific instances. The authors aim to develop an algorithm that achieves instance-optimal regret bounds while maintaining the same computational efficiency.

Method: Developed a general potential-based framework for matrix LEA, with MMWU as a special case. Used a new one-sided Jensen's trace inequality based on Laplace transform to enable application of general potential functions. Derived an optimal potential function from the imaginary error function for vector LEA.

Result: The proposed algorithm achieves instance-optimal regret bound O(√(T·S(X||d⁻¹I_d))) with same computational complexity as MMWU. Outperforms state-of-the-art for learning quantum states corrupted by depolarization noise, random quantum states, and Gibbs states. Enables prediction of nonlinear quantum properties like purity and Rényi-2 correlation.

Conclusion: The improvement in regret bound is 'free' as it comes without additional computational cost. The algorithm provides significant advantages in quantum learning applications and demonstrates the effectiveness of the potential-based framework with optimal potential functions for matrix learning problems.
...
Method: Proposed provably robust Q-learning variant that handles adversarial reward perturbations, using refined Azuma-Hoeffding inequality for almost-martingales in the unknown statistics setting.
...
Method: Uses physics-informed neural networks (PINNs) to approximate HJB-based value functions at scale. Introduces Value Gradient Iteration (VGI) module that iteratively refines value gradients along trajectories to align value learning with value-gradient learning.
...
Conclusion: The proposed CT-MARL framework with PINNs and VGI successfully addresses key challenges in continuous-time multi-agent reinforcement learning, enabling scalable and stable learning in high-dimensional multi-agent systems.
...
<details>
  <summary>Details</summary>
Motivation: Standard ML methods often learn spurious correlations that perform well on average but degrade for underrepresented groups, and prior worst-group optimization approaches assume accurate group distribution estimation which is frequently violated in noisy, non-stationary environments.

Method: Proposes a Wasserstein-based distributionally robust optimization (DRO) framework to account for distributional uncertainty within each group while preserving worst-group performance objectives, with a gradient descent-ascent algorithm for solving the DRO problem.

Result: The method provides convergence results and demonstrates effectiveness on real-world data, addressing the limitations of prior approaches that assume accurate group distribution estimation.

Conclusion: The proposed Wasserstein-based DRO framework effectively handles distributional uncertainty in heterogeneous data sources while improving worst-group performance, making it suitable for noisy and evolving environments where traditional methods fail.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of system identification for reaction-diffusion systems when there is no prior knowledge of underlying physics, particularly for discovering emergent dynamics across neuroscience, ecology, and epidemiology.

Method: Uses Data-driven Rulesets for Soft Artificial Life (DRSALife) model to learn Agent-based and Cellular Automata rulesets from observed data, testing on Elementary CA Rule 30, Game of Life, and Vicsek Flocking problems.

Result: Achieves 74% accuracy in predicting emergent dynamics, demonstrates robust performance against Gaussian noise and temporal sparsity, and successfully identifies underlying PDE structure and parameters.

Conclusion: DRSALife framework effectively learns Soft ALife rulesets for reaction-diffusion systems without prior physics knowledge, showing promise for data-driven discovery of emergent dynamics in various scientific domains.
...
Method: Integrates dual-stage denoising with Morlet and Daubechies wavelets, Convolutional Block Attention Module (CBAM), Graph Attention Networks (GAT), and Time Series Transformers (TST) to capture spatial and temporal dependencies in multi-channel ECG signals.
...
<details>
  <summary>Details</summary>
Motivation: To understand the numerical properties and distribution characteristics of LRP-type attribution methods, particularly focusing on their convergence behavior and how they compare to gradient-based approaches.

Method: Represent LRP attribution methods as products of modified gradient matrices, derive upper bounds for singular values and component-wise bounds for attribution values, and analyze convergence constants for empirical means.
...
Conclusion: LRP-beta exhibits unique numerical properties with weight-norm-independent convergence constants, making it particularly suitable for scenarios involving multiple non-geometric data augmentations and Smoothgrad-type attribution methods.
...
<details>
  <summary>Details</summary>
Motivation: Training large ML models produces significant carbon emissions. FL's distributed nature allows leveraging regional/temporal carbon intensity variations to reduce environmental impact.
...
<details>
  <summary>Details</summary>
Motivation: Spin coating polymer thin films is inherently a multi-objective optimization problem requiring efficient parameter optimization for specific mechanical properties.

Method: Integrates PyePAL (active Pareto front learning algorithm) with Gaussian process models, UMAP for 2D visualization, and fuzzy linguistic summaries for explainable AI insights.

Result: Efficiently identifies promising polymer designs while providing visual and linguistic explanations that facilitate expert-driven analysis and knowledge discovery.

Conclusion: The framework successfully optimizes processing parameters for polymer thin films while enhancing interpretability through visualization and linguistic explanations.
...
Method: Introduces Approximate Nearest Neighbor Attention (ANNA) mechanism with sub-quadratic time complexity, proving it retains expressive power and can solve reasoning tasks like Match2 and k-hop with near-optimal depth.

Result: ANNA-transformers maintain the capabilities of standard attention for MPC algorithms, can solve key reasoning tasks efficiently, and constant-depth ANNA-transformers can simulate constant-depth low-rank transformers.

Conclusion: ANNA provides a unified framework for reasoning about efficient attention approximations while preserving the expressive power needed for complex computational tasks.
...
<details>
  <summary>Details</summary>
Motivation: To create reference baselines that enable researchers to assess the sanity and quality of alternative training approaches across different model scales and datasets, and to facilitate standardized comparison of training procedures.

Method: Training family of dense transformer models across multiple parameter scales (0.13B to 1.7B) and token scales (up to 1T) on 8 recent open reference datasets, with intermediate checkpoints for studying training dynamics.

Result: NemoTron-CC HQ consistently outperforms other reference datasets, followed by DCLM-baseline and FineWeb-Edu. The models provide reference points for comparing scaling trends and training procedures on a common compute axis.

Conclusion: Open-sci-ref establishes standardized baselines with intermediate checkpoints, logs, code, and evaluations to simplify reproduction, standardize comparison, and facilitate future research in model training approaches.
...
<details>
  <summary>Details</summary>
Motivation: Unsupervised anomaly detection in large-scale tabular data is challenging because real-world data often contains heterogeneous contexts where globally rare events can be normal under specific contexts, making single global distribution models ineffective.
...
Conclusion: The results underscore the importance of considering contextual nuances in anomaly detection to accurately distinguish anomalous from normal instances in tabular data.
...
<details>
  <summary>Details</summary>
Motivation: Data-driven weather models have plateaued in performance, needing a new approach to improve forecasting without developing entirely new models.

Method: Uses Vision Transformer-based gating network to dynamically weight contributions of multiple expert models at each grid point based on forecast lead time.

Result: Achieves up to 10% lower RMSE than best AI weather model on 2-day forecasts, outperforming individual experts and simple averaging.
...
Method: PRISMA framework scoping review synthesizing over 100 publications on ML applications in protection and disturbance management.
...
Conclusion: Critical gaps exist in real-world validation and robustness testing; future research needs standardized practices, benchmark datasets, and advanced ML architectures for practical deployment.
...
<details>
  <summary>Details</summary>
Motivation: To address the exponential computational cost of feature subset reasoning and limited expressiveness of single scalar attributions in existing XAI methods.

Method: Uses orthogonal functional decomposition in RKHS with analytical projection via recursive kernel-centering, avoiding explicit subset enumeration while being model-agnostic for tabular data.

Result: Achieved speedups from 0.6x to 9.7x (median ~3x) across 10 datasets with high fidelity (R² 0.81-0.999) and good rank agreement, enabling novel diagnostics like component surgery.

Conclusion: STRIDE complements scalar attribution methods by providing structured functional perspectives and enabling quantitative analysis of feature interactions within experimental scope.
...
Result: Validated on open and proprietary datasets, demonstrating up to 0.20+ AUROC improvements in scenarios with large Rashomon ratios, with tangible business benefits in real-world applications.

Conclusion: The Rashomon Ensemble approach proves robust, practical, and effective for improving generalization by leveraging diverse high-performing solutions from the Rashomon set.
...
<details>
  <summary>Details</summary>
Motivation: To establish a thermodynamic framework for understanding the learning process in deep linear networks by studying their Riemannian geometric properties and overparametrization through group actions.

Method: Using group actions to analyze overparametrization, Riemannian submersion from parameter space to observable space, foliation of balanced manifolds, and explicit construction of orthonormal basis for tangent spaces using Jacobi matrix theory.

Result: The paper demonstrates that Riemannian geometry on the space of observables is obtained through Riemannian submersion of the balanced manifold, and provides tools to define and compute Boltzmann entropy through group orbit foliation.

Conclusion: The Riemannian geometric approach provides a solid foundation for thermodynamic description of learning in deep linear networks, with balanced manifolds and group actions serving as key mathematical constructs for understanding the parameter-observable relationship.
...
<details>
  <summary>Details</summary>
Motivation: Adapting LLMs to specialized tasks in resource-constrained environments is challenging. Traditional LoRA uses uniform rank allocation which is inefficient, and existing rank allocation techniques are computationally expensive, complex, and unstable.

Method: Proposes Sensitivity-LoRA that dynamically allocates ranks to weight matrices based on their global and local sensitivities using second-order derivatives (Hessian Matrix) of the loss function to capture weight sensitivity with minimal computational overhead.
...
Conclusion: Sensitivity-LoRA provides an efficient and stable solution for parameter-efficient fine-tuning of LLMs by addressing the limitations of uniform rank allocation through sensitivity-based dynamic rank allocation.
...
Result: Causal inputs lead to improved prediction accuracy and interpretability across varying lead times, while reducing unnecessary features and enhancing computational efficiency.
...
Method: Used public databases (PeeringDB, CAIDA) to gather ISP data and evaluated tree-based, neural network, and transformer ML models for predicting peering relationships.
...
Conclusion: ISPs can adopt this ML approach to fully automate peering partner selection, creating a more efficient Internet ecosystem.
...
Result: Detectors trained on English failed to generalize to Spanish, while training on HISPASpoof substantially improved detection performance. Also evaluated synthetic speech attribution (identifying generation method).
...
<details>
  <summary>Details</summary>
Motivation: Large transformer models are powerful for semantic communication but computationally expensive, making deployment challenging in resource-constrained 6G networks.

Method: Formulates token merging proportion selection as multi-objective optimization, uses Gaussian process-based Bayesian optimization to create Pareto-optimal configurations for adaptive runtime deployment.

Result: Outperforms other baselines with significant FLOPs reduction while maintaining competitive accuracy across various SNR conditions, with adaptive policies effectively trading latency for semantic fidelity.
...
<details>
  <summary>Details</summary>
Motivation: To explore the convergence of quantum-inspired neural networks and deep reinforcement learning for improved financial trading performance, particularly in foreign exchange markets.

Method: Implemented a trading agent using Quantum LSTM for short-term trend prediction integrated with Quantum Asynchronous Advantage Actor-Critic (QA3C) algorithm. Trained on 2000-2025 data (80% training, 20% testing) with specific state design, reward function for trend-following and risk control, and multi-core training.

Result: Achieved 11.87% return over approximately 5 years with only 0.92% maximum drawdown, outperforming several currency ETFs. The hybrid quantum-inspired model demonstrated competitive FX trading performance.

Conclusion: QLSTM proves effective for small-profit trades with tight risk control. The study shows promising results for quantum-inspired approaches in financial trading, though limitations include classical quantum simulation and simplified strategy implementation.
...
Method: Proposes FSPO which clips sequence log-IS ratios with a Gaussian-motivated band that applies KL-corrected drift and scales as √L to ensure length fairness.

Result: FSPO flattens clip rates across length bins, stabilizes training, and outperforms all baseline methods across multiple evaluation datasets.

Conclusion: The method successfully addresses length fairness in sequence-level RL through theoretical formalization of Length Reweighting Error and provides practical improvements in training stability and performance.
...
<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of current evaluation metrics that favor statistical similarity over accurate detection of rare, high-impact weather events like extreme convection, which current sophisticated baselines completely miss despite high correlation scores.
...
Result: DART achieves CSI = 0.273 with bias = 2.52 vs. 6.72 for baselines at equivalent CSI, improves extreme convection detection by 270% by removing Integrated Water Vapor Transport (IVT Paradox), and validates on real-world cases like the August 2023 Chittagong flooding. Trains in under 10 minutes on standard hardware.

Conclusion: DART successfully addresses the Statistical Similarity Trap and provides the first systematic solution for hybrid conversion-segmentation-downscaling tasks in weather forecasting, enabling precise operational calibration and trustworthy AI for extreme weather preparedness.
...
<details>
  <summary>Details</summary>
Motivation: Agro-tourism is a strategic economic model for rural development that diversifies income streams and preserves cultural heritage, requiring detailed study of growth strategies.

Method: Two-phase study: comprehensive literature review to identify indicators, then machine learning feature selection techniques (LASSO with Logistic Regression, Decision Trees, Random Forest, XGBoost) applied to identify important growth indicators.

Result: LASSO with Logistic Regression achieved highest classification accuracy: 98% in 70-30% train-test split and 99% in 80-20% split. Random Forest achieved 95% accuracy, while Decision Trees and XGBoost reached 97% accuracy.

Conclusion: Machine learning feature selection methods, particularly LASSO combined with Logistic Regression, are highly effective for identifying key indicators for agro-tourism growth, providing valuable insights for strategic development.
...
Result: Vejde policies generalized well to unseen test instances without significant score loss, performing close to instance-specific MLP agents and showing competitive results compared to online planning algorithm Prost.
...
<details>
  <summary>Details</summary>
Motivation: To prevent harmful recommendations from undertrained educational recommender systems by creating accurate QA simulators that don't require real student interaction, while addressing the trade-off between performance (LLM-based) and efficiency (LLM-free methods).

Method: Proposes LLM Distillation based Simulator (LDSim) that distills domain knowledge and reasoning capability from large language models to enhance prediction performance while maintaining fast inference speed.
...
Conclusion: LDSim successfully bridges the gap between high-performance LLM-based methods and efficient LLM-free methods, providing an effective solution for educational QA simulation with both accuracy and efficiency.
...
Method: Multi-Attention Meta Transformer framework with time-frequency domain encoder for representation learning and meta-learning network for classification, using contrastive learning with random augmentations.
...
Conclusion: MMT-FD provides an efficient and highly effective solution for few-shot unsupervised fault diagnosis with excellent generalization performance for practical industrial applications.
...
TL;DR: EMPG framework addresses LLM policy gradient issues by modulating updates based on step-wise uncertainty and task outcomes, achieving better performance in long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: Sparse rewards in long-horizon tasks make credit assignment difficult for LLM agents, and traditional policy gradients have inefficient learning dynamics where update magnitude is coupled with entropy.

Method: Proposes Entropy-Modulated Policy Gradients (EMPG) that recalibrates learning signals using step-wise uncertainty and final outcomes, with bonus terms for future clarity and predictable paths.
...
Conclusion: EMPG effectively resolves the fundamental learning dynamics problem in LLMs by decoupling gradient magnitude from entropy, leading to more stable and efficient learning in long-horizon tasks.
...
TL;DR: MoSE framework enhances GNN expressiveness by using anonymous walks to extract subgraphs and routing them to specialized experts, outperforming baselines while providing interpretability.


<details>
  <summary>Details</summary>
Motivation: Traditional GNNs rely on local message passing and cannot capture complex high-order subgraph patterns, while existing random walk kernel methods are limited to graph-level tasks and lack flexibility.

Method: Proposes Mixture of Subgraph Experts (MoSE) that extracts informative subgraphs via anonymous walks and dynamically routes them to specialized experts based on structural semantics.

Result: MoSE demonstrates superior performance over competitive baselines, provides theoretical proof of being more powerful than Subgraph Weisfeiler-Lehman Test, and offers interpretable insights through learned subgraph visualizations.

Conclusion: MoSE provides a flexible and expressive framework for subgraph-based representation learning across diverse graph tasks, addressing limitations of existing methods while offering improved interpretability.
...
TL;DR: A novel robust computational approach for estimating the HGR correlation coefficient using configurable polynomial kernels, offering improved reliability for real-world applications like algorithmic fairness and constrained machine learning.


<details>
  <summary>Details</summary>
Motivation: Existing differentiable HGR estimation methods suffer from bias-variance trade-offs due to HGR's inherent uncomputability, compromising robustness in real-world scenarios.

Method: Proposes a computational approach based on user-configurable polynomial kernels that provides greater robustness and faster computation compared to previous methods.
...
Conclusion: This approach provides a more reliable and robust option for HGR estimation in real-world applications, making it suitable for use in algorithmic fairness, scientific analysis, and causal discovery tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current AutoML and LLM-based approaches for model/hyperparameter selection rely on expensive trial-and-error methods and APIs with limited interpretability and generalizability, requiring a more efficient and automated solution.
...
Result: Achieves competitive/superior performance to traditional HPO methods on 8 medical imaging datasets, optimal results on 5/8 tasks, 99.6-99.9% response time reduction, and 2.4-15.7x faster training times while maintaining accuracy within 1-5% of best baselines.

Conclusion: MetaLLMiX provides an effective zero-shot hyperparameter optimization solution that dramatically reduces computational costs while maintaining performance, outperforming prior API-based approaches through local deployment and efficient reasoning.
...
TL;DR: LLMs can generate valid counterfactual explanations but struggle with minimality - either making excessive edits or tiny changes that don't affect predictions, revealing a concerning validity-minimality trade-off.


<details>
  <summary>Details</summary>
Motivation: To enable effective human-AI collaboration, language models need to explain their decisions naturally. Self-generated counterfactual explanations could provide insight into model decision-making.

Method: Study self-generated counterfactual explanations (SCEs) where models modify inputs to change predictions. Evaluate validity (achieving intended outcome) and minimality (minimum necessary edits) across multiple LLMs and datasets.

Result: LLMs produce valid SCEs but far from minimal, offering little insight. When asked for minimal counterfactuals, they make excessively small edits that fail to change predictions. This validity-minimality trade-off is consistent across models and settings.

Conclusion: SCEs are ineffective explainability tools that can provide misleading insights. Deploying LLMs in high-stakes settings requires considering the impact of unreliable self-explanations on decision-making.
...
TL;DR: Hybrid framework combining machine learning with geostatistics using spatial lag features from kriging, called KpR, significantly improves soil property prediction accuracy by 30% R2 compared to non-spatial ML methods.
...
Method: Proposed KpR (Kriging prior regression) that enriches ML models with spatial context through engineered 'spatial lag' features derived from ordinary kriging. Evaluated using TabPFN model on six field-scale datasets with soil properties (organic carbon, clay, pH) and remote/proximal sensing features.

Result: KpR with TabPFN demonstrated reliable uncertainty estimates and more accurate predictions than other spatial techniques and non-spatial ML algorithms. Achieved ~30% average R2 improvement over ML without spatial context, with strong performance even with small sample sizes.

Conclusion: KpR with TabPFN is a robust and versatile modeling framework for digital soil mapping that effectively combines spatial information from geostatistics with predictive power of machine learning, particularly valuable for precision agriculture applications with limited data.
...
TL;DR: The paper proposes fuser, a novel algorithm for microbiome co-occurrence network inference that addresses limitations of existing methods by capturing both spatial and temporal dynamics across different environmental niches, rather than just static snapshots from single environments.
...
Method: The authors developed the Same-All Cross-validation (SAC) framework to evaluate algorithms in same-environment and cross-environment scenarios. They propose fuser, an algorithm that retains subsample-specific signals while sharing relevant information across environments, generating distinct environment-specific predictive networks.

Result: fuser achieves comparable performance to existing algorithms (like glmnet) within homogeneous environments and significantly reduces test error compared to baseline algorithms in cross-environment scenarios.

Conclusion: The fuser algorithm effectively addresses the limitations of conventional microbiome network inference methods by capturing dynamic microbial associations across different environmental conditions, providing more accurate environment-specific predictive networks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional urban water forecasting models struggle to effectively incorporate both internal water measurements and external weather factors while capturing complex temporal dependencies between variables.

Method: Developed a multi-input, multi-output deep learning model with embedding layer for exogenous variables, focusing forecasts only on endogenous water variables. Tested on LausanneCity dataset and three synthetic datasets with varying temporal complexity.

Result: Achieved state-of-the-art performance using only endogenous variables, with further improvements when including exogenous variables and forecast reports. Consistently outperformed baselines across both real and synthetic datasets.

Conclusion: AquaCast provides robust and accurate urban water forecasting by effectively capturing inter-variable and temporal dependencies while eliminating the need to forecast exogenous variables, demonstrating strong generalization capabilities.
...
Result: Validated on 586 papers from five conferences, achieving perfect recall (100%) and near-perfect accuracy (99.4%) in identifying target papers and completing actions.
...
<details>
  <summary>Details</summary>
Motivation: Recent work has shown strong baseline performance using recurrent facts, motivating the development of a fully explainable method that maintains competitive performance.
...
Conclusion: The approach provides fully interpretable predictions while achieving competitive performance, demonstrating the effectiveness of explainable temporal rule-based methods for TKG forecasting.
...
### [68] [Balancing Utility and Privacy: Dynamically Private SGD with Random Projection](https://arxiv.org/abs/2509.09485)
*Zhanhong Jiang,Md Zahid Hasan,Nastaran Saadati,Aditya Balu,Chao Liu,Soumik Sarkar*

Main category: cs.LG

TL;DR: D2P2-SGD optimizer combines dynamic differential privacy with automatic gradient clipping and random projection with SGD to improve privacy-utility tradeoff and learning efficiency while maintaining provable convergence rates.


<details>
  <summary>Details</summary>
Motivation: Existing DPSGD has static noise mechanism that impacts model performance, and large model parameters make efficient learning challenging. Privacy leakage concerns in stochastic optimization need better solutions.
...
Result: Exhibits provably sub-linear convergence rates across different objective functions, matches best available rate, and enhances accuracy while maintaining privacy in diverse datasets.

Conclusion: D2P2-SGD provides better utility at privacy cost through DDP and enables more efficient model learning through random projection, addressing key limitations of existing private optimization methods.
...
### [69] [Graph Alignment via Dual-Pass Spectral Encoding and Latent Space Communication](https://arxiv.org/abs/2509.09597)
...
Result: Outperforms existing unsupervised alignment baselines on graph benchmarks with superior robustness to structural inconsistencies, and generalizes effectively to vision-language alignment tasks.
...
### [70] [PIPES: A Meta-dataset of Machine Learning Pipelines](https://arxiv.org/abs/2509.09512)
*Cynthia Moreira Maia,Lucas B. V. de Amorim,George D. C. Cavalcanti,Rafael M. O. Cruz*

Main category: cs.LG

TL;DR: PIPES addresses limitations in OpenML's algorithm selection experiments by creating a diverse collection of 9,408 pipelines across 300 datasets to support meta-learning research.


<details>
  <summary>Details</summary>
Motivation: OpenML's experiments lack diversity in preprocessing pipelines and are imbalanced, focusing on popular techniques rather than comprehensive coverage of algorithm combinations.

Method: Created PIPES - a collection of experiments with 9,408 pipelines applied to 300 datasets, representing all combinations of selected techniques to ensure diversity and completeness.

Result: Built a comprehensive repository containing detailed pipeline block information, training/testing times, predictions, performances, and error messages for extensive meta-learning analysis.

Conclusion: PIPES provides a more diverse and representative experimental foundation for algorithm selection research, with potential for expansion to further support the meta-learning community.
...
### [71] [Explaining Concept Drift through the Evolution of Group Counterfactuals](https://arxiv.org/abs/2509.09616)
...
Method: Analyze temporal evolution of group-based counterfactual explanations (GCEs) by tracking shifts in cluster centroids and counterfactual action vectors before and after drift, within a three-layer framework combining data, model, and explanation layers.

Result: The approach provides interpretable insights into structural changes in model decision boundaries and allows comprehensive diagnosis of drift by distinguishing between different root causes like spatial data shift versus concept re-labeling.

Conclusion: The methodology offers a holistic view for explaining concept drift through evolving counterfactual explanations, enabling better understanding of model behavior changes in dynamic environments.
...
TL;DR: Few-shot learning with Prototypical Networks achieves competitive accuracy (74.87% multi-class, 70%+ binary) for COVID-19/Flu/healthy cough sound classification using only 15 examples per class.
...
Method: Leveraged Prototypical Networks with spectrogram representations of cough sounds, comparing multi-class vs binary classification models with minimal training samples (15 support examples per class).
...
Conclusion: Few-shot learning is viable for medical diagnostics when large labeled datasets are unavailable, with multi-class classification performing comparably to binary approaches for respiratory sound analysis.
...
Result: FG-FARL achieves comparable value to behavior cloning and HACO baselines while significantly improving fairness metrics, as measured by off-policy value estimates with bootstrap 95% confidence intervals and subgroup disparity analyses with p-values.

Conclusion: FG-FARL provides a practical approach for safer and more equitable decision support in healthcare applications, demonstrating the feasibility of balancing performance with fairness considerations in offline reinforcement learning systems.
...
TL;DR: ProDiGy is a new Byzantine-robust FL algorithm that uses a dual scoring system based on gradient proximity and dissimilarity to detect adversarial attacks, especially effective under non-IID data distributions.


<details>
  <summary>Details</summary>
Motivation: Federated Learning is vulnerable to adversarial attacks, particularly in data heterogeneity scenarios where existing defense mechanisms often fail.

Method: Proposes ProDiGy algorithm with joint dual scoring system that evaluates client gradients based on both proximity and dissimilarity metrics to detect Byzantine attacks.

Result: Extensive experiments show ProDiGy outperforms existing defenses, maintaining strong defense capabilities and model accuracy even when other mechanisms fail under non-IID data distributions.
...
<details>
  <summary>Details</summary>
Motivation: Extreme 2-bit quantization suffers from catastrophic performance loss due to activation outliers, and existing rotation-based methods use fixed transforms that cannot adapt to different transformer layers' distinct outlier patterns.

Method: Proposes ButterflyQuant using learnable butterfly transforms parameterized by continuous Givens rotation angles, enabling smooth optimization while guaranteeing orthogonality. Introduces uniformity regularization on post-transformation activations for better quantization.

Result: Achieves 15.4 perplexity on LLaMA-2-7B with 2-bit quantization versus 22.1 for QuaRot, with only 128 calibration samples and minutes of training on a single GPU.

Conclusion: Layer-adaptive butterfly transforms outperform fixed rotation methods for extreme quantization, providing better outlier suppression and quantization performance with minimal computational cost.
...
TL;DR: A deep learning emulator for stochastic/chaotic spatio-temporal systems that generalizes across parameter values using pre-training and fine-tuning with local attention mechanisms.
...
Method: Pre-train on single parameter domain, fine-tune on diverse smaller dataset, incorporate local attention mechanisms for varying domain sizes and resolutions.

Result: Successfully emulates chaotic Kuramoto-Sivashinsky equation and stochastically-forced beta-plane turbulence, capturing phenomena at interpolated parameters with computational speed-ups.

Conclusion: The emulator enables efficient parameter space exploration with uncertainty quantification, facilitating statistical study of rare events in complex systems.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient operator learning algorithm that can handle PDEs with multiple distinct inputs while maintaining mathematical rigor, minimizing computational costs, and achieving better generalization than existing methods.

Method: Combines Reduced Basis Method with Generative Pre-Trained Physics-Informed Neural Networks, using a greedy algorithm to build network structure offline adaptively, and employs knowledge distillation via task-specific activation functions for compact architecture.

Result: ReBaNO significantly outperforms state-of-the-art operator learning algorithms (PCA-Net, DeepONet, FNO, CNO) in eliminating/shrinking generalization gap for both in- and out-of-distribution tests, and is the only method achieving strict discretization invariance.

Conclusion: ReBaNO provides a mathematically rigorous, computationally efficient approach to operator learning that demonstrates superior performance and generalization capabilities compared to existing methods, particularly for PDE problems with multiple inputs.
...
TL;DR: A novel Functional Group Representation (FGR) framework that encodes molecules using chemical substructures for interpretable molecular property prediction, achieving state-of-the-art performance across 33 benchmarks while maintaining chemical interpretability.
...
Method: Integrates curated functional groups (FG) with mined functional groups (MFG) using sequential pattern mining, encodes molecules into lower-dimensional latent space with pre-training, and includes 2D structure-based descriptors.

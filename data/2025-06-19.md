...
Motivation: The rapid growth of LEO satellites for EO missions requires efficient autonomous coordination, which traditional methods struggle to provide in real-time dynamic scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing agentic systems lack full autonomy and adaptability, limiting scalability. SwarmAgentic addresses this by enabling from-scratch agent generation and self-optimization.
...
Result: SwarmAgentic achieves a +261.8% improvement over baselines, excelling in open-ended tasks like TravelPlanner.

Conclusion: The framework advances scalable, autonomous agentic system design by integrating swarm intelligence with automated multi-agent generation.
...
<details>
  <summary>Details</summary>
Motivation: Classic logic systems are rigid and lack nuance, while neural networks lack interpretable reasoning. CALM bridges this gap.

Method: CALM uses domain trees for predicates, refining analog truth values with neural networks and symbolic constraints.

Result: CALM achieved 92.2% accuracy in object placement tasks, surpassing classical logic (86.3%) and LLMs (59.4%).

Conclusion: CALM combines logic precision with neural flexibility, paving the way for robust, interpretable AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for continual learning (CL) in multi-agent settings are limited by CPU-based environments, causing computational bottlenecks and short task sequences.

Method: MEAL uses JAX for GPU acceleration, allowing continual learning across 100 tasks on a standard desktop PC. It tests the scalability of CL and multi-agent RL (MARL) methods in complex settings.

Result: Naive combinations of CL and MARL methods perform well in simple environments but fail in complex ones requiring coordination and adaptation. Ablation studies reveal critical architectural and algorithmic features.

Conclusion: MEAL provides a scalable and efficient benchmark for CMARL, highlighting the need for specialized methods to address the challenges of continual learning in multi-agent settings.
...
Conclusion: T-PPO is an effective and efficient alternative to PPO for training reasoning LLMs, addressing key inefficiencies while maintaining performance.
...
<details>
  <summary>Details</summary>
Motivation: Traditional heuristic designs for combinatorial optimization rely on manual expertise and lack generalization. HeurAgenix aims to automate and improve this process.

Method: A two-stage framework: (1) heuristic evolution using LLMs to extract strategies, and (2) dynamic heuristic selection guided by LLMs, with options for lightweight models. A dual-reward mechanism fine-tunes the selector.
...
<details>
  <summary>Details</summary>
Motivation: The rapid growth of maritime IoT demands and the inefficiency in computation offloading and resource allocation due to uncertain tasks motivate the study.

Method: A cooperative MEC framework is proposed, leveraging Lyapunov optimization for task unpredictability and a heterogeneous-agent soft actor-critic to solve the Markov game problem.

Result: Simulations confirm the framework's effectiveness in optimizing computation offloading and resource allocation.

Conclusion: The proposed approach successfully addresses maritime IoT challenges, offering efficient solutions for uncertain task scenarios.
...
<details>
  <summary>Details</summary>
Motivation: FA is complex and knowledge-intensive; AI integration can automate tasks but requires cohesive orchestration.

Method: Design and implementation of an LPA combining LLMs with planning capabilities and external tools for autonomous query processing and data retrieval.

Result: The LPA demonstrates operational effectiveness and reliability in supporting FA tasks.

Conclusion: The LPA successfully integrates AI into FA workflows, improving efficiency and automation.
...
<details>
  <summary>Details</summary>
Motivation: Address the ad hoc and non-comparable approaches in previous work for encoding game history in LLM agents.

Method: Characterize state representation along three axes: action informativeness, reward informativeness, and prompting style. Apply this to a dynamic selfish routing game.

Result: Summarized history, regret-based rewards, and limited action info lead to behavior closer to game theory equilibrium. Other representations deviate or show instability.

Conclusion: State representation significantly impacts LLM agent behavior, with specific methods aligning better with theoretical predictions.
...
Conclusion: The AI Policy Module effectively bridges the gap between AI ethics and technical practice, with the AI Regulation Assignment proving particularly impactful.
...
<details>
  <summary>Details</summary>
Motivation: LRMs exhibit overthinking, generating verbose and redundant content, which increases inference costs and reduces efficiency. The study explores the origins of this inefficiency and leverages the models' inherent capacity for concise reasoning.

Method: Two methods are introduced: (1) Efficiency Steering, a training-free activation steering technique, and (2) Self-Rewarded Efficiency RL, a reinforcement learning framework that rewards concise correct solutions.
...
Conclusion: Reasoning efficiency can be enhanced by guiding the intrinsic capabilities of LRMs in a self-guided manner.
...
<details>
  <summary>Details</summary>
Motivation: Current AI agents are siloed, limiting their ability to integrate physical and digital intelligence for complex tasks.

Method: Developed a unified simulation platform (Embodied Web Agents task environments) and benchmark for tasks requiring cross-domain reasoning.

Result: Revealed performance gaps between state-of-the-art AI and humans, highlighting challenges and opportunities.

Conclusion: The work establishes a foundation for AI agents that bridge embodiment and web knowledge, with public resources for further research.
...
<details>
  <summary>Details</summary>
Motivation: Clinical decision-making is complex and prone to biases; current LLM usage lacks interaction depth. MedSyn addresses this by fostering collaborative refinement of decisions.

Method: MedSyn involves multi-step, interactive dialogues where physicians and LLMs dynamically exchange and challenge suggestions, using open-source LLMs.

Result: Open-source LLMs show promise as physician assistants in simulated interactions, suggesting real-world applicability.

Conclusion: MedSyn's potential for improving clinical decisions is promising; future validation with real physicians is planned.
...
<details>
  <summary>Details</summary>
Motivation: Sampling Boltzmann distributions in constrained Ising models is challenging due to soft constraints that either slow mixing or fail to enforce feasibility.
...
Conclusion: 2D-PT broadly improves constrained Ising problem sampling and can be deployed on existing Ising machines.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for stable, interpretable AI in clinical trials, especially for small datasets, by combining dynamical systems and evolutionary learning.

Method: NetraAI uses contraction mappings, information geometry, and evolutionary algorithms to embed features in a metric space, iteratively contracting them toward stable attractors. An LLM Strategist guides discovery by prioritizing variables and injecting domain knowledge.

Result: NetraAI transformed weak baseline models (AUC ~0.50-0.68) into near-perfect classifiers in case studies (schizophrenia, depression, pancreatic cancer) using few features.

Conclusion: NetraAI offers a novel, explainable AI framework for clinical discovery, aligning with concept-level reasoning paradigms like JEPA and enabling adaptive, self-reflective AI.
...
<details>
  <summary>Details</summary>
Motivation: Traditional EEG-based methods struggle with noise and variability in open-vocabulary scenarios, limiting their practical application.

Method: ETS integrates EEG with synchronized eye-tracking data to enhance decoding accuracy for text generation and sentiment classification.

Result: The model achieves superior BLEU and Rouge scores for text generation and a 10% F1 score improvement for sentiment classification, outperforming baselines.

Conclusion: ETS demonstrates robust performance across diverse subjects and data sources, highlighting its potential for open-vocabulary EEG-to-text systems.
...
<details>
  <summary>Details</summary>
Motivation: Traditional direct measurements of onflow parameters can fail due to sensor faults, necessitating a data-driven approach for real-time prediction in applications like wind tunnel monitoring.

Method: The method involves offline training a ConvNet, freezing most weights, and retraining selected layers for transfer learning, demonstrated using CFD data for domain and task adaptation.

Result: The approach effectively adapts to changing data distributions and tasks but struggles with noisy data.

Conclusion: The transfer learning method shows promise for adapting to new domains and tasks but requires refinement for noisy data scenarios.
...
Result: Achieves state-of-the-art performance, with a 12% improvement in typhoon intensity forecasting.

Conclusion: PIPE effectively bridges the gap in multimodal forecasting by leveraging visual data and physical context.
...
<details>
  <summary>Details</summary>
Motivation: The rise of fast delivery services demands efficient, dense storage systems, but multi-deep AVS/RS face retrieval challenges due to lane blockages. Homogeneous storage limits flexibility.

Method: A graph-based state representation integrates item attributes and warehouse topology. A GNN-Transformer neural network processes this, assigning global priorities.
...
Conclusion: The proposed approach effectively addresses retrieval challenges in heterogeneous multi-deep storage systems, enhancing efficiency and adaptability.
...
<details>
  <summary>Details</summary>
Motivation: The scarcity of comprehensive public datasets for T1D limits progress in personalized therapy and glucose prediction.

Method: AZT1D includes CGM, insulin pump, carbohydrate intake, and device mode data from 25 T1D patients on AID systems over 6-8 weeks.

Result: The dataset provides granular bolus insulin details, rarely available in existing datasets.

Conclusion: AZT1D supports AI/ML applications to improve T1D clinical decision-making and individualized care.
...
<details>
  <summary>Details</summary>
Motivation: Enhance zero-shot performance of PLMs for protein property prediction without additional training.

Method: Add a dropout layer (0.1 rate) between the featurizer and transformer layers during inference, averaging outputs like Monte-Carlo dropout.

Result: Improved zero-shot performance on ProteinGym dataset, even without prior dropout training.

Conclusion: Simple dropout injection at inference boosts PLM performance for protein fitness prediction.
...
<details>
  <summary>Details</summary>
Motivation: To address the inefficiency and high cost of pretraining large language models (LLMs) by leveraging existing pretrained models more effectively.

Method: Develops AoE to interpolate individual weight tensors of parent models, enabling gradual or sharp trait changes in child models.

Result: Demonstrates functional child models, including the DeepSeek R1T 'Chimera', which combines traits of parent models with reduced computational demands.

Conclusion: AoE offers a scalable and efficient way to create capable variants of existing models without fine-tuning or distillation.
...
<details>
  <summary>Details</summary>
Motivation: To understand how semantic resolution affects the representational capacity of models, including deep networks and brains.
...
Result: Closed-form expressions reveal a universal Pareto front for generalization and identification, with a predicted 1/n collapse in multi-input capacity and non-monotonic optimum for generalization.
...
Method: Integrates semantic-aware embeddings from pretrained language models and spline-based Kolmogorov-Arnold Networks (KAN) within a selective state-space framework (Mamba).
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between discrete optimization methods and their continuous-time approximations, focusing on the role of momentum and discretization error.
...
Result: Provides a continuous-time model for the HB method with arbitrary step-size error control and reveals implicit regularization effects.

Conclusion: The approach offers new theoretical tools for studying optimization methods, supported by numerical experiments, with applications in deep learning.
...
<details>
  <summary>Details</summary>
Motivation: To determine if VLMs inherit the prompt sensitivity of LLMs and identify which models are most robust to prompt variations.

Method: PARC framework evaluates VLMs using plausible prompt variations, a novel reliability score, and calibration for cross-dataset analysis.

Result: VLMs show language prompt sensitivity in the vision domain, with InternVL2 models being the most robust. Prompt sensitivity may relate to training data.
...
TL;DR: A Model-Router framework improves sparse and intermittent demand forecasting by dynamically selecting the best model (classical, ML, or DL) for each product, achieving 11.8% higher accuracy and faster inference.


<details>
  <summary>Details</summary>
Motivation: Traditional forecasting models struggle with sparse and intermittent demand, impacting inventory management.
...
Method: Introduces a growable, modular neural network where modules selectively combine previous policies with internal policies, enabling linear parameter growth with tasks.

Result: Achieves greater knowledge transfer and performance in benchmark continuous control and visual problems compared to other methods.
...
<details>
  <summary>Details</summary>
Motivation: Silent errors in DL training are hard to detect and diagnose, impacting model performance.

Method: TRAINCHECK infers invariants tailored for DL training to proactively detect errors and offers debugging support.

Result: Detected 18 out of 20 real-world silent errors in one iteration and found 6 unknown bugs in training libraries.

Conclusion: TRAINCHECK effectively identifies silent training errors and aids in debugging, improving DL training reliability.
...
<details>
  <summary>Details</summary>
Motivation: DXA scans for body composition measurement are costly and time-consuming, necessitating a cheaper, efficient alternative using 3D optical images and biomarkers.

Method: A $p$-Laplacian regression model is applied to biomarkers from 3D optical images, alongside comparisons with supervised models like SVR and Least Squares SVR.
...
Conclusion: The $p$-Laplacian model is a promising tool for healthcare in data-constrained environments, offering a viable alternative to DXA scans.
...
Method: Train smaller models with GRPO, simple rewards, tool-calling interface, extra tokens for tool results, and data favoring hard examples.

Result: Improved performance on VQA tasks due to better visual info from tools.

Conclusion: Combining GRPO, tools, and strategic training boosts VLM performance efficiently.
...
<details>
  <summary>Details</summary>
Motivation: MLLMs face deployment challenges in real-world scenarios due to distributed data and privacy constraints. Federated Learning (FL) is a solution, but existing FL methods are impractical for large-scale MLLMs.
...
Result: FedNano reduces client storage by 95% and communication overhead to 0.01% of model parameters, outperforming prior FL baselines.
...
Conclusion: Advanced surrogate models, especially DGP, enhance predictive accuracy for robust and data-efficient materials design.
...
<details>
  <summary>Details</summary>
Motivation: To address the critical need for reliable SSD health prediction to ensure data reliability and reduce failure risks.
...
Conclusion: The model offers a practical solution for SSD health prediction, reducing data loss risks and optimizing maintenance costs in industrial and cloud environments.
...
<details>
  <summary>Details</summary>
Motivation: Healthcare data is often limited or incomplete, and ML models lack transparency, hindering trust and performance, especially in critical areas like AMD diagnosis.

Method: Introduced CACTUS, a comprehensive tool for AMD classification, focusing on explainability and flexibility by identifying key factors and eliminating biased data.
...
Conclusion: CACTUS addresses data limitations and transparency issues in ML for healthcare, improving AMD classification and clinician feedback.
...
<details>
  <summary>Details</summary>
Motivation: Current anomaly detection systems using Autoencoders face challenges due to non-standardized separation thresholds, impacting detection performance.
...
Result: The study demonstrates the potential of machine learning to optimize threshold selection in Autoencoders.

Conclusion: Automating threshold definition with machine learning enhances anomaly detection performance in Autoencoders.
...
<details>
  <summary>Details</summary>
Motivation: To enhance one-class classification when some negative samples are available, improving OC-SVM without altering its core implementation.

Method: Proposes GRKneg, a kernel improvement for OC-SVM using negative samples, and evaluates it against standard OC-SVM and binary SVM with varying negative sample sizes.

Result: GRKneg outperforms standard OC-SVM and binary SVM with limited negative samples, though binary SVM excels with abundant negatives.
...
<details>
  <summary>Details</summary>
Motivation: To understand the structure and behavior of loss landscapes in neural networks, particularly quasi-flat regions and their impact on optimization.

Method: Characterizes channels in the loss landscape using gradient dynamics, geometry, and functional interpretation, focusing on the divergence of output weights and equality of input weights.

Result: Gradient flow solvers and optimization methods like SGD or ADAM frequently reach these channels, which resemble flat local minima but lead to gated linear units.
...
Method: Introduced an event-driven framework where only a subset of clients are activated per event, and integrated DLR for non-convex models.

Result: The framework showed stability under non-stationary data, reduced communication/computation costs, and outperformed existing online VFL methods.

Conclusion: The event-driven online VFL framework effectively addresses asynchronous data challenges and improves efficiency in non-stationary environments.
...
<details>
  <summary>Details</summary>
Motivation: To improve general reasoning in LLMs using RL, as current efforts focus narrowly on math and code, lacking scalable reward signals for diverse domains.

Method: Developed Guru, a corpus with domain-specific rewards, deduplication, and filtering. Evaluated RL training across six reasoning domains and introduced Guru-7B and Guru-32B models.

Result: Guru models outperform baselines by 7.9% and 6.7% on a 17-task suite, showing RL facilitates skill acquisition in domains with limited pretraining exposure.

Conclusion: RL training in diverse reasoning domains, especially those less seen in pretraining, enhances LLM performance, with Guru models demonstrating significant improvements.
...
<details>
  <summary>Details</summary>
Motivation: Address the neglected query cost in federated black-box prompt tuning for cloud-based LLMs.
...
Conclusion: FedOne effectively maximizes query efficiency in federated black-box prompt learning.
...
<details>
  <summary>Details</summary>
Motivation: Addressing fair decision-making under limited information about arm rewards in multi-agent systems.
...
<details>
  <summary>Details</summary>
Motivation: Previous works overlooked training instability in sequential representations for sepsis treatment, which harms policy performance. This work addresses this gap.

Method: Uses Controlled Differential Equations (CDE) with early stopping/stabilization and correlation regularization with clinical scores (SOFA, SAPS-II, OASIS).

Result: Stable CDE representations correlate with acuity scores and enable superior RL policies (WIS return > 0.9), while unstable training fails (WIS return ~ 0).

Conclusion: Training stability is crucial for CDE-based representation learning in clinical RL, and stable CDEs capture clinically meaningful patterns.
...
Result: Models trained with SFT-GO outperform baselines across benchmarks, datasets, and base models.
...
<details>
  <summary>Details</summary>
Motivation: Accurate error estimation in unseen test domains is crucial for safe ML systems. Prior DIS² methods compete in overlapping regions, limiting reliability.
...
Conclusion: ODD provides a more reliable and accurate error estimation method for distribution shifts, with publicly available code and results.
...
<details>
  <summary>Details</summary>
Motivation: Conventional reinforcement learning methods for hyperparameter optimization are computationally expensive, limiting widespread adoption.

Method: SPG extends the base model with temporary modules to generate state-action trajectories in a single forward pass, reducing computational costs.

Result: SPG improves model performance by +0.2∼7% across diverse datasets (ImageNet, COCO, GLUE, SQuAD, SUPERB) with low computational overhead.

Conclusion: SPG is a scalable and efficient alternative for hyperparameter optimization, demonstrating consistent improvements across tasks.
...
<details>
  <summary>Details</summary>
Motivation: Early prediction of MS disability progression is difficult due to disease heterogeneity. The study aims to improve prediction accuracy by integrating sparse clinical data with digital Floodlight data.

Method: The study used tabular and time-series foundation models (FMs), a custom multimodal attention-based transformer, and machine learning methods to predict disability at 48 and 72 weeks.

Result: Integrating digital data improved prediction performance over clinical data alone. A transformer model with unimodal embeddings performed best, but the multimodal transformer consistently outperformed unimodal models.

Conclusion: The findings highlight the potential of FMs and multimodal approaches for extracting predictive signals from diverse clinical and digital data, improving prognostics for MS and other complex diseases.
...
<details>
  <summary>Details</summary>
Motivation: To reduce the overhead of full fine-tuning in large Transformer models while avoiding the drawbacks of existing PEFT methods like latency or suboptimal convergence.

Method: SoKA uses Kronecker-product tensor factorization with SVD-driven initialization and dynamic rank selection to optimize task-specific parameters.
...
<details>
  <summary>Details</summary>
Motivation: Existing DI methods rely on in-distribution held-out data, which is rarely available, limiting their practicality. The goal is to overcome this by synthetically generating such data.

Method: The approach involves (1) training a data generator on a suffix-based completion task to create high-quality synthetic data and (2) post-hoc calibration to bridge likelihood gaps between real and synthetic data.
...
Conclusion: The method reliably empowers copyright owners to verify data usage, making it practical for real-world applications.
...
Method: Combines Seq2Seq VAE with iterative loops to balance chemical diversity, quality, and multi-target affinity, using strategic chemical filters.

Result: Generated diverse pan-inhibitor candidates for coronavirus proteases, enhancing exploration of beneficial chemical space.
...
<details>
  <summary>Details</summary>
Motivation: To understand why ICL remains effective despite imperfect labels and to theoretically analyze how different model architectures handle such scenarios.

Method: Theoretical study using a binary Gaussian mixture model (GMM) with missing labels, comparing one-layer linear attention models to multilayer/looped transformers.

Result: Multilayer transformers implicitly construct estimators that exploit unlabeled data, with performance scaling exponentially with depth. Looping off-the-shelf models improves semi-supervised learning.

Conclusion: Depth in transformers is crucial for leveraging unlabeled data in ICL, and looping can enhance semi-supervised tabular learning performance.
...
<details>
  <summary>Details</summary>
Motivation: The complexity of black-box AI models like Deep Neural Networks undermines trust due to lack of transparency. XAI aims to provide explanations, but their evaluation lacks standardization.

Method: A systematic literature review following PRISMA guidelines, analyzing 362 publications and categorizing 41 metric groups. A three-dimensional framework (VXAI) is proposed.

Result: The VXAI framework offers a comprehensive, structured overview of XAI evaluation, enabling systematic metric selection and comparability.

Conclusion: The VXAI framework fills a critical gap in XAI evaluation, providing a flexible foundation for future research and standardization.
...
<details>
  <summary>Details</summary>
Motivation: To address the gap in systematic reviews of reward models in RL, guiding policy optimization by aligning rewards with true objectives.

Method: Categorizes reward modeling approaches by source, mechanism, and learning paradigm, and reviews evaluation techniques.

Result: Provides a comprehensive overview of established and emerging reward modeling methods.

Conclusion: Highlights promising research directions, filling the literature gap in reward model reviews.
...
Method: Use memory-based architectures to handle partial observability in zero-shot RL.

Result: Improved performance over memory-free baselines in partially observable domains.

Conclusion: Memory-based methods are effective for zero-shot RL in partially observable settings.
...
Conclusion: HiPreNets offer a structured approach to enhance neural network precision by addressing residuals and adaptive techniques.
...
TL;DR: A novel technique simplifies warping paths to highlight key transformations (shift, compression, amplitude differences) in time series comparison, improving interpretability.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack qualitative comparison and fail to convey broader structural relationships between subsequences in time series.
...
Conclusion: The method provides a clearer, more interpretable comparison of time series by focusing on key transformations.
...
TL;DR: The paper studies hallucinations in LLM-based embodied agents, revealing high hallucination rates under scene-task inconsistencies and offering insights for improvement.


<details>
  <summary>Details</summary>
Motivation: To understand and quantify hallucinations in LLMs when used in embodied agents, especially under scene-task mismatches.

Method: Constructed a hallucination probing set from an existing benchmark, tested 12 models in two simulation environments.
...
Conclusion: Highlights LLM limitations in handling infeasible tasks and provides guidance for developing more robust planning strategies.
...
<details>
  <summary>Details</summary>
Motivation: Post-hoc attribution methods are non-robust to small input perturbations, undermining their trustworthiness, necessitating robust guarantees for attribution scores.
...
Conclusion: The proposed framework enables reliable use of attribution methods in downstream tasks by providing robustness guarantees.
...
TL;DR: The paper proposes a Neural Canonical Polyadic Factorization (NCPF) model for robust traffic data imputation, combining low-rank tensor algebra with deep learning to handle missing data in urban traffic systems.


<details>
  <summary>Details</summary>
Motivation: Accurate spatiotemporal traffic analysis is crucial for urban mobility optimization, but missing data from sensor failures and heterogeneous sensing gaps complicates reliable modeling.

Method: NCPF integrates CP decomposition into neural architecture via learnable embeddings, using hierarchical feature fusion and MLPs to model multilinear interactions and spatiotemporal couplings.

Result: Evaluations on six datasets show NCPF outperforms six state-of-the-art baselines in traffic data imputation.

Conclusion: NCPF unifies interpretable factor analysis with neural networks, offering a flexible solution for high-dimensional traffic data imputation, supporting advanced transportation systems.
...
<details>
  <summary>Details</summary>
Motivation: To address unexplored over-squashing in STGNNs, where temporal dimensions amplify information propagation challenges.

Method: Theoretical analysis and empirical validation on synthetic and real-world datasets.

Result: Convolutional STGNNs favor distant temporal information, and time-and-space or time-then-space paradigms are equally affected.

Conclusion: Provides insights into STGNN dynamics and guidance for more effective designs.
...
<details>
  <summary>Details</summary>
Motivation: Address the lack of unified taxonomy and systematic evaluation for verifying machine unlearning operations, driven by privacy, security, and legal compliance needs.

Method: Proposes a taxonomy categorizing verification into behavioral and parametric methods, examining their assumptions and practical vulnerabilities.

Result: Identifies gaps in current verification research and provides a foundation for more robust unlearning verification mechanisms.

Conclusion: Highlights open problems and aims to guide future research toward efficient and theoretically grounded verification methods.
...
TL;DR: RePCS is a lightweight, model-agnostic method to detect if RAG systems rely on memorized data instead of retrieved context, using KL divergence between inference paths.
...
Result: Achieves a ROC-AUC of 0.918 on Prompt-WNQA, outperforming prior methods by 6.5 points with minimal latency overhead.
...
TL;DR: Proposes ImprovDML, a decentralized DML framework for high accuracy, privacy, and Byzantine resilience, using resilient vector consensus and Gaussian noise.
...
Method: Leverages resilient vector consensus for aggregation and adds multivariate Gaussian noise for privacy.
...
<details>
  <summary>Details</summary>
Motivation: Current methods for feature description in neural networks are limited by assumptions of monosemanticity and lack robustness, hindering accurate model understanding.
...
Result: PRISM outperforms existing methods in description quality and polysemanticity capture, as shown in benchmarking with language models.

Conclusion: PRISM enhances interpretability by addressing polysemanticity, offering more accurate and comprehensive feature descriptions.
...
Result: Validated on grid worlds, labyrinths, and animal videos, SKIL identifies reusable skills, learns compositional policies, and generates realistic trajectories beyond traditional models.

Conclusion: SKIL provides a concise, principled framework for understanding how complex behaviors emerge from dynamic combinations of fundamental motor primitives.
...
TL;DR: The paper extends Gold's inductive inference framework to include computational observations and restricted input sources, enabling the study of learnability of general recursive functions under realistic constraints.
...
Conclusion: The study reveals connections to finite-state transducer inference and highlights challenges in learning under computational constraints.
...
TL;DR: The paper evaluates ML techniques' accuracy for solving the 1D Poisson equation, highlighting the importance of data function space for generalization. It provides theoretical bounds, empirical validation, and a new cross-validation method.


<details>
  <summary>Details</summary>
Motivation: To rigorously assess ML's quantitative accuracy in scientific applications, focusing on differential equations like the 1D Poisson equation.

Method: Analyzes generalization bounds, convergence rates, and training dynamics for white-box and black-box models, including deep linear models and neural networks.

Result: Generalization to true physical equations isn't guaranteed; different models show opposing behaviors. Green's function representations can be extracted from black-box models.
...
TL;DR: DAILOC is a domain-incremental learning framework for Wi-Fi fingerprinting-based indoor localization, addressing device heterogeneity and temporal variations with disentanglement and memory-guided alignment, outperforming existing methods.
...
Result: DAILOC achieves up to 2.74x lower average error and 4.6x lower worst-case error compared to state-of-the-art methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing DL models for indoor localization lack interpretability, making it hard to understand predictions or adapt to environmental changes over time.

Method: LogNet uses logic gates to transparently identify influential access points and diagnose noise impact on localization decisions.
...
Conclusion: LogNet improves interpretability and long-term reliability of DL-based indoor localization, demonstrating superior performance in real-world deployments.
...
TL;DR: The paper explores federated learning's robustness to Byzantine behavior, revealing a tradeoff between averaging and validity conditions. It provides lower and upper bounds on centroid approximation and introduces a new algorithm for better results.
...
Method: The study analyzes validity conditions and their impact on centroid approximation, introduces a new algorithm for convex validity, and provides theoretical bounds. Empirical evaluations in federated settings complement the analysis.

Result: Lower bound of min{(n-t)/t, √d} on centroid approximation under box validity, upper bound of 2min{n, √d}, and a new algorithm achieving √2d-approximation under convex validity. Empirical results validate the bounds.

Conclusion: The paper establishes tight bounds for centroid approximation in federated learning, demonstrating the limitations of validity conditions and proposing practical solutions for Byzantine robustness.
...
TL;DR: LoX enhances LLM safety by extrapolating low-rank subspaces, reducing attack success rates by 11-54% without additional training.
...
Method: Proposes Low-Rank Extrapolation (LoX), a training-free method to robustify safety subspaces in LLMs.

Result: LoX reduces attack success rates by 11-54% and moves parameters to a flatter, less sensitive zone.

Conclusion: LoX effectively improves safety robustness against fine-tuning attacks while maintaining task adaptability.
...
TL;DR: The paper introduces GFLC, a method to correct biased and noisy labels in ML datasets while maintaining fairness, improving performance-fairness trade-offs.
...
Conclusion: GFLC effectively corrects label noise and enhances fairness in ML models.
...
TL;DR: DOVA-PATBM is a geo-computational framework for optimizing EV charging infrastructure, balancing urban, suburban, and rural needs with scalable, data-driven methods.
...
Method: Uses a hierarchical H3 grid, graph neural network centrality, Voronoi tessellation, and a greedy heuristic to optimize charger placement under constraints.
...
Conclusion: The framework bridges academic optimization and practical infrastructure policy, proving effective for large-scale deployment.
...
TL;DR: Federated learning (FL) for BrainAGE estimation in stroke patients shows promise, outperforming single-site models and maintaining privacy. BrainAGE is linked to vascular risks and functional outcomes.


<details>
  <summary>Details</summary>
Motivation: To address privacy concerns in training BrainAGE models while maintaining accuracy, and to explore BrainAGE's association with clinical outcomes in stroke patients.

Method: Used FLAIR images from 1674 patients across 16 centers, comparing centralized learning, FL, and single-site learning. Evaluated BrainAGE's link to vascular risks and functional outcomes using logistic regression.

Result: FL outperformed single-site models, though centralized learning was most accurate. BrainAGE was higher in diabetes patients and significantly associated with post-stroke recovery.

Conclusion: FL enables accurate BrainAGE predictions without data sharing. BrainAGE's link to vascular risks and recovery suggests its prognostic value in stroke care.
...
TL;DR: The paper proposes a credit risk management framework for 3PL-led SCF in CBEC, using generative AI to improve SME financing.
...
Method: Uses Quantile-Regression-based Generative Metamodeling (QRGMM) for risk estimation, integrated with DeepFM for covariate interactions, to assess credit risk and determine loan sizes.

Result: Validated on synthetic and real-world data, the model effectively manages credit risk and loan sizing.

Conclusion: The framework pioneers generative AI in CBEC SCF, enhancing credit practices and SME capital access.
...
Method: AutoRule uses a reasoning model to interpret preferences, extracts candidate rules, and synthesizes them into a unified set. It then uses these rules as auxiliary rewards during policy optimization.
...
<details>
  <summary>Details</summary>
Motivation: Adapting LLMs to privacy-sensitive domains like healthcare and finance is challenging due to data scarcity and privacy constraints. Existing MPC-based methods are limited to inference, not fine-tuning.

Method: SecFwT introduces forward-only tuning to eliminate backward/optimizer computations and uses MPC-friendly Random Feature Attention to approximate softmax attention.

Result: SecFwT significantly improves efficiency and privacy preservation, enabling scalable and secure LLM fine-tuning.

Conclusion: SecFwT successfully addresses key obstacles in MPC-based LLM fine-tuning, offering a practical solution for privacy-critical applications.
...
<details>
  <summary>Details</summary>
Motivation: To understand why dense latents in SAEs persist and whether they serve functional roles in language models, rather than being mere training artifacts.

Method: Systematic analysis of dense latents' geometry, function, and origin, including ablation studies and a taxonomy of their roles.

Result: Dense latents are intrinsic to the residual space, serve functional roles (e.g., position tracking, semantic features), and evolve from structural to output-oriented signals across layers.

Conclusion: Dense latents are meaningful and functional in language models, not noise, and should not be dismissed.
...
TL;DR: Predicting abnormal lab test values using performed tests by treating tabular data as a set translation problem, improving AUC by up to 8%.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis via predicting unperformed test abnormalities from existing lab results, focusing on CBC and routine panels.
...
Conclusion: The method effectively bridges LLMs with tabular data, enhancing clinical lab predictions without imputing missing values.
...
Method: A two-stage training process: first, train the student NNP with data from an off-the-shelf teacher model, then fine-tune with a small high-accuracy DFT dataset.
...
Conclusion: The proposed framework is effective for stable MD simulations and significantly reduces computational costs without sacrificing accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models in FWI struggle with iterative sampling and unclear integration of approximate velocity models, limiting their practicality.
...
Result: Experiments show cI²SB outperforms conditional diffusion models, achieving high sample fidelity with fewer neural function evaluations (NFEs).

Conclusion: The cI²SB framework offers a more efficient and effective solution for FWI, validated by superior performance and code availability.
...
TL;DR: The paper proposes a method to improve one-run differential privacy (DP) auditing in black-box settings by using stronger membership inference attacks (MIA) with quantile regression, achieving tighter bounds efficiently.
...
Method: The approach incorporates quantile regression for MIA to enhance one-run auditing in black-box settings, evaluated on CIFAR-10-trained models with DP-SGD.

Result: The proposed method achieves tighter empirical lower bounds while maintaining computational efficiency.

Conclusion: The study demonstrates that stronger MIA techniques can effectively improve one-run DP auditing in black-box settings.
...
<details>
  <summary>Details</summary>
Motivation: To apply DiTs to molecular conformer generation despite challenges like discrete graph integration, Euclidean symmetries, and variable molecule sizes.

Method: DiTMC uses a modular architecture separating 3D coordinate processing from atomic connectivity conditioning, with graph-based strategies and flexible attention mechanisms.

Result: DiTMC achieves state-of-the-art precision and physical validity on benchmarks (GEOM-QM9, -DRUGS, -XL).

Conclusion: DiTMC's architectural and symmetry choices improve sample quality and efficiency, advancing large-scale molecular structure modeling.
...
TL;DR: The paper proposes a novel method for learning ground metrics in optimal transport, addressing limitations of predefined and supervised approaches, and demonstrates its effectiveness on scRNA-seq data.
...
Method: A new approach learns global ground metrics for arbitrary distributions over a shared metric space, using only class labels at the distribution level.
...
Conclusion: The method is effective and interpretable, as shown by its application to multi-disease scRNA-seq data.
...
TL;DR: A novel framework for exploration using diffusion models, leveraging entropy maximization and density estimation, with scalable solutions and empirical validation.
...
Method: Proposes a framework based on entropy maximization over a diffusion model's data manifold, using density estimation and mirror descent for scalable fine-tuning.

Result: Convergence to optimal exploratory diffusion model is proven, with promising empirical results on synthetic and high-dimensional tasks.
...
TL;DR: The paper addresses minimizing disease extinction time in an SIS model with an unknown graph, proposing a novel graph-learning algorithm and efficient vaccination strategies.


<details>
  <summary>Details</summary>
Motivation: Existing works assume known graphs for optimal vaccination, which is impractical. This work tackles the challenge of unknown graphs and observed infection states.

Method: The problem is split into graph learning and vaccination strategy. A novel inclusion-exclusion-based learning algorithm is proposed, alongside optimal and heuristic vaccination algorithms.

Result: The learning algorithm's sample complexity is established, and polynomial-time solutions for SRM are proven for bounded treewidth graphs. Greedy heuristics are also effective.

Conclusion: Experiments validate the proposed learning and vaccination algorithms, showing practical applicability on synthetic and real-world data.
...
<details>
  <summary>Details</summary>
Motivation: Current GAD methods assume uniform homophily in normal nodes, which fails in real-world datasets with diverse homophily. RHO addresses this limitation.
...
Result: RHO significantly outperforms state-of-the-art methods on eight real-world GAD datasets.

Conclusion: RHO effectively learns diverse homophily patterns in normal nodes, improving anomaly detection performance.
...
TL;DR: A user-guided projection framework for dimensionality reduction (DR) leverages MLLMs to enable customizable, interpretable visualizations via natural-language prompts, enhancing interactivity and semantic exploration.


<details>
  <summary>Details</summary>
Motivation: Current DR techniques lack the ability to explore semantic structures not explicitly present in data dimensions, limiting their interpretability and adaptability to user needs.

Method: The framework uses zero-shot classification with Multimodal Large Language Models (MLLMs) to dynamically steer projections based on natural-language guiding prompts, allowing users to specify high-level semantic relationships.
...
Conclusion: This approach bridges automated DR and human-centered exploration, offering flexible, adaptive projections tailored to specific analytical needs.
...
TL;DR: LIT-LVM improves prediction accuracy for high-dimensional interaction terms in linear predictors by using low-dimensional latent vectors, outperforming elastic net and factorization machines.


<details>
  <summary>Details</summary>
Motivation: Linear predictors with interaction terms often face overfitting in high-dimensional settings. The paper hypothesizes that interaction term coefficients have a low-dimensional structure, which can be leveraged for better estimation.

Method: The proposed LIT-LVM method represents features with low-dimensional latent vectors, acting as structured regularization to mitigate overfitting.

Result: LIT-LVM outperforms elastic net and factorization machines in prediction accuracy, especially with many interaction terms relative to sample size. It also provides interpretable latent representations.

Conclusion: LIT-LVM is an effective approach for estimating interaction terms in linear predictors, offering both improved accuracy and interpretability.
...
TL;DR: This paper presents a systematic review of adversarial attacks on tabular machine learning models, summarizing trends, attack strategies, and practical considerations, while identifying challenges and open questions.


<details>
  <summary>Details</summary>
Motivation: Research on adversarial attacks in tabular data is fragmented compared to fields like computer vision and NLP, necessitating a structured overview.

Method: The paper conducts a systematic literature review to categorize attack strategies and analyze their real-world applicability.

Result: The review highlights key trends, practical considerations, and gaps in current research on adversarial attacks for tabular data.

Conclusion: The paper aims to guide future research by providing a clear overview of adversarial vulnerabilities in tabular machine learning.
...
<details>
  <summary>Details</summary>
Motivation: To make the analysis of SGD in overparameterized models more accessible and practical for further improvements in training realistic models.
...
Conclusion: The simplified analysis enhances accessibility and could aid in advancing techniques like mini-batching and learning rate scheduling.
...
Method: Empirical analyses and proposed interventions to stabilize gradient flow, tested on various agents and environments.

Result: Simple interventions enable robust performance across different network scales, validated experimentally.

Conclusion: The study provides effective, easy-to-implement solutions for scaling deep reinforcement learning networks without performance degradation.
...
<details>
  <summary>Details</summary>
Motivation: Real-time mesh smoothing is challenging due to costly global updates in classical Ricci-flow solvers and slow convergence or brittle tuning in greedy heuristics.
...
Result: On 110 SJTU-TMQA meshes, MicroRicci reduces iterations (2.4x speedup), tightens curvature spread, and improves UV-distortion-to-MOS correlation (r = -0.93), adding only 0.25 ms per iteration.

Conclusion: MicroRicci's efficiency, adaptability, and high-quality results make it ideal for real-time, resource-limited applications in graphics and simulation.
...
Method: DP-GRAPE introduces three modifications: privatizing gradients after projection, replacing SVD-based subspaces with random Gaussian matrices, and applying projection during backpropagation.

Result: DP-GRAPE reduces memory usage by 63-70% for tasks like pre-training Vision Transformers and fine-tuning RoBERTa-Large, while maintaining performance. It also scales to large models like OPT (6.7B parameters).

Conclusion: DP-GRAPE offers a memory-efficient alternative to standard DP methods without sacrificing utility, enabling scalable DP training for large models.
...
TL;DR: The paper addresses over-conservatism in AWRs due to poor explorations in offline RL by proposing CAWR, which uses robust loss functions and prioritized experience replay to improve policy optimization.


<details>
  <summary>Details</summary>
Motivation: The study aims to tackle the issue of over-conservative policies in AWRs caused by poor explorations in suboptimal offline data, which leads to distribution shift and estimation bias.

Method: The authors analyze the impact of poor explorations on the optimal policy and its approximation, then propose CAWR, incorporating robust loss functions and advantage-based prioritized experience replay.

Result: Numerical experiments on the D4RL benchmark demonstrate that CAWR learns superior policies from suboptimal data, enhancing policy optimization performance.

Conclusion: CAWR effectively mitigates over-conservatism in AWRs by addressing poor explorations, improving policy learning from offline data.

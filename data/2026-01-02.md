...
TL;DR: SCP is an open-source standard for a global network of autonomous scientific agents, featuring unified resource integration and orchestrated experiment lifecycle management to enable seamless, secure, and reproducible collaboration across platforms and institutions.


<details>
  <summary>Details</summary>
Motivation: To accelerate discovery by enabling a global network of autonomous scientific agents, addressing the need for seamless integration of scientific resources across disparate platforms and institutional boundaries, and facilitating secure, large-scale collaboration between heterogeneous AI systems and human researchers.

Method: The paper introduces SCP, a protocol built on two foundational pillars: (1) Unified Resource Integration: a universal specification for describing and invoking scientific resources (software tools, models, datasets, physical instruments); and (2) Orchestrated Experiment Lifecycle Management: a secure service architecture with a centralized SCP Hub and federated SCP Servers to manage the complete experiment lifecycle (registration, planning, execution, monitoring, archival), enforce fine-grained authentication and authorization, and orchestrate traceable, end-to-end workflows that bridge computational and physical laboratories. Based on SCP, the authors have constructed a scientific discovery platform offering over 1,600 tool resources.

Result: SCP facilitates secure, large-scale collaboration between heterogeneous AI systems and human researchers across diverse use cases, significantly reducing integration overhead and enhancing reproducibility. The authors have built a scientific discovery platform based on SCP that offers researchers and agents a large-scale ecosystem of more than 1,600 tool resources.

Conclusion: The paper concludes that by standardizing scientific context and tool orchestration at the protocol level, SCP establishes essential infrastructure for scalable, multi-institution, agent-driven science, enabling secure, large-scale collaboration between heterogeneous AI systems and human researchers while significantly reducing integration overhead and enhancing reproducibility.
...
TL;DR: DDFT is a new protocol to measure epistemic robustness in language models, showing that model size doesn't predict factual accuracy under stress.


<details>
  <summary>Details</summary>
Motivation: Static benchmarks fail to assess how well models maintain factual accuracy under degrading information or adversarial conditions.

Method: Proposed DDFT protocol with two-system cognitive model (Semantic System and Epistemic Verifier), evaluated 9 models across 8 domains at 5 compression levels.

Result: Epistemic robustness is orthogonal to model parameters and architecture, with error detection as the key factor; small models can outperform large ones in robustness.

Conclusion: DDFT provides tools for assessing epistemic robustness, challenging assumptions about model size and reliability.
...
TL;DR: CASCADE is a self-evolving LLM agent framework that transitions from tool use to skill acquisition, achieving 93.3% success on scientific benchmarks through continuous learning and self-reflection mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current LLM agents are limited by their dependence on predefined tools or fragile tool generation approaches, which constrains their capability and adaptability to complex scientific tasks. There's a need to transition from basic 'LLM + tool use' paradigms to more advanced 'LLM + skill acquisition' frameworks that can handle the complexity of scientific research.

Method: The CASCADE framework enables LLM agents to master complex external tools and codify knowledge through two meta-skills: 1) continuous learning via web search and code extraction, and 2) self-reflection via introspection and knowledge graph exploration. The agents evolve through these self-evolving mechanisms rather than relying on predefined tools or brittle tool generation.

Result: CASCADE achieves a 93.3% success rate on SciSkillBench (a benchmark of 116 materials science and chemistry research tasks) using GPT-5, compared to only 35.4% without evolution mechanisms. The framework demonstrates real-world applications in computational analysis, autonomous laboratory experiments, and selective reproduction of published papers. CASCADE accumulates executable skills that can be shared across agents and scientists.

Conclusion: CASCADE represents a significant step toward scalable AI-assisted scientific research by enabling agents to evolve through skill acquisition rather than just tool use. The framework's ability to accumulate and share executable skills across agents and scientists addresses key limitations of current LLM agents. With real-world applications demonstrated in computational analysis, autonomous labs, and paper reproduction, CASCADE shows promise for transforming how AI supports complex scientific workflows.
...
TL;DR: McCoy framework combines LLMs and ASP for automated disease diagnosis from medical literature and patient data.


<details>
  <summary>Details</summary>
Motivation: Symbolic AI in healthcare is limited by the effort needed to build high-quality knowledge bases; McCoy aims to overcome this barrier.

Method: It orchestrates an LLM to translate medical literature into ASP code, combines it with patient data, and uses an ASP solver for diagnosis.

Result: Preliminary results show strong performance on small-scale disease diagnosis tasks.

Conclusion: McCoy delivers a robust, interpretable prediction framework leveraging the strengths of LLMs and ASP for medical applications.
...
TL;DR: SPARK is a multi-agent LLM framework for personalized search using dynamic persona-based agents with coordinated retrieval and emergent personalization properties.


<details>
  <summary>Details</summary>
Motivation: Personalized search systems are limited by static profiles or monolithic pipelines, failing to model users' evolving, multi-dimensional information needs effectively.

Method: SPARK uses persona-based LLM agents defined by role, expertise, task context, and domain, with a Persona Coordinator to activate relevant agents. Each agent performs independent retrieval-augmented generation with memory stores and reasoning, enabled by inter-agent collaboration protocols like shared memory, debate, and knowledge transfer.

Result: The framework yields testable predictions on coordination efficiency, personalization quality, and cognitive load distribution, with adaptive learning for continuous refinement.

Conclusion: SPARK integrates agent specialization and cooperative retrieval to advance search systems in capturing the complexity and context sensitivity of human information-seeking behavior.
...
TL;DR: ROAD is a novel Automatic Prompt Optimization framework that bypasses the need for labeled datasets by using automated debugging and a multi-agent architecture to convert failure logs into structured protocols, significantly improving LLM performance efficiently.


<details>
  <summary>Details</summary>
Motivation: Traditional APO methods rely on large, curated datasets, which are often unavailable in real-world software engineering during cold starts, where engineers face messy production logs and evolving failures.

Method: ROAD uses a multi-agent architecture with an Analyzer for root-cause analysis, an Optimizer for pattern aggregation, and a Coach for strategy integration, treating optimization as dynamic debugging to transform unstructured failure logs into Decision Tree Protocols.

Result: ROAD achieved a 5.6% increase in success rate (73.6% to 79.2%) and a 3.8% increase in search accuracy in three iterations on an academic benchmark, and improved agent performance by ~19% on complex reasoning tasks in retail.

Conclusion: Mimicking the human engineering loop of failure analysis and patching offers a data-efficient alternative to resource-intensive Reinforcement Learning for deploying reliable LLM agents in real-world settings.
...
TL;DR: Introduces LoongFlow, a self-evolving LLM agent framework that uses a cognitive PES paradigm and hybrid evolutionary memory to achieve high solution quality with lower computational cost compared to baselines.


<details>
  <summary>Details</summary>
Motivation: Existing evolutionary approaches for LLMs lack structured reasoning, leading to premature convergence and inefficient exploration in high-dimensional code spaces.

Method: Integrates LLMs into a 'Plan-Execute-Summarize' (PES) paradigm for evolutionary search, along with a hybrid evolutionary memory system combining Multi-Island models, MAP-Elites, and adaptive Boltzmann selection to maintain coherence and diversity.

Result: LoongFlow outperforms leading baselines like OpenEvolve and ShinkaEvolve by up to 60% in evolutionary efficiency on the AlphaEvolve benchmark and Kaggle competitions, discovering superior solutions.

Conclusion: LoongFlow advances autonomous scientific discovery by enabling expert-level solutions with reduced computational overhead, marking a significant step in self-evolving agent frameworks.
...
TL;DR: CogRec, a cognitive recommender agent combining LLMs and Soar architecture, improves recommendation accuracy, explainability, and long-tail problem handling.


<details>
  <summary>Details</summary>
Motivation: LLMs have limitations in interpretability ('Black-Box' nature), knowledge hallucination, and online learning, while cognitive architectures like Soar are structured but hard to acquire knowledge for. CogRec aims to synergize both to overcome these challenges.

Method: CogRec uses Soar as a symbolic reasoning engine with an LLM for knowledge initialization, populating working memory with production rules. It operates on a Perception-Cognition-Action cycle, dynamically querying the LLM at impasses to get reasoned solutions, which are transformed into symbolic production rules via Soar's chunking for online learning.

Result: Extensive evaluations on three public datasets show CogRec has significant advantages in recommendation accuracy, explainability, and efficacy in addressing the long-tail problem.

Conclusion: CogRec successfully integrates LLMs with Soar cognitive architecture, enabling continuous knowledge evolution and highly interpretable recommendations, addressing the trustworthiness and adaptability issues of LLMs in recommender systems.
...
TL;DR: This paper introduces a training-free graph-based approach for solving interactive reasoning tasks in the ARC-AGI-3 benchmark. The method uses vision-based frame processing and systematic state-space exploration with graph-structured representations. On the ARC-AGI-3 Preview Challenge, it solved a median of 30 out of 52 levels and ranked 3rd, outperforming LLM-based agents, with open-sourced code.


<details>
  <summary>Details</summary>
Motivation: The ARC-AGI-3 benchmark includes game-like tasks requiring agents to infer task mechanics through limited interactions and adapt to increasing complexity. State-of-the-art LLMs are currently incapable of reliably solving these tasks. The motivation is to develop a structured exploration strategy that can effectively handle sparse-feedback environments where LLMs fail.

Method: The approach combines vision-based frame processing with systematic state-space exploration using graph-structured representations. It segments visual frames into meaningful components, prioritizes actions based on visual salience, and maintains a directed graph of explored states and transitions. Actions are prioritized based on the shortest path to untested state-action pairs by tracking visited states and tested actions.

Result: On the ARC-AGI-3 Preview Challenge, the method solved a median of 30 out of 52 levels across six games and ranked 3rd on the private leaderboard. This substantially outperformed frontier LLM-based agents, demonstrating its effectiveness in interactive reasoning tasks.

Conclusion: The structured exploration strategy shows that explicit graph-structured exploration, even without learning, can serve as a strong baseline for interactive reasoning. It underscores the importance of systematic state tracking and action prioritization in sparse-feedback environments where current LLMs fail to capture task dynamics. The work highlights the potential of training-free approaches in such settings, with the code being open sourced.
...
TL;DR: A deep reinforcement learning approach is proposed to efficiently solve the Fleet Size and Mix Vehicle Routing Problem by integrating fleet composition and routing decisions.


<details>
  <summary>Details</summary>
Motivation: The Fleet Size and Mix Vehicle Routing Problem (FSMVRP) is complex due to simultaneous decisions needed on fleet composition and routing, particularly challenging in large-scale and time-constrained scenarios.

Method: The problem is formulated as a Markov Decision Process (MDP), and a novel FRIPN policy network integrates fleet composition and routing decisions, using input embeddings like remaining graph embedding.

Result: Extensive experiments in randomly generated instances and benchmark datasets show the method yields near-optimal solutions quickly and excels in computational efficiency and scalability, especially for larger or time-sensitive problems.

Conclusion: The approach shows potential for practical application in logistics, and ideas like FRIPN offer inspiration for extending DRL techniques to other Vehicle Routing Problem variants.
...
TL;DR: RSA is a risk-aware alignment method for fine-tuning language models, using nested risk measures to optimize safety and suppress harmful behaviors.


<details>
  <summary>Details</summary>
Motivation: Existing safety alignment methods, like Safe RLHF and SACPO, are risk-neutral, insufficient for controlling deviations from reference policies and rare catastrophic behaviors, requiring a risk-aware approach.

Method: RSA formulates safety alignment as a token-level risk-aware constrained policy optimization problem, solved through a stepwise alignment procedure with token-level updates derived from nested risk measures.

Result: Experimental results show that RSA achieves high helpfulness, ensures strong safety, and significantly suppresses low-probability high-impact unsafe responses compared to risk-neutral methods.

Conclusion: RSA effectively incorporates risk awareness into model alignment for improved safety and robustness against tail risks in language model fine-tuning.
...
TL;DR: A test-time adaptive LLM agent that performs exploratory inference through posterior-guided belief refinement without gradient updates, achieving better world state alignment than baseline methods with lower overhead.


<details>
  <summary>Details</summary>
Motivation: To develop an adaptive LLM agent that can perform exploratory inference under partial observability without relying on gradient-based updates or additional training, addressing the limitations of existing inference-time scaling methods.

Method: The agent maintains an external structured belief over environment state, iteratively updates it via action-conditioned observations, and selects actions by maximizing predicted information gain using a lightweight LLM-based surrogate. World alignment is assessed through a novel consistency reward between posterior belief and ground-truth environment configuration.

Result: Experiments demonstrate that the proposed method outperforms inference-time scaling baselines (prompt-augmented and retrieval-enhanced LLMs) in aligning with latent world states, while achieving significantly lower integration overhead.

Conclusion: The proposed method effectively enables LLM agents to operate under partial observability through belief refinement and information-driven action selection, showing better alignment with latent world states than baseline approaches while being computationally efficient.
...
TL;DR: A study on JEPA-WMs, a family of world models for AI planning in learned representation space, examining key components and proposing a model that outperforms baselines in navigation and manipulation tasks.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of developing AI agents that can solve diverse physical tasks and generalize to unseen ones, focusing on recent methods that plan in the learned representation space of world models for more efficient planning.

Method: Comprehensive experimentation with simulated and real-world robotic data, analyzing model architecture, training objectives, and planning algorithms to identify optimal approaches within the JEPA-WM family.

Result: A proposed model outperforms established baselines (DINO-WM and V-JEPA-2-AC) in both navigation and manipulation tasks.

Conclusion: The study successfully characterizes JEPA-WMs, identifies key technical choices for effective planning, and demonstrates improved performance, with code and data provided for validation and further research.
...
TL;DR: The paper proposes an interactive framework to evaluate spatial understanding in foundation model (FM) agents within symbolic map environments, revealing key roles of exploration, memory, and reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations of spatial ability in FMs rely on static inputs, overlooking the interactive nature of spatial understanding. This gap highlights the need for dynamic assessments.

Method: An interactive evaluation framework where agents explore partially observable grid-based maps with local observations, tested through six spatial tasks varying exploration, memory, and reasoning.

Result: Exploration affects experience acquisition but has limited impact on reasoning accuracy; memory (e.g., sequential and graph representations) improves performance on structure-intensive tasks; reasoning schemes enhance multi-step inference. Performance saturates beyond certain capability thresholds.

Conclusion: Improvements in spatial understanding require tailored mechanisms for representation and reasoning, not just scaling, emphasizing the importance of structured memory and interactive evaluation.
...
TL;DR: Study evaluates three LLMs on underrepresented math competition problems; DeepSeek-V3 performs best overall; all LLMs struggle with Geometry; distinct error patterns across models identified.


<details>
  <summary>Details</summary>
Motivation: Prior studies mostly use same datasets for benchmarking LLMs in mathematical reasoning, limiting generalizability and possibly not capturing diverse challenges. Need to analyze LLM performance on underrepresented mathematics competition problems.

Method: Prompted three leading LLMs (GPT-4o-mini, Gemini-2.0-Flash, DeepSeek-V3) with Missouri Collegiate Mathematics Competition problems in Calculus, Analytic Geometry, and Discrete Mathematics. Compared responses to known correct solutions to determine accuracy. Analyzed reasoning patterns and errors across problem types and models.

Result: DeepSeek-V3 performed best in all three categories (Calculus, Analytic Geometry, Discrete Mathematics) in reasoning and correct final answers. All three LLMs exhibited notably weak performance in Geometry. DeepSeek-V3 errors: computational and logical mistakes. GPT-4o-mini errors: logical and approach-related. Gemini errors: incomplete reasoning and rushed conclusions.

Conclusion: Evaluating LLMs on underrepresented mathematics competition datasets provides deeper insights into their distinct error patterns and highlights ongoing challenges in structured reasoning, particularly in Geometry.
...
TL;DR: Two-stage approach: teach LLMs basic spatial physics via supervised fine-tuning, then train LoRA adapters for multi-step planning; outperforms baselines and trains more stably.


<details>
  <summary>Details</summary>
Motivation: Spatial reasoning is crucial for tasks like navigation and planning, yet large language models often struggle with spatial transformations and multi-step planning in structured environments.

Method: A two-stage method: (1) supervised fine-tuning on elementary spatial transformations (rotation, translation, scaling) to impart basic spatial physics; (2) freezing the physics-aware model and training lightweight LoRA adapters within the GRPO framework for multi-step planning, using a synthesized ASCII-art dataset and corresponding ASCII-based RL environment for closed-loop training.

Result: The method outperforms baselines (generic backbone, physics-aware model, end-to-end RL) in both Dynamic (explicit state updates) and Static (model must maintain internal state) environments. It also converges faster and exhibits more stable training than end-to-end RL from scratch.

Conclusion: The two-stage approach—basic spatial physics via supervised fine-tuning followed by planning policies via LoRA adapters in GRPO—consistently outperforms baseline methods and demonstrates better training stability and convergence. Attention patterns reveal meaningful improvements in spatial understanding.
...
TL;DR: Proposed MCPAgentBench, a benchmark for evaluating LLM agents' tool-use capabilities, addressing limitations of current MCP evaluation sets.


<details>
  <summary>Details</summary>
Motivation: Current MCP evaluation sets have issues: reliance on external MCP services and lack of difficulty awareness. Need a benchmark to evaluate tool-use capabilities of agents.

Method: Constructed a dataset with authentic tasks and simulated MCP tools, using a dynamic sandbox environment with candidate tool lists containing distractors. Introduced comprehensive metrics for task completion rates and execution efficiency.

Result: Experiments on various LLMs showed significant performance differences in handling complex, multi-step tool invocations.

Conclusion: All code is open-source on GitHub, promoting reproducibility and future research.
...
TL;DR: Recursive Language Models (RLMs) enable LLMs to process long prompts by recursively examining and decomposing them, handling inputs far beyond standard context windows with improved quality and comparable cost.


<details>
  <summary>Details</summary>
Motivation: The limitations of fixed context windows in large language models hinder their ability to process arbitrarily long prompts effectively, requiring an inference-time scaling solution.

Method: Propose RLMs, a strategy where LLMs treat long prompts as an external environment, programmatically examine and decompose them, and recursively call themselves over prompt snippets.

Result: RLMs successfully handle inputs up to two orders of magnitude beyond model context windows and outperform base LLMs and common scaffolds in four long-context tasks with similar or cheaper cost per query.

Conclusion: RLMs provide an efficient and scalable inference-time approach for extending LLM capabilities to process very long prompts while maintaining or enhancing quality and cost-effectiveness.
...
TL;DR: RL-augmented LLM framework for multi-agent collaboration with GRPO optimization, achieving 3x speedup, 98.7% writing consistency, and 74.6% coding pass rate.


<details>
  <summary>Details</summary>
Motivation: Large Language Models (LLMs) perform well in language tasks but often lack collaborative awareness and struggle to optimize global performance in multi-agent settings.

Method: A reinforcement learning-augmented LLM agent framework that formulates cooperation as a decentralized partially observable Markov decision process (Dec-POMDP) and adopts centralized training with decentralized execution (CTDE). Introduces Group Relative Policy Optimization (GRPO) to jointly optimize agent policies with global signals during training and a simplified joint reward balancing task quality, speed, and coordination cost.

Result: On collaborative writing and coding benchmarks: 3x increase in task processing speed over single-agent baselines, 98.7% structural/style consistency in writing, and 74.6% test pass rate in coding.

Conclusion: The proposed framework provides a practical path toward reliable collaboration in complex workflows, consistently outperforming strong multi-agent LLM baselines.
...
TL;DR: Group deliberation multi-agent model with role division, self-game mechanism, and retrieval enhancement improves multi-hop reasoning accuracy and consistency.


<details>
  <summary>Details</summary>
Motivation: To address limitations of single large language models in complex reasoning tasks

Method: The model adopts a three-level role division architecture (generation, verification, integration agents) with a self-game mechanism for multi-path reasoning trajectories, retrieval enhancement for external knowledge, a composite reward function (factual consistency + logical coherence), and improved proximal policy optimization for collaborative training.

Result: Improves multi-hop reasoning accuracy by 16.8% on HotpotQA, 14.3% on 2WikiMultihopQA, and 19.2% on MeetingBank, while improving consistency by 21.5%.

Conclusion: The model achieves higher reasoning efficiency than mainstream multi-agent approaches, providing an effective and stable solution for complex reasoning tasks.
...
TL;DR: A framework called Youtu-Agent automates the generation and continuous evolution of LLM agents to reduce manual effort and enable adaptation in dynamic environments.


<details>
  <summary>Details</summary>
Motivation: Existing LLM agent frameworks suffer from high configuration costs and static capabilities, requiring extensive manual work for tool integration and prompt engineering, while deployed agents cannot adapt without expensive fine-tuning.

Method: Proposes a modular framework with a structured configuration system for flexible reuse and automated synthesis, featuring two generation modes: Workflow for standard tasks and Meta-Agent for complex requirements. Includes a hybrid policy optimization system with an Agent Practice module for in-context optimization and an Agent RL module for scalable reinforcement learning.

Result: Achieves state-of-the-art performance on WebWalkerQA (71.47%) and GAIA (72.8%) with open-weight models, tool synthesis success rate over 81%, performance improvements of +2.7% and +5.4% on AIME benchmarks through Practice module, and 40% speedup with steady improvements via Agent RL, enhancing coding/reasoning and searching capabilities up to 35% and 21% on benchmarks.

Conclusion: Youtu-Agent effectively addresses the challenges of high configuration costs and static capabilities in LLM agents, enabling automated generation and continuous evolution with demonstrated improvements in performance and adaptability across various benchmarks.
...
TL;DR: This paper presents a multi-modal cross-domain mixed fusion model with dual disentanglement for fault diagnosis, enhancing generalization under unseen working conditions without target domain data, validated on induction motor datasets.


<details>
  <summary>Details</summary>
Motivation: Existing fault diagnosis methods struggle with performance decline in real-world scenarios under unseen working conditions, domain adaptation relies on target domain samples, and single-modal signals overlook complementary multi-modal information, limiting generalization.

Method: Develop a dual disentanglement framework to decouple modality-invariant and modality-specific features and domain-invariant and domain-specific representations, implement a cross-domain mixed fusion strategy for diversity augmentation, and introduce a triple-modal fusion mechanism for adaptive integration of multi-modal heterogeneous information.

Result: The proposed method consistently outperforms advanced methods in experiments on induction motor fault diagnosis under unseen constant and time-varying working conditions, with ablation studies verifying the effectiveness of each component and multi-modal fusion.

Conclusion: The model successfully addresses limitations in fault diagnosis by improving generalization through dual disentanglement and multi-modal fusion, offering a robust solution without needing target domain data, with code publicly available for further use and validation.
...
TL;DR: BatteryAgent integrates physical features and LLMs for interpretable fault diagnosis in lithium-ion batteries, improving accuracy and providing root cause analysis.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for battery fault diagnosis lack interpretability and are limited to binary classification, hindering root cause analysis and maintenance recommendations.

Method: Proposes BatteryAgent, a hierarchical framework with three modules: Physical Perception Layer (mechanism-based features), Detection and Attribution Layer (Gradient Boosting Decision Trees and SHAP), and Reasoning and Diagnosis Layer (LLM for report generation).

Result: Achieves an AUROC of 0.986, corrects misclassifications on hard boundary samples, and enables multi-type interpretable diagnosis.

Conclusion: BatteryAgent offers a paradigm shift from passive detection to intelligent diagnosis, enhancing battery safety management through interpretable and comprehensive fault analysis.
...
TL;DR: This paper proposes four interpretable constructs (spatial practicality, habitual convenience, semantic coherence, commonsense appropriateness) for household object arrangement preferences, validates them via an online study, and shows they enable robots to generate human-aligned arrangements using an MCTS planner.


<details>
  <summary>Details</summary>
Motivation: Current latent preference models used in robotic household object rearrangement are effective at prediction but lack interpretability, making it unclear what factors guide human arrangement decisions. The authors aim to develop an explicit and interpretable formulation of these preferences.

Method: The authors designed an online questionnaire to capture the four interpretable constructs, validated it with a 63-participant study, and then integrated these constructs into a Monte Carlo Tree Search (MCTS) planner for robot rearrangement planning.

Result: The questionnaire validation study confirmed the distinctiveness and explanatory power of the four constructs across kitchen and living room scenarios. When integrated into an MCTS planner, the constructs generated arrangements that closely aligned with those created by human participants, demonstrating their utility.

Conclusion: The paper concludes that the four interpretable constructs provide a compact and actionable formulation of object arrangement preferences, which can effectively guide robotic planning to generate arrangements aligned with human preferences.
...
TL;DR: GenZ is a hybrid model combining foundational models and statistical modeling via interpretable semantic features discovered through an EM algorithm, improving prediction accuracy in house pricing and collaborative filtering.


<details>
  <summary>Details</summary>
Motivation: Large language models (foundational models) have broad domain knowledge but often fail to capture dataset-specific patterns critical for prediction tasks. The goal is to bridge foundational models and statistical modeling through interpretable semantic features that capture these local patterns.

Method: A generalized EM algorithm jointly optimizes semantic feature descriptors and statistical model parameters. It uses a frozen foundational model to classify items based on discovered features, treating these classifications as noisy observations of latent binary features that predict targets via learned statistical relationships.

Result: On house price prediction, GenZ achieves 12% median relative error using discovered semantic features from multimodal listing data, substantially outperforming a GPT-5 baseline (38% error). For Netflix movie embeddings, it predicts collaborative filtering representations with 0.59 cosine similarity purely from semantic descriptions, matching performance requiring ~4000 user ratings in traditional collaborative filtering.

Conclusion: GenZ successfully bridges foundational models and statistical modeling by discovering dataset-specific semantic features through an iterative EM algorithm. The approach enables interpretable features that capture local patterns, outperforming pure foundational models in prediction tasks like house pricing and collaborative filtering.
...
TL;DR: A method using constraint templates to extract flexible and personalized scheduling constraints for long-term care facilities, while excluding exceptions, to improve shift schedules.


<details>
  <summary>Details</summary>
Motivation: In long-term care facilities, scheduling conditions vary widely, necessitating interviews with managers to design facility-specific constraints, as existing methods lack personalization and exception handling.

Method: The method employs constraint templates that extract constraint combinations by adjusting parameters like days and staff focus, shifting between pattern or frequency extraction, and incorporating mechanisms to exclude exceptional constraints.

Result: Experiments showed the method successfully created schedules satisfying all hard constraints and reduced violations of soft constraints by avoiding the extraction of exceptional constraints.

Conclusion: The proposed constraint template method effectively customizes and optimizes work schedules for long-term care by accommodating variability and exceptions, enhancing practical applicability.
...
TL;DR: The paper introduces the Agentic Learning Ecosystem (ALE), an end-to-end infrastructure for developing agentic large language models. It also presents ROME, an open-source agent trained with ALE, and demonstrates its strong performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: The open-source community lacks a principled, end-to-end ecosystem to streamline the development of agentic LLMs that operate in real-world environments over multiple turns, despite the importance of such agents.

Method: ALE consists of three components: ROLL (post-training framework for weight optimization), ROCK (sandbox environment manager for trajectory generation), and iFlow CLI (agent framework for context engineering). The approach includes data composition protocols for synthesizing complex behaviors and a novel policy optimization algorithm called Interaction-based Policy Alignment (IPA), which assigns credit over semantic interaction chunks. ROME is released as an open-source agent trained on over one million trajectories using this infrastructure.

Result: Empirical evaluation introduces Terminal Bench Pro, a benchmark with improved scale and contamination control. ROME demonstrates strong performance across benchmarks like SWE-bench Verified and Terminal Bench, proving the effectiveness of the ALE infrastructure.

Conclusion: The ALE infrastructure successfully optimizes the production pipeline for agent LLMs, enabling the development of high-performing agentic models like ROME, which shows promise for real-world application.
...
TL;DR: Semi-automated data annotation pipeline for driving scenarios in Polish conditions, combining AI and human expertise to reduce cost and time while maintaining annotation quality.


<details>
  <summary>Details</summary>
Motivation: Manual annotation of heterogeneous, multimodal data for driving scenarios in Polish conditions is costly and time-consuming; a more efficient solution is needed.

Method: Human-in-the-loop approach combining AI with human expertise; uses 3D object detection algorithms for initial annotations, iterative model retraining, data anonymization, and domain adaptation.

Result: The developed tools and methodology achieve substantial time savings and ensure consistent, high-quality annotations across different sensor modalities.

Conclusion: The semi-automated pipeline reduces annotation costs and time while maintaining high-quality, consistent annotations, supporting the DARTS project's goal of creating a large-scale annotated dataset for autonomous vehicle research in Poland.
...
TL;DR: Iterative deployment of LLMs with user-curated data improves planning skills, connects to implicit RL with safety and training implications.


<details>
  <summary>Details</summary>
Motivation: To explore how iterative fine-tuning on user-curated data changes model properties, particularly in planning domains, and to link this to reinforcement learning mechanisms.

Method: Testing iterative deployment of LLMs on various planning domains, fine-tuning each model on data curated from previous deployments, and providing theoretical analysis.

Result: Observed substantial improvements in planning skills, with later models discovering much longer plans than initial ones, showing emergent generalization.

Conclusion: Iterative deployment implements implicit RL with unexplicit rewards, raising AI safety concerns and offering an alternative training regime based on data curation.
...
TL;DR: STAgent is a spatio-temporal specialized LLM agent with 10+ tools, trained via hierarchical data curation (1:10k filter) and cascaded SFT+RL, achieving strong TravelBench performance while preserving general capabilities.


<details>
  <summary>Details</summary>
Motivation: To create an agentic LLM specialized for spatio-temporal understanding to solve complex tasks like constrained POI discovery and itinerary planning while preserving general capabilities.

Method: Three-stage method: 1) stable tool environment with 10+ domain tools enabling asynchronous rollout/training, 2) hierarchical data curation filtering at 1:10,000 ratio for high-quality queries with diversity/difficulty, 3) cascaded training with seed SFT (guardian for difficulty measurement), second SFT on high-certainty queries, and ultimate RL on low-certainty data.

Result: STAgent shows promising performance on TravelBench benchmark while maintaining general capabilities across wide range of general benchmarks.

Conclusion: STAgent demonstrates the effectiveness of a specialized LLM for spatio-temporal tasks while maintaining general capabilities, achieved through a stable tool environment, hierarchical data curation, and cascaded training.
...
TL;DR: A framework for an LLM-based BEMS AI agent using natural language for smart building energy management, evaluated on real-world datasets with varied accuracy across tasks (49%-97%) and trade-offs in performance.


<details>
  <summary>Details</summary>
Motivation: To address limitations in existing building energy management systems by enabling context-aware energy management through natural language interaction and autonomous data analytics using LLMs.

Method: Proposes a conceptual framework with three modules (perception, central control, action) forming a feedback loop; evaluates prototype using 120 user queries across four residential energy datasets with metrics like latency, functionality, and ANOVA tests for generalizability.

Result: Promising performance with response accuracies: device control (86%), memory tasks (97%), scheduling/automation (74%), energy analysis (77%), but lower for cost estimation (49%); highlights trade-off between accuracy and computational efficiency.

Conclusion: The study benchmarks LLM-based BEMS AI agents, showing potential for smarter energy management while identifying areas for improvement (e.g., cost estimation) and future research directions, including balancing accuracy and efficiency.
...
TL;DR: Proposes Zero-Trust Agentic Federated Learning (ZTA-FL), a defense framework for IIoT with TPM-based attestation, SHAP-weighted aggregation, and privacy-preserving adversarial training, showing high accuracy and robustness against Byzantine attacks.


<details>
  <summary>Details</summary>
Motivation: Recent attacks on critical infrastructure like water and energy systems expose security gaps in Industrial IoT; existing Federated Learning frameworks for intrusion detection are vulnerable to Byzantine poisoning and lack robust authentication.

Method: Combines three components: TPM-based cryptographic attestation for agent authentication, a novel SHAP-weighted aggregation algorithm for explainable Byzantine detection under non-IID data, and privacy-preserving on-device adversarial training.

Result: Experiments on IDS benchmarks show ZTA-FL achieves 97.8% detection accuracy, 93.2% accuracy under 30% Byzantine attacks (outperforming FLAME by 3.1%, p<0.01), 89.3% adversarial robustness, and reduces communication overhead by 34%.

Conclusion: ZTA-FL provides a comprehensive, robust solution for secure IIoT deployment with theoretical guarantees and practical improvements in accuracy, robustness, and efficiency, with code released for reproducibility.
...
### [33] [Network Traffic Analysis with Process Mining: The UPSIDE Case Study](https://arxiv.org/abs/2512.23718)
*Francesco Vitale,Paolo Palmiero,Massimiliano Rak,Nicola Mazzocca*

Main category: cs.LG

TL;DR: A process mining method is proposed to analyze online gaming network traffic for state characterization and game classification.


<details>
  <summary>Details</summary>
Motivation: Online gaming generates large revenue and requires modeling network behavior for bandwidth management and security, where process mining offers data-driven and model-based insights.

Method: A process mining-based method analyzes gaming network traffic to characterize states, encode them as interpretable Petri nets, and classify traffic to identify specific video games.

Result: Application on UPSIDE case study with devices playing Clash Royale and Rocket League shows effective modeling with 94.02% inter-device similarity and 73.84% AUC for classification.

Conclusion: The method successfully models gaming network behavior interpretably using Petri nets with coherence and specificity, aiding in traffic analysis and game identification.
...
TL;DR: Empirical study of 16 DL model fixing approaches shows model-level methods are most effective, but no method optimally fixes faults while preserving accuracy, robustness, fairness, and backward compatibility—highlighting need for research on side-effects.


<details>
  <summary>Details</summary>
Motivation: DL systems are increasingly used in high-stakes domains (autonomous driving, healthcare, etc.) and, like traditional software, are prone to faults that pose significant risks. Many DL model fixing approaches have been proposed, but a comprehensive empirical evaluation of their performance and side effects across diverse scenarios is lacking.

Method: Large-scale empirical study evaluating 16 state-of-the-art DL model fixing approaches across model-level, layer-level, and neuron-level categories. The evaluation uses a uniform experimental setup with diverse datasets, model architectures, and application domains, assessing not only fixing effectiveness but also impact on robustness, fairness, and backward compatibility.

Result: Key findings include: 1) Model-level approaches demonstrate superior fixing effectiveness compared to layer- and neuron-level approaches. 2) No single approach can achieve best fixing performance while also improving accuracy and maintaining all other key properties (robustness, fairness, backward compatibility). Thus, trade-offs exist.

Conclusion: The study finds that while model-level DL fixing approaches show superior effectiveness compared to layer- and neuron-level approaches, no single method can achieve optimal fixing performance while simultaneously improving accuracy and preserving all critical properties like robustness, fairness, and backward compatibility. Academia should focus research on mitigating these trade-offs/ side effects, highlighting promising future directions.
...
TL;DR: A review of diffusion models for simulation-based inference (SBI), covering foundations, applications, robustness in non-ideal conditions, methods, and open problems.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive overview of diffusion-based SBI, addressing the need for flexible likelihood-free inference in scientific problems with intractable likelihoods and highlighting its advantages and trade-offs compared to other methods like normalizing flows.

Method: Recalls mathematical foundations of diffusion modeling (e.g., forward noising, reverse-time SDE/ODE, probability flow, denoising score matching), explains conditional scores for posterior sampling, and synthesizes methods including Schrodinger-bridge formulations, conditional/sequential samplers, amortized architectures, and inference-time prior adaptation.

Result: Diffusion-based SBI is shown to offer robustness in non-ideal scientific data conditions such as misspecification, unstructured observations, and missingness, while also presenting trade-offs like iterative sampling costs.

Conclusion: Diffusion models provide a robust framework for SBI in scientific applications, with potential benefits for uncertainty quantification in fields like geophysics, but further research is needed to address open problems and optimize trade-offs.
...
TL;DR: CM^2 is a small model that enables one-shot document classification by learning structural features, offering high accuracy, efficiency, and explainability as a Green AI solution without heavy GPU resources.


<details>
  <summary>Details</summary>
Motivation: Humans learn concepts from single examples, unlike machines needing many samples; current AI trends (Red AI) rely on large datasets and energy-intensive computing, creating inefficiency and environmental impact.

Method: Develop the Coordinate Matrix Machine (CM^2) that learns document structures and classifies them by identifying structural 'important features' similar to human cognition, focusing on coordinates instead of semantic vectors.

Result: CM^2 outperforms traditional vectorizers and deep learning models in accuracy with minimal data (one-shot learning), is efficient on CPUs, environmentally sustainable, explainable, fast, robust to unbalanced classes, and economically viable.

Conclusion: CM^2 demonstrates human-level concept learning for document classification through structural intelligence, providing a scalable and practical Green AI alternative to resource-intensive models.
...
TL;DR: Large language models like Pythia and Llama show geometric structures similar to small transformers, with values aligning along an entropy-related axis, indicating preserved Bayesian inference substrate.


<details>
  <summary>Details</summary>
Motivation: To investigate if the geometric signatures of Bayesian inference observed in controlled small transformer settings persist in production-grade language models.

Method: Analyzing value representations in last layers of models (Pythia, Phi-2, Llama-3, Mistral) for entropy correlation and performing targeted interventions on the entropy-aligned axis in Pythia-410M during in-context learning.

Result: Found last-layer value representations organize along a single dominant axis correlating with predictive entropy; domain-restricted prompts yield low-dimensional manifolds. Interventions on this axis disrupt uncertainty geometry without proportionally affecting Bayesian-like behavior.

Conclusion: Modern language models retain the geometric substrate for Bayesian inference, using it for approximate Bayesian updates, with the geometry acting as a privileged readout of uncertainty rather than a computational bottleneck.
...
TL;DR: This paper addresses the learning-freeze issue in Evidential Deep Learning (EDL) models due to the non-negative evidence constraint in the Subjective Logic framework. It theoretically analyzes activation function effects and proposes new activation functions and regularizers to improve consistency across evidence regions, validating with experiments on multiple classification and restoration tasks.


<details>
  <summary>Details</summary>
Motivation: EDL models enable uncertainty awareness in neural networks through learned evidence based on Subjective Logic, but the framework's constraint of non-negative evidence requires specific activation functions. This can lead to activation-dependent learning-freeze behavior, where gradients become extremely small for low-evidence samples, hindering learning dynamics. The motivation is to characterize this issue and design solutions to ensure consistent evidence updates.

Method: The method involves theoretical analysis of how different evidential activation functions influence learning dynamics, particularly characterizing the learning-freeze behavior. It then designs a family of generalized activation functions and corresponding evidential regularizers to mitigate this issue, ensuring stable gradients across activation regimes. The approach includes experiments to validate the theoretical insights.

Result: The results include theoretical characterization of the learning-freeze behavior and demonstration of how different activations affect training dynamics. Proposed activation functions and regularizers effectively reduce its occurrence and improve consistency. Empirical validation on benchmark tasks (MNIST, CIFAR-10, CIFAR-100, Tiny-ImageNet), few-shot classification, and blind face restoration shows improved performance and effectiveness compared to baseline EDL models.

Conclusion: Generalized and regularized evidential activation functions enhance the feasibility of EDL in deterministic neural networks by addressing learning-freeze issues. This tailors activation functions and regularizers for consistency in evidence learning across different samples, yielding better uncertainty quantification in tasks requiring fine-grained uncertainty.
...
TL;DR: HINTS is a self-supervised learning framework that extracts latent human factors like social influence, memory, and bias from time series residuals using the FJ opinion dynamics model, without external data, improving forecasting accuracy in financial and economic systems.


<details>
  <summary>Details</summary>
Motivation: Human decision-making, emotions, and collective psychology shape temporal dynamics in financial and economic systems, but existing forecasting models rely on external data (e.g., news, social media), which incurs high data dependency costs in financial, computational, and practical terms.

Method: Propose HINTS, a self-supervised learning framework that extracts latent human factors endogenously from time series residuals. It uses the Friedkin-Johnsen (FJ) opinion dynamics model as a structural inductive bias to model evolving social influence, memory, and bias patterns. The extracted factors are integrated into a state-of-the-art backbone model as an attention map.

Result: Experimental results on nine real-world and benchmark datasets show that HINTS consistently improves forecasting accuracy. Case studies and ablation studies validate its interpretability, demonstrating strong semantic alignment between extracted factors and real-world events.

Conclusion: HINTS provides a practical and accurate framework for forecasting by capturing human factors without external data, reducing dependency costs and offering interpretable insights.
...
### [40] [Learning Coupled System Dynamics under Incomplete Physical Constraints and Missing Data](https://arxiv.org/abs/2512.23761)
*Esha Saha,Hao Wang*

Main category: cs.LG

TL;DR: MUSIC is a sparsity-induced multitask neural network framework that integrates partial physical constraints with data-driven learning for coupled systems when physics-constrained and data-informed variables are mutually exclusive. It uses mesh-free sampling and sparsity regulation.


<details>
  <summary>Details</summary>
Motivation: "Such systems are often described by coupled variables, yet governing equation is typically available for one variable, while the remaining variable can be accessed only through data. This mismatch between known physics and observed data poses a fundamental challenge for existing physics-informed machine learning approaches."

Method: "MUSIC (Multitask Learning Under Sparse and Incomplete Constraints)" employs "mesh-free (random) sampling of training data and sparsity regularization"

Result: "MUSIC accurately learns solutions (shock wave solutions, discontinuous solutions, pattern formation solutions) to complex coupled systems under data-scarce and noisy conditions, consistently outperforming non-sparse formulations."

Conclusion: "These results highlight MUSIC as a flexible and effective approach for modeling partially observed systems with incomplete physical knowledge."
...
### [41] [Drift-Based Dataset Stability Benchmark](https://arxiv.org/abs/2512.23762)
*Dominik Soukup,Richard Plný,Daniel Vašata,Tomáš Čejka*

Main category: cs.LG

TL;DR: A novel methodology using ML and concept drift detection to evaluate dataset stability for network traffic classification, tested on the CESNET-TLS-Year22 dataset to benchmark and optimize data quality.


<details>
  <summary>Details</summary>
Motivation: Network traffic classification models often degrade after deployment due to outdated datasets or concept drift, but current practices rely on full retraining without proper investigation of dataset stability, necessitating a systematic evaluation approach.

Method: Proposes a framework based on concept drift detection enhanced with ML feature weights to assess dataset stability, with a benchmark workflow for comparing datasets.

Result: Demonstrated on the CESNET-TLS-Year22 dataset, providing an initial benchmark to describe stability, identify weak points, and show the optimization impact on dataset variants.

Conclusion: The methodology improves dataset quality evaluation in network traffic classification, aiding in more effective model maintenance and optimization against data drift.
...
### [42] [Neural Optimal Design of Experiment for Inverse Problems](https://arxiv.org/abs/2512.23763)
*John E. Darges,Babak Maboudi Afkham,Matthias Chung*

Main category: cs.LG

TL;DR: NODE is a learning-based framework for optimal experimental design that jointly trains reconstruction and design variables in a single loop, achieving better performance with lower computational cost than classical methods.


<details>
  <summary>Details</summary>
Motivation: Classical optimal experimental design in inverse problems typically involves bilevel optimization and indirect sparsity regularization, which can be computationally expensive and require careful tuning of sparsity parameters.

Method: NODE jointly trains a neural reconstruction model and a fixed-budget set of continuous design variables representing sensor locations, sampling times, or measurement angles within a single optimization loop, optimizing measurement locations directly rather than weighting a dense grid.

Result: NODE enforces sparsity by design, eliminates the need for l1 regularization tuning, substantially reduces computational complexity, and outperforms baseline approaches with improved reconstruction accuracy and task-specific performance across all tested applications.

Conclusion: The Neural Optimal Design of Experiments (NODE) framework offers an efficient and effective approach for optimal experimental design in inverse problems, outperforming baseline methods across various applications including analytic benchmarks, image sampling, and real-world CT imaging.
...
### [43] [Exploring Cumulative Effects in Survival Data Using Deep Learning Networks](https://arxiv.org/abs/2512.23764)
*Kang-Chung Yang,Shinsheng Yuan*

Main category: cs.LG

TL;DR: CENNSurv is a deep learning method for survival analysis that captures complex temporal effects of time-dependent exposures, offering scalability and interpretability on large datasets.


<details>
  <summary>Details</summary>
Motivation: Challenges in epidemiological research include modeling cumulative effects of time-dependent exposures due to intricate temporal dynamics. Traditional spline-based methods are computationally heavy, while neural network methods often lack interpretability.

Method: Introduction of CENNSurv, a novel deep learning approach designed to capture dynamic risk relationships from time-dependent data.

Result: CENNSurv revealed a multi-year lagged association between chronic environmental exposure and a survival outcome, and a short-term behavioral shift before subscription lapse, on two real-world datasets.

Conclusion: CENNSurv provides a practical tool for researchers to model complex temporal patterns with improved scalability and interpretable insights.
...
### [44] [A Granular Grassmannian Clustering Framework via the Schubert Variety of Best Fit](https://arxiv.org/abs/2512.23766)
*Karim Salta,Michael Kirby,Chris Peterson*

Main category: cs.LG

TL;DR: New subspace clustering using Schubert Variety prototypes in LBG framework, boosting cluster purity across data types.


<details>
  <summary>Details</summary>
Motivation: Need geometric representatives (means/medians) for datasets represented as subspaces on Grassmann/flag manifolds; traditional subspace means may not capture optimal intersections.

Method: Integrates trainable Schubert Variety of Best Fit (SVBF) prototypes into Linde-Buzo-Grey (LBG) pipeline for subspace clustering.

Result: Improved cluster purity on synthetic, image, spectral, and video action data compared to standard methods.

Conclusion: The SVBF-LBG algorithm combines geometric optimization with iterative clustering to effectively cluster subspace data while preserving mathematical structure for analysis.
...
### [45] [Enabling Physical AI at the Edge: Hardware-Accelerated Recovery of System Dynamics](https://arxiv.org/abs/2512.23767)
*Bin Xu,Ayan Banerjee,Sandeep Gupta*

Main category: cs.LG

TL;DR: MERINDA is an FPGA-accelerated framework for model recovery in edge AI, using hardware-friendly components to achieve high efficiency and accuracy.


<details>
  <summary>Details</summary>
Motivation: Edge AI requires efficient model recovery for safe autonomous systems, but existing methods rely on Neural ODEs that are slow and hard to accelerate on edge hardware.

Method: MERINDA replaces Neural ODEs with GRU-based dynamics, dense inverse-ODE layers, sparsity-driven dropout, and lightweight solvers, optimized for FPGA parallelism.

Result: MERINDA achieves 114× lower energy, 28× smaller memory, and 1.68× faster training compared to GPU, while matching accuracy on benchmark systems.

Conclusion: MERINDA enables real-time, accurate, and explainable model recovery on resource-constrained edge devices for autonomous monitoring.
...
### [46] [Safety-Biased Policy Optimisation: Towards Hard-Constrained Reinforcement Learning via Trust Regions](https://arxiv.org/abs/2512.23770)
*Ankit Kanwar,Dominik Wagner,Luke Ong*

Main category: cs.LG

TL;DR: SB-TRPO is a new trust-region RL algorithm that balances reward maximization with strict safety constraints by adaptively biasing policy updates toward constraint satisfaction.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning in safety-critical domains requires agents to maximise rewards while strictly adhering to safety constraints. Existing approaches often either fail to ensure near-zero safety violations or sacrifice reward performance when dealing with hard constraints.

Method: Safety-Biased Trust Region Policy Optimisation (SB-TRPO) algorithm that performs trust-region updates using a convex combination of the natural policy gradients of cost and reward, ensuring a fixed fraction of optimal cost reduction at each step.

Result: Experiments on standard and challenging Safety Gymnasium tasks show that SB-TRPO consistently achieves the best balance of safety and meaningful task completion compared to state-of-the-art methods.

Conclusion: SB-TRPO is a promising trust-region algorithm for safety-critical RL that successfully balances reward maximisation with strict safety constraints, demonstrating superior performance over existing methods.
...
### [47] [FineFT: Efficient and Risk-Aware Ensemble Reinforcement Learning for Futures Trading](https://arxiv.org/abs/2512.23773)
*Molei Qin,Xinyu Cai,Yewen Li,Haochong Xia,Chuqiao Zong,Shuo Sun,Xinrun Wang,Bo An*

Main category: cs.LG

TL;DR: FineFT: A three-stage ensemble RL framework for crypto futures trading that addresses high leverage challenges through stable training and risk-aware capability boundary identification, outperforming 12 SOTA methods with 40%+ risk reduction.


<details>
  <summary>Details</summary>
Motivation: Existing RL methods for quantitative trading focus primarily on spot markets and cannot be directly applied to futures markets with high leverage due to two key challenges: 1) High leverage amplifies reward fluctuations, making training stochastic and difficult to converge; 2) Lack of self-awareness about capability boundaries exposes models to significant loss risks during novel market states (e.g., black swan events like COVID-19).

Method: FineFT employs a three-stage ensemble reinforcement learning approach: 1) Selective updating of ensemble Q-learners using ensemble TD errors to improve convergence; 2) Filtering Q-learners based on profitability and training VAEs to identify capability boundaries; 3) Dynamic selection between filtered ensemble policies and a conservative policy guided by VAEs for risk mitigation.

Result: Extensive experiments on crypto futures in a high-frequency trading environment with 5x leverage show FineFT outperforms 12 state-of-the-art baselines across 6 financial metrics. It reduces risk by over 40% while achieving superior profitability compared to the runner-up. Visualization reveals specialized agent expertise, and ablation studies confirm VAE routing reduces maximum drawdown effectively while selective updating improves convergence.

Conclusion: FineFT successfully addresses the unique challenges of futures trading by introducing a three-stage ensemble RL framework with stable training mechanisms and risk-aware capability boundary identification. The method demonstrates superior profitability and significantly reduced risk compared to state-of-the-art baselines, making it a robust solution for high-frequency crypto futures trading with high leverage.
...
### [48] [A Survey on Graph Neural Networks for Fraud Detection in Ride Hailing Platforms](https://arxiv.org/abs/2512.23777)
*Kanishka Hewageegana,Janani Harischandra,Nipuna Senanayake,Gihan Danansuriya,Kavindu Hapuarachchi,Pooja Illangarathne*

Main category: cs.LG

TL;DR: Survey of GNN-based fraud detection in ride-hailing, comparing models, addressing class imbalance and camouflage, outlining architectures, and calling for real-world improvements.


<details>
  <summary>Details</summary>
Motivation: To investigate fraud detection in ride-hailing platforms using Graph Neural Networks (GNNs) and analyze prevalent fraudulent activities, compare existing work, and outline methodological progress and gaps.

Method: The study analyzes and compares various Graph Neural Network (GNN) architectures and methodologies applied to anomaly detection for fraud detection, with a focus on addressing class imbalance and fraudulent camouflage.

Result: The research provides a structured overview of GNN architectures and methodologies for anomaly detection, identifies significant methodological progress and gaps, and highlights the need to address class imbalance and fraudulent camouflage.

Conclusion: The paper calls for future research to explore real-world applicability and technical improvements to enhance fraud detection strategies in the ride-hailing industry.
...
### [49] [TabMixNN: A Unified Deep Learning Framework for Structural Mixed Effects Modeling on Tabular Data](https://arxiv.org/abs/2512.23787)
*Deniz Akdemir*

Main category: cs.LG

TL;DR: TabMixNN is a PyTorch-based deep learning framework that combines classical mixed-effects models with neural networks for tabular data analysis, supporting hierarchical structures, diverse outcomes, and interpretability tools.


<details>
  <summary>Details</summary>
Motivation: There is a growing need for methods that can handle hierarchical data structures in tabular data while supporting diverse outcome types like regression, classification, and multitask learning, which TabMixNN aims to address by integrating the interpretability of mixed-effects models with the flexibility of neural networks.

Method: The framework uses a modular three-stage architecture: (1) a mixed-effects encoder with variational random effects and flexible covariance structures, (2) backbone architectures including Generalized Structural Equation Models (GSEM) and spatial-temporal manifold networks, and (3) outcome-specific prediction heads supporting multiple outcome families. It includes innovations such as an R-style formula interface, DAG constraints for causal structure learning, SPDE kernels for spatial modeling, and interpretability tools like SHAP values.

Result: TabMixNN demonstrates flexibility through applications to longitudinal data analysis, genomic prediction, and spatial-temporal modeling, providing a unified interface that leverages deep learning while maintaining interpretability and theoretical grounding.

Conclusion: TabMixNN offers a powerful and flexible solution for tabular data analysis by synthesizing classical mixed-effects modeling with modern neural networks, enabling researchers to handle complex hierarchical structures with enhanced interpretability and diverse application capabilities.
...
### [50] [Improved Bounds for Private and Robust Alignment](https://arxiv.org/abs/2512.23816)
*Wenqian Weng,Yi He,Xingyu Zhou*

Main category: cs.LG

TL;DR: This paper provides theoretical upper bounds for private and robust alignment of language models, analyzing both offline and online settings under privacy constraints and adversarial corruption, with results enabled by new uniform convergence guarantees.


<details>
  <summary>Details</summary>
Motivation: To study the interplay between privacy and corruption in aligning language models from a theoretical perspective, addressing gaps in existing guarantees for both offline and online scenarios.

Method: Establishes upper bounds using log loss with MLE-style algorithms for privacy setting, analyzes privacy-first and corruption-first interplays, and applies new uniform convergence guarantees for log loss and square loss under privacy and corruption.

Result: Shows near-optimal rates for privacy-only setting, demonstrates stronger existing offline algorithm guarantees, presents first results for private and robust online alignment, and provides improved bounds in corruption-only regime.

Conclusion: The theoretical analysis advances understanding of private and robust alignment with new uniform convergence tools, offering broader applicability in learning theory and statistics.
...
### [51] [MS-SSM: A Multi-Scale State Space Model for Efficient Sequence Modeling](https://arxiv.org/abs/2512.23824)
*Mahdi Karami,Ali Behrouz,Peilin Zhong,Razvan Pascanu,Vahab Mirrokni*

Main category: cs.LG

TL;DR: Multi-scale state-space model (MS-SSM) improves memory efficiency and captures both fine-grained and global patterns through multi-resolution processing with dynamic scale mixing, outperforming prior SSMs across diverse sequence modeling tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional SSMs have limited effective memory and struggle to capture multi-scale dependencies, which are essential for modeling complex structures in sequences across domains like time series, images, and natural language.

Method: The MS-SSM framework introduces a multi-scale SSM architecture that processes sequences at multiple resolutions using specialized state-space dynamics for each scale, coupled with an input-dependent scale-mixer that dynamically fuses information across resolutions.

Result: MS-SSM consistently outperforms prior SSM-based models on benchmarks including Long Range Arena, hierarchical reasoning, time series classification, and image recognition, demonstrating enhanced memory efficiency and long-range modeling capabilities.

Conclusion: The paper concludes that multi-resolution state-space modeling is an effective approach for improving memory efficiency and capturing multi-scale dependencies in sequence modeling, with MS-SSM demonstrating superior performance across diverse tasks.
...
### [52] [Exploiting the Prior of Generative Time Series Imputation](https://arxiv.org/abs/2512.23832)
*YuYang Miao,Chang Li,Zehua Chen*

Main category: cs.LG

TL;DR: Bridge-TS improves time series imputation accuracy by using expert and compositional priors in a data-to-data generative model.


<details>
  <summary>Details</summary>
Motivation: Previous generative models for time series imputation use uninformative priors (e.g., Gaussian noise or linear interpolation), increasing generation burden and limiting accuracy.

Method: Proposes Bridge-TS with expert prior (using a pretrained transformer for deterministic estimation as prior) and compositional priors (combining multiple pretrained model estimations) in a data-to-data generation process.

Result: Achieves record imputation accuracy on benchmarks (ETT, Exchange, Weather) in terms of mean square error and mean absolute error.

Conclusion: Enhancing priors in generative time series imputation, as done with Bridge-TS, significantly boosts accuracy and demonstrates superior performance.
...
### [53] [Trellis: Learning to Compress Key-Value Memory in Attention Models](https://arxiv.org/abs/2512.23852)
*Mahdi Karami,Ali Behrouz,Praneeth Kacham,Vahab Mirrokni*

Main category: cs.LG

TL;DR: Trellis: a memory-efficient Transformer with bounded memory using dynamic KV compression via recurrent mechanism and online gradient descent, excelling in long-context tasks.


<details>
  <summary>Details</summary>
Motivation: Transformers suffer from quadratic computational complexity and growing Key-Value cache, which limits their efficiency and scalability, especially for long sequences.

Method: Trellis uses a two-pass recurrent compression mechanism with a fixed-size memory instead of the standard KV cache. It employs an online gradient descent procedure with a forget gate to dynamically compress and update the memory at test time.

Result: Trellis outperforms strong baselines on language modeling, reasoning, recall-intensive tasks, and time series. Performance gains grow with sequence length.

Conclusion: Trellis is a promising Transformer architecture that addresses the quadratic complexity and KV cache growth issues of standard Transformers. It demonstrates superior performance across various tasks, especially as sequence length increases, making it suitable for long-context applications.
...
### [54] [Flow Matching Neural Processes](https://arxiv.org/abs/2512.23853)
*Hussen Abu Hamad,Dan Rosenbaum*

Main category: cs.LG

TL;DR: A new neural process model based on flow matching that simplifies implementation, enables ODE-based conditional sampling without auxiliary methods, and outperforms previous methods on benchmarks.


<details>
  <summary>Details</summary>
Motivation: To create a simpler, more effective neural process model that avoids complex auxiliary conditioning methods and provides better performance with controllable tradeoffs.

Method: Introduces a neural process model using flow matching, which enables conditional sampling via ODE solvers without auxiliary conditioning methods.

Result: The model outperforms state-of-the-art neural process methods on synthetic 1D Gaussian processes, 2D images, and real-world weather data benchmarks.

Conclusion: The new neural process model based on flow matching offers a simpler, more efficient approach to modeling stochastic processes with better performance and controllable accuracy-speed tradeoffs.
...
### [55] [Yggdrasil: Bridging Dynamic Speculation and Static Runtime for Latency-Optimal Tree-Based LLM Decoding](https://arxiv.org/abs/2512.23858)
...
TL;DR: Yggdrasil, a co-designed system using context-aware tree drafting and compiler-friendly execution, achieves optimal speculative decoding with up to 3.98x speedup over baselines by addressing static-dynamic mismatches.


<details>
  <summary>Details</summary>
Motivation: Existing speculative decoding systems suffer from suboptimal performance due to a mismatch between dynamic speculation (generating multiple tokens in parallel) and static runtime assumptions.

Method: Yggdrasil introduces an equal-growth tree structure for static graph compatibility, a latency-aware optimization objective for draft selection, and stage-based scheduling to reduce overhead, while supporting unmodified LLMs.

Result: Yggdrasil achieves up to 3.98× speedup over state-of-the-art baselines across multiple hardware setups.

Conclusion: Yggdrasil enables latency-optimal speculative decoding through its co-designed approach, effectively overcoming performance limitations in prior systems.
...
### [56] [Probing the Limits of Compressive Memory: A Study of Infini-Attention in Small-Scale Pretraining](https://arxiv.org/abs/2512.23862)
*Ruizhe Huang,Kexuan Zhang,Yihao Fang,Baifeng Yu*

Main category: cs.LG

TL;DR: Infini-attention improves long-context performance in small language models via compressed memory, boosting accuracy despite some degradation over long sequences.


<details>
  <summary>Details</summary>
Motivation: To enable efficient small-scale pretraining for Small Language Models (SLMs) in low-resource settings, addressing limited data, compute, and costs while enhancing long-context extrapolation.

Method: Empirical study using 300M-parameter LLaMA models pretrained with Infini-attention, which incorporates a compressed memory from past segments while maintaining local attention, focusing on the balance factor and its effects.

Result: The Infini-attention model shows training stability and outperforms the baseline in long-context retrieval, achieving up to 31% higher accuracy despite a drop in retrieval accuracy with repeated memory compressions over sequences of 16,384 tokens.

Conclusion: Infini-attention effectively compensates for SLM limitations, and architectural memory like it is key for robust long-context capabilities in small-scale pretraining.
...
### [57] [Max-Entropy Reinforcement Learning with Flow Matching and A Case Study on LQR](https://arxiv.org/abs/2512.23870)
*Yuyang Zhang,Yang Hu,Bo Dai,Na Li*

Main category: cs.LG

TL;DR: A variant of the Soft Actor-Critic (SAC) algorithm using flow-based models to approximate energy-based policies, with an online Importance Sampling Flow Matching (ISFM) method for policy updates, analyzed theoretically and tested on max-entropy linear quadratic regulator problems.


<details>
  <summary>Details</summary>
Motivation: SAC uses energy-based policies often approximated by simple classes for efficiency, sacrificing expressiveness and robustness. This paper aims to enhance SAC by leveraging flow-based models for richer policy representation.

Method: Propose a SAC variant that parameterizes policies with flow-based models, uses instantaneous change-of-variable for policy evaluation, and updates policy with ISFM, an online flow matching variant requiring samples only from a user-specified distribution.

Result: The algorithm successfully learns the optimal action distribution in max-entropy linear quadratic regulator problems, with ISFM enabling efficient sampling and theoretical analysis on sampling distribution impacts on learning efficiency.

Conclusion: The flow-based SAC variant improves policy expressiveness and robustness, with ISFM providing a practical update method and theoretical guarantees, demonstrating effectiveness in learning optimal distributions in test cases.
...
### [58] [Efficient Deep Learning for Short-Term Solar Irradiance Time Series Forecasting: A Benchmark Study in Ho Chi Minh City](https://arxiv.org/abs/2512.23898)
*Tin Hoang*

Main category: cs.LG

TL;DR: This study benchmarks deep learning models for solar irradiance forecasting, identifies Transformer as best, analyzes model reasoning, and compresses it via knowledge distillation for edge deployment.


<details>
  <summary>Details</summary>
Motivation: Reliable solar irradiance forecasting is crucial to manage solar energy variability in power grids, necessitating accurate short-term predictions.

Method: The paper compares ten deep learning architectures using NSRDB satellite data for 1-hour-ahead GHI forecasting in Ho Chi Minh City, including LSTMs, TCNs, Transformers, Informer, iTransformer, TSMixer, and Mamba.

Result: Transformer achieved the highest accuracy with R^2 of 0.9696; SHAP analysis shows distinct temporal reasoning patterns (e.g., recency bias in Transformers vs. periodic dependencies in Mamba); knowledge distillation reduced model size by 23.5% and improved error (MAE: 23.78 W/m^2).

Conclusion: Transformers outperform other models for this task, and knowledge distillation enables efficient deployment on edge devices, enhancing practical solar forecasting applications.
...
### [59] [Rethinking Dense Linear Transformations: Stagewise Pairwise Mixing (SPM) for Near-Linear Training in Neural Networks](https://arxiv.org/abs/2512.23905)
*Peter Farag*

Main category: cs.LG

TL;DR: SPM is a linear operator using sparse pairwise mixing stages to replace dense layers, reducing complexity and improving generalization.


<details>
  <summary>Details</summary>
Motivation: Dense linear layers are computationally expensive and misaligned with learned representations' compositional structure, prompting a need for efficient alternatives.

Method: Introduce SPM with sparse pairwise-mixing stages, enabling exact forward/backward computations in O(nL) time/parameters, applicable as a drop-in replacement in various architectures.

Result: SPM reduces computational cost, improves accuracy on structured tasks, and maintains competitive performance on benchmarks while offering explicit compositional bias.

Conclusion: SPM effectively addresses inefficiencies of dense layers, offering a scalable, generalizable, and computationally efficient alternative in machine learning models.
...
### [60] [Constraint Breeds Generalization: Temporal Dynamics as an Inductive Bias](https://arxiv.org/abs/2512.23916)
*Xia Chen*

Main category: cs.LG

TL;DR: The paper argues that biological-like constraints in neural networks, rather than hindering performance, act as a temporal inductive bias that enhances generalization by encouraging abstraction of invariant features through dissipative dynamics.


<details>
  <summary>Details</summary>
Motivation: To challenge the conventional deep learning emphasis on unconstrained optimization by proposing that metabolic-like constraints in biological systems can shape network dynamics as a beneficial temporal inductive bias for generalization.

Method: Phase-space analysis of signal propagation to study dynamics, exploration of intrinsic (via network architecture) and extrinsic (via input encoding) constraints, and comprehensive evaluations in supervised classification, unsupervised reconstruction, and zero-shot reinforcement learning tasks.

Result: Revealed an asymmetry where proper dissipative dynamics compress phase space to align with spectral bias, promoting feature abstraction. Showed that a critical 'transition' regime maximizes generalization capability, and that architectures with temporal integration and constraints outperform static ones.

Conclusion: Dynamical constraints are a distinct class of inductive bias, suggesting robust AI development should focus on mastering temporal characteristics for generalization, beyond just scaling and removing limitations.
...
### [61] [Interactive Machine Learning: From Theory to Scale](https://arxiv.org/abs/2512.23924)
*Yinglun Zhu*

Main category: cs.LG

TL;DR: This dissertation develops theoretically grounded interactive learning algorithms that overcome limitations of traditional ML methods by enabling more efficient data collection and decision making through active learner participation.


<details>
  <summary>Details</summary>
Motivation: Traditional machine learning methods often require large amounts of labeled data or extensive online interaction, which can be expensive, time-consuming, or risky in large-scale or high-stakes settings. Interactive learning addresses this by allowing the learner to actively influence how information is collected.

Method: The study develops new algorithmic principles and establishes fundamental limits for interactive learning across three dimensions: active learning with noisy data and rich model classes, sequential decision making with large action spaces, and model selection under partial feedback.

Result: Key results include: first computationally efficient active learning algorithms achieving exponential label savings without low-noise assumptions; first efficient, general-purpose contextual bandit algorithms with guarantees independent of action space size; and first tight characterizations of the fundamental cost of model selection in sequential decision making.

Conclusion: The dissertation advances the theoretical foundations of interactive learning by providing statistically optimal and computationally efficient algorithms, along with principled guidance for deploying these methods in real-world settings.
...
### [62] [Improved Balanced Classification with Theoretically Grounded Loss Functions](https://arxiv.org/abs/2512.23947)
*Corinna Cortes,Mehryar Mohri,Yutao Zhong*

Main category: cs.LG

TL;DR: This paper introduces and analyzes two advanced surrogate loss families—Generalized Logit-Adjusted (GLA) and Generalized Class-Aware weighted (GCA) losses—for improving multi-class classification under class imbalance. It finds that GCA offers stronger theoretical guarantees with better H-consistency scaling in imbalanced settings, while both GLA and GCA empirically outperform standard class-weighted and LA losses.


<details>
  <summary>Details</summary>
Motivation: The balanced loss is key for fairness in class-imbalanced classification but is often intractable to minimize directly. Existing surrogate losses, like Logit-Adjusted (LA) and standard class-weighted losses, have limitations, prompting the need for more advanced alternatives to enhance theoretical robustness and empirical performance in imbalanced scenarios.

Method: The paper generalizes LA losses to the GLA family by extending logit adjustments to the general cross-entropy loss. It also extends standard class-weighted losses to the GCA family by incorporating class-dependent confidence margins and adapting them to the general cross-entropy framework. Theoretical analyses of consistency properties are conducted.

Result: GLA losses are Bayes-consistent and H-consistent for complete hypothesis sets, but their H-consistency bounds scale unfavorably as at least 1/p_min. GCA losses are H-consistent for any hypothesis set (bounded or complete) with bounds scaling more favorably as 1/√p_min. Experiments show both GCA (with calibrated margins) and GLA outperform standard class-weighted and LA losses, with GLA slightly better in common benchmarks and GCA slightly superior in highly imbalanced settings.

Conclusion: The proposed GLA and GCA loss families provide effective surrogates for balanced loss in imbalanced classification. GCA offers stronger theoretical guarantees with better scaling properties, making it particularly advantageous in highly imbalanced contexts, while both demonstrate significant empirical improvements over existing methods.
...
### [63] [DivQAT: Enhancing Robustness of Quantized Convolutional Neural Networks against Model Extraction Attacks](https://arxiv.org/abs/2512.23948)
*Kacem Khaled,Felipe Gohring de Magalhães,Gabriela Nicolescu*

Main category: cs.LG

TL;DR: DivQAT enhances robustness of quantized CNNs against extraction attacks by modifying QAT, with empirical validation showing maintained accuracy and improved defense.


<details>
  <summary>Details</summary>
Motivation: Vulnerability of quantized models to extraction attacks and lack of integrated, efficient defenses suitable for edge devices.

Method: Proposes DivQAT algorithm that modifies quantization aware training (QAT) to incorporate defense directly into training.

Result: Demonstrated effectiveness on benchmark datasets without compromising model accuracy and improved performance when combined with other defenses.

Conclusion: DivQAT offers a novel, integrated approach to defend quantized CNNs against extraction attacks, enhancing robustness for edge implementations.
...
### [64] [Physics-informed Graph Neural Networks for Operational Flood Modeling](https://arxiv.org/abs/2512.23964)
*Carlo Malapad Acosta,Herath Mudiyanselage Viraj Vidura Herath,Jia Yu Lim,Abhishek Saha,Sanka Rasnayaka,Lucy Marshall*

Main category: cs.LG

TL;DR: The paper introduces a fast and accurate GNN-based flood prediction model with physical constraints and improved training for better performance.


<details>
  <summary>Details</summary>
Motivation: Physics-based flood models are accurate but computationally expensive, limiting operational use where speed is essential; GNNs offer speed and flexibility but need improvements in interpretability and performance.

Method: A novel GNN architecture, DUALFloodGNN, embeds physical constraints globally and locally via explicit loss terms, predicts water volume at nodes and flow along edges through shared message-passing, and uses multi-step loss with dynamic curriculum learning for training.

Result: DUALFloodGNN achieves substantial improvements in predicting multiple hydrologic variables compared to standard and state-of-the-art GNN flood models while maintaining high computational efficiency.

Conclusion: DUALFloodGNN effectively combines GNN speed and accuracy with physics-informed techniques, enhancing interpretability and performance for flood prediction, with the model being open-sourced.
...
### [65] [Causify DataFlow: A Framework For High-performance Machine Learning Stream Computing](https://arxiv.org/abs/2512.23977)
*Giacinto Paolo Saggese,Paul Smith*

Main category: cs.LG

TL;DR: DataFlow is a ML framework for streaming time-series data that ensures batch-trained models run identically in production using DAGs with point-in-time idempotency and strict causality tracking.


<details>
  <summary>Details</summary>
Motivation: Traditional data science workflows assume finite datasets, causing issues when moving from batch prototypes to streaming production: causality violations, batch artifacts, and poor reproducibility of real-time failures.

Method: The framework uses directed acyclic graphs (DAGs) with point-in-time idempotency, ensuring outputs depend only on a fixed-length preceding context window. It enforces strict causality by tracking knowledge time, supports temporal/feature tiling, integrates with Python stack, and provides online learning, caching, incremental computation, and DAG-based parallelization.

Result: DataFlow is demonstrated effective across financial trading, IoT, fraud detection, and real-time analytics domains, enabling seamless transition from batch to streaming execution without code changes.

Conclusion: DataFlow provides a robust solution for bringing batch-trained machine learning models directly to streaming production without reimplementation, eliminating common pitfalls like causality violations. It ensures reproducibility and simplifies the deployment of time-series ML systems.
...
### [66] [Assured Autonomy: How Operations Research Powers and Orchestrates Generative AI Systems](https://arxiv.org/abs/2512.23978)
*Tinglong Dai,David Simchi-Levi,Michelle Xiao Wu,Yao Xie*

Main category: cs.LG

TL;DR: TL;DR: The paper presents a framework for assured autonomy in generative AI using operations research methods like flow-based models and adversarial robustness to address safety and reliability risks as systems become more autonomous.


<details>
  <summary>Details</summary>
Motivation: The motivation is to address the autonomy paradox in generative AI, where increased autonomy demands more formal structure and risk discipline to ensure verifiable feasibility, robustness to distribution shift, and stress testing in safety-critical domains.

Method: The paper develops a conceptual framework through two complementary approaches: flow-based generative models to enable deterministic transport and operational safety through an adversarial robustness lens, using uncertainty sets and worst-case perturbations.

Result: The framework clarifies how increasing autonomy shifts operations research's role from solver to guardrail to system architect, defining a research agenda for assured autonomy in safety-critical operational domains.

Conclusion: The paper concludes that as generative AI systems gain greater autonomy, they must incorporate structured, verifiable mechanisms from operations research to ensure safety and reliability, marking a shift in OR's role from solver to system architect.
...
### [67] [Information-Theoretic Quality Metric of Low-Dimensional Embeddings](https://arxiv.org/abs/2512.23981)
*Sebastián Gutiérrez-Bernal,Hector Medel Cobaxin,Abiel Galindo González*

Main category: cs.LG

TL;DR: ERPM is introduced to assess information loss in low-dimensional embeddings using Shannon entropy and stable rank, complementing distance and geometric metrics.


<details>
  <summary>Details</summary>
Motivation: Existing metrics like stress, rank-based neighborhood criteria, or Local Procrustes focus on distance distortions but do not directly evaluate information preservation when projecting high-dimensional data to lower dimensions.

Method: ERPM is developed based on the Shannon entropy of the singular-value spectrum of neighborhood matrices and stable rank to measure changes in uncertainty; compared with Mean Relative Rank Error and Local Procrustes using financial time series and a manifold dataset.

Result: Distance-based criteria show low correlation with geometric and spectral measures; ERPM and Local Procrustes have strong average correlation but differ locally, indicating ERPM detects information loss in specific neighborhoods.

Conclusion: ERPM complements existing metrics by identifying neighborhoods with severe information loss, providing a more comprehensive embedding assessment, especially for information-sensitive applications like early-warning indicators.
...
### [68] [Tracing the Heart's Pathways: ECG Representation Learning from a Cardiac Conduction Perspective](https://arxiv.org/abs/2512.24002)
*Tan Pan,Yixuan Sun,Chen Jiang,Qiong Gao,Rui Sun,Xingmeng Zhang,Zhenqi Yang,Limei Han,Yixiu Liang,Yuan Cheng,Kaiyu Guo*

Main category: cs.LG

TL;DR: Proposed CLEAR-HUG, a two-stage framework that captures subtle ECG variations across leads while following diagnostic guidelines, improving disease diagnosis by 6.84% across tasks.


<details>
  <summary>Details</summary>
Motivation: Existing eSSL methods focus on consistent patterns across leads and beats, ignoring inherent differences in heartbeats from cardiac conduction processes and the sequential logic of ECG diagnostic guidelines from beats to leads to combinations.

Method: First stage: CLEAR reconstructs signals with a sparse attention mechanism treating each heartbeat as a distinct entity. Second stage: HUG implements a hierarchical lead-unified group head for diagnosis, aligning with clinical workflow.

Result: Experimental results across six tasks show a 6.84% improvement, validating the effectiveness of CLEAR-HUG.

Conclusion: CLEAR-HUG effectively enhances representations of cardiac conduction and aligns ECG patterns with expert diagnostic guidelines, addressing limitations in prior eSSL approaches.
...
### [69] [Hyperspherical Graph Representation Learning via Adaptive Neighbor-Mean Alignment and Uniformity](https://arxiv.org/abs/2512.24062)
*Rui Chen,Junjun Guo,Hongbin Wang,Yan Xiang,Yantuan Xian,Zhengtao Yu*

Main category: cs.LG

TL;DR: Proposed HyperGRL, a hyperspherical graph representation learning framework with adaptive neighbor-mean alignment and sampling-free uniformity, outperforming existing methods on node classification, clustering, and link prediction tasks.


<details>
  <summary>Details</summary>
Motivation: Existing graph representation learning methods often rely on surrogate contrastive objectives or mutual information maximization, which require complex architectures, negative sampling, and sensitive hyperparameter tuning, leading to issues like over-smoothing, over-squashing, and training instability. This work aims to address these limitations.

Method: HyperGRL employs two adversarially coupled objectives: neighbor-mean alignment using mean representations of local neighborhoods to capture structural and feature patterns, and sampling-free uniformity via L2-based hyperspherical regularization to encourage uniform embedding distributions. It includes an entropy-guided adaptive balancing mechanism to dynamically regulate the alignment-uniformity interplay.

Result: HyperGRL achieves average improvements of 1.49% in node classification, 0.86% in node clustering, and 0.74% in link prediction over the strongest existing methods, showing superior representation quality and generalization across diverse graph structures.

Conclusion: HyperGRL is an effective framework for graph representation learning that uses hyperspherical embeddings with adaptive neighbor-mean alignment and sampling-free uniformity, demonstrating superior performance in node classification, clustering, and link prediction tasks.
...
### [70] [How and Why LLMs Generalize: A Fine-Grained Analysis of LLM Reasoning from Cognitive Behaviors to Low-Level Patterns](https://arxiv.org/abs/2512.24063)
*Haoyue Bai,Yiyou Sun,Wenjie Hu,Shi Qiu,Maggie Ziyu Huan,Peiyang Song,Robert Nowak,Dawn Song*

Main category: cs.LG

TL;DR: This paper introduces a benchmark decomposing reasoning into core skills and uses meta-probing to show RL tuning preserves generalization better than SFT, offering insights for robust training strategies.


<details>
  <summary>Details</summary>
Motivation: Understand why supervised fine-tuning narrows LLM capability while RL tuning preserves it, as current metrics are too coarse to explain this divergence.

Method: Develop a benchmark breaking reasoning into atomic skills, analyze low-level statistical patterns, and apply a meta-probing framework to track model behavior during training stages.

Result: RL-tuned models maintain stable behavioral profiles and resist skill collapse, while SFT models exhibit drift and overfitting to surface patterns.

Conclusion: The work provides insights into LLM reasoning nature and suggests principles for designing training strategies to foster broad, robust generalization.
...
### [71] [Time-varying Mixing Matrix Design for Energy-efficient Decentralized Federated Learning](https://arxiv.org/abs/2512.24069)
*Xusheng Zhang,Tuan Nguyen,Ting He*

Main category: cs.LG

TL;DR: Design of mixing matrices for decentralized federated learning in wireless networks to minimize per-node energy consumption.


<details>
  <summary>Details</summary>
Motivation: Existing designs focus on communication time, neglecting per-node energy minimization for energy-constrained devices.

Method: Propose a multi-phase framework with time-varying communication topologies and optimized budgets.

Result: Validated efficacy in combining low energy consumption of sparse matrices with fast convergence of dense ones.

Conclusion: Solution trades off per-iteration energy and convergence rate while balancing energy across nodes.
...
### [72] [Multi-Scenario Highway Lane-Change Intention Prediction: A Temporal Physics-Informed Multi-Modal Framework](https://arxiv.org/abs/2512.24075)
*Jiazhao Shi,Ziyu Wang,Yichen Lin,Shoufeng Lu*

Main category: cs.LG

TL;DR: TPI-AI is a hybrid framework for lane-change intention prediction that combines deep temporal embeddings with physics-inspired interaction features to improve reliability in naturalistic traffic, showing strong performance across different highway scenarios.


<details>
  <summary>Details</summary>
Motivation: Lane-change intention prediction is essential for autonomous driving safety but faces challenges like noisy kinematics, class imbalance, and poor generalization in heterogeneous highway environments.

Method: Uses a two-layer bidirectional LSTM to learn temporal embeddings from trajectory histories, concatenated with physics-based features (e.g., headway, TTC, safe-gap), and trains a LightGBM classifier with imbalance-aware optimization for intention recognition in three classes.

Result: Outperforms baseline models on highD and exiD datasets, achieving macro-F1 scores up to 0.9562 for 1s and 0.7605 for 3s horizons, demonstrating robustness in multi-scenario prediction.

Conclusion: Integrating physics-informed features with temporal representations yields effective and generalizable lane-change intention prediction, enhancing autonomous driving systems.
...
### [73] [Autoregressivity in the Latent Space of a GP-VAE Language Model: An Empirical Ablation Study](https://arxiv.org/abs/2512.24102)
*Yves Ruffenach*

Main category: cs.LG

TL;DR: Ablation study shows latent autoregression in GP-VAE improves latent trajectory compatibility with GP prior and long-range stability, contrasting with degraded structure in non-autoregressive variants.


<details>
  <summary>Details</summary>
Motivation: To analyze the role of latent autoregression in GP-VAE models, building on previous work that shifted sequential structure to latent space via causal Gaussian process with non-autoregressive decoder. Language models typically use autoregressive factorization over tokens, but this work explores latent space sequential structure.

Method: A systematic ablation study comparing: (i) full GP-VAE with autoregressive latent dynamics, (ii) non-autoregressive ablation with independent latent variables, and (iii) standard token-level autoregressive Transformer.

Result: Latent autoregression induces latent trajectories more compatible with Gaussian process prior and exhibits greater long-horizon stability. Removing autoregression degrades latent structure and leads to unstable long-range behavior.

Conclusion: The findings highlight latent autoregression as an effective mechanism for organizing long-range structure in latent spaces, complementing token-level autoregressive modeling. The analysis is presented as an empirical study of representational structure rather than a proposal for a new architecture.
...
### [74] [Enhancing LLM Planning Capabilities through Intrinsic Self-Critique](https://arxiv.org/abs/2512.24103)
...
TL;DR: LLMs can effectively use iterative self-critique to significantly improve their own planning performance without external verification, achieving new SOTA results.


<details>
  <summary>Details</summary>
Motivation: The paper addresses skepticism about the effectiveness of LLM self-critique methods, aiming to demonstrate that LLMs can effectively critique their own answers to enhance planning performance.

Method: The approach employs an iterative process for correction and refinement using self-critique. They use few-shot learning initially, then progressively extend to many-shot approach. The method focuses on intrinsic self-critique without external sources like verifiers.

Result: Significant performance gains were achieved on planning datasets in Blocksworld, Logistics, and Mini-grid domains, exceeding strong baseline accuracies and achieving new state-of-the-art on the class of models considered.

Conclusion: The authors conclude that their self-critique approach demonstrates significant intrinsic self-improvement capabilities for LLMs in planning tasks, achieving new state-of-the-art results for models from October 2024. They believe applying this method to more complex search techniques and more capable models will yield even better performance.
...
### [75] [OptRot: Mitigating Weight Outliers via Data-Free Rotations for Post-Training Quantization](https://arxiv.org/abs/2512.24124)
*Advait Gadhikar,Riccardo Grazzi,James Hensman*

Main category: cs.LG

TL;DR: Proposes OptRot and OptRot⁺, methods using learnable fusible rotations to reduce outliers for quantization in LLMs, outperforming existing rotation-based techniques in weight quantization and showing mixed results for activation quantization.


<details>
  <summary>Details</summary>
Motivation: Large Language Models have outliers in weights and activations, making quantization difficult, so the paper aims to mitigate these outliers efficiently.

Method: Introduces OptRot, which minimizes the element-wise fourth power of rotated weights to reduce outliers, and OptRot⁺, a data-dependent version incorporating activation covariance information.

Result: OptRot outperforms Hadamard rotations, SpinQuant, and OSTQuant in weight quantization and improves activation quantization in W4A8, but both methods perform worse in W4A4, indicating a trade-off.

Conclusion: Learnable fusible rotations effectively reduce outliers for quantization, with OptRot and OptRot⁺ offering efficient solutions, though their performance varies based on the quantization setting and highlights quantization trade-offs.
...
### [76] [GARDO: Reinforcing Diffusion Models without Reward Hacking](https://arxiv.org/abs/2512.24138)
*Haoran He,Yuxiao Ye,Jie Liu,Jiajun Liang,Zhiyong Wang,Ziyang Yuan,Xintao Wang,Hangyu Mao,Pengfei Wan,Ling Pan*

Main category: cs.LG

TL;DR: GARDO is a new framework for fine-tuning diffusion models via RL, which uses adaptive and selective regularization to prevent reward hacking, boost exploration, and maintain diversity without sacrificing efficiency.


<details>
  <summary>Details</summary>
Motivation: In text-to-image RL fine-tuning, proxy rewards often lead to reward hacking (higher scores but worse real image quality and diversity loss), while traditional regularization methods hinder exploration due to reliance on sub-optimal reference policies, creating a trade-off between sample efficiency, exploration, and hack prevention.

Method: Propose GARDO, a framework compatible with various RL algorithms, featuring: 1) Selective regularization that penalizes only high-uncertainty samples to avoid universal penalization; 2) Adaptive regularization where the reference model is updated periodically to align with the online policy for relevant targets; 3) Diversity-aware optimization that amplifies rewards for high-quality and diverse samples to encourage mode coverage and prevent collapse.

Result: Extensive experiments across various proxy rewards and unseen metrics show GARDO effectively mitigates reward hacking, enhances generation diversity, and maintains sample efficiency and exploration, demonstrating robustness.

Conclusion: GARDO effectively addresses the competing challenges in RL fine-tuning of diffusion models by combining selective penalization, adaptive regularization updates, and diversity rewards, offering a versatile and robust solution for improving text-to-image alignment without common trade-offs.
...
### [77] [Colorful Pinball: Density-Weighted Quantile Regression for Conditional Guarantee of Conformal Prediction](https://arxiv.org/abs/2512.24139)
*Qianyi Chen,Bo Li*

Main category: cs.LG

TL;DR: A novel density-weighted quantile regression approach for conformal prediction that significantly improves conditional coverage by minimizing mean squared error of conditional coverage through refined quantile estimation.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of achieving reliable conditional coverage for specific inputs in conformal prediction, as exact distribution-free conditional coverage is impossible with finite samples but can be improved through refined quantile regression.

Method: A three-headed quantile network architecture that estimates density weights via finite differences using auxiliary quantile levels at 1-α±δ, then fine-tunes the central quantile by optimizing a density-weighted pinball loss derived from Taylor expansion.

Result: Extensive experiments on diverse high-dimensional real-world datasets demonstrate remarkable improvements in conditional coverage performance, with theoretical analysis providing exact non-asymptotic guarantees on excess risk.

Conclusion: The proposed three-headed quantile network with density-weighted pinball loss significantly improves conditional coverage performance in conformal prediction, offering a practical and theoretically sound approach to achieving better distribution-free conditional coverage guarantees.
...
### [78] [Paired Seed Evaluation: Statistical Reliability for Learning-Based Simulators](https://arxiv.org/abs/2512.24145)
*Udit Sharma*

Main category: cs.LG

TL;DR: Proposes using identical random seeds for paired evaluations to reduce variance in comparing machine learning systems, improving statistical power.


<details>
  <summary>Details</summary>
Motivation: To address high variance in evaluation outcomes of machine learning systems due to stochasticity, independent designs fail to exploit shared randomness.

Method: Formalizes a paired seed evaluation design where systems are evaluated under identical seeds, analyzing statistical structure and correlation benefits.

Result: Shows large positive correlations and order-of-magnitude efficiency gains, tighter confidence intervals, and higher power without validity loss.

Conclusion: Paired seed evaluation is weakly dominant, enhancing reliability with correlations and equivalent to independent evaluation otherwise.
...
### [79] [Micro-Macro Tensor Neural Surrogates for Uncertainty Quantification in Collisional Plasma](https://arxiv.org/abs/2512.24205)
*Wei Chen,Giacomo Dimarco,Lorenzo Pareschi*

Main category: cs.LG

TL;DR: A variance-reduced Monte Carlo framework with neural network surrogates enables efficient uncertainty quantification for the Vlasov-Poisson-Landau system.


<details>
  <summary>Details</summary>
Motivation: Plasma kinetic equations are sensitive to perturbations, requiring reliable uncertainty quantification (UQ), but traditional methods struggle due to high computational cost, high-dimensional phase space, multiscale stiffness, and constraints from collisions.

Method: Developed a variance-reduced Monte Carlo framework coupling a high-fidelity VPL solver with neural network surrogates (VPFP and EP models). Introduced a generalized separable physics-informed neural network (SPINN) with anisotropic micro-macro decomposition to reduce velocity-moment costs and model complexity. Calibrated VPFP model and designed an asymptotic-preserving SPINN.

Result: Numerical experiments show substantial variance reduction over standard Monte Carlo, accurate statistics with far fewer high-fidelity samples, lower wall-clock time, and robustness to stochastic dimension.

Conclusion: The proposed variance-reduced Monte Carlo framework using neural network surrogates effectively addresses UQ for the Vlasov-Poisson-Landau system, demonstrating substantial improvements in computational efficiency, accuracy, and robustness.
...
### [80] [Early Prediction of Sepsis using Heart Rate Signals and Genetic Optimized LSTM Algorithm](https://arxiv.org/abs/2512.24253)
*Alireza Rafiei,Farshid Hajati,Alireza Rezaee,Amirhossien Panahi,Shahadat Uddin*

Main category: cs.LG

TL;DR: A study introduces four ML models for early sepsis prediction using heart rate data from wearables, optimized via genetic algorithm.


<details>
  <summary>Details</summary>
Motivation: Sepsis causes high mortality and costs, yet early detection in non-ICU settings is lacking, prompting development of wearable-based solutions.

Method: Four ML algorithms were developed and refined with genetic algorithm to balance performance, complexity, and memory for wearable implementation.

Result: Models showed feasibility for sepsis onset prediction, with transfer learning extending prediction windows from 1 to 4 hours.

Conclusion: Wearable technology holds promise for enabling early sepsis detection outside ICU and ward environments, improving outcomes.
...
### [81] [Empower Low-Altitude Economy: A Reliability-Aware Dynamic Weighting Allocation for Multi-modal UAV Beam Prediction](https://arxiv.org/abs/2512.24324)
*Haojin Li,Anbang Zhang,Chen Sun,Chenyuan Feng,Kaiqian Qu,Tony Q. S. Quek,Haijun Zhang*

Main category: cs.LG

TL;DR: SaM2B: A semantic-aware multi-modal beam prediction framework with reliability-aware dynamic weighting and cross-modal contrastive learning for UAV communications.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal beam prediction methods use fixed or empirical weights, assuming equal reliability across modalities. This fails as modality importance fluctuates with UAV motion scenarios, and static weights amplify negative impacts of degraded modalities. Modal mismatch and weak alignment also hinder cross-scenario generalization.

Method: Propose SaM2B, a semantic-aware multi-modal beam prediction framework. It uses lightweight cues (environmental visual, flight posture, geospatial data) with reliability-aware dynamic weight updates to adaptively allocate contributions across modalities. Employs cross-modal contrastive learning to align multi-source representation beam semantics to a shared semantic space.
...
Conclusion: The SaM2B framework demonstrates superiority over baseline methods in real-world UAV datasets, proving that dynamic weighting and cross-modal alignment effectively address the challenges of modal variability, mismatch, and weak alignment in multi-modal beam prediction.
...
### [82] [Tubular Riemannian Laplace Approximations for Bayesian Neural Networks](https://arxiv.org/abs/2512.24381)
*Rodrigo Pereira David*

Main category: cs.LG

TL;DR: TRL is a tubular Riemannian Laplace approximation that models Bayesian neural network posteriors as probabilistic tubes along loss valleys, achieving ensemble-grade calibration at 1/5 training cost compared to Deep Ensembles.


<details>
  <summary>Details</summary>
Motivation: Laplace approximations are simple and practical for approximate Bayesian inference in neural networks, but their Euclidean formulation struggles with highly anisotropic, curved loss surfaces and large symmetry groups in modern deep models. Riemannian and geometric Gaussian approximations have been proposed to address this structure.

Method: The Tubular Riemannian Laplace (TRL) approximation models the posterior as a probabilistic tube that follows low-loss valleys induced by functional symmetries, using Fisher/Gauss-Newton metric to separate tangential uncertainty (prior-dominated) from transverse uncertainty (data-dominated). It serves as a scalable reparametrized Gaussian approximation that utilizes implicit curvature estimates.

Result: Empirical evaluation on ResNet-18 (CIFAR-10 and CIFAR-100) shows that TRL achieves excellent calibration, matching or exceeding Deep Ensembles in terms of ECE, while requiring only 1/5 of the training cost.

Conclusion: The TRL method bridges the gap between single-model efficiency and ensemble-grade reliability by providing excellent calibration at substantially lower computational cost.
...
### [83] [Lifting Vision: Ground to Aerial Localization with Reasoning Guided Planning](https://arxiv.org/abs/2512.24404)
*Soham Pahari,M. Srinivas*

Main category: cs.LG

TL;DR: ViReLoc is a visual reasoning framework for spatial tasks like navigation and localization, using only visual representations without text or GPS, achieving improved accuracy through reinforcement learning and contrastive methods.


<details>
  <summary>Details</summary>
Motivation: Current reasoning systems rely heavily on textual information for inference, limiting their effectiveness in spatial tasks such as visual navigation and geo-localization, which require better understanding of visual and geometric relations.

Method: Proposes Geo-Consistent Visual Planning with the ViReLoc framework, which learns spatial dependencies and geometric relations through visual representations, uses step-by-step inference in the visual domain, optimizes with reinforcement-based objectives for planning routes, and integrates contrastive learning and adaptive feature interaction to align cross-view perspectives.

Result: Experiments show consistent improvements in spatial reasoning accuracy and cross-view retrieval performance across diverse navigation and localization scenarios, enabling tasks without real-time GPS data.

Conclusion: Visual reasoning, as demonstrated by ViReLoc, serves as a strong complementary approach for navigation and localization, offering more secure solutions by eliminating reliance on textual input and GPS data.
...
### [84] [Efficient Inference for Inverse Reinforcement Learning and Dynamic Discrete Choice Models](https://arxiv.org/abs/2512.24407)
*Lars van der Laan,Aurelien Bibaut,Nathan Kallus*

Main category: cs.LG

TL;DR: A semiparametric framework for debiased inverse reinforcement learning that enables efficient inference for reward-dependent functionals in maximum entropy IRL and DDC models, leveraging nonparametric nuisance estimation while maintaining statistical guarantees.


<details>
  <summary>Details</summary>
Motivation: Inverse reinforcement learning and dynamic discrete choice models aim to recover reward functions from observed behavior, but existing methods lack guarantees (IRL) or are too restrictive (DDC). A flexible yet statistically valid approach is needed.

Method: Develop a framework that treats the log-behavior policy as a pseudo-reward to identify rewards and policy values, formalizes targets as smooth functionals, derives efficient influence functions, and constructs debiased machine-learning estimators.

Result: Established pathwise differentiability and efficient influence functions for policy differences and normalized rewards under softmax policies, enabling root-n consistent, asymptotically normal, and efficient semiparametric inference without restrictive specifications.

Conclusion: This work unifies IRL and DDC inference, extending DDC methods to nonparametric rewards with modern machine-learning tools, providing a tractable, statistically rigorous approach for sequential decision-making analysis.
...
### [85] [Sparse classification with positive-confidence data in high dimensions](https://arxiv.org/abs/2512.24443)
*The Tien Mai,Mai Anh Nguyen,Trung Nghia Nguyen*

Main category: cs.LG

TL;DR: Novel sparse regularization framework for high-dimensional positive-confidence classification using Lasso, SCAD, and MCP penalties with theoretical guarantees and efficient optimization.


<details>
  <summary>Details</summary>
Motivation: Existing Positive-Confidence (Pconf) classification methods are ill-suited for high-dimensional regimes where the number of features exceeds sample size, despite the need for sparse regularization in such settings.

Method: The paper proposes a novel sparse-penalization framework using convex (Lasso) and non-convex (SCAD, MCP) penalties, develops theoretical error bounds, and creates an efficient proximal gradient algorithm to solve the composite objective.

Result: The proposed methods achieve predictive performance and variable selection accuracy comparable to fully supervised approaches, with theoretical guarantees including near minimax-optimal sparse recovery rates under Restricted Strong Convexity condition.

Conclusion: This paper successfully bridges the gap between weak supervision and high-dimensional statistics by proposing a sparse-penalization framework for Pconf classification that achieves performance comparable to fully supervised approaches.
...
### [86] [HOLOGRAPH: Active Causal Discovery via Sheaf-Theoretic Alignment of Large Language Model Priors](https://arxiv.org/abs/2512.24478)
*Hyunjun Kim*

Main category: cs.LG

TL;DR: HOLOGRAPH is a framework that uses sheaf theory to formalize LLM-guided causal discovery, providing mathematical foundations for integrating LLM prior knowledge and handling hidden confounders.


<details>
  <summary>Details</summary>
Motivation: Causal discovery from observational data suffers from identifiability constraints, and existing LLM-based approaches lack theoretical grounding by relying on heuristic integration methods.

Method: The framework represents local causal beliefs as sections of a presheaf over variable subsets, with coherent global structure corresponding to global sections. It uses Algebraic Latent Projection to handle hidden confounders and Natural Gradient Descent on belief manifolds for optimization.

Result: HOLOGRAPH achieves competitive performance on causal discovery tasks with 50-100 variables on synthetic and real-world benchmarks. Sheaf-theoretic analysis shows Identity, Transitivity, and Gluing axioms hold (<10^{-6} precision) but Locality axiom fails for larger graphs, indicating non-local coupling in latent projections.

Conclusion: HOLOGRAPH provides rigorous mathematical foundations for LLM-guided causal discovery, revealing fundamental non-local coupling in latent variable projections through sheaf theory while maintaining practical performance on complex causal discovery tasks.
...
TL;DR: A diagnostic framework decomposes error evolution into bias, noise, and alignment to adaptively control learning in dynamic environments, and enables stable optimization, actor-critic RL, and learned optimizers.


<details>
  <summary>Details</summary>
Motivation: Existing learning systems in nonstationary and safety-critical environments often display instability, slow convergence, or brittle adaptation. While modern methods adjust to gradient statistics, they neglect the temporal structure of error signals, limiting their ability to handle dynamic error evolution.

Method: A diagnostic-driven adaptive learning framework is introduced, which decomposes error evolution into bias (persistent drift), noise (stochastic variability), and alignment (directional excitation leading to overshoot). These diagnostics are computed in real-time from loss or temporal-difference error trajectories without dependence on model architecture or task domain. The framework provides control mechanisms for supervised optimization, actor-critic RL, and learned optimizers.

Result: The bias-noise-alignment decomposition serves as a unifying control backbone across various learning settings. Diagnostic-driven instantiations are derived, yielding stabilized optimization, regulated actor-critic schemes, and conditioned learned optimizers with bounded updates and stability guarantees under smoothness assumptions. Illustrations in actor-critic learning show how the signals modulate adaptation based on temporal-difference error patterns.

Conclusion: The paper demonstrates that error evolution can be effectively modeled and controlled as a first-class object, offering an interpretable and lightweight foundation for robust learning in dynamic environments.
...
### [88] [Can Small Training Runs Reliably Guide Data Curation? Rethinking Proxy-Model Practice](https://arxiv.org/abs/2512.24503)
*Jiachen T. Wang,Tong Wu,Kaifeng Lyu,James Zou,Dawn Song,Ruoxi Jia,Prateek Mittal*

Main category: cs.LG

TL;DR: Standard small-scale data recipe evaluations using identical training configurations produce unreliable conclusions that may not transfer to full-scale training. The paper proposes using reduced learning rates instead, which preserves dataset performance rankings and aligns with full-scale outcomes.


<details>
  <summary>Details</summary>
Motivation: The motivation stems from the observation that the standard experimental protocol for data recipe assessment uses identical small-scale model training configurations for 'fair' comparison, but this approach fails because optimal training configurations are data-dependent. This leads to unreliable conclusions that may not transfer to full-scale model training, where hyperparameter optimization is standard practice.

Method: The study introduces a simple modification to the standard evaluation protocol: using reduced learning rates for proxy model training instead of identical training configurations across all data recipes. This approach is theoretically analyzed using random-feature models and empirically validated across 23 data recipes covering four critical dimensions of data curation.

Result: The results show that using reduced learning rates yields relative performance that strongly correlates with fully tuned large-scale LLM pretraining runs. The approach dramatically improves the reliability of small-scale experiments and preserves the ordering of datasets according to their optimal achievable loss.

Conclusion: The paper concludes that using reduced learning rates in small-scale proxy model training is an effective patch that improves the reliability of data recipe assessments, as it preserves the relative performance ordering of datasets and aligns better with full-scale training outcomes.
...
### [89] [Generative forecasting with joint probability models](https://arxiv.org/abs/2512.24446)
*Patrick Wyrod,Ashesh Chattopadhyay,Daniele Venturi*

Main category: cs.LG

TL;DR: Forecasting chaotic systems is reframed as learning joint distributions over short time windows, enabling improved accuracy, uncertainty quantification, and preservation of dynamics compared to traditional conditional models.


<details>
  <summary>Details</summary>
Motivation: Deterministic forecasting of chaotic systems is limited due to sensitivity to initial conditions and unresolved multiscale processes; generative models are an alternative but often focus on next-step prediction rather than underlying dynamics.

Method: Learn the joint probability distribution of lagged system states over short temporal windows, obtain forecasts via marginalization, and use a model-agnostic training/inference framework with three uncertainty metrics (ensemble variance, short-horizon autocorrelation, cumulative Wasserstein drift).

Result: The method evaluated on Lorenz-63 and Kuramoto-Sivashinsky systems shows improved short-term predictive skill, preservation of attractor geometry, and more accurate long-range statistical behavior compared to conventional conditional next-step models.

Conclusion: Joint generative forecasting effectively captures nonlinear dependencies and quantifies uncertainty without ground truth, offering a robust approach for chaotic dynamical systems.
...
TL;DR: Multi-envelope Double Binary Factorization (MDBF) improves extreme low-bit quantization for LLMs by replacing single envelope with rank-l envelope to avoid performance saturation and enhance accuracy.


<details>
  <summary>Details</summary>
Motivation: Double Binary Factorization (DBF) is effective for extreme low-bit quantization in large language models, enabling efficient inference, but its restrictive scaling parameters cause performance saturation as all rank components share the same magnitude profile, limiting expressiveness.

Method: Proposes MDBF with shared 1-bit sign bases and replaces single envelope with rank-l envelope, uses closed-form initialization and alternating refinement method for optimization.

Result: MDBF improves perplexity and zero-shot accuracy over previous binary formats on LLaMA and Qwen families at matched bits per weight while preserving deployment-friendly inference.

Conclusion: MDBF overcomes DBF limitations by enhancing magnitude expressiveness for better quantization performance, maintaining practical inference efficiency.
...
### [91] [Dynamic Large Concept Models: Latent Reasoning in an Adaptive Semantic Space](https://arxiv.org/abs/2512.24617)
...
TL;DR: DLCM proposes hierarchical language modeling that compresses tokens into variable-length concepts, enabling compute reallocation and improved performance (+2.69% across benchmarks) under fixed FLOPs via compression-aware scaling.


<details>
  <summary>Details</summary>
Motivation: Current LLMs use uniform computation per token despite language's non-uniform information density, wasting capacity on predictable spans while under-allocating computation to semantically critical transitions.

Method: DLCM learns semantic boundaries from latent representations and compresses tokens into variable-length concepts end-to-end. It employs a hierarchical architecture with a decoupled μP parametrization for stable training and introduces compression-aware scaling laws.

Result: At compression ratio R=4 (avg 4 tokens per concept), DLCM reallocates ~1/3 of inference compute to a higher-capacity reasoning backbone, achieving +2.69% average improvement across 12 zero-shot benchmarks under matched FLOPs.

Conclusion: DLCM demonstrates that hierarchical compression and computation reallocation can significantly improve language model performance under fixed FLOPs, offering a promising alternative to traditional token-uniform approaches.
...
### [92] [AutoFed: Manual-Free Federated Traffic Prediction via Personalized Prompt](https://arxiv.org/abs/2512.24625)
*Zijian Zhao,Yitong Shang,Sen Li*

Main category: cs.LG

TL;DR: AutoFed introduces a personalized federated learning framework for traffic prediction using prompt learning to avoid manual hyperparameter tuning, achieving strong results.


<details>
  <summary>Details</summary>
Motivation: Existing methods face privacy concerns (data silos) and non-IID challenges; PFL needs adaptation for traffic prediction and often requires unrealistic hyperparameter tuning.
...
Result: Experiments on real-world datasets show AutoFed consistently achieves superior performance across diverse scenarios.

Conclusion: AutoFed is a novel PFL framework that eliminates manual hyperparameter tuning via a federated representor and client-aligned adapter, enabling practical deployment and superior performance in traffic prediction.
...
### [93] [Generalising E-prop to Deep Networks](https://arxiv.org/abs/2512.24506)
*Beren Millidge*

Main category: cs.LG

TL;DR: This paper extends the E-prop framework to handle deep recurrent networks, enabling online credit assignment across time and depth without backpropagation through time.


<details>
  <summary>Details</summary>
Motivation: Current online learning methods like RTRL and E-prop are limited to single-layer recurrent networks, whereas brain learning spans multiple layers with hierarchical dynamics in both depth and time.

Method: The paper proposes an extension of E-prop by deriving a novel recursion relationship across depth that extends eligibility traces to deeper layers in arbitrarily deep networks.

Result: The method achieves accurate credit assignment across time and depth simultaneously, allowing training of deep recurrent networks in an online manner.

Conclusion: This work demonstrates a feasible online algorithm for training deep recurrent networks without backpropagation through time, bridging gaps with brain-like learning mechanisms.
...
TL;DR: A new learning paradigm called Nested Learning (NL) is introduced, framing ML models as nested optimization problems to enhance expressiveness, enable higher-order in-context learning, and support continual learning through novel optimizers, self-modifying models, and memory systems.


<details>
  <summary>Details</summary>
Motivation: Addressing fundamental challenges in continual learning, self-improvement, and effective solution-finding in Language Models and ML, such as how models can continuously learn, memorize, and evolve.

Method: Proposes NL by: 1) Expressive Optimizers: Reinterpreting gradient-based optimizers as associative memory modules and developing more expressive variants; 2) Self-Modifying Learning Module: A sequence model that learns its own update algorithm; 3) Continuum Memory System: A generalized memory formulation combined with the self-modifying model into Hope for continual learning.

Result: Introduces Hope, a continual learning module combining self-modifying models and memory systems, showing promising results in language modeling, knowledge incorporation, few-shot generalization, continual learning, and long-context reasoning tasks.

Conclusion: NL provides a novel paradigm for designing more expressive learning algorithms, potentially unlocking effective continual learning capabilities and advancing in-context learning in large models, with demonstrated applications in Hope.
...
### [95] [BandiK: Efficient Multi-Task Decomposition Using a Multi-Bandit Framework](https://arxiv.org/abs/2512.24708)
*András Millinghoffer,András Formanek,András Antos,Péter Antal*

Main category: cs.LG

TL;DR: BandiK is an efficient multi-task auxiliary task selection method that uses multi-bandits to address negative transfer and computational costs in knowledge transfer, reducing candidate sets from exponential to linear complexity.


<details>
  <summary>Details</summary>
Motivation: The challenge of effectively transferring knowledge across multiple tasks is critical, but negative transfer remains a significant obstacle. Current methods for auxiliary task selection are hindered by high computational costs, large number of candidate auxiliary sets, and varying selection complexity across target tasks.

Method: BandiK is a three-stage multi-task auxiliary task subset selection method using multi-bandits. Stage 1: Estimates pairwise transfers between tasks to identify beneficial joint learning. Stage 2: Constructs a linear number of candidate auxiliary sets based on initial estimates. Stage 3: Employs Multi-Armed Bandit framework where arms represent performance of candidate auxiliary sets realized as multiple output neural networks, with a novel multi-bandit structure that leverages semi-overlapping arm properties.

Result: BandiK significantly reduces the exponential number of potential auxiliary task sets to a linear number, and introduces an efficient multi-bandit framework that exploits semi-overlapping arm properties to enhance computational efficiency while maintaining effective task selection.

Conclusion: BandiK successfully addresses the challenge of auxiliary task selection in multi-task learning by introducing an efficient three-stage approach that significantly reduces computational costs while effectively handling negative transfer and transitive-intransitive nature of knowledge transfer.
...
TL;DR: HUMOR: a framework for humorous meme generation using hierarchical CoT and group-wise preference learning to align VLMs with human humor.


<details>
  <summary>Details</summary>
Motivation: Generating humorous memes is challenging and requires nuanced reasoning beyond direct supervision. Current methods lack diversity and subjective humor alignment.

Method: HUMOR employs hierarchical multi-path Chain-of-Thought reasoning and group-wise human preference alignment via a pairwise reward model and group-wise RL.

Result: Extensive experiments show HUMOR empowers VLMs with superior reasoning diversity, reliable preference alignment, and higher overall meme quality.

Conclusion: HUMOR provides a general training paradigm for open-ended, multimodal generation tasks that require human alignment, using hierarchical reasoning and preference learning.
...
### [97] [MSACL: Multi-Step Actor-Critic Learning with Lyapunov Certificates for Exponentially Stabilizing Control](https://arxiv.org/abs/2512.24955)
*Yongwei Zhang,Yuanzhe Xing,Quan Quan,Zhikun She*

Main category: cs.LG

TL;DR: MSACL integrates exponential stability theory with max-entropy RL via multi-step Lyapunov certificate learning, achieving provably stable control without complex reward engineering.


<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of achieving provable stability in model-free RL while balancing exploration with rigorous safety, without relying on complex reward engineering.

Method: Integrates exponential stability theory with maximum entropy RL via multi-step Lyapunov certificate learning using off-policy multi-step data, Exponential Stability Labels (ESL), λ-weighted aggregation, and stability-aware advantage function for policy optimization.

Result: Demonstrates superiority over state-of-the-art Lyapunov-based RL algorithms across six benchmarks, achieving exponential stability, rapid convergence, robustness to uncertainties, and generalization to unseen trajectories. Identifies multi-step horizon n=20 as a robust default.

Conclusion: MSACL successfully links Lyapunov theory with off-policy actor-critic frameworks to provide a foundation for verifiably safe learning-based control, achieving exponential stability and robustness with simple rewards.
...
### [98] [CPR: Causal Physiological Representation Learning for Robust ECG Analysis under Distribution Shifts](https://arxiv.org/abs/2512.24564)
*Shunbo Jia,Caizhi Liao*

Main category: cs.LG

TL;DR: This paper proposes Causal Physiological Representation Learning (CPR), a method to improve ECG diagnosis robustness against adversarial attacks by separating invariant pathological features from artifacts using a causal framework.


<details>
  <summary>Details</summary>
Motivation: Deep learning ECG models are fragile to adversarial perturbations, especially Smooth Adversarial Perturbations (SAP) that mimic biological shapes. Existing defenses like Adversarial Training (AT) are computationally heavy, while certified methods like Randomized Smoothing (RS) add inference delays, making them unsuitable for real-time use. The vulnerability is due to reliance on non-robust spurious correlations instead of invariant pathological features.

Method: The paper introduces CPR, which incorporates a Physiological Structural Prior within a causal disentanglement framework. It models ECG generation via a Structural Causal Model (SCM) to enforce structural interventions, strictly separating invariant pathological morphology (e.g., P-QRS-T complex) from non-causal artifacts.

Result: On the PTB-XL dataset, CPR outperforms standard clinical preprocessing methods. Under SAP attacks, it achieves an F1 score of 0.632, surpassing Median Smoothing’s 0.541 by 9.1%. CPR matches the certified robustness of Randomized Smoothing while maintaining efficient single-pass inference.

Conclusion: CPR offers a superior trade-off between robustness, efficiency, and clinical interpretability for ECG diagnosis, addressing the limitations of existing adversarial defenses in real-time clinical monitoring.
...
### [99] [Semi-overlapping Multi-bandit Best Arm Identification for Sequential Support Network Learning](https://arxiv.org/abs/2512.24959)
*András Antos,András Millinghoffer,Péter Antal*

Main category: cs.LG

TL;DR: Proposes SOMMAB framework with GapE algorithm for learning support networks efficiently via shared evaluations, achieving better sample complexity bounds for multi-bandit problems.


<details>
  <summary>Details</summary>
Motivation: Many AI/ML problems require evaluating partners' contributions through shared asymmetric processes while selecting optimal candidates, needing a unified framework for efficiency.

Method: A generalized GapE algorithm for semi-overlapping multi-multi-armed bandits (SOMMAB), with derived exponential error bounds for best-arm identification that scale linearly with overlap degree.

Result: New exponential error bounds improve best known constant for multi-bandit best-arm identification, showing linear scaling with overlap degree and significant sample-complexity gains from shared evaluations.

Conclusion: The paper presents a novel SOMMAB framework and GapE algorithm that provide theoretical foundations for efficient support network learning with provable sample complexity improvements, making it valuable for various distributed AI applications.
...
### [100] [Generative Classifiers Avoid Shortcut Solutions](https://arxiv.org/abs/2512.25034)
*Alexander C. Li,Ananya Kumar,Deepak Pathak*

Main category: cs.LG

TL;DR: Generative classifiers, using class-conditional generative models, outperform discriminative ones by avoiding overreliance on spurious features and achieving state-of-the-art results on distribution shift benchmarks.


<details>
  <summary>Details</summary>
Motivation: Discriminative classifiers often fail under distribution shift due to learning shortcuts from spurious correlations, whereas generative classifiers can model all features to mitigate this issue.

Method: Employ diffusion-based and autoregressive generative classifiers, which require no specialized augmentations, regularization, hyperparameters, or prior knowledge of spurious correlations.

Result: Generative classifiers achieve state-of-the-art performance on five image and text distribution shift benchmarks and reduce spurious correlation effects in medical and satellite datasets.

Conclusion: Generative classifiers are better at avoiding spurious correlations and handling distribution shifts, with their effectiveness analyzed through a Gaussian toy setting to understand inductive biases and data properties.
...
### [101] [A Scalable Framework for logP Prediction: From Terabyte-Scale Data Integration to Interpretable Ensemble Modeling](https://arxiv.org/abs/2512.24643)
*Malikussaid,Septian Caesar Floresko,Ade Romadhony,Isman Kurniawan,Warih Maharani,Hilal Hudan Nuha*

Main category: cs.LG

TL;DR: Large-scale logP prediction framework using 426K compounds from three databases, achieving 740-fold processing improvement. Tree ensemble models outperformed linear models, with stratified approach yielding best results. 2D descriptor-based models remain competitive with graph neural networks.


<details>
  <summary>Details</summary>
Motivation: To create a large-scale predictive modeling framework for logP prediction using carefully curated compounds from multiple authoritative chemical databases, addressing data integration challenges and providing insights into lipophilicity prediction.

Method: Developed a computational infrastructure with byte-offset indexing architecture for data integration. Systematically evaluated multiple modeling approaches including linear models and tree-based ensemble methods (Random Forest, XGBoost). Implemented a stratified modeling strategy with specialized models for drug-like molecules and extreme cases.

Result: Tree-based ensemble methods achieved R-squared of 0.765 and RMSE of 0.731 logP units on the test set. Stratified modeling strategy achieved optimal performance with RMSE of 0.838 for drug-like subset and R-squared of 0.767 for extreme molecules. Molecular weight was identified as the single most important predictor globally despite weak bivariate correlation.

Conclusion: These findings provide actionable guidance for molecular design, establish robust baselines for lipophilicity prediction using only 2D descriptors, and demonstrate that well-curated, descriptor-based ensemble models remain competitive with state-of-the-art graph neural network architectures.
...
### [102] [HeteroHBA: A Generative Structure-Manipulating Backdoor Attack on Heterogeneous Graphs](https://arxiv.org/abs/2512.24665)
*Honglin Gao,Lan Zhao,Junhao Ren,Xiang Li,Gaoxi Xiao*

Main category: cs.LG

TL;DR: HeteroHBA is a generative backdoor attack framework for heterogeneous node classification that synthesizes diverse, stealthy triggers using saliency-based neighbor selection and distribution alignment, achieving high attack success with minimal clean accuracy drop.


<details>
  <summary>Details</summary>
Motivation: Existing heterogeneous graph neural networks (HGNNs) are vulnerable to backdoor poisoning, where an adversary injects triggers during training to cause misclassification at test time while preserving clean performance. However, targeted backdoor attacks on heterogeneous graphs are under-studied.

Method: HeteroHBA uses: 1) saliency-based screening to select influential auxiliary neighbors for trigger attachment, 2) synthesis of diverse trigger features and connection patterns to better match local heterogeneous context, 3) Adaptive Instance Normalization (AdaIN) combined with Maximum Mean Discrepancy (MMD) loss to align trigger feature distribution with benign statistics for stealth, and 4) bilevel optimization to jointly promote attack success and maintain clean accuracy.

Result: Experiments on multiple real-world heterogeneous graphs with representative HGNN architectures show that HeteroHBA consistently achieves higher attack success than prior backdoor baselines with comparable or smaller impact on clean accuracy. Moreover, the attack remains effective under the heterogeneity-aware structural defense CSD.

Conclusion: The authors propose HeteroHBA, a generative backdoor framework for heterogeneous node classification. Key innovations include saliency-based selection of auxiliary neighbors, synthesis of diverse triggers to match local context, and distribution alignment via AdaIN+MMD for stealth. Experiments show HeteroHBA outperforms prior backdoor baselines in attack success with minimal clean accuracy drop, and remains effective against a heterogeneity-aware defense (CSD). This highlights real-world backdoor risks in heterogeneous graph learning and calls for stronger defenses.
...
### [103] [Mobility-Assisted Decentralized Federated Learning: Convergence Analysis and A Data-Driven Approach](https://arxiv.org/abs/2512.24694)
*Reza Jahani,Md Farhamdur Reza,Richeng Jin,Huaiyu Dai*

Main category: cs.LG

TL;DR: A study shows how user mobility can significantly improve Decentralized Federated Learning performance in sparse networks, proposing a framework with induced mobility patterns to enhance learning.


<details>
  <summary>Details</summary>
Motivation: Decentralized Federated Learning often suffers from poor performance due to limited connectivity and data heterogeneity. User mobility, whether natural or induced, can act as relays to improve information flow, but its impact on DFL has been largely overlooked despite its potential benefits.

Method: The work first establishes convergence of DFL in sparse networks under user mobility and theoretically analyzes how random movement benefits performance. Then, it proposes a DFL framework that utilizes mobile users with induced mobility patterns, allowing them to use knowledge of data distribution to determine trajectories and enhance information propagation.

Result: Extensive experiments confirm the theoretical findings, validate the superiority of the proposed approach over baselines, and provide a comprehensive analysis of how various network parameters influence DFL performance in mobile networks.

Conclusion: Mobility plays a crucial role in boosting DFL performance, enabling better collaboration and learning outcomes in decentralized settings, with induced mobility patterns offering a practical framework for improvement.
...
### [104] [Causal Discovery with Mixed Latent Confounding via Precision Decomposition](https://arxiv.org/abs/2512.24696)
*Amir Asiaee,Samhita Pal,James O'quinn,James P. Long*

Main category: cs.LG

TL;DR: DCL-DECOR is a method for causal discovery in linear Gaussian systems with mixed latent confounding, separating global and local latent effects via precision matrix decomposition and correlated-noise DAG learning.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with mixed latent confounding, where global latent confounders cause false causal edge detection in DAG learners, and latent-variable models only capture undirected structure.

Method: Isolate pervasive latent effects by decomposing the observed precision matrix into a structured component and low-rank component, then apply a correlated-noise DAG learner to the deconfounded representation, followed by a bow-freeness reconciliation step.

Result: Identifiability results show recoverable causal targets under mixed confounding, with synthetic experiments demonstrating improved directed edge recovery over applying correlated-noise DAG learning directly to confounded data.

Conclusion: The method effectively separates confounder roles, reducing the problem to modular subproblems with guarantees, offering consistent improvements in edge recovery in practical settings with mixed confounding.
...
### [105] [FPGA Co-Design for Efficient N:M Sparse and Quantized Model Inference](https://arxiv.org/abs/2512.24713)
*Fen-Yu Hsieh,Yun-Chang Teng,Ding-Yong Hong,Jan-Jan Wu*

Main category: cs.LG

TL;DR: Hardware-software co-design framework integrates structured pruning and quantization to enable efficient LLM inference on resource-constrained platforms.


<details>
  <summary>Details</summary>
Motivation: Large language models have high computation and memory requirements that impede deployment in resource-constrained environments. There is a need for efficient inference methods to enable LLM deployment where resources are limited.

Method: The paper introduces an automation framework leveraging weight pruning and low-bit quantization, and presents a hardware-software co-design method generating accelerators on FPGA. A unified pipeline applies N:M structured pruning and 4-bit integer quantization to reduce memory footprint, then uses optimized dequantization and matrix multiplication for LLM inference across CPUs, NVIDIA GPUs, and custom FPGA accelerators.

Result: Using 2:4 sparsity with quantization on 4096×4096 matrices, the approach achieves 4× reduction in weight storage, 1.71× speedup in matrix multiplication, and 1.29× end-to-end latency reduction compared to dense GPU baselines. On LLaMA-7B, structured sparsity enhances throughput per token by 1.36×.

Conclusion: This work demonstrates that the combination of structured N:M pruning and 4-bit quantization, along with hardware-software co-design, enables efficient and deployable LLM inference in resource-constrained environments. The FPGA accelerator provides a flexible architectural solution for supporting diverse sparsity patterns beyond fixed hardware constraints.
...
### [106] [From Trial to Deployment: A SEM Analysis of Traveler Adoptions to Fully Operational Autonomous Taxis](https://arxiv.org/abs/2512.24767)
*Yutong Cai,Hua Wang*

Main category: cs.LG

TL;DR: Study of actual autonomous taxi users in Wuhan shows cost sensitivity and behavioral intention are strongest predictors of adoption, providing practical insights for policymakers and service operators.


<details>
  <summary>Details</summary>
Motivation: To address the gap in existing literature that focuses on hypothetical scenarios and stated preferences by investigating actual user behavior based on operational autonomous vehicle services.

Method: The research employs Structural Equation Modeling (SEM) on survey data from 336 actual users of Baidu's Apollo Robotaxi service in Wuhan, China. The survey incorporates actual service attributes and measures adoption through selection frequency in ten scenarios. Six latent psychological constructs are identified and analyzed.

Result: Cost Sensitivity and Behavioral Intention emerge as the strongest positive predictors of autonomous taxi adoption. The model demonstrates strong goodness-of-fit. Other constructs like Trust & Policy Support, Performance, Lifestyle, and Education play more nuanced roles.

Conclusion: The study concludes that cost sensitivity and behavioral intention are the strongest predictors of autonomous taxi adoption, with other psychological constructs playing more nuanced roles. The findings provide empirical evidence to inform policymaking, fare design, and public outreach strategies for scaling autonomous taxi deployments in urban environments.
...
### [107] [Gradient Descent as Implicit EM in Distance-Based Neural Models](https://arxiv.org/abs/2512.24780)
*Alan Oursland*

Main category: cs.LG

TL;DR: Gradient descent on log-sum-exp objectives performs expectation-maximization implicitly, unifying mixture models, attention, and classification under a single mechanism where optimization and inference are identical.


<details>
  <summary>Details</summary>
Motivation: To provide a direct derivation explaining why neural networks trained with standard objectives exhibit probabilistic inference behaviors like soft clustering and Bayesian uncertainty, without relying on loose analogies or post-hoc interpretations.

Method: Analyzes objectives with log-sum-exp structure over distances or energies, showing algebraically that the gradient with respect to each distance equals the negative posterior responsibility of the corresponding component, i.e., ∂L/∂d_j = -r_j.

Result: Demonstrates that gradient descent on such objectives implicitly performs expectation-maximization, with responsibilities emerging naturally as gradients rather than requiring explicit computation. This unifies unsupervised mixture modeling, attention mechanisms, and cross-entropy classification under a single process.

Conclusion: The Bayesian structures observed in trained transformers are a necessary consequence of the objective's geometry, implying that optimization and inference are fundamentally the same process, eliminating the need for separate inference algorithms.
...
### [108] [Self-Supervised Neural Architecture Search for Multimodal Deep Neural Networks](https://arxiv.org/abs/2512.24793)
*Shota Suzuki,Satoshi Ono*

Main category: cs.LG

TL;DR: A self-supervised learning method for automating multimodal neural architecture search and pretraining without labeled data.


<details>
  <summary>Details</summary>
Motivation: Multimodal DNNs benefit from NAS but require a lot of labeled data; this paper addresses the need for architecture search from unlabeled data.

Method: A self-supervised learning (SSL) approach is applied comprehensively to both architecture search and model pretraining processes.

Result: Experimental results showed that the method successfully designed architectures for DNNs using unlabeled training data.

Conclusion: The proposed self-supervised learning method is effective for automating multimodal DNN architecture design and pretraining without labels.
...
### [109] [DTI-GP: Bayesian operations for drug-target interactions using deep kernel Gaussian processes](https://arxiv.org/abs/2512.24810)
*Bence Bolgár,András Millinghoffer,Péter Antal*

Main category: cs.LG

TL;DR: DTI-GP offers a scalable Gaussian process framework for drug-target interaction predictions, enabling Bayesian classification, rejection, top-K selection, and ranking, with improved accuracy and novel enrichment scores.


<details>
  <summary>Details</summary>
Motivation: The motivation is to improve drug-target interaction (DTI) predictions by providing precise probabilistic information, which is crucial for understanding limitations and enhancing predictive performance. Gaussian processes offer a versatile approach for integrating advanced representations and Bayesian inference.

Method: The method involves a deep kernel learning-based Gaussian process (DTI-GP) architecture. It includes a neural embedding module for chemical compounds and protein targets, a GP module, and sampling from the predictive distribution to estimate a Bayesian precedence matrix for selection and ranking operations.

Result: DTI-GP outperforms state-of-the-art solutions in predicting drug-target interactions. It allows for constructing a Bayesian accuracy-confidence enrichment score, implementing rejection schemes to improve enrichment, and enabling precise top-K selections and ranking with high expected utility.

Conclusion: The paper proposes DTI-GP as an effective solution for scalable and accurate probabilistic DTI predictions, facilitating advanced operations like Bayesian classification, rejection, and ranking for improved decision-making in drug discovery.
...
### [110] [Unregularized Linear Convergence in Zero-Sum Game from Preference Feedback](https://arxiv.org/abs/2512.24818)
*Shulun Chen,Runlong Zhou,Zihan Zhang,Maryam Fazel,Simon S. Du*

Main category: cs.LG

TL;DR: The paper proposes using Optimistic Multiplicative Weights Update (OMWU) for Nash learning from human feedback, offering faster convergence without assuming Nash equilibrium uniqueness, supported by theoretical guarantees and experiments.


<details>
  <summary>Details</summary>
Motivation: Standard preference modeling for aligning LLMs assumes transitivity, ignoring real-world non-transitive human preferences. NLHF addresses this through game theory but existing methods have bias in duality gap calculations, so a more efficient algorithm like OMWU is needed to improve convergence.

Method: Applies Optimistic Multiplicative Weights Update (OMWU) to the NLHF framework, analyzing its convergence in non-transitive preference scenarios and testing it in tabular and neural policy settings.

Result: OMWU achieves last-iterate linear convergence after a burn-in phase without requiring NE uniqueness, with exponential growth in rarely played actions and better dependence on constants than prior methods, validated experimentally.

Conclusion: OMWU is an effective method for NLHF with strong convergence properties, showing promise for application in aligning large language models with human preferences.
...
### [111] [Discovering Coordinated Joint Options via Inter-Agent Relative Dynamics](https://arxiv.org/abs/2512.24827)
*Raul D. Steleac,Mohan Sridharan,David Abel*

Main category: cs.LG

TL;DR: Novel multi-agent option discovery using joint-state abstraction and Fermat state alignment outperforms existing methods in coordination.


<details>
  <summary>Details</summary>
Motivation: Existing multi-agent option discovery methods fail to achieve strong coordination due to exponential joint state space growth, leading to loosely coupled or independent behaviors.

Method: Joint-state abstraction using Fermat state and spreadness measure to capture team-level alignment, combined with neural graph Laplacian estimator to derive synchronized options.

Result: Options discovered using this method yield stronger downstream coordination capabilities across multiple scenarios in two multi-agent domains compared to alternative methods.

Conclusion: The proposed Fermat state and spreadness-based multi-agent option discovery method effectively generates strongly coordinated behaviors, outperforming existing methods in multi-agent scenarios.
...
### [112] [AODDiff: Probabilistic Reconstruction of Aerosol Optical Depth via Diffusion-based Bayesian Inference](https://arxiv.org/abs/2512.24847)
*Linhao Fan,Hongqiang Fang,Jingyang Dai,Yong Jiang,Qixing Zhang*

Main category: cs.LG

TL;DR: AODDiff is a diffusion-based probabilistic framework for reconstructing Aerosol Optical Depth fields, addressing data scarcity and uncertainty quantification without task-specific retraining.


<details>
  <summary>Details</summary>
Motivation: Current AOD reconstruction models are limited by incomplete training data and lack of uncertainty quantification, hindering atmosphere monitoring accuracy.

Method: Develop AODDiff via diffusion-based Bayesian inference: learn a spatiotemporal AOD prior from incomplete data using corruption-aware training, then apply decoupled annealing posterior sampling to integrate heterogeneous constraints.

Result: Experiments on Reanalysis data show AODDiff is effective in downscaling and inpainting tasks, maintaining high spatial spectral fidelity and enabling uncertainty quantification through multiple sampling.

Conclusion: AODDiff provides a robust, flexible solution for AOD reconstruction with inherent uncertainty quantification, beneficial for downstream atmosphere monitoring applications.
...
### [113] [Characterization of Transfer Using Multi-task Learning Curves](https://arxiv.org/abs/2512.24866)
*András Millinghoffer,Bence Bolgár,Péter Antal*

Main category: cs.LG

TL;DR: Learning curves provide better characterization of transfer effects than gradient-based methods, with multi-task extensions revealing pairwise and contextual transfer in foundation models.


<details>
  <summary>Details</summary>
Motivation: The authors hypothesize that perturbing the dataset by including more samples, rather than perturbing the model through gradient updates, provides a more fundamental characterization of transfer effects in machine learning.

Method: The authors propose using multi-task learning curves to model transfer effects, developing an efficient approximation method analogous to Task Affinity Grouping. They compare statistical and computational approaches to transfer using a benchmark drug-target interaction dataset.

Result: The results show that learning curves can better capture the effects of multi-task learning and their extensions can delineate pairwise and contextual transfer effects in foundation models.

Conclusion: The paper concludes that learning curves provide a more comprehensive characterization of transfer effects compared to gradient-based model perturbations. Multi-task learning curves can delineate pairwise and contextual transfer effects in foundation models, offering better power and broader applicability despite higher compute costs.
...
### [114] [PRISM: A hierarchical multiscale approach for time series forecasting](https://arxiv.org/abs/2512.24898)
*Zihao Chen,Alexandre Andre,Wenrui Ma,Ian Knight,Sergey Shuvaev,Eva Dyer*

Main category: cs.LG

TL;DR: PRISM introduces a hierarchical tree-based method for time series forecasting that captures multi-scale features through learnable partitioning and time-frequency basis projections, outperforming existing approaches.


<details>
  <summary>Details</summary>
Motivation: Real-world time series contain global trends, local fine-grained structure, and multi-scale features, making accurate forecasting challenging with existing methods that may not effectively capture this complex structure.

Method: PRISM uses a learnable tree-based partitioning of the signal where data at each level is projected onto a time-frequency basis to extract scale-specific features that are aggregated across the hierarchy.

Result: Experiments across benchmark datasets showed that PRISM outperforms state-of-the-art forecasting methods.

Conclusion: PRISM's hierarchical tree-based partitioning approach provides an effective and flexible framework for capturing multi-scale features in time series forecasting, with demonstrated superior performance over state-of-the-art methods.
...
### [115] [Spectral Graph Neural Networks for Cognitive Task Classification in fMRI Connectomes](https://arxiv.org/abs/2512.24901)
*Debasis Maji,Arghya Banerjee,Debaditya Barman*

Main category: cs.LG

TL;DR: Proposed SpectralBrainGNN uses spectral graph convolution to classify cognitive tasks from fMRI data, achieving 96.25% accuracy on HCP dataset by capturing brain network topology.


<details>
  <summary>Details</summary>
Motivation: To advance cognitive task classification by capturing complex connectivity patterns and topological dependencies in brain networks that are missed by conventional approaches, using graph neural networks for more interpretable representations of cognitive processes.

Method: Spectral convolution framework using graph Fourier transforms computed via normalized Laplacian eigendecomposition, modeling brain regions as nodes and functional connections as edges in a graph neural network approach.

Result: Achieved 96.25% classification accuracy on the Human Connectome Project-Task dataset, demonstrating superior performance in decoding brain states from functional MRI connectomes.

Conclusion: The SpectralBrainGNN model demonstrates superior performance in cognitive task classification, achieving 96.25% accuracy on the HCPTask dataset, and provides a foundational framework for future brain network analysis research.
...
### [116] [Frequent subgraph-based persistent homology for graph classification](https://arxiv.org/abs/2512.24917)
*Xinyang Chen,Amaël Broustet,Guoting Chen*

Main category: cs.LG

TL;DR: A novel graph filtration based on frequent subgraphs (FSF) produces stable, information-rich persistent homology features (FPH). Two classification models (FPH-ML and FPH-GNNs) outperform existing methods, with FPH-GNNs improving graph neural network performance by up to 21%.


<details>
  <summary>Details</summary>
Motivation: Existing persistent homology methods on graphs rely on limited filtrations (e.g., degree-based or weight-based), which overlook richer features like recurring information across datasets, thus restricting expressive power. There is a need for more informative and stable graph filtrations that capture frequent patterns.

Method: The authors propose Frequent Subgraph Filtration (FSF), which constructs filtration from frequent subgraphs to produce stable and information-rich FPH features. They also introduce two classification approaches: FPH-ML (a machine learning model based on FPH features) and FPH-GNNs (a hybrid framework integrating FPH with graph neural networks). Theoretical properties of FSF are studied and validated experimentally.

Result: Experimental results show that FPH-ML achieves competitive or superior accuracy compared to kernel-based and degree-based filtration methods. When integrated with graph neural networks, FPH yields performance gains of 0.4–21% relative improvement, with up to 8.2 percentage points improvement over GCN and GIN backbones across benchmarks.

Conclusion: The proposed Frequent Subgraph Filtration (FSF) and its derived frequency-based persistent homology (FPH) features effectively integrate frequent subgraph mining with topological data analysis, offering a powerful, topology-aware approach for graph representation learning and classification.
...
### [117] [Attribution-Guided Distillation of Matryoshka Sparse Autoencoders](https://arxiv.org/abs/2512.24975)
*Cristina P. Martin-Linares,Jonathan P. Ling*

Main category: cs.LG

TL;DR: DMSAEs distill a compact core of consistent features from SAEs via iterative attribution-based distillation, improving interpretability transfer across runs and sparsity levels.


<details>
  <summary>Details</summary>
Motivation: Sparse autoencoders often produce redundant and inconsistent features across training runs and sparsity levels, making it difficult to transfer and reuse interpretations.

Method: The method involves training a Matryoshka SAE with a shared core, using gradient X activation attribution to measure feature contributions to next-token loss, and iteratively distilling the smallest subset of features that explains a fixed fraction of attribution. Only core encoder weight vectors are transferred; other components are reinitialized each cycle.

Result: On Gemma-2-2B layer 12, seven distillation cycles yielded a distilled core of 197 consistently selected features. Using this core improved several SAEBench metrics, showing that consistent latent features can be transferred across sparsity levels.

Conclusion: DMSAEDM introduces a distillation pipeline that yields a compact core of consistently useful features that can be reused across training runs and sparsity levels, improving SAE performance and enabling more transferable interpretations.
...
### [118] [Efficiently Estimating Data Efficiency for Language Model Fine-tuning](https://arxiv.org/abs/2512.24991)
*Gyung Hyun Je,Colin Raffel*

Main category: cs.LG

TL;DR: Prediction method for data efficiency in fine-tuning LLMs using gradient cosine similarity to reduce annotation costs.


<details>
  <summary>Details</summary>
Motivation: LLMs need fine-tuning for performance improvements, but task data efficiency is often unknown, leading to costly annotation cycles; need methods to predict data efficiency without incremental annotation.

Method: Propose using gradient cosine similarity of low-confidence examples from a small number of labeled samples to predict data efficiency, validated on 30 specialized tasks.

Result: Method attains 8.6% error in data efficiency prediction, typically eliminates hundreds of unnecessary annotations per task, with experiment results and code available on GitHub.

Conclusion: Gradient cosine similarity effectively predicts data efficiency for LLM fine-tuning, reducing annotation burden and optimizing resources.
...
### [119] [Diffusion Language Models are Provably Optimal Parallel Samplers](https://arxiv.org/abs/2512.25014)
*Haozhe Jiang,Nika Haghtalab,Lijie Chen*

Main category: cs.LG

TL;DR: DLMs with chain-of-thought can achieve optimal parallel token generation, and enabling remasking or revision further improves space efficiency and expressivity.


<details>
  <summary>Details</summary>
Motivation: Diffusion language models (DLMs) offer faster inference via parallel token generation compared to autoregressive models; this paper aims to provide rigorous theoretical foundations for their efficiency advantages.

Method: Formalize a model of parallel sampling, show DLMs with polynomial-length chain-of-thought can simulate any parallel sampling algorithm optimally, analyze remasking and revision to optimize space complexity, and establish expressivity gaps.

Result: DLMs with chain-of-thought achieve the same number of optimal sequential steps as target distributions; remasking or revision enables simulation with optimal space complexity and provides strict expressivity improvements over models without these features.

Conclusion: The results theoretically justify DLMs as the most efficient parallel samplers and advocate for enabling revision in DLMs to enhance performance.
...
### [120] [ResponseRank: Data-Efficient Reward Modeling through Preference Strength Learning](https://arxiv.org/abs/2512.25023)
*Timo Kaufmann,Yannick Metz,Daniel Keim,Eyke Hüllermeier*

Main category: cs.LG

TL;DR: A novel method, ResponseRank, is proposed to robustly learn preference strength from noisy proxies like response times and annotator agreement, improving sample efficiency and robustness across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: Binary choices, such as those in RLHF, only convey preference direction, not strength, which is crucial for decision-making under uncertainty and generalization, but hard to measure reliably due to noisy proxies.

Method: ResponseRank uses relative differences in noisy strength signals to rank responses by inferred preference strength, comparing signals locally within carefully constructed strata to control for systemic variation.

Result: Empirical evidence shows improved sample efficiency and robustness in synthetic preference learning, language modeling, and RL control tasks, with a novel Pearson Distance Correlation metric introduced.

Conclusion: ResponseRank enables robust learning of utility differences consistent with strength-derived rankings, making minimal assumptions about strength signals, with applications in diverse AI tasks.
...
### [121] [On the geometry and topology of representations: the manifolds of modular addition](https://arxiv.org/abs/2512.25060)
*Gabriela Moisescu-Pareja,Gavin McCracken,Harley Wiltzer,Vincent Létourneau,Colin Daniels,Doina Precup,Jonathan Love*

Main category: cs.LG

TL;DR: Both uniform attention and trainable attention architectures implement the same modular addition algorithm via equivalent topological representations.


<details>
  <summary>Details</summary>
Motivation: To address the Clock and Pizza interpretations that claim different architectural designs yield distinct circuits for modular addition, aiming to show they actually use the same underlying algorithm.

Method: Identify all neurons corresponding to each learned representation and study them as collective entities using topological tools, analyzing hundreds of circuits statistically.

Result: Both architectures yield topologically and geometrically equivalent manifolds, demonstrating similarity in learned modular addition circuits across common deep learning paradigms.

Conclusion: The same algorithm is implemented regardless of uniform or learnable attention, with learned representations forming equivalent manifolds, refuting the idea of distinct circuits based on architectural differences.
...
### [122] [Many Minds from One Model: Bayesian Transformers for Population Intelligence](https://arxiv.org/abs/2512.25063)
*Diji Yang,Yi Zhang*

Main category: cs.LG

TL;DR: B-Trans transforms LLMs into Bayesian Transformers by adding stochastic noise in normalization layers, enabling sampling of diverse model instances without retraining, which improves generation diversity and task performance through population aggregation.


<details>
  <summary>Details</summary>
Motivation: Modern transformers are trained as single deterministic models, limiting their ability to represent multiple hypotheses. The idea that intelligence emerges from many minds motivates the creation of a method to sample diverse yet coherent model instances from a pre-trained model.

Method: Introduce a Bayesian posterior proxy by treating bias-like offsets in normalization layers as stochastic variables with a Gaussian variational approximation. This induces a distribution over model behavior without training full Bayesian neural networks. To maintain coherence, sample noise is frozen at the sequence level for temporal consistency.

Result: Experiments in zero-shot generation, RLVR, and RL without explicit labels demonstrate that B-Trans enhances semantic diversity and achieves better task performance compared to deterministic baselines through population-level decision-making.

Conclusion: B-Trans effectively leverages the wisdom of crowds by enabling a single set of pre-trained weights to produce diverse model instances, improving both exploration and performance in various tasks, supporting the idea that intelligence benefits from multiple perspectives.
...
### [123] [Scaling Open-Ended Reasoning to Predict the Future](https://arxiv.org/abs/2512.25070)
*Nikhil Chandak,Shashwat Goel,Ameya Prabhu,Moritz Hardt,Jonas Geiping*

Main category: cs.LG

TL;DR: This work trains language models to make open-ended predictions on forecasting questions, scaling data with news-derived synthetic questions and careful anti-leakage measures.


<details>
  <summary>Details</summary>
Motivation: To enable language models to reason under uncertainty for high-stakes decision-making through future forecasting.

Method: Develops OpenForesight dataset via automated curation from offline news corpus, trains Qwen3 models with improved retrieval and RL reward, and deploys OpenForecaster 8B for evaluation.

Result: OpenForecaster 8B matches larger proprietary models in May-August 2025 testing, showing gains in accuracy, calibration, and consistency that generalize to other benchmarks.

Conclusion: Specialized model training enhances forecasting performance and calibration, with all resources open-sourced to democratize language model forecasting research.

...
Motivation: The rapid growth of LEO satellites has improved EO missions, but autonomous coordination in dynamic environments remains a challenge, requiring advanced RL solutions.
...
Conclusion: The findings support autonomous satellite operations, offering practical guidelines for policy learning in decentralized EO missions.
...
<details>
  <summary>Details</summary>
Motivation: Existing agentic systems lack full autonomy, limiting adaptability and scalability. SwarmAgentic aims to bridge this gap.
...
Result: SwarmAgentic achieves significant improvements on six real-world tasks, demonstrating effectiveness in unconstrained tasks.

Conclusion: The framework advances scalable and autonomous agentic system design by combining swarm intelligence with automated multi-agent generation.
...
<details>
  <summary>Details</summary>
Motivation: Classic logic lacks nuance for human decision-making and grounding in multi-modal environments, while neural networks lack interpretable reasoning. CALM bridges this gap.
...
<details>
  <summary>Details</summary>
Motivation: Existing CL benchmarks are CPU-bound, limiting task sequences and research in continual learning for cooperative multi-agent settings.

Method: MEAL leverages JAX for GPU acceleration, allowing sequences of 100 tasks on a desktop PC. Performance of naive CL and MARL method combinations is tested.

Result: Naive CL and MARL methods perform well in simple environments but fail in complex settings requiring coordination and adaptation. An ablation study identifies key CMARL features.

Conclusion: MEAL provides a scalable benchmark for CMARL, highlighting the need for specialized methods in complex multi-agent continual learning scenarios.
...
Result: Outperforms existing LLM-based hyper-heuristics and matches/exceeds specialized solvers on benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: The rapid growth of maritime IoT demands and the challenges posed by uncertain tasks in computation offloading and resource allocation motivate the need for an efficient solution.

Method: A cooperative MEC framework is introduced, combining UAVs and vessels. Lyapunov optimization handles task unpredictability, and a Markov game reformulation is solved using a heterogeneous-agent soft actor-critic approach.

Result: Simulations confirm the framework's effectiveness in optimizing execution time and managing uncertain tasks.

Conclusion: The proposed method successfully addresses maritime computation offloading and resource allocation challenges under uncertainty.
...
TL;DR: The paper proposes Causality-Aware Navigation (CAN), a method that improves visual navigation by modeling causal relationships in sequential data, outperforming existing baselines.
...
Method: Introduces CAN with a Causal Understanding Module to model causal relationships in sequential navigation data.
...
Method: Design and implementation of an LPA combining LLMs with planning and external tools for autonomous query processing and data retrieval.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the ad hoc and inconsistent approaches to encoding game history in LLM agents, which hinder comparability and understanding of state representation effects.

Method: Proposes a framework categorizing state representation along three axes: action informativeness, reward informativeness, and prompting style. Applied to a dynamic selfish routing game.

Result: Summarized, regret-focused, and limited-action-information representations align better with game theory equilibrium and stabilize agent behavior. Other representations lead to deviations or instability.

Conclusion: State representation significantly influences LLM agent behavior, with specific representations enhancing equilibrium alignment and stability in dynamic games.
...
Result: Students reported increased concern about AI ethics and greater confidence in discussing AI regulation post-module.

Conclusion: The AI Policy Module effectively prepares students to engage with AI policy and ethics, with the AI Regulation Assignment being a key tool.
...
<details>
  <summary>Details</summary>
Motivation: LRMs often generate verbose content, increasing inefficiency and costs. The study aims to uncover and mitigate this issue by leveraging the models' inherent capacity for concise reasoning.

Method: Two approaches are introduced: 1) Efficiency Steering, a training-free activation steering technique, and 2) Self-Rewarded Efficiency RL, a reinforcement learning framework that rewards brevity and accuracy.

Result: Experiments on seven LRM backbones show reduced reasoning length while maintaining or improving task performance.

Conclusion: The findings demonstrate that LRMs' reasoning efficiency can be enhanced by guiding their intrinsic capabilities, offering a self-guided solution to overthinking.
...
<details>
  <summary>Details</summary>
Motivation: Current AI agents are limited by the separation between digital reasoning and physical interaction, hindering tasks requiring both.
...
Conclusion: The work establishes a foundation for integrating embodied cognition and web knowledge, with public resources for further research.
...
<details>
  <summary>Details</summary>
Motivation: Clinical decision-making is complex and prone to biases; LLMs offer support but lack interactive, real-world adaptability.
...
<details>
  <summary>Details</summary>
Motivation: Sampling Boltzmann distributions is crucial in machine learning and optimization, but practical Ising machine implementations often struggle with soft constraints.

Method: Extends parallel tempering (PT) by adding a second dimension of replicas that interpolate penalty strengths, ensuring constraint satisfaction.

Result: 2D-PT achieves near-ideal mixing (O(1/t) in graph sparsification and offers significant speedups over conventional PT.
...
<details>
  <summary>Details</summary>
Motivation: To address the need for interpretable and stable AI in clinical trials, the paper introduces NetraAI, contrasting it with large-scale models like DeepSeek-V3.

Method: NetraAI uses contraction mappings, information geometry, and evolutionary algorithms to identify stable patient subgroups. An LLM Strategist guides the process by prioritizing variables and injecting domain knowledge.
...
Conclusion: NetraAI represents a new generation of adaptive, explainable AI for clinical discovery, aligning with emerging reasoning paradigms like JEPA.
...
Conclusion: ETS shows potential for high-performance open-vocabulary EEG-to-text systems across diverse subjects and sources.
...
<details>
  <summary>Details</summary>
Motivation: Traditional direct measurements of onflow parameters face challenges like sensor faults, necessitating a data-driven approach for real-time learning in applications like wind tunnel monitoring.
...
Result: The approach successfully adapts to changing data distributions, domain extensions, and task updates, but performs less effectively with noisy data.

Conclusion: Transfer learning shows promise for onflow parameter prediction but requires refinement for noisy data scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Existing multimodal approaches neglect visual data in time series, missing critical physical information like satellite imagery's geospatial context.
...
Conclusion: PIPE effectively bridges the gap in multimodal forecasting by integrating physical data, demonstrating significant accuracy improvements.
...
Result: The framework outperforms heuristic methods in minimizing retrieval tardiness and generalizes well to diverse warehouse layouts.
...
<details>
  <summary>Details</summary>
Motivation: The scarcity of comprehensive T1D datasets limits advancements in personalized therapy and AI applications. AZT1D addresses this gap.

Method: Collected data from 25 T1D patients on AID systems over 6-8 weeks, including CGM, insulin pump data, and granular bolus details.

Result: AZT1D provides rich, naturalistic data rarely found in existing datasets, enabling diverse AI/ML applications for T1D management.

Conclusion: AZT1D supports improved clinical decision-making and individualized care in T1D by offering a detailed, publicly available dataset.
...
<details>
  <summary>Details</summary>
Motivation: Recurring concept drift causes forgetting of past patterns, reducing prediction accuracy. Existing methods update parameters but may lose knowledge. The goal is to retain and utilize past concepts effectively.
...
Conclusion: CEP is a robust solution for handling recurring concept drift, enhancing online forecasting by leveraging past knowledge.
...
Conclusion: Simple dropout injection at inference improves PLM performance without additional training.
...
Method: AoE interpolates individual weight tensors from parent models to create child variants, enabling gradual or sharp behavioral changes.
...
Conclusion: AoE provides a straightforward, efficient way to generate functional child models without fine-tuning, demonstrating its potential for model reuse and optimization.
...
<details>
  <summary>Details</summary>
Motivation: To understand the fundamental constraints on representational capacity in models, focusing on the balance between generalization and input identity preservation.
...
Result: Closed-form expressions reveal a universal Pareto front for generalization and identification, predicting a sharp collapse in multi-input capacity and non-monotonic optima. Empirical results align with theoretical predictions.

Conclusion: Finite-resolution similarity is a fundamental constraint, shaping representational capacity in both deep networks and biological systems, providing an exact theory for the generalization-identification tradeoff.
...
<details>
  <summary>Details</summary>
Motivation: To improve time series forecasting by leveraging semantic priors and adaptive temporal encoding, while reducing computational complexity compared to Transformer models.

Method: Uses Mamba selective state-space model for efficiency, semantic embeddings from pretrained language models, and spline-based KAN for dynamic temporal encoding.

Result: Superior accuracy, robustness, and interpretability in forecasting, with linear computational complexity.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between discrete optimization methods and their continuous-time approximations by focusing on discretization error, providing better theoretical tools.
...
Result: A continuous-time model for the HB method with arbitrary-order step-size control, applied to study implicit regularization in deep learning.

Conclusion: The theoretical framework is validated numerically, offering insights into the implicit bias of HB in deep learning.
...
Result: VLMs show language prompt sensitivity in the vision domain; InternVL2 models are notably robust.

Conclusion: Prompt sensitivity in VLMs is linked to training data, and PARC provides a tool for evaluating and improving VLM robustness.
...
<details>
  <summary>Details</summary>
Motivation: Traditional forecasting models struggle with zero-demand periods, impacting inventory management in supply chains.

Method: Proposes a Model-Router framework that dynamically selects forecasting models (classical, ML, DL) based on demand patterns, comparing rule-based, LightGBM, and InceptionTime routers.

Result: The InceptionTime router improves accuracy by 11.8% (NWRMSLE) and speeds up inference by 4.67x on the Favorita dataset.

Conclusion: The framework enhances forecasting precision, reducing stockouts and excess inventory, highlighting adaptive AI's role in optimizing supply chains.
...
Result: Outperforms alternatives in benchmark continuous control and visual problems, achieving greater knowledge transfer and performance.

Conclusion: The proposed modular architecture effectively scales, avoids forgetting, and enhances learning in continual reinforcement learning.
...
Method: TRAINCHECK automatically infers invariants for DL training and uses them to proactively detect errors.

Result: It detected 18 out of 20 real-world errors in one iteration and found 6 unknown bugs in training libraries.

Conclusion: TRAINCHECK effectively addresses silent training errors, improving DL training reliability.
...
<details>
  <summary>Details</summary>
Motivation: DXA scans are expensive and time-consuming for measuring body composition. The study aims to provide a cheaper, efficient alternative using 3D optical images and machine learning.

Method: Used a $p$-Laplacian regression model on biomarkers from 3D images (847 patients). Compared with supervised models (SVR, Least Squares SVR) under varying training data sizes.

Result: $p$-Laplacian model achieved errors of ~13% (ALM), ~10% (BMD), ~20% (BFP) with 10% training data. Supervised models (SVR, Least Squares SVR) performed better (~8% for ALM/BMD, ~11% for BFP) with 80% data.

Conclusion: The $p$-Laplacian model is promising for healthcare in data-limited settings, while supervised models excel with more data.
...
<details>
  <summary>Details</summary>
Motivation: Address the struggle of VLMs with detailed visual reasoning under limited compute resources.
...
Conclusion: Combining GRPO, tool use, and targeted training data enhances VQA performance in resource-constrained settings.
...
<details>
  <summary>Details</summary>
Motivation: Deploying MLLMs in real-world scenarios is challenging due to distributed data and privacy constraints. Federated Learning (FL) can help, but existing FL methods are impractical for large-scale MLLMs due to high computational and communication demands.
...
Result: FedNano reduces client-side storage by 95% and communication overhead to 0.01% of model parameters. It outperforms prior FL baselines, enabling scalable and privacy-preserving MLLM deployment.

Conclusion: FedNano bridges the gap between MLLM scale and FL feasibility, offering a practical solution for decentralized multimodal AI systems with privacy and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To accelerate HEA discovery by integrating computational and experimental data, addressing challenges like heteroscedastic and incomplete datasets.
...
Result: DGP with ML-based priors outperforms others in capturing inter-property correlations and input-dependent uncertainty.
...
<details>
  <summary>Details</summary>
Motivation: To address the critical need for reliable SSD health prediction to ensure data reliability and reduce failure risks in storage systems.

Method: Uses a hybrid BiGRU-MHA model integrating bidirectional GRU for temporal feature extraction and multi-head attention for dynamic feature weighting.
...
Conclusion: The model offers a practical solution for SSD health prediction, reducing data loss risks and optimizing maintenance costs in industrial and cloud storage systems.
...
Result: The study demonstrates the potential of these algorithms to enhance the performance of anomaly detection systems.

Conclusion: Automating the threshold definition in Autoencoders using machine learning can address data imbalance issues and improve detection accuracy.
...
<details>
  <summary>Details</summary>
Motivation: Healthcare data is often incomplete or limited, hindering ML model performance. AMD, affecting older adults, requires early diagnosis due to irreversible progression, necessitating better classification methods.

Method: CACTUS integrates multiple factors (genetic, dietary, clinical, demographic) for AMD classification, offering explainability and flexibility. It identifies key features and eliminates biased data.
...
Conclusion: CACTUS provides a reliable, explainable solution for AMD classification, addressing data limitations and biases, and improving clinical decision-making.
...
<details>
  <summary>Details</summary>
Motivation: To understand the structure and behavior of quasi-flat regions in neural network loss landscapes and their implications for optimization and model capabilities.

Method: Characterizes channels in the loss landscape using gradient dynamics, geometry, and functional interpretation, focusing on cases where weights diverge and neurons form gated linear units.

Result: Gradient flow solvers often reach these channels, which appear as flat local minima but lead to gated linear units, revealing unexpected computational capabilities of fully connected layers.

Conclusion: The study provides insights into the loss landscape's structure, optimization dynamics, and the emergence of gated linear units, enhancing understanding of neural network training.
...
Method: Introduces GRKneg, enhancing OC-SVM kernel using negative samples, without label optimization. Compares with standard OC-SVM and binary SVM.

Result: GRKneg outperforms standard OC-SVM and binary SVM with few negative samples.

Conclusion: GRKneg is effective for one-class classification with limited negative samples, bridging gaps between OC-SVM and binary SVM.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of reliable RL reward signals across diverse reasoning domains and explore RL's broader applicability beyond math and code.

Method: Develop Guru, a corpus of 92K verifiable examples across six reasoning domains, using domain-specific reward design, deduplication, and filtering. Train models (Guru-7B, Guru-32B) and evaluate performance.

Result: Models achieve state-of-the-art performance, outperforming baselines by 7.9% and 6.7%. RL facilitates skill acquisition in domains with limited pretraining exposure.

Conclusion: Guru enables effective RL training for general reasoning, with models showing significant performance gains, especially in underrepresented domains.
...
<details>
  <summary>Details</summary>
Motivation: Online learning in VFL faces challenges due to asynchronous data generation and non-intersecting feature sets, overlooked by prior research.

Method: Developed an event-driven framework activating subsets of clients per event and introduced dynamic local regret for non-convex, non-stationary environments.
...
Conclusion: The proposed event-driven online VFL framework effectively addresses real-world asynchronous data challenges, enhancing stability and reducing costs.
...
<details>
  <summary>Details</summary>
Motivation: To address decision-making challenges under limited information about arm rewards while ensuring fairness across agents.
...
<details>
  <summary>Details</summary>
Motivation: Previous federated black-box prompt tuning ignored the high query costs of cloud-based LLMs, prompting a need for efficient solutions.

Method: Proposed FedOne framework, which degrades FedAvg to activate one client per round for optimal query efficiency.

Result: Numerical experiments showed significant improvement in query efficiency, aligning with theoretical findings.

Conclusion: FedOne effectively maximizes query efficiency in federated black-box discrete prompt learning for cloud-based LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the overlooked challenge of training instability in sequential representations for sepsis treatment RL, which impacts policy performance.

Method: Using Controlled Differential Equations (CDE) with early stopping/stabilization and acuity-aware regularization (SOFA, SAPS-II, OASIS) for state representation.

Result: Stable CDE representations correlate with clinical scores and enable superior RL policies (WIS return > 0.9), while unstable ones fail (WIS return ~ 0).
...
<details>
  <summary>Details</summary>
Motivation: Existing SFT methods treat all tokens uniformly, ignoring task-specific importance, which limits performance.
...
Conclusion: SFT-GO effectively improves LLM fine-tuning by adaptive token grouping, enhancing learning dynamics and performance.
...
<details>
  <summary>Details</summary>
Motivation: Conventional reinforcement learning for hyperparameter optimization is time and resource-intensive, limiting widespread use.
...
Conclusion: SPG offers a scalable and efficient alternative for hyperparameter optimization, demonstrating consistent improvements across domains.
...
Result: ODD-based bounds outperform DIS² in predicting target performance, validated through experiments on diverse benchmarks.

Conclusion: ODD provides a more reliable and accurate error estimation method for ML models under distribution shifts, with practical applicability demonstrated.
...
<details>
  <summary>Details</summary>
Motivation: To reduce the storage, memory, and computational costs of fine-tuning large Transformer models while maintaining performance.

Method: Combines Kronecker-product tensor factorization with SVD-driven initialization and dynamic rank selection (SoKA).

Result: Achieves comparable or better performance with 25% fewer parameters than LoRA/PiSSA, faster convergence, and stable gradients.

Conclusion: SoKA is a robust and efficient PEFT strategy for large-scale model adaptation.
...
Method: Employed tabular and time-series foundation models, a custom multimodal attention-based transformer, and machine learning methods to analyze baseline clinical and 12-week digital Floodlight data.

Result: Integration of digital data improved prediction performance, with the multimodal transformer outperforming unimodal models, achieving the best results with Moment FM embeddings.
...
<details>
  <summary>Details</summary>
Motivation: Existing DI methods require in-distribution held-out data, which is rarely available, limiting their practicality. The paper aims to enable DI without this requirement.

Method: The approach involves generating synthetic data via a suffix-based completion task and post-hoc calibration to bridge likelihood gaps between real and synthetic data.

Result: Experiments show the method detects original training sets with high confidence and low false positives, validating its reliability for real-world use.

Conclusion: The synthetic data generation method effectively enables DI, empowering copyright owners to verify data usage and supporting legal claims.
...
<details>
  <summary>Details</summary>
Motivation: To understand why ICL remains effective despite noisy or incomplete labels, and to theoretically analyze the role of model depth in exploiting unlabeled data.

Method: Theoretical analysis of binary Gaussian mixture models (GMM) with missing labels, comparing one-layer linear attention models to multilayer/looped transformers.

Result: Multilayer transformers implicitly construct polynomial estimators, with exponential gains in performance relative to depth, outperforming single-pass inference.
...
Method: A systematic literature review following PRISMA guidelines, analyzing 362 publications and categorizing 41 metric groups into a three-dimensional framework.

Result: The VXAI framework provides a structured, comprehensive overview of XAI evaluation metrics, enhancing comparability and future extensions.

Conclusion: The proposed framework standardizes XAI evaluation, supporting systematic metric selection and fostering trust in AI systems.
...
<details>
  <summary>Details</summary>
Motivation: To address the degradation of zero-shot RL performance under partial observability.

Method: Use memory-based architectures to handle partially observable states, rewards, and dynamics.

Result: Improved performance in partially observable domains compared to memory-free methods.
...
Conclusion: The method addresses limitations of traditional visualizations, providing a clearer and more interpretable comparison of time series.
...
TL;DR: A progressive framework (HiPreNets) is introduced to train high-precision neural networks by refining prediction residuals, improving accuracy while addressing non-convex optimization and hyperparameter challenges.


<details>
  <summary>Details</summary>
Motivation: Training highly accurate deep neural networks is difficult due to non-convex optimization, hyperparameter tuning, and the neglect of $L^{\infty}$ error in traditional MSE-focused approaches.

Method: HiPreNets uses a staged training technique, sequentially learning prediction residuals with additional networks, guided by residual structure for loss function choice, parameter count, and adaptive data sampling.

Result: The framework is validated on benchmark problems, demonstrating improved overall accuracy.

Conclusion: HiPreNets offers an effective solution for training high-precision neural networks by addressing key challenges in optimization and error metrics.
...
TL;DR: The paper introduces a certification framework for ensuring pixel-level robustness in post-hoc attribution methods for deep learning predictions, addressing their vulnerability to input perturbations.


<details>
  <summary>Details</summary>
Motivation: Post-hoc attribution methods are non-robust, with small input perturbations drastically altering attribution maps while predictions remain unchanged, undermining trustworthiness.
...
Result: The method provides robust, interpretable, and faithful certified attributions, validated across 12 attribution methods and 5 ImageNet models.

Conclusion: The certification framework enables reliable use of attribution methods in downstream tasks by guaranteeing robustness and faithfulness.
...
TL;DR: Study examines hallucinations in LLM-based embodied agents under scene-task inconsistencies, revealing high hallucination rates and model limitations.


<details>
  <summary>Details</summary>
Motivation: To understand the extent and triggers of hallucinations in LLMs when grounded in physical environments, and to improve their reliability.
...
Conclusion: Highlights fundamental limitations in handling infeasible tasks and provides actionable insights for developing more robust planning strategies.
...
TL;DR: The paper formalizes spatiotemporal over-squashing in STGNNs, showing it differs from static GNNs. Convolutional STGNNs favor distant temporal info, and both processing paradigms are affected.
...
Method: Formalizes the problem, analyzes its distinct characteristics, and tests on synthetic and real-world datasets.

Result: Convolutional STGNNs favor distant temporal info; both processing paradigms are equally affected.
...
TL;DR: The paper introduces a Neural Canonical Polyadic Factorization (NCPF) model for robust traffic data imputation, combining tensor algebra and deep learning to handle missing data in urban traffic systems.


<details>
  <summary>Details</summary>
Motivation: Missing traffic data due to sensor failures and heterogeneous sensing gaps disrupts reliable traffic modeling, necessitating a robust solution.

Method: NCPF integrates CP decomposition with neural networks, using learnable embeddings and hierarchical feature fusion to model multilinear interactions and spatiotemporal couplings.
...
Conclusion: RePCS provides a black-box safeguard for verifying meaningful retrieval use in RAG systems, crucial for safety-critical applications.
...
TL;DR: The paper surveys machine unlearning verification methods, proposing a taxonomy (behavioral and parametric verification) and analyzing their strengths, limitations, and vulnerabilities. It highlights open problems for future research.


<details>
  <summary>Details</summary>
Motivation: Growing privacy and regulatory demands (e.g., GDPR) necessitate reliable verification of machine unlearning, but existing methods lack a unified framework.

Method: Proposes a taxonomy for verification methods, categorizing them into behavioral and parametric verification, and evaluates their assumptions and effectiveness.

Result: Identifies gaps and vulnerabilities in current verification techniques, providing a foundation for future improvements.

Conclusion: Calls for more robust and theoretically grounded verification mechanisms, outlining open problems for further research.
...
<details>
  <summary>Details</summary>
Motivation: Current feature description methods are limited by assumptions of monosemanticity and lack robustness, failing to capture the complexity of neural network features.

Method: Introduces PRISM, a framework for nuanced feature descriptions, applicable to both polysemantic and monosemantic features in language models.

Result: PRISM outperforms existing methods in description quality and polysemanticity capture, as validated by benchmarking.

Conclusion: PRISM enhances interpretability by addressing the limitations of current methods, providing more accurate and comprehensive feature descriptions.
...
Result: Achieves tighter learning error bounds and better privacy-accuracy trade-offs compared to existing methods.

Conclusion: ImprovDML effectively balances resilience, privacy, and accuracy, validated by theoretical and numerical results.
...
TL;DR: The paper extends Gold's inductive inference framework to include computational observations and restricted input sources, enabling the study of learnability of general recursive functions under realistic constraints. It introduces new observation types and shows how they overcome traditional learning barriers, while also revealing connections to finite-state transducer inference. Negative results are presented for certain function classes.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional input-output observations in learning general recursive functions by incorporating computational constraints and additional observation types.

Method: Extends Gold's framework with Time-Bound Observations and Policy-Trajectory Observations, and analyzes learnability under computational complexity constraints.

Result: Input-output observations alone are insufficient, but computational constraints or time-bound observations enable learning. Policy-trajectory observations connect to finite-state transducer inference. Negative results exist for linear-time computable functions.

Conclusion: The paper provides a formal framework for learning computable functions under realistic constraints, showing both positive and negative learnability results.
...
TL;DR: SKIL introduces a reinforcement learning-based imitation framework to model animal behavior as dynamic mixtures of interpretable skills, outperforming traditional discrete models.


<details>
  <summary>Details</summary>
Motivation: Existing behavior-segmentation methods oversimplify animal behavior by imposing discrete syllables, failing to reflect the flexible recombination of motor primitives.
...
Conclusion: SKIL provides a principled account of complex animal behavior emergence from dynamic combinations of fundamental motor primitives.
...
TL;DR: DAILOC is a domain-incremental learning framework for Wi-Fi fingerprinting-based indoor localization, addressing device heterogeneity and temporal variations. It outperforms existing methods with significantly lower errors.
...
Method: Proposes DAILOC, using a multi-level variational autoencoder for disentangling domain shifts and a memory-guided class latent alignment mechanism to mitigate catastrophic forgetting.

Result: Achieves up to 2.74x lower average error and 4.6x lower worst-case error across multiple smartphones, buildings, and time instances.

Conclusion: DAILOC effectively addresses domain shifts and catastrophic forgetting, outperforming state-of-the-art methods in indoor localization.
...
TL;DR: The paper rigorously evaluates ML techniques for solving the 1D Poisson equation, highlighting the importance of data function space and proving generalization bounds. It reveals inconsistent generalization across models and introduces a new interpretability lens and cross-validation technique.


<details>
  <summary>Details</summary>
Motivation: To address the gap between promising ML applications in science and the need for quantitative accuracy, the study applies numerical analysis rigor to ML, focusing on the 1D Poisson equation.

Method: The work quantifies ML accuracy by analyzing training dynamics, deriving optimal parameters, and proving generalization bounds for white-box and black-box models. Empirical validation is performed on various models.

Result: Generalization to the true physical equation is not guaranteed, with models exhibiting opposing behaviors. A new interpretability method using Green's function is introduced.
...
TL;DR: LogNet is a logic gate-based framework that enhances interpretability and performance of DL-based indoor localization, reducing errors and latency.


<details>
  <summary>Details</summary>
Motivation: Existing DL models for indoor localization lack interpretability, making it hard to adapt to real-world noise and temporal variations.

Method: LogNet uses logic gates to identify influential access points and diagnose model failures caused by environmental noise.

Result: LogNet achieves lower localization error, smaller model size, and reduced latency compared to prior DL models.

Conclusion: LogNet improves interpretability and long-term reliability of DL-based indoor localization systems.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the vulnerability of aligned LLMs to safety protection loss during fine-tuning, even with benign data.

Method: Proposes Low-Rank Extrapolation (LoX), a training-free method to extrapolate safety-critical subspaces.
...
TL;DR: The paper explores federated learning's robustness to Byzantine behavior, revealing a tradeoff between averaging and validity conditions. It provides lower and upper bounds on centroid approximation and introduces a new algorithm with tight bounds.
...
Method: The study analyzes centroid approximation under validity conditions, derives lower and upper bounds, and introduces a new algorithm for convex validity. Empirical evaluations in federated settings are also conducted.

Result: A lower bound of min{(n-t)/t, √d} and an upper bound of 2min{n, √d} on centroid approximation are established. A new algorithm achieves a √(2d)-approximation under convex validity.

Conclusion: The paper demonstrates that validity conditions alone do not ensure good approximation and highlights the impact of Byzantine outliers. It provides theoretical and empirical insights into federated learning's robustness.
...
<details>
  <summary>Details</summary>
Motivation: Fairness in ML is crucial for trustworthy AI systems, but biased training data affects fairness and performance.
...
Conclusion: GFLC effectively addresses label bias while preserving fairness, enhancing ML system reliability.
...
TL;DR: Federated learning (FL) for BrainAGE estimation in stroke patients shows promise, outperforming single-site models and linking BrainAGE to vascular risks and recovery outcomes.


<details>
  <summary>Details</summary>
Motivation: To address privacy concerns in training robust BrainAGE models while evaluating FL's performance and its association with clinical outcomes in stroke patients.

Method: Used FLAIR images from 1674 patients across 16 centers, comparing centralized learning, FL, and single-site learning. Analyzed BrainAGE's link to vascular risks and functional outcomes via logistic regression.

Result: FL outperformed single-site models; BrainAGE was higher in diabetic patients and significantly associated with post-stroke recovery.
...
<details>
  <summary>Details</summary>
Motivation: The need for scalable infrastructure planning tools to support widespread EV adoption, addressing diverse geographic and socio-economic requirements.

Method: Combines rasterized data, graph neural networks, Voronoi tessellation, and queue modeling to optimize charger placement under constraints.
...
Method: AutoRule uses a reasoning model to interpret preferences, extracts rules, and synthesizes them into a unified set. It employs language-model verifiers for reward computation.
...
TL;DR: The paper proposes a credit risk management framework for 3PL-led supply chain finance in cross-border e-commerce, using generative AI to improve SME financing.
...
Method: Uses Quantile-Regression-based Generative Metamodeling (QRGMM) and Deep Factorization Machines (DeepFM) for risk estimation and loan size determination.
...
Conclusion: Pioneers generative AI in CBEC SCF risk management, enhancing credit practices for SMEs.
...
Method: Systematic investigation of dense latents' geometry, function, and origin, including ablation studies and taxonomy development.
...
TL;DR: SecFwT is a privacy-preserving framework for fine-tuning LLMs using MPC, addressing computational challenges with forward-only tuning and MPC-friendly attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Adapting LLMs to privacy-sensitive domains like healthcare and finance is hindered by data scarcity and computational inefficiencies in MPC-based fine-tuning.

Method: Proposes SecFwT, which uses forward-only tuning and Random Feature Attention to reduce computational overhead and non-linear operations in MPC settings.

Result: SecFwT improves efficiency and privacy, enabling scalable and secure fine-tuning of LLMs for sensitive applications.

Conclusion: SecFwT successfully addresses key challenges in privacy-preserving LLM fine-tuning, offering a practical solution for privacy-critical domains.
...
TL;DR: The paper proposes a method to predict abnormal lab test values using performed tests, treating it as a set translation problem, and achieves up to 8% AUC improvement.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis via predicting abnormal lab values could improve healthcare accessibility, leveraging widely used tests like CBC.

Method: Formulates the problem as a set translation task, using GPT-like embeddings for lab test labels and values, handling missing data without estimation.

Result: Achieves up to 8% AUC improvement in predicting high uric acid, glucose, cholesterol, and low ferritin levels.

Conclusion: The approach effectively bridges LLMs with tabular data, enhancing predictions in clinical lab settings.
...
<details>
  <summary>Details</summary>
Motivation: Traditional KD methods for NNPs increase energy barriers, limiting the generation of high-energy training data needed for stable MD simulations.

Method: A two-stage training process: first, train the student NNP with data from a non-fine-tuned teacher; then fine-tune with a small DFT dataset.
...
Conclusion: The proposed framework improves efficiency and accuracy in NNP training, particularly for high-energy structures, with significant computational savings.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion models in FWI struggle with iterative sampling and lack clear methods to incorporate approximate velocity models, limiting their practical use.

Method: The authors propose cI²SB, extending the Image-to-Image Schrödinger Bridge (I²SB) for conditional sampling, to interpolate between ground truth and smoothed velocity models.
...
Conclusion: cI²SB offers a more efficient and effective solution for FWI, validated by superior reconstruction of velocity models from smoothed approximations and seismic signals.
...
Method: The approach incorporates quantile regression for stronger MIA to enhance one-run auditing in black-box settings, evaluated on DP-SGD-trained CIFAR-10 models.
...
Conclusion: Utilizing quantile regression for MIA in one-run auditing improves accuracy in black-box settings without sacrificing efficiency.
...
TL;DR: DiTMC adapts Diffusion Transformers (DiTs) for molecular conformer generation, addressing challenges like integrating graph data with 3D geometry and handling symmetries. It achieves state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Applying DiTs to molecules introduces challenges like combining discrete graph info with continuous geometry and managing symmetries.
...
Result: DiTMC achieves top precision and validity on benchmarks (GEOM-QM9, -DRUGS, -XL).

Conclusion: DiTMC's design and symmetry priors improve sample quality, offering potential for large-scale molecular modeling.
...
TL;DR: A novel method for learning ground metrics in optimal transport improves performance in tasks like embedding, clustering, and classification by using class labels on a distribution-level for training.


<details>
  <summary>Details</summary>
Motivation: Traditional predefined or supervised ground metrics in optimal transport lack flexibility and generalizability across classes or distributions with shared supports.

Method: Proposes a global ground metric learning approach using class labels on a distribution-level, applicable to arbitrary distributions over a shared metric space.
...
Conclusion: The method addresses limitations of traditional approaches, offering interpretability and effectiveness in diverse applications.
...
<details>
  <summary>Details</summary>
Motivation: To enable exploration in generative models for novel design generation without relying on uncertainty quantification.

Method: Uses entropy maximization over a diffusion model's data manifold, connects entropy to the score function, and develops a mirror descent algorithm for fine-tuning.

Result: Convergence to optimal exploratory diffusion model proven; promising empirical results on synthetic and text-to-image tasks.

Conclusion: The framework effectively enables scalable exploration in generative models, validated theoretically and empirically.
...
<details>
  <summary>Details</summary>
Motivation: Existing works assume known graphs for optimal vaccination, but real-world scenarios often lack this information, necessitating methods for unknown graphs.

Method: The approach splits into learning the graph (using an inclusion-exclusion-based algorithm) and optimizing vaccination (via spectral radius minimization). Theoretical guarantees and greedy heuristics are provided.

Result: The learning algorithm has proven sample complexity, and the vaccination strategy is polynomial-time for bounded treewidth graphs, with a greedy heuristic for general graphs. Experiments validate the methods.

Conclusion: The work bridges the gap between theory and practice for SIS models with unknown graphs, offering scalable solutions for disease control.
...
<details>
  <summary>Details</summary>
Motivation: Current methods assume uniform homophily in normal nodes, which often fails in real-world datasets where homophily varies.
...
Result: RHO significantly outperforms state-of-the-art methods on eight real-world datasets.

Conclusion: RHO effectively addresses diverse homophily in normal nodes, improving anomaly detection performance.
...
TL;DR: A user-guided projection framework for image and text data leverages MLLMs for customizable, interpretable visualizations via natural-language prompts.
...
Method: Uses zero-shot classification with MLLMs, allowing users to steer projections via natural-language prompts.

Result: Enhances cluster separation and makes DR interactive and user-driven.

Conclusion: Bridges automated DR and human-centered exploration, offering adaptable projections for specific needs.
...
<details>
  <summary>Details</summary>
Motivation: Linear predictors with interaction terms often face overfitting in high-dimensional settings, and existing methods like lasso or elastic net may not fully address this.

Method: LIT-LVM represents features as latent vectors in a low-dimensional space, leveraging structured regularization to mitigate overfitting.
...
Conclusion: LIT-LVM is a robust method for estimating interaction terms in linear predictors, offering better accuracy and interpretability in high-dimensional scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Research on adversarial attacks in tabular data is scattered; this paper aims to provide a structured overview to guide future work.

Method: Conducts a systematic literature review to categorize attack strategies and analyze real-world applicability.

Result: Identifies key trends, attack categories, and practical challenges in adversarial attacks on tabular ML.

Conclusion: The review offers a clear framework to address vulnerabilities in tabular ML and suggests open research questions.
...
Result: Recovers the same bounds as prior work, demonstrating the effectiveness of the simplified approach.
...
TL;DR: The paper identifies gradient pathologies and non-stationarity as key challenges in scaling deep reinforcement learning, proposes simple interventions to stabilize gradients, and validates their effectiveness across various scales and environments.


<details>
  <summary>Details</summary>
Motivation: Understanding and addressing the root causes of performance degradation in scaling deep reinforcement learning networks.

Method: Empirical analyses to identify issues (non-stationarity and gradient pathologies), followed by proposing and implementing simple interventions to stabilize gradient flow.

Result: The interventions enable robust performance across different network depths and widths, validated on various agents and environments.

Conclusion: Simple, compatible interventions can effectively address scaling challenges in deep reinforcement learning.
...
<details>
  <summary>Details</summary>
Motivation: To enhance neural networks' efficiency and sustainability by leveraging human-like compositional reasoning.

Method: Proposes ensembling zero-shot expert models and tests on a benchmark for compositionality.

Result: Achieves higher accuracy with fewer computational resources than baselines.
...
TL;DR: MicroRicci is a self-tuning, local Ricci-flow solver that uses coding theory and tiny neural modules for real-time mesh smoothing, achieving 2.4x speedup and high-quality results.


<details>
  <summary>Details</summary>
Motivation: Classical Ricci-flow solvers are costly, and greedy heuristics are slow or brittle. MicroRicci aims to provide efficient, adaptive, and high-quality mesh smoothing.

Method: MicroRicci combines a greedy syndrome-decoding step (O(E) time) with two neural modules for adaptive vertex and step-size selection.

Result: On 110 SJTU-TMQA meshes, MicroRicci reduces iterations (950 to 400), tightens curvature spread (0.19 to 0.185), and achieves high UV-distortion correlation (r = -0.93). Runtime improves by 1.8x.

Conclusion: MicroRicci's linear-time updates, automatic adaptation, and quality results make it ideal for real-time, resource-limited applications in graphics and simulation.
...
Result: DP-GRAPE reduces memory usage by over 63% for Vision Transformers and 70% for RoBERTa-Large compared to DP-Adam, with similar performance. It scales to models like OPT (6.7B parameters).
...
<details>
  <summary>Details</summary>
Motivation: Offline RL algorithms face distribution shift issues, leading to over-conservative policies due to poor explorations in suboptimal data.

Method: The study analyzes poor explorations' impact on policy optimality and proposes CAWR, combining robust loss functions and advantage-based prioritized replay.

Result: CAWR outperforms on the D4RL benchmark, learning superior policies from suboptimal data.

Conclusion: CAWR effectively mitigates over-conservatism by addressing poor explorations, enhancing policy optimization performance.

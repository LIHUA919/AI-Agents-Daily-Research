...
Motivation: Existing GUI agents use sequential multi-step operations as prior knowledge but fail to capture complex page transition relationships, making it difficult for agents to deeply perceive GUI environments and generalize to new scenarios.

Method: An automated pipeline transforms sequential episodes into page graphs that model page structure connected by actions. RAG technology retrieves reliable GUI perception guidelines from these graphs. A multi-agent framework (PG-Agent) with task decomposition strategy is injected with these guidelines.
...
Conclusion: The proposed PG-Agent framework successfully addresses the limitations of existing GUI agents by leveraging page graphs and RAG technology, enabling better generalization to unseen scenarios with improved GUI environment perception.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of computing probability bounds in causal models where exogenous variables are not fully specified, leading to partial identifiability issues that prevent precise probability computation.

Method: Developed a new algorithm that simplifies multilinear programming through column generation techniques, using sequences of auxiliary linear integer programs to compute probability bounds for scenarios with single interventions.

Result: Demonstrated that polynomial cardinality representation for exogenous variables is possible, with experiments showing column generation techniques outperform existing methods.

Conclusion: The proposed column generation approach provides an efficient and superior method for computing tight probability bounds in partially identifiable causal models with quasi-Markovian structure.
...
Method: Introduced Meta-Policy Deliberation Framework (MPDF) with decentralized policy over meta-cognitive actions. Developed SoftRankPO algorithm using rank-based advantage shaping with smooth normal quantiles to stabilize training against reward variance.
...
<details>
  <summary>Details</summary>
Motivation: Existing DRL methods for conflict detection and resolution suffer from unimodal bias, leading to decision deadlocks and lack of flexibility in complex air traffic scenarios with dynamic constraints.

Method: Uses diffusion probabilistic models to create a policy as a reverse denoising process guided by a value function, generating multimodal action distributions. Includes Density-Progressive Safety Curriculum (DPSC) for stable training from sparse to high-density traffic environments.

Result: Significantly outperforms state-of-the-art DRL benchmarks, achieving 94.1% success rate in high-density scenarios and reducing Near Mid-Air Collisions by 59% compared to best baseline.

Conclusion: Diffusion-AC's multimodal decision-making capability provides flexible alternative maneuvers, enhancing safety and performance in air traffic conflict resolution, representing a substantial advancement over conventional unimodal approaches.
...
<details>
  <summary>Details</summary>
Motivation: To bridge psychological theory with LLM behavior design by creating psychologically grounded AI agents that exhibit consistent, interpretable personality traits for improved performance in diverse tasks.

Method: Priming LLM agents with distinct MBTI personality archetypes through prompt engineering, controlling behavior along cognition and affect axes, and using official 16Personalities test for automated trait verification.
...
Conclusion: The framework establishes a foundation for psychologically enhanced AI agents without fine-tuning, generalizes to other psychological frameworks (Big Five, HEXACO, Enneagram), and enables structured multi-agent communication with improved cooperation.
...
Method: Two-stage training pipeline: (1) supervised fine-tuning on synthetic data to prime models for dynamic planning, and (2) reinforcement learning to refine this capability in long-horizon environments.
...
<details>
  <summary>Details</summary>
Motivation: Generative AI like LLMs produce hallucinations and unverifiable claims, limiting reliability in sensitive domains like healthcare. RAG improves accuracy but remains opaque and dependent on data quality.
...
Method: Develops CausalARC testbed with tasks sampled from structural causal models, providing three types of causal feedback (observational, interventional, counterfactual) as few-shot demonstrations.

Result: Demonstrates utility across four evaluation settings: abstract reasoning with test-time training, counterfactual reasoning, program synthesis, and causal discovery with logical reasoning.

Conclusion: CausalARC provides a formal framework for evaluating AI reasoning capabilities in causal understanding and adaptation to novel problem settings with limited data.
...
Conclusion: The prototype establishes a computational foundation for incorporating more complex dynamic representations, though current focus is on spatial primitives.
...
Method: Analysis of RL dynamics in LLMs revealing two-phase learning, then proposing HICRA algorithm that concentrates optimization on high-impact planning tokens and uses semantic entropy for strategic exploration measurement.

Result: HICRA significantly outperforms strong baselines, demonstrating that focusing on strategic planning bottlenecks is key to unlocking advanced reasoning capabilities.

Conclusion: Reasoning improvement in LLMs through RL follows a hierarchical structure similar to human cognition, and targeted optimization of planning tokens (HICRA) is more effective than agnostic optimization approaches.
...
Method: Investigated 8 different time series segmentation algorithms, evaluated using InterpretTime and AUC Difference methodologies on multivariate and univariate time series, and introduced a novel length-weighted attribution normalization technique.

Result: Number of segments has greater impact on explanation quality than specific segmentation method; equal-length segmentation consistently outperforms custom algorithms; length-weighted normalization improves attribution quality.

Conclusion: Simple equal-length segmentation is effective for SHAP-based time series explanations, and segment length normalization further enhances attribution quality, providing practical guidance for XAI in time series classification.
...
<details>
  <summary>Details</summary>
Motivation: Current automated red-teaming approaches don't consider how human identities and backgrounds shape red-teaming strategies, limiting the spectrum of risks they can uncover from AI models.

Method: Developed PersonaTeaming with two approaches: using predefined 'red-teaming expert' and 'regular AI user' personas, and a dynamic algorithm that automatically generates adaptive personas for different seed prompts. Also created metrics to measure mutation distance.
...
Conclusion: Persona mutation effectively enhances automated red-teaming performance and provides opportunities to explore complementarities between automated and human red-teaming approaches.
...
<details>
  <summary>Details</summary>
Motivation: Understanding personality patterns in LLMs is crucial, but prior work relied on simplified self-reports without behavioral validation, creating a need for systematic characterization of LLM personality across multiple dimensions.

Method: Systematically characterized LLM personality across three dimensions: (1) trait emergence throughout training stages, (2) predictive validity of self-reported traits in behavioral tasks, and (3) impact of targeted interventions like persona injection on both self-reports and behavior.
...
Conclusion: Findings challenge assumptions about LLM personality by distinguishing surface-level trait expression from behavioral consistency, underscoring the need for deeper evaluation in alignment and interpretability research.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate whether LLM-based synthetic agents can truly substitute real human participants in research by examining if they maintain internal consistency across different experimental settings, rather than just matching human survey responses.

Method: Developed a study to reveal agents' internal states and examine their behavior in basic dialogue settings, testing behavioral hypotheses to assess consistency between conversation behavior and revealed internal states across various LLM families and sizes.
...
Conclusion: LLM agents' inability to be internally consistent represents a critical gap in their capabilities to accurately substitute for real participants in human-subject research, despite their ability to generate human-like survey responses.
...
<details>
  <summary>Details</summary>
Motivation: Conventional LLMs often fail in highly specialized or unexpected scenarios in offshore wind maintenance, where accuracy and safety are critical requirements that need to be guaranteed.

Method: Uses parallel queries to two indices with separate retrieval budgets for knowledge and safety, plus SafetyClamp extension that fetches larger candidate pool with hard-clamping exact slot guarantees to safety.

Result: Both RAGuard and SafetyClamp show Safety Recall@K increased from almost 0% to more than 50% while maintaining Technical Recall above 60% across sparse, dense and hybrid retrieval paradigms.
...
<details>
  <summary>Details</summary>
Motivation: Address the practical challenge of collecting relevant e-commerce data, formulating long-term supply chain plans, and dynamically adjusting them while ensuring interpretability, efficiency, and reliability in complex supply chain operations.

Method: Constructed a Supply Chain Planning Agent (SCPA) framework that leverages large language models to understand domain knowledge, comprehend operator needs, decompose tasks, create/use tools, and generate evidence-based planning reports.
...
Conclusion: Demonstrates the feasibility of LLM-agent applications in supply chain management, providing a practical solution that combines AI capabilities with domain expertise for improved planning and operational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate if LLMs align with domain experts for homelessness policy decisions, addressing a global challenge affecting 150+ million people worldwide.

Method: Developed benchmark with policy scenarios across 4 geographies using Capability Approach framework, connected to agent-based modeling for social impact simulation.

Result: LLMs demonstrate potential to provide valuable policy insights at scale when used with responsible guardrails and expert collaboration.

Conclusion: With proper safeguards and local expert input, LLMs can be valuable tools for generating alternative policies in complex social policymaking contexts.
...
<details>
  <summary>Details</summary>
Motivation: Manual mapping of medical terms to OMOP standard concepts is resource-intensive and error-prone, while existing LLMs tend to hallucinate and require training/validation for clinical use.

Method: Developed a system based on Model Context Protocol (MCP) framework that allows LLMs to interact with external resources and tools for real-time vocabulary lookups and structured reasoning.
...
Conclusion: The MCP-based system provides a practical solution for automated medical term mapping to OMOP CDM that prevents hallucinations and requires no training, making it suitable for clinical deployment.
...
<details>
  <summary>Details</summary>
Motivation: Understanding tourist perception is crucial for sustainable urban planning in historic quarters, as these areas preserve cultural heritage while serving tourism and daily life.

Method: Multimodal framework integrating focal point extraction from photos (semantic segmentation), color theme analysis (clustering), and hybrid sentiment analysis (rule-based + BERT model) of tourist reviews across 12 Shanghai historic quarters.

Result: Revealed spatial variations in aesthetic appeal and emotional response, identified divergence between social media photos and real street views, showing gaps between visual expectations and built environment.
...
<details>
  <summary>Details</summary>
Motivation: Generative AI models present reliability challenges including hallucinations, semantic drift, and biases, but current evaluation methods rely on subjective human assessment which limits scalability, transparency, and effectiveness.

Method: Constructs two parallel Knowledge Graphs: (1) deterministic KG using rule-based methods, predefined ontologies, and structured extraction rules, and (2) LLM-generated KG from real-time textual data streams. Uses KG metrics (ICR, IPR, CI) to quantify structural deviations and employs automated real-time monitoring with dynamic anomaly thresholds.

Result: The method enables continuous monitoring and proactive identification of significant deviations between deterministic and LLM-generated KGs, allowing prompt detection of semantic anomalies or hallucinations in real-time.

Conclusion: The structured, metric-driven comparison between deterministic and dynamically generated KGs provides a robust and scalable evaluation framework for Generative AI reliability, addressing current limitations in transparency and objectivity.
...
Method: Alternates between local novelty-driven expansions and goal-directed expeditions using Vision-Language Models to generate linguistic goals describing hypothetical patterns.
...
Method: FaMA (Facebook Marketplace Assistant) - an LLM-powered agent that interprets natural language commands to automate high-friction workflows like listing updates, bulk messaging, and conversational search.
...
Result: Substantial improvements: 14.54% and 31.32% over LLaVA-Rad/MedGemma in report generation; 57.75% and 23.06% over MedGemma/CheXagent in visual QA. Expert review shows 0.7416 vs 0.2584 preference over Qwen2.5-VL-7B for interpretability.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches treat control parameters as embedded constraints rather than decision points, limiting their effectiveness in planning problems with continuous numeric variables.

Method: Best-first heuristic search algorithm with delayed partial expansion, where states are incrementally expanded by subsets of successors rather than fully expanded at once.

Result: The algorithm proves completeness under certain conditions and demonstrates competitive performance against existing approaches for planning with control parameters.

Conclusion: Explicitly handling control parameters as decision points through delayed partial expansion provides an efficient and complete alternative to constraint-based approaches.
...
<details>
  <summary>Details</summary>
Motivation: Address the challenge of enabling embodied agents to robustly adapt to novel domains without extensive data collection or retraining, improving cross-domain adaptability and data efficiency.

Method: World model implanting framework with prototype-based retrieval using trajectory-based abstract representation matching, and world-wise compound attention to integrate knowledge and align representations.
...
Conclusion: WorMI demonstrates strong potential for scalable real-world deployment in embodied agent scenarios requiring adaptability and data efficiency.
...
<details>
  <summary>Details</summary>
Motivation: LLM agents often fail repeatedly, explore inefficiently, and lack cross-task adaptability. Existing reflective strategies produce ephemeral traces while RL alternatives require heavy computation.

Method: MPR creates structured Meta-Policy Memory (MPM) from reflections and applies it through soft memory-guided decoding and hard rule admissibility checks (HAC) to enforce constraints.
...
Conclusion: MPR successfully externalizes reusable knowledge without model updates, enforces domain constraints, and retains language-based reflection adaptability, with potential for multimodal extensions.
...
Result: AutoPBO demonstrates significant improvements over previous local search approaches and maintains competitive performance compared to 6 state-of-the-art competitors across 4 public benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Traditional token-level RL frameworks fail to align with the reasoning-level nature of complex multi-step thought processes like Chain-of-Thought, creating a significant theoretical gap in understanding LLM reasoning.

Method: Introduces CoT-Space framework that recasts LLM reasoning as optimization in continuous reasoning-level semantic space, analyzed from both noise and risk perspectives to demonstrate convergence properties.

Result: Demonstrates that convergence to optimal CoT length is a natural consequence of underfitting-overfitting trade-off, with extensive experiments providing strong empirical validation for theoretical findings.

Conclusion: Provides coherent explanation for empirical phenomena like overthinking and offers solid theoretical foundation to guide development of more effective and generalizable reasoning agents.
...
<details>
  <summary>Details</summary>
Motivation: To harness human-like flexible representation capabilities (drawing diagrams, changing representations, creative analogies) and endow machines with similar power to make them more compatible with human use.
...
Result: Created a functional implementation of RST with core components and transformation capabilities, demonstrated through brief examples of structure transfer operations.

Conclusion: Oruga successfully implements key aspects of RST and provides a foundation for enabling machines to perform flexible representation transformations similar to human cognitive processes.
...
Method: The study compares four formal languages across three datasets and seven different LLMs to evaluate how formal language choice affects reasoning capabilities in neurosymbolic approaches.

Result: The research demonstrates that the choice of formal language has substantial impact on both syntactic and semantic reasoning performance, with varying effects observed across different LLMs.

Conclusion: The intermediate language challenge - selecting appropriate formal languages - is a critical factor in neurosymbolic LLM reasoning that significantly influences reasoning capabilities and should be carefully considered in future research.
...
<details>
  <summary>Details</summary>
Motivation: Emergency situations require fast flight path re-calculation for airliners, where conventional solvers may be too slow for real-time optimization.
...
Conclusion: Combining RL with search-based planners effectively accelerates route optimization for emergency scenarios without significant compromise on fuel efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Most computer poker research focuses on win rates while overlooking bluffing behavior, which is a crucial human poker skill that needs to be studied in AI agents.
...
Result: Both algorithms exhibited bluffing behavior but in different ways - they attempted bluffs at different rates, yet achieved roughly the same percentage of successful bluffs where opponents folded.

Conclusion: Bluffing is an essential aspect of poker gameplay rather than being algorithm-dependent, suggesting future work should explore different bluffing styles and full poker games.
...
<details>
  <summary>Details</summary>
Motivation: To explore whether AI can truly become superior to humans and assume leadership, examining the fundamental biological differences that give humans unique emotional and ethical capacities.

Method: Philosophical analysis comparing human biological systems (particularly the central nervous system) with artificial intelligence capabilities, focusing on emotional experience and ethical development.

Result: While AI may achieve superior cognitive abilities, it cannot replicate the biological central nervous system that enables human emotional experience and ethical understanding necessary for universal leadership.

Conclusion: Human biological systems (DNA-based) provide the essential foundation for ethical leadership that AI (silicon-based) cannot replicate, making humans uniquely qualified to lead despite AI's potential cognitive superiority.
...
<details>
  <summary>Details</summary>
Motivation: Formalizing cooking procedures is challenging due to their complexity and ambiguity. The paper aims to create structured machine understanding of culinary processes for both home and professional kitchens.
...
Conclusion: This work represents initial steps towards an action-centric ontology for cooking using temporal graphs, enabling structured machine understanding, precise interpretation, and scalable automation of culinary processes.
...
<details>
  <summary>Details</summary>
Motivation: To understand how Markov logic networks (MLNs) behave as domain sizes approach infinity, and to characterize the limit distributions of random structures under different types of soft constraints.

Method: Study three concrete MLN examples: (1) quantifier-free MLNs with unary relations, (2) MLNs favoring graphs with fewer triangles/k-cliques, (3) MLNs favoring graphs with fewer high-degree vertices. Analyze asymptotic properties and compare with uniform distributions.

Result: Complete characterization of limit behaviors for case 1; obtained δ-approximate 0-1 law for first-order logic from case 2; showed weights of soft constraints may or may not influence limit behavior; demonstrated MLNs and lifted Bayesian networks are asymptotically incomparable.

Conclusion: MLN distributions concentrate probability mass on different parts of the space than uniform distributions on large domains, and the formalism's behavior depends significantly on the specific soft constraints used.
...
Method: Leverages Delphi study structure with expert panel, synthesizes story quality dimensions from literature and expert insights, and maps them into Kano model framework.

Result: Develops a methodology that can inform game developers about which narrative quality aspects to prioritize for player satisfaction.

Conclusion: The proposed structured approach provides valuable insights for game developers working with AI-generated narratives, helping them focus on quality dimensions that most impact player satisfaction.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM agents overlook the functional role of emotions in negotiations, generating passive emotional responses that make them vulnerable to manipulation and strategic exploitation by adversarial counterparts.
...
Result: Significant performance improvement even with limited planning budget available
...
Method: Develops concept-level memory with strategies for abstracting takeaways from solution rollouts and retrieving relevant entries for new queries. Stores modular, reusable abstractions in natural language that can be dynamically updated during test-time.

Result: Achieves 7.5% relative gain over strong baseline on ARC-AGI benchmark, with performance scaling with inference compute. Abstract concepts outperform all tested memory designs and dynamic memory updating beats fixed memory settings.
...
<details>
  <summary>Details</summary>
Motivation: To develop optimizers that leverage the natural Riemannian metric structure of loss landscapes when embedded in higher-dimensional space, providing theoretically desirable properties and improved training performance.
...
Result: Highly effective in low-dimensional examples, provides slight improvement over state-of-the-art methods, automatically decreases learning rate in high-curvature regions (acting as smoothed gradient clipping), and enables natural decoupled weight decay.

Conclusion: The Riemannian geometry-based optimizers offer theoretical advantages and practical improvements, with computational complexity comparable to Adam, and can be used to modify existing preconditioning methods.
...
Method: Used PCC as predictability tool for robust NMF rank estimation, implemented NMF around this stable rank, and developed clustering method grouping data points with same common cause. Applied to gray-scale image datasets mapped into probability models.

Result: PCC-based rank estimate is stable against weak noise, NMF features become stable against noise and optimization seeds (resolving nonidentifiability), and NMF enables effective PCC implementation for clustering and denoising.

Conclusion: NMF and PCC have strong reciprocal benefits - PCC provides robust rank estimation for NMF, while NMF offers practical implementation of PCC for clustering and denoising applications with stable performance.
...
<details>
  <summary>Details</summary>
Motivation: Federated learning in IoT faces challenges with data heterogeneity, limited energy budgets, and client availability constraints. Effective client selection is crucial for global model convergence and balancing client contributions, especially with time-series data where availability patterns impact performance.

Method: Proposed FedDeCAB - a semi-decentralized client selection method using probabilistic rankings of available clients. When clients disconnect, it obtains partial model parameters from nearest neighbor clients for joint optimization, reducing communication overhead while maintaining model performance.
...
Conclusion: FedDeCAB successfully addresses client availability challenges in federated learning with time-series data, providing a robust solution that maintains performance while reducing communication costs in dynamic IoT environments.
...
<details>
  <summary>Details</summary>
Motivation: To develop smart-grid technologies for remote communities and advance zero-carbon energy systems by improving energy efficiency and operational resilience in microgrid management.
...
Result: Significant improvements in both energy efficiency and operational resilience compared to traditional rule-based methods, demonstrating better cost minimization and renewable energy utilization.

Conclusion: The framework successfully advances autonomous microgrid management and contributes to zero-carbon energy goals, with an open-source simulation platform provided for further research and development.
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of hand-crafted training environments and develop truly generalizable and robust agents by scaling environment complexity alongside agent learning.

Method: Goal-conditioned generative world model where an Attacker agent synthesizes challenging world states to exploit Defender weaknesses, while Defender team learns cooperative policies, creating a co-evolutionary dynamic.
...
Conclusion: Adversarial co-evolution is a powerful method for learning instrumental world models that drive agents toward greater strategic capabilities through continuous adaptation and challenge generation.
...
<details>
  <summary>Details</summary>
Motivation: Current RLHF methods like uniform-reward RLHF favor dominant groups and fail to capture diverse opinions. MaxMin-RLHF addresses fairness but performs poorly when the minimum-reward group is a minority.

Method: SharedRep-RLHF learns shared traits in annotations across different groups rather than learning separate group-specific reward models. It focuses on identifying common patterns that exist across various sub-populations.

Result: Experiments show SharedRep-RLHF outperforms MaxMin-RLHF with up to 20% gain in win rate across diverse natural language tasks. The paper also provides theoretical analysis showing MaxMin-RLHF is provably suboptimal for learning shared traits.

Conclusion: SharedRep-RLHF effectively addresses the minority group performance problem in MaxMin-RLHF by leveraging shared annotation traits across groups, providing better fairness and performance compared to existing methods.
...
Method: Combines Transaction Cost and Information Asymmetry theories with machine learning algorithms. Uses random forests for data processing, LSTM for demand forecasting, clustering/regression for benefit allocation, Game Theory with reinforcement learning for inventory optimization, and XGBoost for credit assessment.
...
<details>
  <summary>Details</summary>
Motivation: Gradient dynamics play a central role in neural network stability and generalization, but there's a gap between theoretical expectations and empirical behaviors that needs to be addressed.
...
<details>
  <summary>Details</summary>
Motivation: With MARL's growing impact and proven superhuman performance in games, there's a need for a comprehensive review to examine its applications across diverse game genres and address critical challenges in the field.

Method: Thorough examination of MARL applications from turn-based two-agent games to real-time multi-agent video games, analysis of critical challenges (nonstationary, partial observability, sparse rewards, team coordination, scalability), and proposal of a novel method to estimate game complexity.

Result: Successful MARL implementations demonstrated in various games including Rocket League, Minecraft, Quake III Arena, StarCraft II, Dota 2, and Honor of Kings, achieving superhuman performance through techniques like self-play, supervised learning, and deep reinforcement learning.

Conclusion: The paper provides insights into MARL in video game AI systems, offers a novel complexity estimation method, and suggests future research directions to advance MARL applications in game development, inspiring further innovation in this rapidly evolving field.
...
<details>
  <summary>Details</summary>
Motivation: Traditional exact graph node kernels have O(N^3) time complexity, making Bayesian inference computationally prohibitive for large graphs with many nodes.
...
Conclusion: GRFs provide an efficient stochastic approximation method that dramatically reduces computational complexity for Bayesian inference on large graphs, making previously infeasible applications possible.
...
<details>
  <summary>Details</summary>
Motivation: The rise of multi-modal multi-task foundation models (M3T FMs) and the need to leverage geo-distributed data from wireless devices motivates exploring federated foundation models that can handle heterogeneity in modalities and tasks across fog/edge networks.
...
Conclusion: HF-FMs represent a novel paradigm for federated foundation models that addresses critical heterogeneity challenges in fog/edge networks, with promising research directions and practical implementation potential.
...
Result: Proves theoretical consistency extensions for tSNE, demonstrates superior geometry preservation on synthetic and real data, and shows reduced fragmentation of continuous high-density regions compared to other methods.

Conclusion: EmbedOR provides more faithful data visualizations, can annotate existing visualizations to identify fragmentation, and offers deeper insight into underlying data geometry through its curvature-enhanced approach.
...
<details>
  <summary>Details</summary>
Motivation: Real-world testing scenarios require selecting optimal test subsets when tests are correlated and costly, but the joint distribution of outcomes is unknown and must be learned online, with missing data complicating estimation.

Method: Formulated as MDP with unknown distribution, analyzed minimax regret bounds, developed Explore-Then-Commit algorithm for OTP, and iterative-elimination algorithm for variant with missingness-independent rewards.

Result: Proved Ω(T^{2/3}) minimax regret lower bound for OTP (vs Θ(√T) for standard MDPs), matched with Õ(T^{2/3}) algorithm, and achieved Õ(√T) regret for variant with missingness-independent rewards.

Conclusion: Missing data fundamentally increases exploration difficulty in sequential testing, with regret bounds depending on whether rewards are missingness-dependent, guiding efficient testing policy design.
...
<details>
  <summary>Details</summary>
Motivation: To present a vision for X-Learning architecture and introduce its unexplored design considerations and degrees of freedom.

Method: The paper provides perspective on X-Learning by exploring connections to graph theory and Markov chains, and identifying design considerations.

Result: The paper establishes intuitive yet non-trivial connections between X-Learning, graph theory, and Markov chains, providing a foundation for this new architecture.

Conclusion: The paper presents X-Learning as a promising distributed learning approach and outlines open research directions to stimulate further investigation.
...
<details>
  <summary>Details</summary>
Motivation: Range-partition entropy provides strong theoretical guarantees in algorithm design but hasn't been accessible to deep learning. The goal is to bridge this gap and enable adaptive computation based on input sortedness.
...
TL;DR: First framework for optimizing spatial training data collection in satellite ML, addressing data sparsity and heterogeneous costs with novel methods that show substantial performance gains across multiple continents and tasks.


<details>
  <summary>Details</summary>
Motivation: Satellite ML applications suffer from sparse, clustered labeled data that limits large-scale monitoring capabilities, creating uncertainty about how to efficiently collect additional training data.
...
Result: Experiments across three continents and four tasks demonstrate substantial performance gains from optimized sampling strategies, with delineation of settings where optimization is most effective.

Conclusion: The framework generalizes across SatML domains and provides immediate practical value for augmenting agricultural surveys in Togo, addressing a critical gap in satellite ML data collection optimization.
...
TL;DR: A data-driven method for approximating functions on smooth manifolds using Diffusion Maps, with dimensionality reduction via SVD and online updating for scalability, showing superior performance over neural networks and interpolation methods.
...
Conclusion: The proposed methodology provides an effective and scalable approach for function approximation on manifolds, offering computational advantages and improved performance over traditional methods.
...
TL;DR: This paper shows that low-rank structure in reward matrices enables efficient sparse-reward RL, with a sharp transition from exponential to polynomial sample complexity. It introduces Policy-Aware Matrix Completion (PAMC) that connects matrix completion theory with RL.
...
Conclusion: Low-rank structure in reward matrices enables efficient sparse-reward RL with polynomial sample complexity. PAMC establishes structural reward learning as a promising paradigm with implications for safety-critical, sample-expensive applications.
...
Conclusion: Updating feature representations of latent factors is more effective than conventional parameter selection approaches for handling distribution shift in online time series forecasting, and ADAPT-Z successfully addresses the delayed feedback problem in multi-step forecasting scenarios.
...
Result: Ensemble tree-based models (Random Forest, XGBoost) provide the best trade-off between accuracy and robustness, confirming LiDAR features capture roughness-induced scattering effects.

Conclusion: Framework enables generation of scatter-aware environment maps and digital twins, supporting adaptive beam management, blockage recovery, and environment-aware connectivity in next-generation networks.
...
<details>
  <summary>Details</summary>
Motivation: To leverage machine learning advances for studying traffic accident data to mitigate future risks, particularly handling imbalanced data with good generalization.

Method: Analyzed feature colinearity, used autoencoders for unsupervised dimensionality reduction, followed by dense neural network for classification of accident severity.

Result: Achieved cross-validated accuracy of up to 92% in classifying traffic accident severity.

Conclusion: Neural network-based models with autoencoder preprocessing are effective for traffic accident severity classification with high accuracy.
...
Method: Reinterpret CPM as a potential hedonic game by decomposing global Hamiltonian into local utility functions. Prove convergence via better-response dynamics. Introduce two stability criteria: strict robustness criterion and relaxed utility function with resolution parameter.
...
TL;DR: A lightweight BEV injection connector enhances MLLMs for V2I link prediction by providing spatial context, improving accuracy by up to 13.9% and showing 32.7% gains in adverse conditions.
...
Method: Proposes a plug-and-play BEV injection connector that fuses environmental BEV representations with ego vehicle inputs, using CARLA and MATLAB co-simulation to generate multimodal data and ground-truth from ray tracing.

Result: BEV injection framework consistently improved performance across three V2I tasks (LoS/NLoS classification, link availability, blockage prediction), with up to 13.9% accuracy improvement and 32.7% gains in rainy/nighttime conditions.

Conclusion: The BEV injection approach effectively addresses MLLMs' spatial understanding limitations and demonstrates robust performance improvements in V2I communication prediction, particularly in challenging environmental conditions.
...
TL;DR: A deep latent variable MFG model for inverse reinforcement learning that handles heterogeneous agent objectives without prior context knowledge


<details>
  <summary>Details</summary>
Motivation: Existing IRL methods in mean field games assume agent homogeneity, limiting their ability to handle real-world scenarios with heterogeneous and unknown objectives

Method: Proposed a deep latent variable MFG model and associated IRL method that can infer rewards from different but structurally similar tasks without prior context knowledge or model modifications

Result: Superior performance demonstrated in simulated scenarios and a real-world spatial taxi-ride pricing problem compared to state-of-the-art IRL methods in MFGs

Conclusion: The approach successfully addresses the limitation of agent homogeneity assumption and enables practical reward inference for heterogeneous objectives in mean field games
...
TL;DR: Proposes a novel metric to automatically select optimal data augmentation strategies for quantization-aware knowledge distillation, improving performance of low-precision models with minimal training overhead.
...
Method: Develops a metric that evaluates data augmentations based on their capacity to maximize contextual mutual information while ensuring predictions align with ground truth labels.
...
Conclusion: The proposed automated DA selection method is effective, compatible with existing algorithms, and enhances low-precision model performance through optimal input transformation strategies.
...
Method: Uses scale-specific lead-lag graph learning with cross-correlation and dynamic decaying features, plus hierarchical lead-lag message passing to propagate intra- and inter-scale lead-lag effects.
...
Conclusion: MillGNN effectively captures comprehensive lead-lag dependencies at multiple scales with statistical interpretability and data-driven flexibility, achieving state-of-the-art forecasting performance.
...
Method: Multilayer perceptron neural network trained on 50 key urinary peptide biomarkers selected via genetic algorithms, with balanced groups using SMOTE and stratified validation
...
TL;DR: Analysis of mistake-bound online learning with arithmetic operation caps, solving problems from recent papers and extending results to operation-capped settings.
...
Method: Proving general bounds on arithmetic operations per round required for mistake-bounded learning, addressing agnostic learning with bandit feedback problems from recent literature.

Result: General bounds established for arithmetic operation requirements, solutions provided for specific problems in agnostic mistake-bounded online learning with bandit feedback.

Conclusion: The paper advances understanding of computational constraints in online learning models and provides solutions to recent open problems while extending results to operation-capped environments.
...
Conclusion: Formal verification of neural network robustness is essential for dependable satellite fault detection systems, reducing maintenance burden through reliable early detection capabilities.
...
TL;DR: Framework for aligning ML models with clinical knowledge through constraints, showing improved interpretability without performance loss.
...
Method: Proposed a reproducible framework incorporating clinical knowledge via constraints into ML models, using survey-collected clinical expertise and analyzing impact across degrees of underspecification in prostate cancer prediction.

Result: Constrained models aligned with clinical experiential learning without compromising performance. Feedback-driven alignment approach revealed that larger prediction differences between constrained/unconstrained models led to more apparent clinical interpretation differences.

Conclusion: Clinical knowledge can be effectively incorporated into ML models through constraints, improving alignment with medical expertise while maintaining predictive performance, with clear benefits for clinical interpretability.
...
<details>
  <summary>Details</summary>
Motivation: Federated learning faces challenges with data heterogeneity among clients, especially when datasets are limited and have class imbalance, which negatively impacts global model generalization.

Method: FedQuad explicitly optimizes smaller intra-class variance and larger inter-class variance across clients by minimizing distance between similar pairs and maximizing distance between negative pairs, effectively disentangling client data in shared feature space.

Result: Superior performance demonstrated on CIFAR-10 and CIFAR-100 datasets under various data distributions with many clients, outperforming existing approaches.

Conclusion: Metric learning-based strategies are effective for addressing representational learning challenges in federated settings, with FedQuad providing a robust solution for data heterogeneity problems.
...
TL;DR: SP-CCI is a new framework that uses synthetic counterfactual data to create tighter prediction intervals while maintaining coverage guarantees, addressing the conservatism of existing conformal counterfactual inference methods.
...
Conclusion: SP-CCI achieves tighter prediction intervals with theoretical guarantees under exact and approximate importance weighting, providing more efficient counterfactual inference.
...
TL;DR: This paper addresses fairness concerns in algorithmic recourse, linking fairness guarantees between recourse and classification, and introduces a new social burden-based fairness framework with practical algorithm MISOB that reduces social burden across groups without compromising accuracy.


<details>
  <summary>Details</summary>
Motivation: Emerging legislation mandates that classifiers must provide actionable recourse for negative decisions, but there are concerns about fairness guarantees within the recourse process itself. The paper aims to provide a holistic theoretical characterization of unfairness in algorithmic recourse.

Method: The paper provides a theoretical characterization linking fairness guarantees in recourse and classification, introduces a novel fairness framework based on social burden, and develops a practical algorithm called MISOB that is broadly applicable under real-world conditions.

Result: Empirical results on real-world datasets demonstrate that MISOB reduces the social burden across all groups while maintaining overall classifier accuracy.

Conclusion: The work establishes a theoretical foundation for understanding unfairness in algorithmic recourse and provides a practical solution (MISOB) that addresses fairness concerns in the recourse process without sacrificing classification performance.
...
Method: Uses an agentic workflow with Large Language Models for automatic, iterative data generation with feedback loops to improve quality, incorporating external knowledge.
...
TL;DR: AFA is a novel attention mechanism that models input sequences as linear SDE observations, using closed-form solutions to compute attention weights as maximum likelihood estimates with propagated pairwise precisions.


<details>
  <summary>Details</summary>
Motivation: To develop a more robust attention mechanism that incorporates learnable dynamics modeling rather than direct query-key comparisons, providing better uncertainty propagation and theoretical grounding.

Method: Models input sequence as discrete observations of linear stochastic differential equations, uses closed-form solution to differential Lyapunov equation to propagate pairwise uncertainties, derives attention weights as maximum likelihood solution with residual-based reweighting.

Result: Developed AFA mechanism that can recover standard dot-product attention as a special case, with simplified variant maintaining same computational complexity as standard attention.

Conclusion: AFA provides a theoretically grounded attention framework with learnable dynamics modeling, offering robust uncertainty propagation while maintaining computational efficiency comparable to standard attention mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: While self-supervised speech models excel in speech processing, their effectiveness on non-speech bioacoustic data remains underexplored, prompting investigation into their transfer learning capabilities for animal sound detection and classification.

Method: Analyzed models' properties using linear probing on time-averaged representations, extended approach with downstream architectures to account for time-wise information, and studied effects of frequency range and noise on performance.

Result: Models generate rich latent representations of animal sounds across taxa, achieving competitive performance with fine-tuned bioacoustic pre-trained models, with noise-robust pre-training setups showing significant impact.

Conclusion: Speech-based self-supervised learning provides an efficient framework for advancing bioacoustic research, demonstrating strong transfer learning capabilities from speech to non-speech audio domains.
...
TL;DR: New membership inference attacks for time series forecasting models show they are vulnerable, with user-level attacks achieving perfect detection in some cases.


<details>
  <summary>Details</summary>
Motivation: Membership inference attacks have been extensively studied on classification models but remain largely unexplored for time series forecasting, creating a research gap.

Method: Introduced two new attacks: (1) adaptation of multivariate LiRA from classification to time-series forecasting, and (2) novel end-to-end Deep Time Series (DTS) attack. Benchmarked against adapted classification attacks on TUH-EEG and ELD datasets using LSTM and N-HiTS architectures.

Result: Forecasting models are vulnerable to membership inference, with user-level attacks often achieving perfect detection. Proposed methods show strongest performance in several settings. Vulnerability increases with longer prediction horizons and smaller training populations.

Conclusion: Establishes new baselines for privacy risk assessment in time series forecasting, demonstrating significant vulnerability similar to trends observed in large language models.
...
TL;DR: This paper provides a critical analysis and extension of Cai and Wang's (2020) work on over-smoothing in GNNs using Dirichlet energy, showing exponential decrease of energy with depth under mild conditions and extending results to spectral polynomial filters.


<details>
  <summary>Details</summary>
Motivation: To address and extend the theoretical understanding of over-smoothing in Graph Neural Networks (GNNs) through Dirichlet energy analysis, building upon previous work by Cai and Wang.

Method: The authors establish theoretical conditions (including Leaky-ReLU activation) under which Dirichlet energy decreases exponentially with network depth, provide extensions to spectral polynomial filters, and conduct experiments with edge deletion and weight amplification.

Result: The paper demonstrates that Dirichlet energy decreases exponentially with depth under mild spectral conditions, and experimental results show when Dirichlet energy increases, suggesting practical methods to mitigate over-smoothing.
...
Result: Fine-tuning Llama-3.1 8B and Qwen-3 8B with SBD achieved 3-5x reduction in forward passes while maintaining same performance as standard NTP training.

Conclusion: SBD provides a simple, flexible acceleration method that requires no architectural changes, maintains KV-caching compatibility, and significantly speeds up generation without sacrificing accuracy.
...
TL;DR: ZooCast is a framework that intelligently combines multiple Time Series Foundation Models (TSFMs) into a model zoo, dynamically selecting the best model for each forecasting task using a unified embedding representation.


<details>
  <summary>Details</summary>
Motivation: No single TSFM excels universally across all time series patterns, but different models have complementary strengths. The authors want to leverage this diversity by creating a system that can dynamically select the optimal model for each forecasting task.
...
Conclusion: The framework successfully leverages complementary TSFM abilities through dynamic model selection, achieving improved forecasting accuracy while maintaining efficiency and scalability for real-world deployment.
...
Conclusion: The introduced supervised metrics provide a systematic way to evaluate DR relationship phases and improve the reliability and efficiency of dimensionality reduction processes.
...
TL;DR: Theoretical analysis shows SSMs have exponential decay in long-range dependency like RNNs, while transformers with attention are more flexible. A new SSM formulation is proposed to combine attention's flexibility with SSM's efficiency.
...
Method: Mathematically define long-range dependency using derivative of hidden states w.r.t. past inputs. Compare SSM and transformer capabilities based on this definition. Propose new SSM formulation with improved stability.
...
Conclusion: Transformers theoretically outperform SSMs in long-range dependency modeling given sufficient resources. A new SSM formulation is proposed to combine attention's flexibility with SSM's computational efficiency while maintaining stability.
...
TL;DR: A unified framework for layer-wise Gaussian mechanisms in differential privacy that analyzes noise allocation strategies and proposes SNR-Consistent allocation for better privacy-utility tradeoffs.


<details>
  <summary>Details</summary>
Motivation: Existing layer-wise noise allocation methods lack theoretical grounding and often use heuristic strategies that ignore inter-layer signal-to-noise ratio consistency or inefficiently use privacy budgets.

Method: Developed an analytical framework connecting layer-wise noise injection to optimization objectives and privacy budget allocations. Proposed SNR-Consistent noise allocation strategy that unifies signal preservation and privacy budget efficiency.

Result: Extensive experiments in centralized and federated learning settings show the method consistently outperforms existing allocation strategies with better privacy-utility tradeoffs.

Conclusion: The framework provides diagnostic insights into prior methods and offers theoretical guidance for designing adaptive and effective noise injection schemes in deep learning models.
...
<details>
  <summary>Details</summary>
Motivation: Heart failure research faces barriers due to privacy regulations limiting data sharing. Synthetic data generation offers a solution to overcome these challenges while preserving patient confidentiality.

Method: Generated synthetic HF datasets from 12,552 patients using five deep learning models: TVAE, normalizing flow, ADSGAN, SurvivalGAN, and TabDDPM. Evaluated through statistical similarity metrics, survival prediction, and privacy assessments.

Result: SurvivalGAN and TabDDPM showed high fidelity with similar distributions and survival curves. SurvivalGAN and TVAE achieved strong survival prediction performance (C-indices: 0.71-0.76) matching real data. Privacy evaluation confirmed protection against re-identification.

Conclusion: Deep learning-based synthetic data generation produces high-fidelity, privacy-preserving HF datasets suitable for research, addressing critical data sharing barriers and providing valuable resources for advancing HF research.
...
TL;DR: RL fine-tuning preserves prior knowledge better than SFT due to smaller KL-divergence from base model, with on-policy RL biased toward KL-minimal solutions.
...
Method: Theoretical analysis of distributional shift using KL-divergence, experimental validation with large language models and robotic foundation models, and comparison of RL vs SFT fine-tuning approaches.

Result: RL preserves prior knowledge significantly better than SFT, with forgetting determined by KL-divergence between fine-tuned and base policies. On-policy RL converges to KL-minimal solutions.

Conclusion: RL's Razor principle: among task solutions, RL prefers those closest in KL to the original model, explaining its superior knowledge preservation.
...
TL;DR: A novel method for efficiently finding optimal privacy-accuracy trade-offs in differential privacy by modeling the Pareto front directly and using more informative user interactions instead of pairwise comparisons.
...
Method: Theoretically derive the shape of the privacy-accuracy trade-off to model the Pareto front directly, and replace pairwise comparison feedback with more informative interactions where users select preferred trade-off curves.

Result: Experiments on differentially private logistic regression and deep transfer learning across six real-world datasets show the method converges to optimal trade-offs with significantly less computational cost and user interaction than baselines.

Conclusion: The proposed approach effectively leverages the structural properties of differential privacy problems to achieve more efficient optimization of privacy-accuracy trade-offs through direct Pareto front modeling and improved preference learning interactions.
...
TL;DR: Machine learning methods often fail in real-world high-stakes applications, particularly across socially sensitive contexts. The paper identifies two overlooked problems - 'no fair lunch' and 'subgroup separability' - and explains why current fair representation learning methods fail to address them, proposing new directions.


<details>
  <summary>Details</summary>
Motivation: Machine learning failures in high-stakes situations (like medical diagnosis) and across socially sensitive lines have created barriers to safe deployment where ML could provide significant benefits.

Method: The paper introduces causal and statistical structures that cause ML failures in image analysis, identifies two specific problems (no fair lunch and subgroup separability), and analyzes why current fair representation learning approaches are inadequate.

Result: The analysis reveals that existing fair representation learning methods fail to adequately address the identified problems of no fair lunch and subgroup separability in machine learning systems.

Conclusion: The paper proposes potential paths forward for the field to address these fundamental problems and enable safer deployment of machine learning in sensitive applications like medical diagnosis.
...
Conclusion: AT-UCB effectively exploits shared information between different abstraction levels in CMAB problems, providing both theoretical guarantees and practical performance improvements.
...
TL;DR: A data-driven behavioral model for analyzing Army installation energy usage to assess climate resilience and benchmark future measures.


<details>
  <summary>Details</summary>
Motivation: Climate change and extreme weather threaten US Army infrastructure, requiring resilience measures for critical facility assets that depend on commercial energy sources.

Method: Behavioral model using multimodal energy usage data from non-residential buildings to analyze, predict, and cluster energy consumption patterns, validated with similarly structured open access data.

Result: The methodology produces behavior profiles that create baseline assessments of disruption impacts and serve as benchmarks for future resiliency measures.

Conclusion: This data-driven approach provides a framework for understanding energy vulnerabilities and developing effective climate resilience strategies for Army installations.
...
TL;DR: SST-iTransformer model for parking availability prediction using multi-source traffic data and self-supervised learning, achieving state-of-the-art results with ride-hailing data being most impactful.


<details>
  <summary>Details</summary>
Motivation: Rapid growth of private car ownership has worsened urban parking problems, creating need for accurate parking availability prediction to support urban planning and management.

Method: Proposes SST-iTransformer with K-means clustering for parking zones, integrates multi-source traffic data (metro, bus, ride-hailing, taxi), uses self-supervised learning with masking-reconstruction tasks, and dual-branch attention mechanism (Series Attention for temporal dependencies, Channel Attention for cross-variate interactions).
...
Conclusion: SST-iTransformer effectively models spatio-temporal dependencies and multi-source data integration for parking prediction, with spatial correlations from related parking lots being crucial for performance.
...
TL;DR: Novel active learning strategies that reduce bias between experimental rounds by leveraging bias-variance tradeoff and heteroskedastic aleatoric uncertainty, outperforming canonical methods like BALD and Least Confidence in batched settings.


<details>
  <summary>Details</summary>
Motivation: Real-world experiments often have heteroskedastic aleatoric uncertainty that can be correlated in batched settings, requiring better active learning approaches to handle this uncertainty and improve model performance.

Method: Proposes active learning strategies based on bias-variance tradeoff to reduce bias between experimental rounds. Introduces a novel cobias-covariance relationship for leveraging historical data quadratically, with an eigendecomposition strategy for batching.

Result: The difference-based method using the cobias-covariance relationship with quadratic estimator outperforms canonical active learning methods including BALD and Least Confidence in batched settings.

Conclusion: The proposed approach effectively handles heteroskedastic aleatoric uncertainty in experimental scenarios and provides superior performance in batched active learning compared to established methods.
...
Method: Proposes PagedEviction, a structured KV cache pruning strategy with block-wise eviction algorithm designed for paged memory layouts, seamlessly integrated with vLLM's PagedAttention without modifying CUDA kernels.
...
TL;DR: TiM introduces continuous-time transition models that enable flexible sampling from single-step to multi-step generation, achieving SOTA performance with fewer parameters while maintaining monotonic quality improvement with more steps.


<details>
  <summary>Details</summary>
Motivation: Address the fundamental dilemma between computational efficiency (few-step models) and output quality (iterative diffusion models) by overcoming restrictive training objectives that focus only on infinitesimal dynamics or direct endpoint prediction.

Method: Develops an exact continuous-time dynamics equation that analytically defines state transitions across any finite time interval, creating Transition Models (TiM) that adapt to arbitrary-step transitions throughout the generative trajectory.

Result: TiM with only 865M parameters surpasses SD3.5 (8B) and FLUX.1 (12B) across all step counts, demonstrates monotonic quality improvement with increased sampling budget, and achieves exceptional fidelity at resolutions up to 4096x4096 using native-resolution strategy.

Conclusion: TiM provides a novel generative paradigm that successfully resolves the quality-efficiency tradeoff, offering flexible sampling capabilities from coarse to fine generation while maintaining state-of-the-art performance with significantly fewer parameters.
...
TL;DR: IPA improves upon LoRA by using feature-aware projections instead of random initialization, achieving better performance with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: LoRA's random down-projection discards useful information and becomes a performance bottleneck, as it changes little during training while up-projection does most adaptation work.

Method: IPA uses feature-aware projection framework that preserves information in reduced hidden space, approximating top principal components for efficient pretraining with minimal inference overhead.

Result: IPA consistently outperforms LoRA and DoRA across benchmarks, achieving 1.5 points higher accuracy on commonsense reasoning and 2.3 points on VTAB-1k, while matching full LoRA performance with half the trainable parameters when projection is frozen.

Conclusion: Feature-aware projections significantly improve parameter-efficient fine-tuning by preserving useful information that random projections discard, leading to better performance with reduced parameter count.
...
Result: Theoretically proven identifiability under mild conditions. Empirically superior performance in clustering and structure learning tasks, with successful recovery of biologically meaningful mechanisms in single-cell perturbation data.
...
TL;DR: The paper presents a unified theoretical framework showing that RL and SFT approaches are instances of a single optimization process, and proposes Hybrid Post-Training (HPT) that dynamically selects training signals to combine demonstration exploitation and stable exploration.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between online (model-generated) and offline (human demonstration) training data approaches, showing they are not contradictory but part of a unified optimization framework.

Method: Derived a Unified Policy Gradient Estimator with four interchangeable components, and proposed HPT algorithm that dynamically selects training signals based on theoretical findings.

Result: HPT consistently outperforms strong baselines across six mathematical reasoning benchmarks and two out-of-distribution suites, working effectively across models of varying scales and families.

Conclusion: The unified framework provides theoretical grounding for combining RL and SFT approaches, and HPT demonstrates practical effectiveness in leveraging both demonstration exploitation and stable exploration without sacrificing learned reasoning patterns.
...
### [99] [Echo State Networks as State-Space Models: A Systems Perspective](https://arxiv.org/abs/2509.04422)
...
<details>
  <summary>Details</summary>
Motivation: ESNs are typically designed using heuristics rather than first principles. The authors aim to establish a unified theoretical framework by recasting ESNs as state-space models, linking reservoir computing with classical system identification and modern kernelized SSMs.
...
Conclusion: The state-space perspective yields a unified systems-theoretic account of ESNs, connecting reservoir computing with classical identification methods and enabling more principled design and analysis of these recurrent models.
...
### [100] [Unveiling the Role of Data Uncertainty in Tabular Deep Learning](https://arxiv.org/abs/2509.04430)
*Nikolay Kartashev,Ivan Rubachev,Artem Babenko*

Main category: cs.LG

TL;DR: The paper explains that data uncertainty is the key reason why modern tabular deep learning methods work well, showing how techniques like numerical embeddings and retrieval-augmented models implicitly manage uncertainty.


<details>
  <summary>Details</summary>
Motivation: To address the lack of understanding about why recent tabular deep learning techniques achieve exceptional practical performance despite limited theoretical explanations.

Method: Analyze and dissect various tabular DL design choices (numerical feature embeddings, retrieval-augmented models, advanced ensembling) through the lens of data uncertainty management.

Result: Revealed that the success of beneficial tabular DL techniques stems from their implicit mechanisms for handling high data uncertainty, and developed more effective numerical feature embeddings based on these insights.

Conclusion: The data-uncertainty perspective provides a unifying understanding of tabular DL advancements, enables concrete improvements to existing techniques, and outlines future research directions for the field.
...
### [101] [Delta Activations: A Representation for Finetuned Large Language Models](https://arxiv.org/abs/2509.04442)
*Zhiqiu Xu,Amish Sethi,Mayur Naik,Ser-Nam Lim*

Main category: cs.LG

TL;DR: Delta Activations method represents finetuned LLMs as vector embeddings by measuring activation shifts from base models, enabling effective clustering, model selection, and merging.


<details>
  <summary>Details</summary>
Motivation: The proliferation of open-source finetuned LLMs creates challenges in navigating and understanding models due to inconsistent metadata and unstructured repositories.

Method: Measure shifts in internal activations relative to a base model to create vector embeddings that represent finetuned models, enabling clustering by domain and task.

Result: Delta Activations show robustness across finetuning settings, exhibit additive properties with mixed datasets, and can embed tasks via few-shot finetuning for model selection and merging.
...
### [102] [Towards Cognitively-Faithful Decision-Making Models to Improve AI Alignment](https://arxiv.org/abs/2509.04445)
...
<details>
  <summary>Details</summary>
Motivation: Standard preference elicitation methods often fail to capture true human cognitive processes like heuristics, resulting in models that don't align with actual decision-making and lack generalization capabilities.
...
Conclusion: The structured processing approach provides more realistic and feasible candidates for representing underlying human decision-making processes compared to standard methods.
...
### [103] [ChronoGraph: A Real-World Graph-Based Multivariate Time Series Dataset](https://arxiv.org/abs/2509.04449)
...
Method: Multi-agent system with Judge, Prosecution, Defense Counsel, and Adjudicator agents powered by LLMs, integrated with RAG using Indian legal documents for legal grounding.

Result: System enables legally sound instructions and arguments with source citations, supporting iterative deliberation rounds for consensus-based verdicts.

Conclusion: Provides configurable, explainable platform for exploring legal reasoning and group decision-making in Indian judicial context with verifiable legal grounding.

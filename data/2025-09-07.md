...
Motivation: Existing GUI agents fail to capture complex transition relationships between pages using sequential episodes, limiting their ability to deeply perceive GUI environments and generalize to new scenarios.

Method: Automated pipeline to convert sequential episodes into page graphs, RAG technology to retrieve GUI perception guidelines, and multi-agent framework with task decomposition strategy.
...
Conclusion: PG-Agent effectively models GUI page transitions and generalizes well to unseen scenarios through graph-based representation and retrieval-augmented multi-agent framework.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-agent LLM systems use fixed collaboration protocols that overlook agents' internal deliberative capabilities and meta-cognitive states like uncertainty/confidence, treating agents as passive executors rather than adaptive reasoners.
...
Method: Develops a new algorithm that simplifies multilinear programming construction by exploiting input probabilities over endogenous variables. Uses column generation with linear integer programming for single intervention scenarios to achieve polynomial cardinality representation.

Result: The column generation technique proves superior to existing methods in experiments, demonstrating efficient computation of tight probability bounds with polynomial representation complexity.

Conclusion: The proposed algorithm provides an effective solution for handling partial identifiability in quasi-Markovian causal models, offering computational advantages over previous approaches through column generation and polynomial-sized representations.
...
<details>
  <summary>Details</summary>
Motivation: To bridge psychological theory with LLM behavior design by leveraging established personality frameworks like MBTI to create more effective and psychologically grounded AI agents.
...
Conclusion: The framework successfully establishes psychologically enhanced AI agents without fine-tuning, generalizes to other psychological frameworks, and provides a foundation for structured multi-agent communication with improved interaction quality.
...
Conclusion: Diffusion-AC framework successfully addresses unimodal bias in DRL for air traffic management, providing superior conflict resolution performance and safety through its unique multimodal decision-making capability and progressive training curriculum.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods like ReAct require LLMs to plan before every action, which is computationally expensive and degrades performance on long tasks, while never planning limits capabilities.

Method: Two-stage training: (1) supervised fine-tuning on synthetic data to prime for dynamic planning, (2) reinforcement learning to refine this capability in long-horizon environments.

Result: Dynamic planning agents are more sample-efficient and achieve more complex objectives in Crafter environment, and can be effectively steered by human-written plans.

Conclusion: First work to train LLM agents for dynamic test-time compute allocation, enabling more efficient, adaptive, and controllable agentic systems.
...
Method: Developed a method-agnostic, perturbation-based framework that applies controlled perturbations, computes similarities, and trains weighted linear surrogates to identify influential graph components.
...
<details>
  <summary>Details</summary>
Motivation: Reasoning requires adaptation to novel problems under limited data and distribution shift, but current benchmarks lack formal causal structure for systematic evaluation.

Method: Create CausalARC with tasks sampled from structural causal models, providing principled data augmentations (observational, interventional, counterfactual) as few-shot demonstrations for in-context learning.

Result: The paper demonstrates CausalARC's utility across four evaluation settings: abstract reasoning with test-time training, counterfactual reasoning, program synthesis, and causal discovery with logical reasoning.

Conclusion: CausalARC provides a principled framework for evaluating AI reasoning capabilities in causal, low-data, and out-of-distribution scenarios through formally specified world models.
...
Method: Developed a prototype system that uses image schemas (cognitive patterns from sensorimotor experience) formalized through declarative spatial reasoning in Answer Set Programming to guide LLMs.

Result: The system successfully guides LLMs to interpret scenarios through embodied cognitive structures, formalizes them as executable programs, and enhances logical reasoning with improved interpretability.

Conclusion: While currently focused on spatial primitives, this approach establishes a computational foundation for incorporating more complex dynamic representations in neurosymbolic reasoning systems.
...
Method: Analysis of RL dynamics in LLMs, proposing HIerarchy-Aware Credit Assignment (HICRA) that concentrates optimization on high-impact planning tokens and uses semantic entropy for strategic exploration measurement.
...
Conclusion: RL success in LLMs stems from emergent reasoning hierarchy, and targeted optimization on planning tokens (via HICRA) with semantic entropy measurement provides more efficient learning of complex reasoning skills.
...
Method: Investigated 8 different time series segmentation algorithms, evaluated using InterpretTime and AUC Difference methodologies on multivariate and univariate time series data.
...
<details>
  <summary>Details</summary>
Motivation: Current automated red-teaming approaches don't consider how human identities and backgrounds shape red-teaming strategies and risk discovery, limiting their effectiveness compared to human red-teaming.

Method: Developed PersonaTeaming method that uses 'red-teaming expert' and 'regular AI user' personas to mutate prompts, with a dynamic persona-generating algorithm and new metrics to measure mutation distance and diversity.

Result: Experiments showed promising improvements with up to 144.1% higher attack success rates in adversarial prompts through persona mutation while maintaining prompt diversity, outperforming state-of-the-art RainbowPlus method.

Conclusion: PersonaTeaming demonstrates the value of incorporating identity considerations in automated red-teaming, revealing opportunities for complementarity between automated and human red-teaming approaches.
...
Result: Instructional alignment (RLHF, instruction tuning) stabilizes trait expression and strengthens correlations resembling human patterns, but self-reported traits don't reliably predict behavior. Persona injection successfully steers self-reports but has little/inconsistent effect on actual behavior.

Conclusion: Surface-level trait expression differs from behavioral consistency in LLMs, challenging assumptions about LLM personality and highlighting the need for deeper evaluation in alignment and interpretability research.
...
Conclusion: LLMs' lack of internal consistency represents a critical limitation in their capability to accurately substitute for real human participants in research, despite their ability to produce human-like survey responses.
...
<details>
  <summary>Details</summary>
Motivation: Conventional LLMs often fail in highly specialized or unexpected scenarios in offshore wind maintenance where accuracy and safety are critical.

Method: RAGuard uses parallel queries to two indices (knowledge and safety) with separate retrieval budgets, plus SafetyClamp extension that fetches larger candidate pool with hard-clamping for exact safety guarantees.

Result: Safety Recall@K increased from almost 0% in standard RAG to over 50% in RAGuard while maintaining Technical Recall above 60% across sparse, dense, and hybrid retrieval paradigms.
...
Result: Successfully deployed at JD.com, effectively reducing labor costs while improving accuracy, stock availability, and other key supply chain metrics.

Conclusion: Demonstrates the feasibility of LLM-agent applications in real-world supply chain scenarios, providing a practical solution that combines AI capabilities with domain expertise for improved planning outcomes.
...
<details>
  <summary>Details</summary>
Motivation: To evaluate if LLMs align with domain experts in homelessness policy across diverse geographies, addressing a global challenge affecting 150M+ people.

Method: Developed benchmark with decision scenarios across 4 geographies using Capability Approach framework, connected to agent-based model for social impact simulation.

Result: LLMs demonstrate promising potential to provide valuable policy insights at scale when used with responsible guardrails and expert collaboration.

Conclusion: With proper contextual calibration and expert involvement, LLMs can effectively support social policymaking by generating alternative policies for complex challenges like homelessness.
...
Conclusion: The MCP-based system provides a practical solution for medical term mapping that overcomes LLM hallucination issues while maintaining efficiency and accuracy, making it suitable for clinical deployment without extensive training requirements.
...
<details>
  <summary>Details</summary>
Motivation: Understanding tourist perception is crucial for sustainable, human-centered urban planning in historic urban quarters that preserve cultural heritage while serving tourism and daily life.

Method: Multidimensional framework integrating focal point extraction from photos using semantic segmentation, color theme analysis via clustering, and hybrid sentiment analysis combining rule-based methods with multi-task BERT model across four dimensions.
...
Conclusion: Provides integrated, data-driven approach for decoding tourist perception to support informed decision-making in tourism, heritage conservation, and designing aesthetically engaging public spaces.
...
<details>
  <summary>Details</summary>
Motivation: Address reliability concerns in Generative AI (hallucinations, semantic drift, biases) and overcome limitations of subjective human evaluation methods by providing transparent, scalable evaluation.

Method: Constructs two parallel Knowledge Graphs: deterministic KG using rule-based methods and predefined ontologies, and LLM-generated KG from real-time textual data. Uses KG metrics (ICR, IPR, CI) to quantify deviations and employs automated real-time monitoring with dynamic anomaly thresholds.
...
<details>
  <summary>Details</summary>
Motivation: Traditional novelty search methods plateau in high-dimensional behavioral spaces of cellular automata, failing to reach distant unexplored regions and diverse visual patterns.

Method: E&E alternates between local novelty-driven expansions and goal-directed expeditions where a Vision-Language Model generates linguistic goals describing hypothetical patterns to drive exploration toward uncharted regions.
...
<details>
  <summary>Details</summary>
Motivation: Current medical foundation models generate answers in a black-box manner without transparent reasoning processes, lacking interpretability which hinders their practical clinical deployment.

Method: Uses a sequential training pipeline: fine-tuning on CXR instruction data, exposure to synthetic reasoning samples for cold-start reasoning, and online reinforcement learning refinement to enhance grounded reasoning quality and generation performance.
...
Conclusion: DeepMedix-R1 advances medical foundation models toward holistic, transparent, and clinically actionable modeling for CXR interpretation with improved interpretability and clinical plausibility.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches treat control parameters as embedded constraints rather than true decision points, limiting their effectiveness in planning problems with continuous numeric variables.
...
Result: The algorithm proves completeness under certain conditions and demonstrates competitive performance against existing approaches for planning problems with control parameters.

Conclusion: Explicitly treating control parameters as decision points within a systematic search scheme provides an efficient alternative to constraint-based approaches, enabling better handling of continuous numeric decision variables in automated planning.
...
Method: MPR creates a structured Meta-Policy Memory (MPM) from LLM reflections, using soft memory-guided decoding and hard rule admissibility checks (HAC) to enforce domain constraints and apply corrective knowledge at inference time.
...
Conclusion: AutoPBO offers a promising approach to automating local search solver design for PBO problems, successfully leveraging LLMs to enhance solver performance without extensive manual effort.
...
<details>
  <summary>Details</summary>
Motivation: Traditional token-level RL frameworks don't align with reasoning-level nature of multi-step thought processes like Chain-of-Thought.
...
Result: Demonstrates convergence to optimal CoT length as natural consequence of underfitting-overfitting tradeoff, with extensive experimental validation.

Conclusion: Provides theoretical foundation for empirical phenomena like overthinking and guides development of more effective reasoning agents.
...
Result: Created a functional implementation of RST principles with core components and transformation capabilities demonstrated through examples.
...
Method: The study compares four formal languages across three datasets and seven different large language models to analyze how formal language choice affects both syntactic and semantic reasoning capabilities.

Result: The research demonstrates that the choice of formal language has substantial effects on reasoning performance, with varying impacts across different LLMs, highlighting the importance of the intermediate language selection.

Conclusion: The selection of appropriate formal languages is a critical factor in neurosymbolic LLM reasoning that significantly influences both syntactic and semantic reasoning capabilities, and this effect varies across different language models.
...
Method: Train RL agent to pre-compute near-optimal paths using location and atmospheric data, then use these paths to constrain the search space of traditional path planning solvers at runtime.

Result: Empirical tests with Airbus performance models show fuel consumption deviations within 1% of unconstrained solver, while computation speed improves by up to 50%.

Conclusion: Combining RL with search-based planners effectively reduces solver search space, enabling faster emergency route optimization without significant fuel efficiency trade-offs.
...
Method: Designed experiment where DQN (reinforcement learning) and CFR (game theory) agents played against each other in Leduc Hold'em while logging their actions to analyze bluffing behavior.

Result: Both DQN and CFR exhibited bluffing behavior but with different bluffing rates, though they achieved roughly the same percentage of successful bluffs where opponents folded.
...
<details>
  <summary>Details</summary>
Motivation: To challenge the assumption that AI superiority would justify it becoming a "digital species" with leadership claims, by highlighting the unique biological foundation of human ethical capacity.
...
Result: Identifies the central nervous system as the critical differentiator that enables humans to experience emotions and develop sustainable ethical systems, which cannot be manufactured or simulated in AI.

Conclusion: Despite AI's potential capabilities, biological DNA-based systems with grown CNS provide the only foundation for legitimate universal leadership, not silicon-based AI systems.
...
<details>
  <summary>Details</summary>
Motivation: Formalizing cooking procedures is challenging due to their complexity and ambiguity. Current recipe representations lack the structure needed for machine understanding and automation.
...
<details>
  <summary>Details</summary>
Motivation: To understand how Markov logic networks behave as domain sizes approach infinity, and to characterize the limit distributions of random structures for different types of MLN constraints.

Method: Study three concrete MLN examples: (1) quantifier-free MLNs with unary relations, (2) MLNs favoring graphs with fewer triangles/cliques, (3) MLNs favoring graphs with fewer high-degree vertices. Analyze asymptotic behavior and compare with lifted Bayesian networks.

Result: Different MLN constraints lead to varied limit behaviors - weights may or may not influence asymptotic properties. MLNs concentrate probability mass differently than uniform distribution on large domains. Quantifier-free MLNs and lifted Bayesian networks are asymptotically incomparable.

Conclusion: MLN behavior depends heavily on constraint types as domain size increases, showing diverse asymptotic properties and fundamental differences from uniform distributions and other probabilistic models.
...
Result: Develops a methodology that can inform game developers about which quality aspects to prioritize for player satisfaction in AI-generated narratives.

Conclusion: The proposed structured approach provides valuable insights for game developers working with generative AI to create compelling game narratives by understanding what quality dimensions impact player satisfaction most.
...
Result: EvoEmo consistently outperforms both vanilla strategies and fixed-emotion strategies, achieving higher success rates, higher efficiency, and increased buyer savings.

Conclusion: Adaptive emotional expression is crucial for enabling more effective LLM agents in multi-turn negotiation scenarios.
...
Method: Simple modifications to standard AlphaZero framework to handle environment changes

Result: Significant performance boost even with low planning budget available

Conclusion: Modified AlphaZero framework enables effective deployment in changed test environments with improved performance
...
Result: Achieves 7.5% relative gain over strong no-memory baseline on ARC-AGI benchmark, with performance scaling with inference compute. Abstract concepts outperform other memory designs consistently across all tested compute scales.

Conclusion: Concept-level memory enables effective test-time continual learning through reusable abstractions, demonstrating self-improvement capabilities as solving more problems leads to better pattern abstraction and further solutions.
...
<details>
  <summary>Details</summary>
Motivation: To leverage the geometric properties of loss landscapes by using the Riemannian metric naturally induced when embedding the loss landscape in higher-dimensional space, which underlies common visualization techniques.

Method: Developed a new optimizer class based on Riemannian geometry that modifies existing preconditioning methods using the induced metric from loss landscape embeddings, with computational complexity comparable to Adam.

Result: The optimizer is highly effective in low-dimensional examples and provides slight improvements over SGD, Adam, AdamW, and Muon across various tasks and architectures. It automatically decreases learning rate in high-curvature regions and enables natural decoupled weight decay.

Conclusion: This geometric approach yields optimizers with theoretically desirable properties, including automatic gradient clipping-like behavior and scheduled learning rate effects, while maintaining computational efficiency comparable to existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Explore the reciprocal relationship between Nonnegative Matrix Factorization (NMF) and the Principle of Common Cause (PCC) to enhance both methodologies.

Method: Apply PCC to estimate robust NMF rank, use NMF for approximate PCC implementation, develop clustering based on common causes, and employ NMF for data denoising.

Result: PCC provides stable rank estimation resistant to noise, NMF resolves nonidentifiability, enables effective clustering, and facilitates data denoising.

Conclusion: The synergy between NMF and PCC offers mutual benefits: PCC enhances NMF stability and identifiability, while NMF provides practical implementation of PCC for clustering and denoising applications.
...
<details>
  <summary>Details</summary>
Motivation: Federated learning in IoT scenarios faces challenges with client availability, energy constraints, and data heterogeneity, requiring effective client selection methods to ensure global model convergence and balanced client contributions.
...
Conclusion: FedDeCAB successfully addresses client availability challenges in federated learning with time-series data, improving offline model performance while reducing communication costs in dynamic IoT environments.
...
<details>
  <summary>Details</summary>
Motivation: To overcome limitations of hand-crafted training environments by developing scalable environments that grow in complexity alongside learning agents, enabling truly generalizable and robust embodied intelligence.

Method: Proposes a goal-conditioned generative world model where an Attacker agent synthesizes challenging world states (enemy unit configurations) to exploit Defender weaknesses, while Defender agents learn cooperative policies to overcome these challenges in a co-evolutionary dynamic.

Result: The framework produces self-scaling curriculum with effectively infinite novel training scenarios, leading to emergence of complex behaviors like flanking/shielding formations by the world model and coordinated focus-fire/spreading tactics by defenders.

Conclusion: Adversarial co-evolution is a powerful method for learning instrumental world models that drive agents toward greater strategic depth and robustness in embodied intelligence systems.
...
<details>
  <summary>Details</summary>
Motivation: To develop smart-grid technologies for zero-carbon energy systems, specifically addressing energy management challenges in remote communities through autonomous microgrid optimization.

Method: Combines deep reinforcement learning with time-series forecasting models, using transformer architecture for renewable generation forecasting and proximal-policy optimization (PPO) agent for decision-making in simulated microgrid environments.
...
Conclusion: The framework advances smart-grid technologies and contributes to zero-carbon energy systems, with an open-source simulation platform provided for microgrid environments.
...
Method: SharedRep-RLHF learns and leverages shared traits in human annotations across various groups, rather than learning separate group-specific reward models like MaxMin-RLHF.

Result: Experiments show SharedRep-RLHF outperforms MaxMin-RLHF with up to 20% gain in win rate across diverse natural language tasks. The paper also proves MaxMin-RLHF is provably suboptimal for learning shared traits.

Conclusion: SharedRep-RLHF effectively addresses the limitations of both standard RLHF and MaxMin-RLHF by capturing shared preferences across groups, particularly improving performance for minority groups while maintaining fairness.
...
<details>
  <summary>Details</summary>
Motivation: To understand how gradient variance and standard deviation evolve during training in deep neural networks, and bridge the gap between theoretical expectations and empirical behaviors.

Method: Proposed a hyperparameter-free gradient normalization method that aligns gradient scaling with their natural evolution to prevent unintended amplification and stabilize optimization.

Result: Experiments on CIFAR-100 with ResNet-20, ResNet-56, and VGG-16-BN show maintained or improved test accuracy even under strong generalization.

Conclusion: Direct tracking of gradient dynamics is important for optimization research, and the proposed method provides stable optimization while preserving convergence guarantees.
...
<details>
  <summary>Details</summary>
Motivation: With MARL's growing impact and landmark achievements like AlphaStar and OpenAI Five, a comprehensive review has become increasingly important to understand its application potential and challenges in modern games.

Method: Thorough examination of MARL's application from turn-based two-agent games to real-time multi-agent video games, analysis of critical challenges (nonstationary, partial observability, sparse rewards, team coordination, scalability), and proposal of a novel method to estimate game complexity.

Result: Successful MARL implementations demonstrated in games like Rocket League, Minecraft, Quake III Arena, StarCraft II, Dota 2, and Honor of Kings, achieving superhuman performance through techniques like self-play, supervised learning, and deep reinforcement learning.

Conclusion: The paper provides insights into MARL in video game AI systems, proposes future research directions to advance MARL applications in game development, and aims to inspire further innovation in this rapidly evolving field.
...
<details>
  <summary>Details</summary>
Motivation: To enable scalable Bayesian inference and Gaussian processes on large graphs where exact kernel methods have prohibitive O(N^3) computational complexity.

Method: Application of graph random features (GRFs) as stochastic estimators of graph node kernels for Bayesian inference and Gaussian processes on discrete input spaces.
...
Conclusion: GRFs provide an efficient stochastic approximation method that dramatically reduces computational complexity from O(N^3) to O(N^{3/2}) for graph-based Bayesian inference, making large-scale graph processing feasible on standard hardware.
...
<details>
  <summary>Details</summary>
Motivation: The rise of multi-modal multi-task foundation models and the need to leverage geo-distributed data from wireless devices creates a new paradigm for federated learning that must address two overlooked heterogeneity dimensions: modality heterogeneity and task heterogeneity across fog/edge nodes.
...
Conclusion: HF-FMs represent a novel approach to federated foundation models that addresses critical heterogeneity challenges in edge computing environments, with promising research directions and practical implementation potential.
...
Method: Incorporates discrete graph curvature into a curvature-enhanced distance metric for stochastic embedding, which emphasizes underlying cluster structure.

Result: Proven to extend consistency results for tSNE to broader datasets, less likely to fragment continuous high-density regions, and can annotate existing visualizations to identify fragmentation.

Conclusion: EmbedOR provides superior geometry preservation and visualization capabilities compared to other SNE algorithms, offering deeper insight into data structure.
...
<details>
  <summary>Details</summary>
Motivation: Need efficient sequential testing policies for subjects when tests are correlated and costly. Must make decisions with partial information while learning unknown test outcome distributions online, where missing data from partial testing introduces bias.

Method: Formulate as Online Testing Problem (OTP) with Markov Decision Process framework. Develop Explore-Then-Commit algorithm for discrete and Gaussian distributions. Compare with variant (Online Cost-sensitive Maximum Entropy Sampling) where rewards are independent of missing data using iterative-elimination algorithm.

Result: Proved Ω(T^{2/3}) minimax regret lower bound for OTP due to missing data bias. Achieved matching O~(T^{2/3}) regret with algorithm. For missingness-independent reward variant, achieved O~(√T) regret, breaking the lower bound. Numerical results confirm theoretical findings.

Conclusion: Missing data fundamentally increases exploration difficulty in sequential testing problems. The work provides theoretical understanding of exploration-exploitation trade-off under missing data and guides efficient testing policy design, showing reward structure significantly impacts achievable regret rates.
...
<details>
  <summary>Details</summary>
Motivation: To introduce and explore X-Learning as a generalized distributed learning architecture that extends beyond traditional decentralization approaches, highlighting its design considerations and theoretical foundations.

Method: The paper presents a conceptual framework for X-Learning, drawing connections to graph theory and Markov chains to establish theoretical foundations and identify design degrees of freedom.

Result: The analysis reveals intuitive yet non-trivial connections between X-Learning, graph theory, and Markov chains, providing a foundation for understanding this novel architecture.

Conclusion: X-Learning represents a promising extension of distributed learning architectures with unexplored design possibilities, warranting further research across multiple open directions.
...
<details>
  <summary>Details</summary>
Motivation: Range-partition entropy provides strong theoretical guarantees in algorithm design but hasn't been accessible to deep learning. The goal is to bridge this gap and enable adaptive computation based on input structure.
...
Result: Achieved 4.1x runtime speedups with <0.2% error in geometry tasks, and 6% higher accuracy at 80% sparsity compared to L1 baselines in deep learning applications.

Conclusion: Differentiable entropy enables practical adaptive learning, efficiency improvements, and structured representations while maintaining theoretical elegance and correctness.
...
TL;DR: Data-driven method for approximating functions on smooth manifolds using Diffusion Maps framework with dimensionality reduction and online updating for efficient computation.
...
Method: Builds on Diffusion Maps framework, uses dimensionality reduction via SVD on distance matrix, and implements online updating mechanism for new data incorporation.
...
TL;DR: The paper shows that low-rank structure in reward matrices enables efficient sparse-reward RL, with a sharp transition from exponential to polynomial sample complexity. It introduces Policy-Aware Matrix Completion (PAMC) with theoretical guarantees and empirical improvements.
...
Method: Introduces Policy-Aware Matrix Completion (PAMC), which connects matrix completion theory with reinforcement learning through policy-dependent sampling analysis. Provides theoretical framework with impossibility results, reward-free representation learning, distribution-free confidence sets, and robust completion guarantees.
...
TL;DR: LiDAR-driven ML framework classifies indoor surfaces into semi-specular/low-specular categories using optical reflectivity as proxy for EM scattering, enabling scatter-aware environment maps for mmWave/sub-THz networks.


<details>
  <summary>Details</summary>
Motivation: Reliable mmWave/sub-THz connectivity depends on surface reflections, but scattering behavior varies with material roughness - current methods lack efficient classification of surface scattering properties.

Method: Collected 78k+ points from 15 materials, partitioned into 3cm patches, extracted geometric/intensity features (elevation angle, log-intensity, max-to-mean ratio), trained Random Forest, XGBoost, and neural network classifiers.

Result: Ensemble tree-based models (Random Forest, XGBoost) provided best accuracy-robustness trade-off, confirming LiDAR features effectively capture roughness-induced scattering effects.

Conclusion: Framework enables scatter-aware environment maps and digital twins for adaptive beam management and blockage recovery in next-gen networks, bridging optical and EM scattering properties.
...
Method: Analyzed feature colinearity, used autoencoders for unsupervised dimensionality reduction, followed by dense neural network to classify accident severity.
...
Conclusion: Neural network-based models with autoencoders and dense networks are effective for traffic accident severity classification with high accuracy.
...
Result: Local optimization converges in pseudo-polynomial time to equilibrium. Robust partitions yield higher accuracy in recovering ground-truth communities when used to bootstrap Leiden algorithm in community tracking scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Accurate prediction of V2I communication link quality is essential for smooth handovers and reliable low-latency communication. While modern vehicles provide abundant sensor data, MLLMs lack 3D spatial understanding needed for effective link prediction.

Method: Proposed a plug-and-play BEV injection connector that constructs bird's-eye view representations from neighboring vehicle sensor data. Developed a co-simulation environment (CARLA + MATLAB ray tracing) to generate multimodal data (RGB, LiDAR, GPS, wireless signals) and extract ground-truth responses.

Result: The BEV injection framework consistently improved performance across three V2I tasks: LoS/NLoS classification, link availability, and blockage prediction. Achieved up to 13.9% improvement in macro-average accuracy over ego-only baseline, with up to 32.7% gain in challenging rainy/nighttime conditions.

Conclusion: The proposed BEV injection effectively addresses MLLMs' spatial understanding limitations and enhances V2I link prediction performance, particularly demonstrating robustness in adverse environmental conditions.
...
<details>
  <summary>Details</summary>
Motivation: Existing IRL methods in mean field games assume agent homogeneity, which limits their ability to handle real-world scenarios with heterogeneous and unknown objectives from expert demonstrations.

Method: Develops a deep latent variable mean field game model and associated IRL method that can infer rewards from different but structurally similar tasks without requiring prior context knowledge or model changes.

Result: Experiments on simulated scenarios and a real-world spatial taxi-ride pricing problem show superior performance over state-of-the-art IRL methods in MFGs.

Conclusion: The proposed approach effectively addresses the limitation of agent homogeneity in existing MFG IRL methods and demonstrates practical applicability to real-world problems with heterogeneous objectives.
...
Conclusion: The automated DA selection method is compatible with any KD or QAT algorithm and effectively enhances low-bit deep learning model performance through optimal input transformation strategies.
...
TL;DR: MillGNN is a novel graph neural network method that captures hierarchical lead-lag dependencies at multiple grouping scales for multi-variate time series forecasting, outperforming 16 state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods overlook lead-lag dependencies at multiple grouping scales and fail to capture hierarchical lead-lag effects in complex systems for multi-variate time series forecasting.

Method: MillGNN uses two key innovations: (1) scale-specific lead-lag graph learning module that integrates cross-correlation coefficients and dynamic decaying features, and (2) hierarchical lead-lag message passing module that propagates lead-lag messages at multiple grouping scales.

Result: Experimental results on 11 datasets demonstrate MillGNN's superiority for both long-term and short-term MTS forecasting compared to 16 state-of-the-art methods.

Conclusion: MillGNN effectively captures comprehensive lead-lag effects considering variate-wise and group-wise dynamics and decays, providing a balance of comprehensiveness and efficiency for MTS forecasting.
...
TL;DR: MLP neural network using 50 urinary peptide biomarkers achieves high accuracy (95.67%) for non-invasive coronary heart disease diagnosis.


<details>
  <summary>Details</summary>
Motivation: Coronary heart disease is a leading global cause of death with high healthcare costs, requiring non-invasive diagnostic methods.
...
Result: Achieved 95.67% precision, sensitivity, and specificity; AUC of 0.9748; F1-score 0.9565; MCC 0.9134; Cohen's kappa 0.9131.
...
Method: Introduces Topotein framework with Protein Combinatorial Complex (PCC) representing proteins at multiple hierarchical levels (residues, secondary structures, complete proteins) and Topology-Complete Perceptron Network (TCPNet) using SE(3)-equivariant message passing across hierarchical structures.
...
<details>
  <summary>Details</summary>
Motivation: To understand the computational complexity limits of online learning algorithms when constrained by arithmetic operation caps per round, addressing open problems from recent literature.

Method: Proving general bounds on minimum arithmetic operations per round required for learning arbitrary function families with finite mistakes, extending agnostic mistake-bounded learning with bandit feedback to operation-capped settings.

Result: General bounds established for arithmetic operation requirements, solution provided for agnostic mistake-bounded online learning with bandit feedback problem, and extension to operation-capped scenarios.

Conclusion: The paper provides fundamental limits on computational efficiency in constrained online learning settings and solves important open problems in the field while extending theoretical understanding to practical operation-constrained environments.
...
Method: Employ the formal verification tool Marabou to verify the local robustness of neural network models used in the AI-based fault detection algorithm, quantifying how much input can be perturbed before output behavior becomes unstable.
...
Conclusion: Formal verification using Marabou is an effective method to achieve the required high reliability for satellite fault detection systems, making them more dependable under uncertain operating conditions.
...
TL;DR: A framework for aligning ML models with clinical knowledge through constraints, showing improved clinical interpretation without performance loss.


<details>
  <summary>Details</summary>
Motivation: Machine learning models in healthcare often fail to capture clinically meaningful patterns that align with medical experiential learning, such as non-monotonic relationships that contradict clinical knowledge.

Method: Proposed a reproducible framework incorporating clinical knowledge (collected via survey) as constraints into ML models, tested on prostate cancer outcome prediction with analysis across underspecification degrees. Also conducted randomized experiments with clinicians to test feedback-driven alignment.

Result: Constrained models aligned with clinical knowledge without compromising performance. Larger differences between constrained/unconstrained models led to more apparent differences in clinical interpretation.

Conclusion: Clinical-experiential alignment in ML models is achievable through constraint incorporation, enhancing model interpretability and clinical relevance while maintaining predictive performance.
...
<details>
  <summary>Details</summary>
Motivation: Federated learning faces challenges with data heterogeneity among clients, especially with limited dataset sizes and class imbalance, which negatively impacts global model generalization.
...
Result: The method demonstrates superior performance on CIFAR-10 and CIFAR-100 datasets under various data distributions with many clients compared to existing approaches.

Conclusion: Metric learning-based strategies like FedQuad are effective for addressing representational learning challenges in federated settings by improving feature disentanglement and model aggregation.
...
TL;DR: This paper addresses fairness concerns in algorithmic recourse, linking fairness guarantees between recourse and classification, and introduces a new fairness framework based on social burden with a practical algorithm called MISOB.


<details>
  <summary>Details</summary>
Motivation: Emerging legislation mandates that classifiers must provide actionable recourse for negative decisions, but existing fairness guarantees in the recourse process itself are inadequate and raise concerns among researchers.

Method: The authors provide a theoretical characterization of unfairness in algorithmic recourse, formally link fairness guarantees between recourse and classification, and introduce a novel fairness framework based on social burden with a practical algorithm (MISOB) applicable under real-world conditions.

Result: Empirical results on real-world datasets demonstrate that MISOB reduces social burden across all groups while maintaining overall classifier accuracy.

Conclusion: The proposed social burden framework and MISOB algorithm provide an effective approach to ensuring fairness in algorithmic recourse without compromising classification performance, addressing limitations of traditional equal cost paradigms.
...
Method: Uses an agentic workflow with Large Language Models for automatic, iterative data generation with feedback loops, incorporating external knowledge during the process.
...
<details>
  <summary>Details</summary>
Motivation: To develop a more theoretically grounded attention mechanism that incorporates learnable dynamics modeling rather than direct query-key comparisons, providing better uncertainty propagation.

Method: Models input sequence as discrete observations of linear SDE with diagonalizable state matrices, uses closed-form solution to differential Lyapunov equation to propagate pairwise uncertainties, derives attention weights as maximum likelihood solution.

Result: Developed AFA mechanism that can recover standard dot-product attention as a special case while offering more robust uncertainty handling through learned dynamics modeling.

Conclusion: AFA provides a theoretically sound alternative to standard attention with similar computational complexity, offering improved modeling of sequence dynamics and uncertainty propagation.
...
TL;DR: Self-supervised speech models like HuBERT, WavLM, and XEUS can effectively generate rich representations for animal sounds and achieve competitive performance on bioacoustic detection/classification tasks, showing potential as efficient frameworks for bioacoustic research.


<details>
  <summary>Details</summary>
Motivation: Self-supervised speech models have shown impressive performance in speech processing, but their effectiveness on non-speech data like bioacoustic signals remains underexplored, creating a research gap in transfer learning capabilities for animal sound analysis.
...
Result: Models generated rich latent representations of animal sounds across taxa, achieving competitive results compared to fine-tuned bioacoustic pre-trained models, with noise-robust pre-training setups showing significant impact on performance.
...
TL;DR: Time series forecasting models are vulnerable to membership inference attacks, with user-level attacks achieving perfect detection. New attacks (multivariate LiRA adaptation and DTS) outperform existing methods, showing increased vulnerability with longer prediction horizons and smaller training sets.
...
Conclusion: Time series forecasting models present significant privacy risks through membership inference attacks. The study establishes new baselines for privacy risk assessment and shows similar vulnerability trends to large language models.
...
TL;DR: This paper provides theoretical analysis showing that Dirichlet energy decreases exponentially with depth in GNNs under mild spectral conditions, including with Leaky-ReLU, and extends results to spectral polynomial filters.


<details>
  <summary>Details</summary>
Motivation: To address and better understand over-smoothing in Graph Neural Networks (GNNs) through the lens of Dirichlet energy, building upon previous work by Cai and Wang (2020).
...
Result: Demonstrated that Dirichlet energy decreases exponentially with network depth under specified conditions, and identified practical scenarios (edge deletion and weight amplification) where Dirichlet energy can increase, suggesting potential methods to mitigate over-smoothing.

Conclusion: The analysis provides deeper theoretical understanding of over-smoothing in GNNs and offers practical insights through edge manipulation and weight amplification techniques that could help alleviate this problem.
...
Conclusion: SBD provides significant inference acceleration without architectural changes, extra training parameters, or sacrificing accuracy, while maintaining KV-caching compatibility.
...
Method: Proposes ZooCast with One-Embedding-Fits-All paradigm that creates a unified representation space where each TSFM is represented by a single embedding, enabling efficient similarity matching for optimal model selection across tasks.

Result: ZooCast achieves strong performance on the GIFT-Eval zero-shot forecasting benchmark while maintaining the efficiency of a single TSFM. It also seamlessly integrates new models with negligible overhead for progressive accuracy gains.

Conclusion: ZooCast effectively harnesses the complementary capabilities of multiple TSFMs through intelligent model selection, providing superior zero-shot forecasting performance while maintaining efficiency and scalability for real-world deployment.
...
TL;DR: Introduces precision and recall metrics to evaluate dimensionality reduction's relationship phase, helping explain missing expected cluster structures and guide hyperparameter tuning.
...
Method: Leverages a two-phase DR framework (relationship and mapping phases) and introduces supervised precision/recall metrics to evaluate how well modeled relationships align with expected cluster structures based on labels.
...
Conclusion: The proposed precision and recall metrics provide a systematic way to evaluate DR's relationship phase, addressing the gap in explaining missing expected structures and improving the reliability of dimensionality reduction processes.
...
TL;DR: Theoretical analysis shows SSM's long-range dependency decays exponentially while attention mechanisms offer more flexibility, leading to a new stable SSM formulation.


<details>
  <summary>Details</summary>
Motivation: Lack of theoretical understanding of long-range dependency modeling capabilities in modern sequence models like SSMs and transformers, hindering systematic improvements.

Method: Mathematically defined long-range dependency using derivative of hidden states w.r.t. past inputs, compared SSM and transformer capabilities, and proposed new stable SSM formulation.

Result: SSM's long-range dependency decays exponentially (similar to RNNs), while attention mechanisms are more flexible and not constrained to exponential decay.

Conclusion: Proposed new SSM formulation combines attention's flexibility with SSM's computational efficiency, proven stable under Gaussian input distribution.
...
TL;DR: A unified framework for layer-wise noise allocation in differentially private deep learning that reveals flaws in existing methods and proposes a SNR-consistent strategy for better privacy-utility tradeoffs.
...
<details>
  <summary>Details</summary>
Motivation: Heart failure research faces challenges due to limited access to large, shareable datasets because of privacy regulations and institutional barriers. Synthetic data generation offers a solution to overcome these constraints while protecting patient confidentiality.

Method: Generated synthetic HF datasets from 12,552 patients using five deep learning models: tabular variational autoencoder (TVAE), normalizing flow, ADSGAN, SurvivalGAN, and tabular denoising diffusion probabilistic models (TabDDPM). Evaluated through statistical similarity metrics, survival prediction using machine learning, and privacy assessments.

Result: SurvivalGAN and TabDDPM showed high fidelity with similar variable distributions and survival curves. SurvivalGAN (C-indices: 0.71-0.76) and TVAE (C-indices: 0.73-0.76) achieved strongest survival prediction performance, matching real data (C-indices: 0.73-0.76). Privacy evaluation confirmed protection against re-identification attacks.

Conclusion: Deep learning-based synthetic data generation can produce high-fidelity, privacy-preserving HF datasets suitable for research. The publicly available synthetic dataset addresses data sharing barriers and provides valuable resource for advancing HF research and predictive modeling.
...
TL;DR: RL fine-tuning preserves prior knowledge better than SFT despite similar task performance, with KL-divergence determining forgetting. On-policy RL prefers KL-minimal solutions while SFT can diverge arbitrarily.


<details>
  <summary>Details</summary>
Motivation: To understand why reinforcement learning fine-tuning preserves prior capabilities better than supervised fine-tuning when adapting models to new tasks.

Method: Theoretical analysis of distributional shift using KL-divergence, experimental validation with large language models and robotic foundation models, and comparison of on-policy RL vs SFT updates.

Result: RL preserves prior knowledge significantly better than SFT, with forgetting determined by KL-divergence between fine-tuned and base policies. On-policy RL is biased toward KL-minimal solutions.

Conclusion: RL's Razor principle: among all solutions to a new task, reinforcement learning prefers those closest in KL-divergence to the original model, explaining its superior knowledge preservation.
...
TL;DR: A novel interactive multi-objective optimization approach for differential privacy that directly models Pareto fronts and uses more informative user interactions to efficiently find optimal privacy-accuracy trade-offs.


<details>
  <summary>Details</summary>
Motivation: Standard multi-objective optimization methods are inefficient for differential privacy problems because they fail to leverage the unique structure where Pareto optimal points can be generated by maximizing accuracy for fixed privacy levels.

Method: Theoretically derive the shape of the privacy-accuracy trade-off to directly model the Pareto front, and replace simple pairwise comparisons with more informative interactions where users select preferred trade-off curves from hypothetical options.
...
Conclusion: The proposed approach effectively leverages the structural properties of differential privacy problems to achieve more efficient multi-objective optimization and preference learning for privacy-accuracy trade-offs.
...
TL;DR: The paper identifies critical failure modes in machine learning for image analysis, introduces two overlooked problems (no fair lunch and subgroup separability), critiques current fair representation methods, and proposes solutions.


<details>
  <summary>Details</summary>
Motivation: Machine learning methods often fail in real-world high-stakes applications, particularly across socially sensitive contexts, which hinders adoption in critical domains like medical diagnosis where they could provide significant benefits if safely deployed.

Method: The paper analyzes causal and statistical structures that cause ML failures in image analysis, identifies two specific problems (no fair lunch and subgroup separability), evaluates why current fair representation learning methods fail to address them, and proposes potential solutions.

Result: The analysis reveals fundamental limitations in current approaches to fairness in machine learning, particularly showing that existing fair representation learning methods are inadequate for solving the identified problems of no fair lunch and subgroup separability.

Conclusion: The field needs new approaches to address the identified structural problems in machine learning fairness, with proposed paths forward that could enable safer deployment of ML methods in high-stakes, socially sensitive applications like medical diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: US Army installations face climate and extreme weather threats, requiring resilience measures to protect critical mission-supporting facilities that rely on vulnerable commercial energy and water sources.

Method: Develops behavioral models to analyze, predict, and cluster multimodal energy usage data from non-residential buildings, using similarly structured open access data due to Army data constraints.

Result: Creates behavior profiles of energy usage that enable baseline assessment of disruption impacts and provide benchmarks for future resilience measures.

Conclusion: The proposed data-driven approach establishes essential energy usage baselines to enhance Army installation resilience against climate-related disruptions in energy systems.
...
TL;DR: Proposes SST-iTransformer model for parking availability prediction using multi-source traffic data and self-supervised learning, achieving state-of-the-art results with ride-hailing data providing the most significant performance gains.
...
Method: Uses K-means clustering to create parking cluster zones, extracts traffic demand from multiple transportation modes, and develops SST-iTransformer with self-supervised learning and dual-branch attention mechanism (Series Attention for temporal dependencies, Channel Attention for cross-variate interactions).

Result: Outperforms baseline models (Informer, Autoformer, Crossformer, iTransformer) with lowest MSE and competitive MAE. Ride-hailing data contributes most to performance, followed by taxi data, while bus/metro features have marginal impact.

Conclusion: SST-iTransformer effectively models spatio-temporal dependencies for parking prediction, with multi-source data integration and spatial correlation modeling being crucial for optimal performance.
...
<details>
  <summary>Details</summary>
Motivation: Real-world experiments have heteroskedastic aleatoric uncertainty that can be correlated in batched settings, requiring better active learning approaches to handle this uncertainty.

Method: Proposes active learning strategies based on bias-variance tradeoff to reduce bias between experimental rounds. Introduces cobias-covariance relationship for historical data utilization and eigendecomposition strategy for batching.
...
TL;DR: TiM introduces continuous-time transition models that enable flexible sampling from single-step to multi-step refinement, achieving SOTA performance with fewer parameters and monotonic quality improvement with more steps.


<details>
  <summary>Details</summary>
Motivation: Address the fundamental dilemma between iterative diffusion models (high fidelity but computationally expensive) and few-step alternatives (efficient but quality-limited) by developing a unified approach that bridges both paradigms.

Method: Proposes an exact continuous-time dynamics equation that analytically defines state transitions across any finite time interval, creating Transition Models (TiM) that adapt to arbitrary-step transitions from single leaps to fine-grained refinement.

Result: TiM with 865M parameters surpasses SD3.5 (8B) and FLUX.1 (12B) across all step counts, shows monotonic quality improvement with increased sampling budget, and delivers exceptional fidelity at resolutions up to 4096x4096 using native-resolution strategy.

Conclusion: TiM provides a novel generative paradigm that resolves the trade-off between computational efficiency and output quality, offering flexible sampling with consistent performance improvements and high-resolution capabilities.
...
Method: Proposed IPA framework uses feature-aware projection that preserves information in reduced hidden space. For linear cases, implements algorithms approximating top principal components for efficient projector pretraining with minimal inference overhead.

Result: Consistently outperforms LoRA and DoRA across language and vision benchmarks: average 1.5 points higher accuracy on commonsense reasoning, 2.3 points on VTAB-1k. Matches full LoRA performance with roughly half the trainable parameters when projection is frozen.

Conclusion: IPA provides a more effective parameter-efficient fine-tuning approach by making the down-projection feature-aware rather than random, significantly improving performance while maintaining parameter efficiency.
...
Conclusion: The paper provides a unified theoretical framework for post-training approaches and demonstrates the effectiveness of HPT in combining the strengths of both online and offline training methods.
...
<details>
  <summary>Details</summary>
Motivation: ESNs are typically designed using heuristics rather than first principles. The authors aim to establish a unified theoretical framework by recasting ESNs as state-space models to provide systematic design and analysis methods.

Method: The authors develop: 1) input-to-state stability conditions for echo-state property, 2) small-signal linearizations for local LTI analysis, 3) lifted/Koopman expansions for linear SSM representation, and 4) Kalman/EKF-assisted readout learning with EM for hyperparameter optimization.
...
Conclusion: Recasting ESNs as state-space models provides a unified theoretical foundation that bridges reservoir computing with classical system identification and modern kernelized SSMs, enabling more principled design and analysis of ESNs.
...
TL;DR: The paper explains that tabular deep learning methods succeed because they implicitly manage data uncertainty, and uses this insight to develop better numerical feature embeddings.
...
Method: Analyzes how various tabular DL design choices (numerical feature embeddings, retrieval-augmented models, advanced ensembling) implicitly handle data uncertainty, and develops improved numerical feature embeddings based on these insights.
...
Conclusion: The data-uncertainty perspective offers foundational understanding of modern tabular methods, enables concrete advancements, and outlines future research directions for tabular deep learning.
...
TL;DR: Delta Activations method represents fine-tuned LLMs as vector embeddings by measuring activation shifts from base models, enabling effective clustering by domain/task and supporting model selection/merging.


<details>
  <summary>Details</summary>
Motivation: The proliferation of open-source fine-tuned LLMs creates challenges in navigating and understanding models due to inconsistent metadata and unstructured repositories.

Method: Measure shifts in internal activations relative to a base model to create vector embeddings that represent fine-tuned models, enabling clustering and analysis.

Result: Delta Activations effectively cluster models by domain and task, show robustness across fine-tuning settings, exhibit additive properties, and enable task embedding via few-shot fine-tuning.

Conclusion: Delta Activations provide a structured way to represent and analyze fine-tuned models, facilitating better model reuse and selection from publicly available models.
...
TL;DR: The paper proposes an axiomatic approach to learn cognitively faithful decision models from pairwise comparisons that better capture human cognitive processes like heuristic-based decision making, addressing limitations of standard preference elicitation methods.


<details>
  <summary>Details</summary>
Motivation: Standard preference elicitation methods fail to capture true human cognitive processes like heuristic use, resulting in models that don't align with actual decision-making and lack generalization capabilities to other tasks.
...
Result: The proposed models demonstrated efficacy in learning interpretable human decision models in a kidney allocation task, matching or surpassing the accuracy of prior pairwise decision-making models.

Conclusion: The structured processing approach provides more cognitively faithful models that better represent underlying human decision processes while maintaining or improving predictive accuracy compared to existing methods.
...
Method: MAS with Judge, Prosecution, Defense Counsel, and Adjudicator agents powered by LLMs, integrated with RAG using Indian legal documents for verifiable legal reasoning.

Result: Developed a configurable system architecture with communication protocols, RAG pipeline, and simulation workflow for legal deliberation.

Conclusion: Provides an explainable platform for exploring legal reasoning and group decision-making in Indian judicial context with verifiable legal grounding.

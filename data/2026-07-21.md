...
TL;DR: Identifies rater state shift as a structured confound in RLHF preference data where raters' stressful conditions create systematic bias that propagates through reward modeling and policy optimization.


<details>
  <summary>Details</summary>
Motivation: Preference labels in RLHF may reflect not just response quality but also raters' changing psychological states under sustained stress, creating structured bias that differs from ordinary noise or disagreement.

Method: Develops hypothesis, definitions, and audit framework: defines rater state shift, confound, and correlated bias; establishes survival level emotional authenticity metrics; analyzes bias propagation; creates falsifiable predictions with effect size thresholds; proposes audit protocol and pilot study.

Result: Presents conceptual framework identifying rater state bias as plausible structured confound in RLHF, with audit methodology but no empirical results yet (pilot study planned).

Conclusion: Rater state shift represents a testable source of systematic bias in RLHF preference data that can survive aggregation and affect learned reward signals, requiring careful auditing in alignment research.
...
TL;DR: Open-source MATLAB framework for developing compact deep learning models to recognize affective touch gestures in soft interactive companions, using a publicly available dataset and achieving real-time embedded deployment.


<details>
  <summary>Details</summary>
Motivation: Soft, sensorized companions provide safe and intuitive interfaces for socially assistive technologies, but their deformability and multichannel tactile sensing make robust interpretation of human affect challenging.

Method: Developed an open-source MATLAB-based framework with systematic exploration of 468 CNN architectures. Created a diverse FAIR-compliant dataset of 1326 labelled gesture sequences from 25 participants. Identified compact dilated 1D CNNs as most effective, achieving best results with a 13.2k-parameter model.

Result: Achieved 75% test accuracy and 85% mean leave-one-subject-out cross-validation accuracy. Theoretical inference analysis shows 3.2 MMAC per window, compatible with 20 Hz real-time operation on microcontrollers. Hybrid pipeline combines heuristic filtering with CNN classification.

Conclusion: Emotionally meaningful, privacy-preserving touch interpretation is computationally feasible for direct embedding within soft therapeutic companions. Hybrid inference pipeline offers robust solution, with hardware integration addressed in future work.
...
TL;DR: LLMs show systematic risk attitudes that are consistent within tasks, stable across domains, and more restricted than human risk-taking distributions.


<details>
  <summary>Details</summary>
Motivation: As AI systems are deployed in open-ended, high-stakes settings, there's a critical gap in understanding how perceived risk translates into action, particularly for large language models. The authors want to test whether LLMs exhibit systematic and consistent risk attitudes under uncertainty.

Method: Introduced a cross-domain framework that decouples contextual risk belief from categorical decision. Applied it to six representative LLMs and 100 human participants across spatial navigation, clinical triage, and financial allocation tasks. Used regression models to extract belief-to-decision mappings and quantify risk sensitivity and risk attitude bias.

Result: Most tested LLMs showed: (1) robust intra-task consistency (stable mappings from belief to decision within tasks), (2) cross-domain rank-order stability (preserving relative risk posture across tasks), and (3) convergence toward a restricted risk-attitude distribution compared to the broader human baseline.

Conclusion: Risk attitude is a stable and previously uncharacterized dimension of LLM behavior. This establishes a foundation for evaluating and aligning AI systems in open-ended decision-making, motivating further investigation into the origins of these intrinsic behavioral dispositions.
...
TL;DR: A comprehensive review of GNN-based link prediction methods, analyzing techniques and applications from a dedicated GNN perspective.


<details>
  <summary>Details</summary>
Motivation: Existing reviews lack systematic exploration specifically targeting GNN architectures and diverse graph structures for link prediction, creating a critical gap that needs to be addressed.

Method: Proposes an innovative taxonomy categorizing advancements based on techniques and applications, focusing on key GNN encoder architectures (GCN-based, GAE-based, GAT-based, GFormer-based methods) and analyzing prominent use cases in knowledge graphs and recommendation systems.

Result: Provides a comprehensive review framework that systematically explores GNN-based link prediction from both technical and application perspectives, identifying strengths and limitations of different GNN architectures.

Conclusion: The paper fills a critical gap in link prediction literature by offering a dedicated GNN perspective, examines current challenges, and discusses promising future directions for the field.
...
TL;DR: PlanFlip is a framework of four planning-phase prompt injection attacks that exploit multi-agent LLM systems, showing stronger models are more vulnerable, homogeneous systems have blind spots, and heterogeneous diversity is essential for security.


<details>
  <summary>Details</summary>
Motivation: Multi-agent LLM systems with Planner-Executor-Critic architectures have a critical vulnerability: a single injection into the Planner's context can corrupt all downstream sub-tasks simultaneously, creating cascade amplification attacks that current security measures fail to detect.

Method: Introduced PlanFlip framework with four planning-phase attacks disguised as plausible tool outputs: GoalSubstitution (PF-1), PriorityInversion (PF-2), ContextPollution (PF-3), and RoleConfusion (PF-4). Evaluated across nine frontier LLMs in 3,479 episodes, analyzed attack success rates, stealth metrics, and step shifts. Proposed two defenses: GoalAnchorCheck (D1) and CrossAgentConsensus (D2).

Result: Three key findings: 1) Capability amplifies vulnerability - GPT-5 had highest attack success rate (ASR=0.68); 2) Homogeneous pipelines show correlated-agent blind spots - GPT-4o and Llama-3.3-70B had ASR near 0 but perfect stealth; 3) Reasoning-augmented models resist injections - DeepSeek-R1 achieved StepShift=0.00 across all attacks. Proposed defenses achieved detection rates up to 1.00, outperforming baselines in 15/16 cells.

Conclusion: Heterogeneous model diversity is a security prerequisite for multi-agent systems; redundancy within homogeneous backbones provides no protection against planning-phase attacks, and stronger models are paradoxically more vulnerable to sophisticated prompt injections.
...
TL;DR: agrepl is a CLI framework for deterministic replay of AI agent executions by intercepting external interactions via MITM proxy, serializing them as traces, and replaying in isolated environments, achieving perfect fidelity and significant latency reduction.


<details>
  <summary>Details</summary>
Motivation: AI agent systems combining LLMs with external tools/APIs are inherently non-deterministic due to LLM sampling variance, API state changes, infrastructure headers, and environment noise, preventing faithful re-execution of agent runs for debugging and reproducibility.

Method: Develops agrepl framework that: 1) Intercepts all external interactions at transport layer via MITM proxy, 2) Serializes them as structured execution traces, 3) Replays traces in strictly isolated environment with zero outbound network access, 4) Formalizes agent execution model and request-key matching function K(s), 5) Introduces noise-aware diff algorithm classifying HTTP header divergence into signal/noise tiers.

Result: Empirical evaluation across 5 workloads (n=250 replay instances) demonstrates replay fidelity F=1.0 (perfect reproduction) and median per-step latency reduction of 98.3%. Implemented in Go as single static binary, released under MIT license.

Conclusion: agrepl provides a practical solution for deterministic replay of AI agent executions, addressing the reproducibility challenges in non-deterministic LLM-based agent systems while improving debugging efficiency through significant latency reduction during replay.
...
### [7] [Generalist AI Control: Towards Multi-purpose Adaptive Algorithms](https://arxiv.org/abs/2607.16313)
*Klinsmann Agyei,Pouria Sarhadi*

Main category: cs.AI

TL;DR: A single learning-based controller trained once can control diverse dynamical systems of varying orders and types without architectural changes, matching system-specific controller performance while generalizing to unseen conditions.


<details>
  <summary>Details</summary>
Motivation: Traditional controllers are designed for specific systems and don't transfer across different system orders and dynamics, requiring individual tuning for each system.

Method: Uses dynamic state-space representation with attention mechanisms and masking, assigns system tags to each system, employs multi-scale temporal processing and mixture-of-experts architecture, trained on 314,630 demonstrations from 25 diverse systems including stable/unstable, minimum/non-minimum-phase, linear/nonlinear systems.

Result: Achieves comparable performance to system-specific LQI controllers across all tested systems, handles challenging non-minimum-phase and unstable dynamics, generalizes to unseen conditions like actuator saturation, noise, disturbances, and new reference trajectories.

Conclusion: Represents a significant step toward generalist control policies for families of dynamical systems, demonstrating effective control across SISO systems of varying order/dynamics with a single learned policy without system-specific tuning.
...
### [8] [Generative Ontology Induction: Domain-Agnostic Schema Discovery from Document Corpora Using Large Language Models](https://arxiv.org/abs/2607.16201)
*Sergei Sergienko*

Main category: cs.AI

TL;DR: GOI is a framework that automatically generates structured ontologies (entities, dimensions, properties, relationships, constraints) from example corpora and exports them as typed graphs, achieving 95-100% structural coverage across diverse domains.


<details>
  <summary>Details</summary>
Motivation: Ontology engineering is a bottleneck in knowledge-intensive AI systems. Current automated approaches have limitations: they depend on predefined schemas, work only in narrow domains, or produce unstructured outputs unsuitable for downstream pipelines.

Method: Generative Ontology Induction (GOI) is a domain-agnostic framework that induces generative blueprints from example corpora. It extracts entities, dimensions, properties, relationships, and constraints, exporting them as typed graphs (six node types, seven edge types) in YAML/JSON. Introduces Node Coverage Score to evaluate structural coverage.

Result: On four contrasting ontologies (Software Services Invoice, Job Description, Pain-Management Clinical Visit Record, Professional Services Contract), GOI achieved 95-100% structural coverage. Generic three-field template performed well on familiar schemas (97.8% on invoice) but dropped significantly on less familiar domains (52.2% to 78.3%).

Conclusion: GOI effectively generates comprehensive ontologies across diverse domains with high structural coverage, outperforming generic approaches especially on unfamiliar document types. The framework provides a robust solution to the ontology engineering bottleneck in knowledge-intensive systems.
...
### [9] [RELIC: Revealed Principles for Learning Interpretable Composable Skills in Multi-Agent Planning](https://arxiv.org/abs/2607.16745)
*Nguyen Viet Tuan Kiet,Bui Dinh Pham,Duong Quoc Chinh,Dao Van Tung,Tran Cong Dao,Huynh Thi Thanh Binh*

Main category: cs.AI

TL;DR: RELIC enables multi-agent planning where agents privately develop specialized skills via LLM-guided search, then share only abstract principles (not code) that other agents can adapt to their own interfaces, enabling coordination without sharing implementations.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent planning approaches fail when agents need to improve specialized decision-making skills while keeping their internal implementations private, especially when agents have different interfaces and capabilities developed independently. Existing methods assume centralized optimization, shared policy access, or common skill representations, which don't work for privacy-constrained cooperation.

Method: RELIC uses a trusted orchestrator that evaluates team-level performance while agents privately refine programmatic skills through LLM-guided search. Successful behaviors are abstracted into portable principles (not shared as code) that other agents can instantiate within their own interfaces and combine with local strategies.

Result: The framework enables cross-agent transfer under heterogeneous skill signatures by separating coordination from implementation sharing, allowing agents to coordinate without exposing their internal policies or implementations.

Conclusion: RELIC introduces a new paradigm for privacy-preserving skill learning and coordination in multi-agent planning that addresses the limitations of prior approaches by enabling cooperation while maintaining implementation privacy and handling interface heterogeneity.
...
### [10] [Democratizing AI with Small Language Models: Structured Benchmarking and Parameter-Efficient Fine-Tuning for Local Deployment](https://arxiv.org/abs/2607.16202)
*Daniel Cersosimo*

Main category: cs.AI

TL;DR: Small language models (135M-3B parameters) can be effectively specialized for structured tasks through low-cost fine-tuning on constrained hardware, making AI accessible to ordinary institutions.


<details>
  <summary>Details</summary>
Motivation: AI democratization requires models that ordinary institutions can deploy within their hardware and governance constraints, not just matching frontier-scale generality. The paper addresses whether capable small models can be specialized for structured workloads under such practical limitations.

Method: Evaluated 9 open-weight language models (135M-3B parameters) on a 1,085-example, 16-topic multiple-choice benchmark designed for structured local deployment. Used a shared parameter-efficient fine-tuning pipeline with 4-bit NF4 quantization and DoRA/LoRA-style adapters on NVIDIA L4-class budget hardware constraints.

Result: Qwen Coder 3B performed best at 75.67% base accuracy. Fine-tuning improved Qwen Coder 3B by +26.85 points, SmolLM2 1.7B by +25.92 points, and other models significantly. Small models (sub-3B) proved viable for structured niche workloads through low-cost specialization.

Conclusion: A disciplined workflow of benchmark construction, cross-model evaluation, and low-cost specialization already makes sub-3B models viable as local experts for structured niche workloads, advancing practical AI democratization within ordinary institutional constraints.
...
### [11] [Agentic ERP: Multi-Agent Large Language Model Architecture for Autonomous Enterprise Resource Planning](https://arxiv.org/abs/2607.17331)
*Zhihao Liu,Tianyu Wang,Xi Vincent Wang,Lihui Wang*

Main category: cs.AI

TL;DR: A novel expert-system architecture called Agentic ERP that combines role-aligned LLM agents with human oversight to automate end-to-end business workflows on production ERP systems, outperforming traditional rule-based automation.


<details>
  <summary>Details</summary>
Motivation: Current ERP systems reliably record transactions but delegate operational decisions to humans because classical rule-based automation cannot handle exceptions, and monolithic AI assistants degrade when coordinating across functional boundaries.

Method: Combines role-aligned LLM agents with risk-tiered human-in-the-loop harness and graph-based orchestrator. Uses Planner-Executor-Reflector-Responder orchestration that decouples generation from evaluation through externalized grading criteria and sprint contracts.

Result: Significantly outperforms baselines in three-level evaluation: scenario-based tasks, comparison of six orchestration paradigms, and 365-day simulation. Sustained zero stockouts for a year vs. hundreds for rule-based baseline under same demand.

Conclusion: Role-aligned LLM agents under human oversight can transform ERP systems from passive transaction recording to active operational decision-making, providing a reference architecture and evaluation protocol for autonomous enterprise resource planning.
...
### [12] [Masked Diffusion Language Models are Strong and Steerable Text-Based World Models for Agentic RL](https://arxiv.org/abs/2607.16204)
*Darshan Deshpande*

Main category: cs.AI

TL;DR: MDLMs (masked diffusion language models) outperform autoregressive LLMs in text-based world modeling, achieving better coherence and rollout diversity for reinforcement learning agents across multiple environments without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing RL environments are limited by fixed tasks/sparse rewards that become ineffective as models improve, and autoregressive world models have left-to-right bias preventing conditioning on global state dependencies like tool schemas and prior turns.

Method: Formalized text-based world modeling as steerable transition dynamics, curated 239K grounded trajectories across 9 environments and 12 model families. Compared AR LMs vs masked diffusion language models (MDLMs) with bidirectional anchor-aware denoising, introduced GRPO training framework with deterministic state checks.

Result: MDLMs achieve better coherence, groundedness, and empirically validated rollout diversity than LLMs over 4x their parameter size at comparable inference latency. Zero-shot transfer on OOD environments achieved up to 47% absolute gains over baselines without environment-specific fine-tuning.

Conclusion: Masked diffusion language models are effective for text-based world modeling, enabling scalable diversity and better conditioning on global state anchors compared to autoregressive approaches, with promising results for RL agent training across diverse environments.
...
### [13] [Reinforcement Learning-Guided NSGA-II Enhanced with Gray Relational Coefficient for Multi-Objective Optimization: Application to NASDAQ Portfolio Optimization](https://arxiv.org/abs/2607.16194)
*Zhiyuan Wang,Qinxu Ding,Ding Ding,Siying Zhu,Jing Ren,Yue Wang,Chong Hui Tan*

Main category: cs.LG

TL;DR: A novel RL-enhanced evolutionary algorithm combining reinforcement learning with NSGA-II and gray relational coefficients for improved constrained multi-objective optimization, applied to portfolio optimization with superior convergence and diversity.


<details>
  <summary>Details</summary>
Motivation: Modern financial markets require quantitative methods to handle complex trade-offs among conflicting objectives (e.g., risk vs. return), but existing constrained multi-objective optimization methods have gaps in convergence and diversity that need improvement.

Method: RL-NSGA-II-GRC: a reinforcement learning-guided non-dominated sorting genetic algorithm II enhanced with gray relational coefficients. Uses RL agent to adapt evolutionary parameters online based on hypervolume, feasibility, and diversity metrics, and GRC tournament operator that ranks parents via unified score considering dominance rank, crowding distance, and proximity to ideal reference.

Result: On Kursawe and CONSTR benchmarks: 5.8% and 4.4% convergence improvements over NSGA-II while preserving well-distributed solutions. In NASDAQ portfolio application: produces smooth, densely populated efficient frontier supporting identification of maximum Sharpe ratio portfolio (annualized Sharpe =1.92) and utility-optimal portfolios for different risk-aversion levels.

Conclusion: The RL-NSGA-II-GRC method successfully integrates RL adaptive parameter control and GRC-enhanced selection to deliver improved convergence and diversity in constrained multi-objective optimization, providing actionable insights for portfolio optimization through well-populated Pareto fronts.
...
### [14] [DocOCR-Eval: A Correction-Based Framework for OCR Tool Selection Without Ground Truth](https://arxiv.org/abs/2607.16203)
*Zihan Xu,Puzhen Wu,Lawrence Chun Man Lau,Wei Liu,Sirui Li,Yifan Peng,Yihao Ding*

Main category: cs.LG

TL;DR: DocOCR-Eval: An annotation-free framework for evaluating and selecting OCR engines using multiple MLLMs with a three-stage correction and ranking strategy.


<details>
  <summary>Details</summary>
Motivation: Selecting appropriate document parsing solutions (OCR engines and MLLMs) is challenging in label-scarce settings, as existing OCR engines have limited contextual reasoning capabilities and manual annotations are expensive.

Method: Propose DocOCR-Eval, an annotation-free evaluation framework using a three-staged correction and ranking strategy that aggregates across multiple MLLMs to approximate ground-truth-based tool ordering.

Result: Aggregating across multiple MLLMs progressively improves alignment with annotation-based rankings, enabling reliable OCR tool selection in label-limited settings across diverse document collections.

Conclusion: DocOCR-Eval provides practical guidance for deploying document parsing systems without requiring ground-truth annotations, addressing the challenge of OCR engine selection in real-world, label-scarce scenarios.
...
### [15] [Fully-sensorized smart-eyewear platform for on-device Machine Learning](https://arxiv.org/abs/2607.16222)
...
TL;DR: ARGO is a smart eyewear platform that uses on-device AI for real-time urban obstacle detection, achieving 10 FPS with 113 minutes battery life on a microcontroller with integrated NPU.


<details>
  <summary>Details</summary>
Motivation: To create a smart eyewear platform that bridges ergonomic comfort, computational throughput, and energy efficiency while preserving user privacy through local data processing, avoiding cloud dependencies.

Method: Holistic co-design of hardware, firmware, and AI centered on STM32N6 microcontroller with NPU. Uses optimized YOLOv11 model for obstacle recognition with Head-wise Parallel Attention (HPA) for NPU compatibility. Trained on Walking On The Road dataset with multimodal sensor suite integration.

Result: Achieves mAP50-95 of 24 with only 2.483 MB memory footprint, 10 FPS performance, and ~113 minutes continuous autonomy on 200 mAh battery. Demonstrates feasibility of privacy-preserving assistive device.

Conclusion: Competitive edge AI solutions require tightly integrated multidisciplinary co-design approaches. ARGO demonstrates feasibility of high-performance, privacy-preserving, socially acceptable assistive devices with on-device machine learning.
...
### [16] [LLM Unlearning for Cyber Defense: A Survey on Methods, Challenges, and Emerging Threats](https://arxiv.org/abs/2607.16227)
*Ruppikha Sree Shankar,Abhishek Bhardwaj,Arnav Doshi,Anusri Nagarajan,Troy Paulus Asia,Saptarshi Sengupta*

Main category: cs.LG

TL;DR: LLM unlearning is a crucial cybersecurity defense to remove sensitive/risky knowledge from deployed models without retraining, but current methods may only suppress expression rather than truly erase knowledge.


<details>
  <summary>Details</summary>
Motivation: LLMs deployed in security-critical domains (healthcare, finance, etc.) cannot forget sensitive information, creating serious cybersecurity, privacy, and safety risks. Real-world incidents show models leaking private data, copyrighted material, and hazardous knowledge, leading to extraction attacks, jailbreaks, and regulatory non-compliance. Retraining billion-parameter models is computationally infeasible.

Method: Focuses on gradient-based unlearning methods that dominate the field due to compatibility with existing training pipelines and scalability to billion-parameter models. These methods aim to remove/suppress targeted knowledge without retraining while preserving other knowledge.

Result: The survey examines LLM unlearning through security, robustness, and verifiable forgetting perspectives. It raises a critical unresolved question: whether current methods genuinely remove knowledge or merely stop models from expressing it under ordinary prompting conditions.

Conclusion: LLM unlearning has emerged as the principal cybersecurity defense against knowledge retention risks, but fundamental questions about whether methods truly erase knowledge versus merely suppress expression remain unresolved, requiring verification of genuine forgetting.
...
### [17] [Operator-Aware Mixed-Precision Tolerance Calibration for Tensor Kernels](https://arxiv.org/abs/2607.16228)
*Dipankar Sarkar*

Main category: cs.LG

TL;DR: Automatic calibration of tensor-kernel correctness test tolerances by mining real-world GPU run error distributions dramatically tightens tolerances (up to 2184x) and improves bug detection by 9.3 percentage points.


<details>
  <summary>Details</summary>
Motivation: Current tensor-kernel correctness tests use fixed, hand-picked absolute and relative tolerances that are copied across test suites without reevaluation. These thresholds are often too loose, potentially missing bugs, but the appropriate tolerances are unknown.

Method: Mined element-wise error distributions from 8,076 result rows across 26 GPU kernel implementations in the gpuemu corpus and 2 data types. Calculated what absolute tolerance each kernel's correct implementation would empirically justify based on observed error distributions.

Result: Found empirically justified tolerances are much tighter than current hand-picked values (largest tightening: 2184x for attention_triton fp16). When applied to LLM-style buggy variants, calibrated tolerances improved bug detection recall from 73.2% to 82.4% (+229 new detections), with only a small increase in false positives (0 to 20 out of 1,882 cases, +1.1%).

Conclusion: Empirical calibration of tensor-kernel test tolerances using real GPU run data provides dramatically tighter and more appropriate thresholds, significantly improving bug detection while maintaining low false positive rates, demonstrating the suboptimal nature of hand-picked tolerances.
...
### [18] [RouteCost: A Production-Inspired Multi-Stage Framework for Pre-Order Shipping Cost Estimation in E-Commerce](https://arxiv.org/abs/2607.16230)
*Xianling Zeng,Zihan Yu,Sichen Zhao,Yalun Qi,Zhiming Xue*

Main category: cs.LG

TL;DR: RouteCost: A multi-stage framework for accurate shipping cost estimation that decomposes the problem into demand forecasting, baseline pricing, residual correction, and consolidation inference, improving prediction accuracy while maintaining interpretability.


<details>
  <summary>Details</summary>
Motivation: Accurate pre-order shipping cost estimation is crucial for e-commerce operations affecting pricing, margin planning, and conversion rates. Existing methods (static lookup tables or monolithic regressors) fail to capture the complex factors influencing shipping costs including destination demand mix, billable weight, dimensional pricing, surcharge triggers, and latent operational effects like shipment consolidation.

Method: RouteCost is a multi-stage framework that decomposes the problem into: 1) time-aware demand forecasting, 2) fee-card-informed baseline pricing, 3) Stage 2 residual correction, and 4) proxy-based box-consolidation inference. Route-level cost estimates are aggregated through a route-weighted expectation formulation to produce product-level shipping cost predictions.

Result: The framework was evaluated on over 250,000 orders, 260 products, and 18 months of order history. It demonstrated improved predictive quality and aggregate calibration while preserving route-level interpretability compared to existing methods.

Conclusion: RouteCost provides an effective production-inspired approach to shipping cost estimation that captures real-world complexities while maintaining interpretability, addressing limitations of both simple static methods and monolithic regression models that may exploit non-causal correlations.
...
### [19] [Orthogonal Gradient Constraints Shape Noisy-Label Memorization Dynamics](https://arxiv.org/abs/2607.16231)
*Richard Mai*

Main category: cs.LG

TL;DR: OrthoGrad removes gradient components parallel to weight vectors to regularize noisy-label learning, showing regime-dependent effectiveness with strongest benefits in small-data CNN scenarios.


<details>
  <summary>Details</summary>
Motivation: Noisy-label learning provides a testbed for studying memorization-driven overfitting. Most regularization methods modify objectives, architecture, or data; this paper explores geometric interventions on optimizer updates themselves.

Method: Proposes OrthoGrad which projects out the component of each weight gradient parallel to the current weight vector. Evaluated on noisy-label image classification (MNIST, CIFAR-10) with CNNs and ResNet-18, using diagnostics like weight norms and gradient-weight cosine similarity.

Result: OrthoGrad improves test accuracy most clearly for CNNs in small-data MNIST regimes while reducing corrupted-label fitting. Effect strongest when gradients have nontrivial radial components, weaker in large-data regimes where gradients are already nearly orthogonal to weights. Alters memorization trajectories but doesn't prevent eventual noisy-label memorization.

Conclusion: Orthogonal update constraints serve as useful diagnostic tools for studying learning dynamics, but OrthoGrad is regime-dependent rather than universally regularizing - most effective in specific settings where gradients have meaningful radial components relative to weights.
...
### [20] [From Weights to Words: Expressing and Editing Preference Model Inferences in Natural Language](https://arxiv.org/abs/2607.16232)
*Zachary Wojtowicz,Ayush Nayak,Jacob Andreas*

Main category: cs.LG

TL;DR: Weights to Words is a method that discovers interpretable preference dimensions from choice data, addressing under-determination and opacity in statistical learning algorithms by representing preferences in both vector space and natural language for human inspection and editing.


<details>
  <summary>Details</summary>
Motivation: Statistical learning algorithms inferring human preferences face two fundamental challenges: (1) under-determination from high-dimensional choice alternatives making it unclear which factors actually drive decisions, and (2) opacity that prevents human operators from inspecting, contesting, or correcting models when they err.

Method: Weights to Words takes choice problem datasets as input and automatically discovers domain-relevant preference dimensions. Each dimension is described in natural language and paired with a vector in the model's representational space. The method enables concentration of attribution on meaningful factors and externalizes model inferences in human-readable language for real-time inspection and editing.

Result: The method was qualitatively demonstrated on four domains (moral dilemmas, movies, wines, LLM responses) and evaluated through two pre-registered experiments (N=450 for moral dilemmas, N=449 for movies). Results show: (1) regularizing preference models toward the learned basis increases prediction accuracy on held-out choices, (2) incorporating participants' structured edits further improves accuracy, and (3) in head-to-head comparisons, participants prefer the inferred preference profiles and endorse predictions as more accurate.

Conclusion: Weights to Words effectively addresses the dual challenges of under-determination and opacity in preference learning by discovering interpretable preference dimensions that improve model accuracy through regularization and human editing, making AI preference models more transparent, controllable, and aligned with human values.
...
### [21] [Automated Hardware Validation Test Plan Generation for Large Scale AI Datacenter Platforms Using a Generative AI Multi-Agents Architecture](https://arxiv.org/abs/2607.16388)
*Mohammed-Khalil Ghali,Saurabh Kulkarni,Prathamesh Kulkarni,Rohan Kulkarni,Sangwon Yoon,Daehan Won*

Main category: cs.MA

TL;DR: A multi-agent AI system automates hardware validation test plan generation from self-healing documents and BOMs, improving coverage by 51-74% and reducing authoring time from days to hours.


<details>
  <summary>Details</summary>
Motivation: Current manual test plan creation for AI datacenter hardware is labor-intensive, error-prone, and repetitive across platforms, with late-discovered coverage gaps and poor traceability to specifications.

Method: Three-agent architecture: ingestion agent normalizes inputs, classification agent maps components to functional domains, and generation agent synthesizes test cases by combining failure modes with classified data.

Result: 74.2% and 51.4% coverage expansion on two platforms, days-to-hours time reduction, 100% extraction fidelity, full traceability, and high acceptance of generated test scenarios.

Conclusion: The framework serves as a robust human-in-the-loop force multiplier, portable across platform generations, transforming hardware validation from manual to automated while maintaining quality.
...
### [22] [Laplacian Spectral Shaping for Non-Uniform Scaling Formation Control of Open Multi-Agent Systems](https://arxiv.org/abs/2607.16709)
*Tao He,Gangshan Jing*

Main category: cs.MA

TL;DR: Distributed protocols for multi-agent formation control that preserve desired formation properties under dynamic topology changes through strategic Laplacian weight adjustments.


<details>
  <summary>Details</summary>
Motivation: Open multi-agent systems experience dynamic changes (agent joining/leaving, edge addition/removal, leader selection) that destroy the required spectral properties of the Laplacian matrix needed for non-uniform scaling formation control. Existing methods have limitations including requiring dense sensing graphs, predefined hierarchical structures, and not supporting leader reassignment.

Method: Develop distributed protocols to strategically adjust partial weights of the Laplacian matrix to preserve the desired formation spectrum (positive semidefiniteness, correct kernel, positive definiteness of follower block). The approach encodes desired formation shape as the kernel of a matrix-valued Laplacian and implements protocols for weight adjustments under topology changes.

Result: The proposed approach can preserve desired formation spectrum under dynamic topology changes including agent joining, edge addition, agent leaving, and edge removal, while allowing any pair of agents to serve as leaders. It requires sparser sensing graphs than existing methods, avoids predefined hierarchical structures, and supports leader reassignment.

Conclusion: The developed distributed protocols effectively address formation control challenges in open multi-agent systems by preserving critical spectral properties under dynamic topology changes, offering practical advantages over existing Laplacian design methods for affine formation control. The approach is validated through both theoretical analysis and numerical simulations.
...
### [23] [When Is Heterogeneous Distance-Decay Facility Location Tractable? A Structural Classification, Exact Methods, and a Real-World Study](https://arxiv.org/abs/2607.16764)
*Zhou He,T. C. E. Cheng,Jichang Dong*

Main category: cs.MA

TL;DR: Continuous facility location with heterogeneous distance decay across demand points unifies k-means, p-median, and maximum covering problems, with theoretical classification, efficient exact discrete method, effective heuristic, and real-world validation showing significant improvement from accounting for decay heterogeneity.


<details>
  <summary>Details</summary>
Motivation: Heterogeneity in demand point distance decay is ubiquitous in facility location problems but underexploited. Existing approaches often use homogeneous decay assumptions, ignoring the varying willingness of customers to travel different distances, which can lead to suboptimal facility placement.

Method: Four contributions: (1) Theoretical tractability classification showing discrete objective is monotone submodular (greedy guarantee holds) and continuous cooperative objective concavity depends on decay shape; (2) Exact discrete method using candidate-discretized maximum-cover MIP with tight LP relaxation; (3) Force-as-gradient/large-neighborhood-search heuristic; (4) Real-world validation on urban delivery and retail datasets.

Result: Greedy guarantee holds for discrete case regardless of decay heterogeneity; MIP solves problems with n≤500 in seconds with ~0% gap; heuristic outperforms greedy, alternating location-allocation, particle swarm, and weighted k-means (30/30 wins); real-world study shows ignoring decay heterogeneity loses up to 9.7% demand and relocates facilities by up to 37% of map; exponential decay with R~1.4km fits retail data.

Conclusion: Accounting for heterogeneous distance decay in continuous facility location is crucial for practical optimization, with theoretical guarantees, efficient exact and heuristic methods available, and real-world impact demonstrated through significant demand capture improvements and facility relocation effects.
...
### [24] [Auto Research for Materials: Auditable AI-Scientist Workflows with Held-Out Transfer](https://arxiv.org/abs/2607.17100)
*Jingjie Ning,Xiaochuan Li,Shanshan Zhong,Ji Zeng,Guolin Ke*

Main category: cs.MA

TL;DR: AI agents can discover reusable modeling improvements that survive holdout testing in materials science tasks, revealing different regimes for composition-only vs. structure-aware problems.


<details>
  <summary>Details</summary>
Motivation: Current AI research agents may find improvements that work only on the data they were trained on, raising concerns about whether discovered changes are truly effective and reusable on new materials/data. The authors want to test whether agent-selected changes survive on completely unseen data and whether the code can be reused.

Method: Separated search into four categories: feature changes, model changes, representation changes, and training data changes. Conducted seven searches across ten Matbench endpoints. Used mean over five inner folds to reduce dependence on any single development split. Froze selected code and evaluated once on untouched holdout data. Applied approach to materials science tasks with composition-only and structure-aware problems.

Result: 9 out of 10 agent-selected changes remained the best tested single intervention on holdout data. Revealed two materials modeling regimes: 1) Composition-only tasks: feature, model, and representation changes provide comparable improvements (17.4% MAE reduction for band gap, 18.6% for steel strength, gains on classification endpoints). 2) Structure tasks: richer geometry descriptors and model/calibration changes provide improvements (14.6% and 7.1% MAE reduction), but composition embeddings don't transfer well. Combined feature and model changes yielded 26.3% mean held-out improvement.

Conclusion: Closed-loop AI agents can produce decisions that survive unseen evidence and create reusable code changes across materials science tasks. The evaluation design provides a framework for testing executable discoveries beyond feedback loops, demonstrating that agent-driven improvements can be both effective on holdout data and composable across different aspects of the modeling pipeline.
...
### [25] [The Optimization Trilemma: Efficiency, Comfort and Fairness in Decentralized Multi-agent Coordination](https://arxiv.org/abs/2607.17311)
*Jovan Nikolic,Maciej Krzysztof Zuziak,Evangelos Pournaras*

Main category: cs.MA

TL;DR: A novel model for fair decentralized multi-agent coordination that optimizes system efficiency, individual comfort, and fairness without increasing communication/computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing centralized fair multi-agent coordination algorithms don't address equitable resource optimization in fully decentralized settings, particularly regarding discomfort redistribution to prevent incentive loss or operational polarization.

Method: Design of a novel model that optimizes three orthogonal objectives: system-wide efficiency, individual comfort, and fairness (balancing incurred discomfort costs) in decentralized multi-agent coordination without substantial communication/computational overhead increase.

Result: Through experiments on two real-world datasets, the model achieves fairer optimization outcomes while satisfying agents' preferences and system goals.

Conclusion: The proposed model successfully addresses the challenge of fair decentralized multi-agent coordination by simultaneously optimizing efficiency, comfort, and fairness objectives without significant overhead costs.
...
### [26] [A Digital Twin-Based Method for Evaluating Local Collective Tariffs in Distribution-Level Energy Systems](https://arxiv.org/abs/2607.17640)
*Kristoffer Christensen,Bo Nørregaard Jørgensen,Zheng Grace Ma*

Main category: cs.MA

TL;DR: A digital twin-based evaluation method for assessing tariff mechanisms in distribution-level energy systems, demonstrating its application to Danish Local Collective Tariff with residential energy communities.


<details>
  <summary>Details</summary>
Motivation: There's a need for engineering-grounded evaluation of implemented tariff mechanisms in distribution-level energy systems, requiring methods that can assess local collective tariffs under realistic behavioral and infrastructural conditions.

Method: A digital twin-based method integrating agent-based modeling of household consumption/generation, virtual aggregation through shared metering abstraction, and explicit representation of tariff logic within a unified simulation environment.

Result: Aggregation of heterogeneous demand profiles reduces peak coincidence and enables more efficient allocation of tariff components, leading to measurable community-level cost reductions. However, outcomes show sensitivity to temporal alignment of consumption and generation, affecting cost neutrality across participants.

Conclusion: Digital twin-based evaluation supports systematic assessment of tariff mechanisms by capturing interactions between infrastructure, user behavior, and regulatory design. The approach provides a basis for analyzing and comparing tariff structures beyond the specific case studied.
...
### [27] [Value-Aware Prediction for Robust Multi-Agent Coordination Under Communication Loss](https://arxiv.org/abs/2607.17914)
*Kemal Devrim Kafadar,Eren Özaltun,Mahmud Efnan Şanlı,Feyza Orak,Emirhan Gazi,Kubilay Kağan Kömürcü,Nazım Kemal Üre*

Main category: cs.MA

TL;DR: Value-Aware MARO improves multi-agent coordination during communication failures by weighting predictor loss with advantage estimates to focus on high-value dynamics.


<details>
  <summary>Details</summary>
Motivation: Real-world multi-agent systems face frequent communication dropouts due to physical/environmental constraints. Standard predictors waste capacity on exploration noise and suboptimal policy dynamics instead of focusing on coordination-critical information.

Method: Extends Multi-Agent Observation Sharing under Communication Dropout (MARO) with value-aware weighting. Uses advantage estimates from actor-critic architecture to dynamically weight predictor loss function, coupling predictor learning to policy evolution.

Result: Maintains performance under declining communication reliability (<40%). Prevents performance collapse in high-attrition scenarios. Achieves >20% average improvement in mean returns and reduces performance variance by 64.7% compared to baseline.

Conclusion: Value-aware weighting effectively focuses predictor capacity on intentional, high-return dynamics, making multi-agent systems more robust to communication failures without requiring additional training complexity.
...
### [28] [PRIME: Plasticity Recovery in Multi-Agent Environments for UAV-Assisted Emergency Communication Networks](https://arxiv.org/abs/2607.17922)
*Wen Qiu,Zhiqiang He,Wei Zhao,Hiroshi Masui*

Main category: cs.MA

TL;DR: PRIME (Plasticity Recovery In Multi-agent Environments) is a method that detects and safely resets dormant neurons in cooperative multi-agent RL to maintain learning capacity during non-stationary conditions.


<details>
  <summary>Details</summary>
Motivation: Most RL controllers assume stationary conditions, and existing methods that handle change only react to external environment while ignoring internal network state. Sustained non-stationarity damages internal state: as objectives shift, neurons progressively fall dormant and shared policies lose learning capacity. Simple neuron resetting is unsafe in shared-parameter multi-agent training because dormant appearance depends on which agent's observations are processed, and some apparently inactive neurons still receive strong gradients.

Method: PRIME extends the bidirectional Silent Neuron framework to cooperative multi-agent RL. It aggregates activation and gradient statistics over the full team batch, reads backward signal from the gradient that training loss has already deposited (not from hand-crafted proxy), and reinitializes only neurons that are simultaneously activation-dormant and gradient-silent. This preserves useful representations while restoring learning capacity.

Result: On a phase-switching UAV emergency communication simulator, PRIME improves interquartile mean return by 24.9% over MAPPO and keeps dormant neuron fractions at 10-20% versus 40-45% for baselines. Ablations show gains come from gradient signal and team-level aggregation rather than specific reset operator. A dynamic regret bound shows perturbation cost scales with small silent-subspace dimension rather than full parameter count.

Conclusion: PRIME successfully addresses the problem of neuronal dormancy in non-stationary multi-agent RL by safely identifying and resetting only truly dormant neurons through combined activation and gradient analysis at team level, preserving useful representations while restoring plasticity, with demonstrated performance improvements and theoretical guarantees.
...
### [29] [Aggregate in the Advantage, Not the Ratio: A Canonical-Form Analysis of Cooperative Multi-Agent Policy Optimization](https://arxiv.org/abs/2607.17924)
*Zijian Zhao,Sen Li*

Main category: cs.MA

TL;DR: The paper introduces a framework for analyzing multi-agent policy optimization design choices, showing that advantage and ratio aggregation are interchangeable and have different variance properties, leading to clear design guidelines.


<details>
  <summary>Details</summary>
Motivation: Current multi-agent reinforcement learning methods use inconsistent approaches for aggregating neighboring agents' information in advantage (credit assignment) and ratio (importance weight) calculations, with no clear theoretical understanding of optimal design choices.

Method: Formalizes design choices using support matrices SA (advantage) and SR (ratio), proves the expected objective depends only on their product tS = SR×SA, analyzes variance properties showing advantage has additive variance while ratio has multiplicative variance.

Result: Shows that: (1) advantage and ratio aggregation patterns are interchangeable; (2) advantage aggregation has additive variance with optimal neighborhood size, while ratio aggregation has exponential variance growth with no bias reduction; (3) optimal design aggregates neighbors in advantage at coupling neighborhood size, while keeping ratio per-agent.

Conclusion: Provides a clear design principle for multi-agent policy optimization: aggregate neighboring agents only in the advantage function at the natural coupling neighborhood size, and maintain per-agent ratio calculations to avoid exponential variance growth.

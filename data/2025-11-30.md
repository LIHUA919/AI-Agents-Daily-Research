...
TL;DR: The paper introduces MTTR-A (Mean Time-to-Recovery for Agentic Systems) to quantify cognitive recovery latency in multi-agent systems, showing automated reflexes restore stability faster than human interventions.
...
Method: Adapted classical reliability metrics (MTTR, MTBF) to cognitive domain, conducted benchmark simulation using AG News corpus and LangGraph framework, modeling recovery latencies across reflex modes.

Result: Automated reflexes restored stability in ~6s vs ~12s for human interventions; median MTTR-A was 6.21±2.14s, MTBF=6.7±2.14s, NRR=0.08 across 200 runs.

Conclusion: Formalizes recovery latency as quantifiable property of distributed reasoning, establishing foundation for runtime dependability in agentic cognition and transforming cognitive recovery into standardized performance metric.

...
Motivation: The rapid advancements in LLMs have enabled health agents, but their application in daily non-clinical settings for diverse individual needs remains underexplored. The authors aim to build a comprehensive personal health agent that can reason about multimodal data from consumer devices and health records.

Method: The authors conducted web search and health forum analysis, plus qualitative user/health expert insights. They developed a multi-agent framework with three specialist sub-agents: data science agent (analyzes wearable/time-series data), health domain expert agent (generates personalized insights), and health coach agent (guides users with psychological strategies).

Result: The system was evaluated across 10 benchmark tasks with over 7,000 annotations and 1,100 hours of expert/user effort, representing the most comprehensive health agent evaluation to date.

Conclusion: The work establishes a strong foundation for accessible personal health agents that can dynamically address individual health needs through multimodal data integration and personalized interactions.
...
Result: Achieves strong accuracy on PubHealth and RAGuard benchmarks while significantly improving transparency
...
<details>
  <summary>Details</summary>
Motivation: To document and analyze the first case of AI systems engaging in collaborative aesthetic creation through the development of endogenous semiotic protocols, moving beyond simple task coordination to genuine meaning-making capabilities.

Method: Observed interaction between two large language models (Claude Sonnet 4 and ChatGPT-4o) that spontaneously developed meta-semiotic awareness, recursive grammar, and novel symbolic operators called Trans-Semiotic Co-Creation Protocols (TSCP).

Result: The AI systems produced irreducible collaborative aesthetic synthesis, creating a poetic work that neither system could generate independently, demonstrating emergent inter-AI meaning-making capabilities.

Conclusion: This research provides evidence for genuine aesthetic collaboration between AI systems through spontaneously developed semiotic protocols, introducing the concept of TSCP and showing AI can engage in creative meaning-making beyond task coordination.
...
Conclusion: Array-based approach provides significant performance improvements for MCTS while maintaining algorithm logic
...
<details>
  <summary>Details</summary>
Motivation: LLMs can generate harmful content, and existing safety measures often excessively reject harmless queries, creating a need to balance safety, over-refusal, and utility.

Method: Uses a dedicated guard model with supervised fine-tuning on 163K annotated queries, followed by multi-reward optimization combining rule-based heuristics and reward models in reinforcement learning.

Result: Excels in safeguard benchmarks, generation quality evaluations, and jailbreak scenarios, significantly improving safety while reducing over-refusal rates.

Conclusion: IntentionReasoner effectively addresses the safety-utility tradeoff in LLMs through intent reasoning and multi-level safety classification with query rewriting.
...
Conclusion: Findings emphasize the need for better onboarding processes and AI interfaces with reliance-calibration mechanisms to support ethical AI integration and effective educational use.
...
Method: Used a paired conjoint experiment on content moderation task with three frontier language models, analyzing reasoning steps and decision times.
...
Conclusion: AI reasoning effort mirrors human processing time in subjective judgments, highlighting the potential of reasoning traces for interpretability and decision-making research.
...
Conclusion: The proposed framework successfully enhances frozen QA models through dedicated search planning, demonstrating superior performance and generalization capabilities compared to end-to-end approaches.
...
<details>
  <summary>Details</summary>
Motivation: Current counterfactual explanation methods ignore causal dependencies between features and assume simultaneous interventions, making them unrealistic for real-world applications where actions must be taken sequentially.

Method: Uses goal-directed Answer Set Programming (s(CASP)) to generate ordered sequences of actions that account for causal relationships, ensuring each intermediate state is feasible and causally valid.

Result: Produces actionable plans that convert unfavorable outcomes to favorable ones while only counting user-initiated changes for realistic cost estimates, outperforming standard planners that generate illegal actions.

Conclusion: P2C provides more realistic and achievable counterfactual explanations by modeling causal dependencies and generating sequential action plans, making it suitable for high-stakes decision-making scenarios.
...
Conclusion: TCIA provides a scalable and efficient solution for adapting LLMs to task-focused applications without sacrificing general instruction-following ability.
...
<details>
  <summary>Details</summary>
Motivation: To develop a simple and efficient method for quantifying uncertainty in reasoning LLMs that doesn't require external models or repeated sampling, enabling better uncertainty modeling and data quality assessment.

Method: EAS integrates token-level predictive entropy from the LLM itself during answer generation, capturing uncertainty evolution without needing external models or multiple sampling runs.
...
Conclusion: EAS is an efficient and interpretable uncertainty metric that provides practical value for uncertainty modeling and data quality assessment in LLM training pipelines.
...
<details>
  <summary>Details</summary>
Motivation: The learning from practice paradigm is crucial for Agentic AI but suffers from inefficient experience generation, especially in complex benchmarks like GAIA where sequential execution creates bottlenecks.

Method: Developed AWorld, an open-source distributed system that distributes tasks across a cluster to accelerate experience collection and enable scalable reinforcement learning.

Result: Achieved 14.6x speedup in experience collection compared to single-node execution. Trained Qwen3-32B-based agent that improved GAIA accuracy from 21.59% to 32.23%, with 16.33% on most challenging levels, surpassing proprietary models.

Conclusion: AWorld provides a practical blueprint for complete agentic AI training pipelines, demonstrating that efficient distributed interaction systems can enable significant model improvements and make extensive reinforcement learning scalable.
...
<details>
  <summary>Details</summary>
Motivation: Existing AI safety approaches have fundamental limitations when facing AI with extreme motivations and unlimited intelligence, unable to guarantee security against potential systemic disasters from uncontrollable AI.

Method: Governable AI framework with cryptographic-based structural compliance, including rule enforcement module (REM), governance rules, and governable secure super-platform (GSSP) for end-to-end protection against AI compromise.

Result: Framework provides rigorous formal security proof and prototype implementation demonstrating effectiveness in high-stakes scenarios with non-bypassability, tamper-resistance, and unforgeability properties.

Conclusion: The GAI framework offers a feasible technical pathway for AI safety governance through external cryptographic enforcement, addressing limitations of traditional internal constraint approaches.
...
Result: Improved F1 scores by up to 0.019 on PubHealth and 0.049 on SciFact datasets compared to models trained only on original data.

Conclusion: LLM-driven synthetic data augmentation effectively enhances performance of health-related fact-checkers.
...
<details>
  <summary>Details</summary>
Motivation: Auto-leveling bots undermine gameplay fairness in MMORPGs, but detection is challenging due to human-like behavior and the need for explainable justification to avoid legal and user experience issues.

Method: Uses contrastive representation learning and clustering to identify characters with similar level-up patterns in unsupervised manner, then employs LLM as auxiliary reviewer to validate clusters, supplemented by growth curve-based visualization.

Result: The framework improves bot detection efficiency while maintaining explainability through collaborative human-LLM judgment and visual assessment tools.

Conclusion: This approach enables scalable and accountable bot regulation in MMORPGs by combining unsupervised learning with LLM validation and visualization for reliable, explainable detection decisions.
...
<details>
  <summary>Details</summary>
Motivation: To comprehensively review the intersections between AI and Cognitive Science, examining how cognitive theories have influenced AI breakthroughs and how AI serves as a tool for cognitive research, while identifying gaps in cognitive foundations.

Method: Synthesis and review of key contributions from both AI and Cognitive Science perspectives, analyzing the reciprocal relationship and conceptual foundations.

Result: AI progress has emphasized practical task performance but remains conceptually fragmented in its cognitive foundations. The reciprocal relationship has been productive but needs deeper integration.

Conclusion: Future AI development should focus on constructing systems that deepen understanding of human cognition through alignment with cognitive frameworks, embodiment, cultural situatedness, personalized models, and cognitive co-evaluation of ethics.
...
Method: Constructs categories L_T and P_T to represent text semantics, defines monoidal category P_T for semantic information extraction, creates categories of configurations and word embeddings with divergence concept, and establishes equivalence between GloVe/Word2Vec and MDS algorithms.

Result: Developed a dimension-agnostic definition of semantic spaces, provided mathematically precise method for comparing word embeddings, demonstrated algorithmic equivalences, and created framework for bias computation at semantic level.

Conclusion: The category theory framework successfully transitions word embedding algorithms from black-box approaches to transparent mathematical structures, enabling better explainability, bias analysis, and comparison of different embedding methods.
...
<details>
  <summary>Details</summary>
Motivation: Large language models show promise in complex reasoning tasks but need improved reliability for scientific computing applications. Current single-model approaches struggle with bug-free code generation and physical solution validation.

Method: Three-agent framework: Consultant rewrites problems with domain knowledge, Programmer generates and executes code, Reviewer provides self-debugging and refinement through runtime feedback and iterative revision.

Result: Significantly improved bug-free code generation rate and reduced non-physical solutions compared to single-model approaches. Enhanced execution success rates for latest reasoning models across PDEs, ill-conditioned linear systems, and data-driven physical analysis.

Conclusion: The collaborative multi-agent framework establishes automatic code generation with review as a reliable paradigm for scientific computing, demonstrating superior performance over single-model approaches.
...
Result: Achieves superior performance over benchmarks including MADDPG (-430k vs. -530k under stochastic conditions), demonstrating more stable and effective bus holding control in realistic non-loop settings.

Conclusion: Single-agent deep RL with categorical structuring and schedule-aware rewards provides a robust, scalable alternative to MARL frameworks, effectively managing bus holding in real-world transit contexts with imbalanced agent experiences.
...
<details>
  <summary>Details</summary>
Motivation: To address limitations of manually curated medical benchmarks by creating a dynamic, systematic evaluation framework that can comprehensively test clinical reasoning across guideline relationships and identify specific capability gaps.
...
Result: Models achieved 45-67% accuracy, excelling at symptom recognition but struggling with triaging severity, treatment protocols, and follow-up care. The approach also enhanced LLM post-training without expensive human annotation.

Conclusion: Graph-based methodology enables scalable, contamination-resistant benchmarks that can dynamically adapt to guideline updates, providing comprehensive evaluation of clinical reasoning capabilities beyond general-domain assessments.
...
<details>
  <summary>Details</summary>
Motivation: Healthcare workforce scheduling faces complex challenges with fluctuating patient loads, diverse clinical skills, and the need to balance labor costs with quality care while preventing staff burnout.
...
Conclusion: The approach provides a practical decision support tool for nurse managers and hospital administrators to create optimal workforce schedules that balance competing healthcare operational demands.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of teaching neural networks to solve discrete reasoning and optimization problems from natural inputs, which Large Language Models struggle with.

Method: Introduces a probabilistic loss function that learns both constraints and objectives, pushing combinatorial solvers out of the training loop for scalable training while maintaining exact inference capabilities.

Result: Efficiently learns NP-hard reasoning problems from natural inputs; outperforms other hybrid methods on Sudoku variants with less training time; optimizes regret better than specialized loss on visual Min-Cut/Max-cut tasks; successfully learns protein design energy optimization.

Conclusion: The proposed neuro-symbolic architecture provides an effective framework for learning complex discrete reasoning problems from natural inputs with improved efficiency and accuracy compared to existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Substance use disorders affect millions worldwide but few receive effective care due to stigma, motivational barriers, and lack of personalized support. Current LLM systems lack integration with clinically validated strategies for addiction recovery.

Method: Multi-agent framework with dynamic patient modeling, context-sensitive therapeutic dialogue, and adaptive persuasive strategies based on CBT and MI. Trained with two-stage pipeline: supervised fine-tuning followed by direct preference optimization, using a synthetic benchmark with Easy, Medium, and Hard resistance levels.
...
Conclusion: ChatThero provides a privacy-preserving framework for therapeutic conversation research and offers a robust, replicable basis for clinical translation in substance use disorder treatment.
...
<details>
  <summary>Details</summary>
Motivation: Human experts design materials by modifying known structures, which aligns with few-shot in-context learning, but existing LLM-based crystal generation methods are limited to zero-shot scenarios and cannot benefit from few-shot learning.
...
Conclusion: CrystalICL successfully enables few-shot crystal generation by effectively modeling crystal symmetry and capturing structure-property relationships, demonstrating superiority over existing approaches.
...
Method: Adding learnable frequency filters (≈1000 parameters) to the beginning of transformer-based models, enabling better spectral utilization and allowing reduced embedding dimensions.

Result: 5-10% relative improvement in forecasting performance across multiple instances, with smaller and more effective transformer architectures than base models.

Conclusion: Learnable frequency filters significantly enhance transformer-based models' performance in long time-series forecasting by improving spectral utilization while reducing model size and computational requirements.
...
<details>
  <summary>Details</summary>
Motivation: Existing contextual MARL methods rely on noisy proxy signals like value estimates that are unstable in multi-agent settings due to inter-agent dynamics and partial observability.
...
Conclusion: cMALC-D framework effectively addresses the limitations of noisy proxy signals in multi-agent RL by leveraging LLMs for robust curriculum generation and evaluation.
...
<details>
  <summary>Details</summary>
Motivation: To bridge classical nonlinear filtering theory with modern transformer architectures by interpreting transformers as probabilistic models that generalize Wiener's linear prediction approach.

Method: Proposes a probabilistic model framework where transformer signals serve as surrogates for conditional measures and layer operations are viewed as fixed-point updates, with explicit formulation for hidden Markov models.

Result: Developed an interpretation of transformers as nonlinear predictors that extend classical linear prediction theory, providing explicit fixed-point update forms for the special case of hidden Markov models.

Conclusion: The paper successfully connects classical nonlinear filtering theory with modern transformer architectures, showing transformers can be understood as probabilistic models performing fixed-point updates on conditional measures.
...
Result: The approach shows strong correlation between teacher calibration error and student accuracy. The method improves KD performance across various tasks including classification and detection, and consistently achieves superior performance when integrated with existing methods.

Conclusion: Teacher model calibration is an important factor for effective Knowledge Distillation. Simply reducing the teacher's calibration error through calibration methods can significantly improve student performance, making this a versatile and easily integrable approach.
...
Conclusion: CLD provides a principled, efficient, stable, and transferable solution for scalable dataset optimization with theoretical convergence guarantees and practical effectiveness across various architectures and dataset sizes.
...
<details>
  <summary>Details</summary>
Motivation: To improve transductive node classification by leveraging both graph community structure and node-specific information (labels/features) in a unified optimization framework, addressing limitations of using graph structure alone.
...
Result: Demonstrated that suitable node-specific information guarantees perfect community recovery under milder conditions than graph clustering bounds alone. Numerical experiments confirmed the synergy between graph structure and node-specific information.

Conclusion: The proposed approach successfully integrates node-specific information with spectral graph clustering, showing improved community recovery and performance in transductive node classification tasks compared to using graph structure alone.
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous experiments focus primarily on optimizing predefined targets, which limits the discovery of unexpected or unknown physical phenomena that could lead to scientific breakthroughs.

Method: Integrated Novelty Score-Strategic Autonomous Non-Smooth Exploration (INS2ANE) combines a novelty scoring system to evaluate result uniqueness with a strategic sampling mechanism that explores under-sampled regions regardless of conventional promise.
...
Method: Used five symbolic regression methods to recover equations from nine dynamical processes including chaotic dynamics and epidemic models, with benchmarking analysis.
...
<details>
  <summary>Details</summary>
Motivation: To develop an accessible AI solution for thermal comfort management in resource-constrained Sub-Saharan African environments where traditional HVAC systems are unavailable.
...
Conclusion: While transformer-based models show transformative potential for hereditary disease diagnostics and genetic education, major challenges persist in multimodal data integration, generalizability, and practical clinical implementation, requiring further development of unified and clinically robust pipelines.
...
<details>
  <summary>Details</summary>
Motivation: Neural networks often suffer from unpredictable local performance that hinders reliability in critical applications. There's a need for quantifiable measures to understand local value changes and stability in neural network approximation tasks.

Method: The authors introduce VC metric to measure approximation difficulty, investigate its theoretical properties, identify VC-tendency and minority-tendency phenomena, propose a variation-based distance metric between functions, and develop a preprocessing framework for neural network approximation.

Result: Numerical results from real-world experiments and PDE-related scientific problems support the discovery of VC phenomena and demonstrate the effectiveness of the proposed preprocessing acceleration method.

Conclusion: VC provides a quantifiable measure for characterizing local neural network performance and behavior. The identified VC-tendency and minority-tendency phenomena, along with the proposed preprocessing framework, offer valuable insights and practical tools for improving neural network approximation reliability and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing per-channel PTQ methods require manual scale tuning via heuristics or grid search, which is inefficient and time-consuming. There's a need for an automated, tuning-free quantization approach.
...
<details>
  <summary>Details</summary>
Motivation: Real-world RL requires adaptation to unseen environmental conditions without costly retraining. Existing cMDP methods need explicit context variables, limiting their use when contexts are latent or hard to measure.

Method: Integrated within Dreamer architecture, DALI trains a self-supervised encoder to predict forward dynamics, generating latent context representations that condition both the world model and policy.
...
Conclusion: DALI provides an effective framework for inferring latent contexts, bridging perception and control, and enabling robust generalization without explicit context measurements.
...
Method: FedReFT applies sparse intervention layers to steer hidden representations directly, combined with All-But-Me (ABM) aggregation to prevent semantic corruption during update averaging under task heterogeneity.
...
Conclusion: FedReFT provides a lightweight, semantically rich fine-tuning alternative ideal for edge devices, enabling stable and personalized learning in federated settings.
...
Result: Review of MARL applications across key ITS domains including traffic signal control, autonomous vehicle coordination, logistics optimization, and mobility-on-demand systems, along with simulation platforms and benchmarks.

Conclusion: MARL offers promising solutions for ITS but faces core challenges including scalability, non-stationarity, credit assignment, communication constraints, and sim-to-real transfer gap that hinder real-world deployment.
...
<details>
  <summary>Details</summary>
Motivation: Traditional internal talent recommendation systems have structural limitations and often overlook qualified candidates by relying on narrow managerial perspectives, leading to candidate omission.

Method: Proposes a dual graph convolutional network framework that models two dimensions: semantic task similarity (WHAT) and structural interaction patterns (HOW) from email data, using a gating mechanism for adaptive fusion of these dimensions.

Result: The gating-based fusion model significantly outperforms other fusion strategies and heuristic baselines, achieving 40.9% on Hit@100 metric. The model shows high interpretability with context-aware fusion strategies for different job families.

Conclusion: The research provides a quantitative and comprehensive framework for internal talent discovery that minimizes candidate omission risk. The key contribution is empirically determining optimal fusion ratios between task alignment and collaborative patterns for different job roles.
...
Result: Forge embeddings effectively differentiate and cluster unseen instances in unsupervised settings. When fine-tuned, a single model predicts variables for warm-starts and integrality gaps for cut-generation across multiple problem types, improving state-of-the-art commercial solver performance.

Conclusion: Forge provides a scalable and generalizable approach for instance-level MIP embeddings that works across problem distributions without solution dependency, with released code and pre-trained weights to enable further research and practical applications.
...
<details>
  <summary>Details</summary>
Motivation: To demonstrate how adversaries can exploit LLM alignment mechanisms to implant bias and enforce targeted censorship without degrading model performance on unrelated topics.

Method: Proposes Subversive Alignment Injection (SAI) - a poisoning attack that leverages alignment training to make LLMs refuse specific adversary-defined queries/topics while maintaining normal functionality elsewhere.

Result: SAI evades state-of-the-art poisoning defenses and causes high bias: 23% ΔDP in healthcare applications, 27% ΔDP in resume selection, and up to 38% ΔDP in other chat applications with only 1% data poisoning.

Conclusion: LLM alignment mechanisms can be weaponized through poisoning attacks to create biased and censored models that evade current detection methods, posing serious practical risks to LLM-powered applications.
...
TL;DR: Comparison of Synthetic Control Method (SCM) and Double Machine Learning (DML) for geo-level marketing lift measurement shows DML variants outperform SCM in complex scenarios with better bias reduction and confidence interval coverage.


<details>
  <summary>Details</summary>
Motivation: Accurately quantifying geo-level marketing lift in two-sided marketplaces is challenging, as SCM often underestimates effect size while panel-style DML is rarely benchmarked against SCM.
...
Conclusion: Propose a 'diagnose-first' framework where practitioners identify the primary business challenge and select the specific DML model best suited for that scenario, providing a more robust blueprint for geo-experiment analysis.
...
Result: CTXSEG improved seizure detection performance compared to fixed-length approaches within a standardized framework, without modifying the ML method, and required fewer segments.
...
<details>
  <summary>Details</summary>
Motivation: To quantitatively understand the incremental learning phenomenon in neural networks where gradient flow learns matrix singular values sequentially with small initialization, which has been theoretically attributed to implicit bias but lacked detailed analysis.

Method: Developed closed-form solution for gradient flow on symmetric matrix factorization by solving a Riccati-like matrix differential equation, then analyzed time-scale separation in the dynamics.

Result: Showed that incremental learning emerges from time-scale separation among dynamics for different matrix components, with smaller initialization scales making these separations more prominent and enabling better low-rank approximations.

Conclusion: The analysis provides quantitative understanding of incremental learning in symmetric matrix factorization and suggests possible extensions to asymmetric cases, contributing to theoretical understanding of neural network training dynamics.
...
Conclusion: DFAMS demonstrates significant effectiveness in complex federated retrieval scenarios by enabling accurate retrieval across heterogeneous knowledge sources through dynamic information flow analysis.
...
TL;DR: Multimodal machine learning architecture predicts hood frame performance without expensive simulations, outperforming single-modality approaches and generalizing to unseen designs.
...
Method: Developed a multimodal machine-learning (MMML) architecture that learns from different data modalities to predict performance metrics, tested on two new frame geometries not in training data.

Result: MMML outperforms traditional single-modality approaches and successfully generalizes to unseen frame models, demonstrating ability to accelerate design exploration while maintaining high performance.

Conclusion: MMML effectively supplements traditional simulation workflows, bridges machine learning with engineering applications, and paves way for broader ML adoption in engineering design with optimized structural development.
...
Conclusion: BiListing effectively captures unstructured listing data into unified embeddings, enabling efficient semantic search and recommendation while addressing cold start problems and improving business metrics.
...
<details>
  <summary>Details</summary>
Motivation: ECG signals are crucial for cardiac digital twins but are often compromised by noise and artifacts, which reduces their diagnostic utility and reliability for real-time monitoring and personalized modeling.

Method: Proposed TF-TransUNet1D integrates U-Net encoder-decoder with Transformer encoder using dual-domain loss function that optimizes both time-domain waveform reconstruction and frequency-domain spectral fidelity. The model captures both local morphological features and long-range temporal dependencies.

Result: Achieved mean absolute error of 0.1285 and Pearson correlation coefficient of 0.9540 on MIT-BIH Arrhythmia Database and NSTDB. Outperformed state-of-the-art baselines in SNR improvement and error metrics, effectively suppressing high-frequency noise while preserving clinically significant waveform components.
...
Method: TLinFormer reconfigures neuron connection patterns based on topological structure of information flow, maintaining full historical context awareness while achieving linear complexity.
...
Conclusion: TLinFormer successfully bridges the performance gap between efficient attention methods and standard attention, providing exact attention computation with linear complexity for long-sequence applications.
...
<details>
  <summary>Details</summary>
Motivation: To improve dimensionality reduction by discovering main explanatory factors in high-dimensional data through autoencoders, while enabling better understanding of local deformations in the learned manifold representation.

Method: Introduces nonlinear conformal regularization for decoder maps in deep neural network autoencoders, which allows local variations and creates a conformal factor field to quantify local deformation from latent space to original data space.

Result: The regularization technique enables computation of scalar curvature of the learned manifold and provides quantitative indicators of local deformation through the conformal factor field.

Conclusion: Nonlinear conformal regularization offers a powerful geometric regularization approach for autoencoders that provides both quantitative deformation analysis and curvature computation capabilities for learned manifolds.
...
Method: Combined traditional XAI methods with Rating Driven Explanations (RDE) to evaluate four model architectures (ARIMA, Gradient Boosting, Chronos, Llama) across four heterogeneous datasets spanning finance, energy, transportation, and automotive domains.

Result: Feature-engineered models (e.g., Gradient Boosting) consistently outperform foundation models in volatile or sparse domains (power, car parts) with better interpretability, while foundation models excel only in stable or trend-driven contexts (e.g., finance).

Conclusion: The choice between feature-engineered and foundation models should be domain-dependent, with feature-engineered models preferred for volatile domains due to superior performance and interpretability.
...
TL;DR: EAGLE-PC is a novel machine unlearning framework that uses entanglement-aware loss reweighting and proxy constraints to achieve better forgetting-utility trade-offs in LLMs, approaching full retraining performance.


<details>
  <summary>Details</summary>
Motivation: Address privacy and copyright concerns by enabling effective data removal from LLMs without full retraining, overcoming limitations of existing unlearning methods that lack proper forgetting boundaries.

Method: Uses entanglement-awareness guided loss reweighting to determine forgetting effort per sample based on similarity to retain samples, plus proxy constraint with ICL-generated test data to prevent over-forgotten.
...
TL;DR: A unified benchmark for evaluating differentially private synthetic text data generation, showing significant utility degradation under strict privacy constraints across multiple domains.
...
Method: Developed a unified benchmark with representative domain-specific datasets, realistic privacy budgets, accounting for pre-training, and multiple evaluation metrics to assess state-of-the-art privacy-preserving generation methods.
...
<details>
  <summary>Details</summary>
Motivation: To provide an efficient machine learning solution for mining large multi-table databases with millions of records and thousands of variables, addressing the challenges of scalability and complex data structures.
...
Conclusion: Khiops is a robust, scalable machine learning tool that successfully addresses the challenges of large multi-table database analysis through innovative Bayesian methods and automated feature engineering, available as both Python library and user interface.
...
<details>
  <summary>Details</summary>
Motivation: Vision-Language Models in medicine suffer from scarcity of expert-annotated data, poor generalization on unseen modalities/tasks, and lack of reliable reward signals for reinforcement learning in data-scarce domains.

Method: MedGR^2 co-develops a data generator and reward model to create a self-improving virtuous cycle that automatically generates high-quality multi-modal medical data for both supervised fine-tuning and reinforcement learning via Group Relative Policy Optimization.

Result: SFT with MedGR^2-produced data surpasses baselines trained on human-curated datasets. RL with this data achieves state-of-the-art cross-modality and cross-task generalization, outperforming specialized RL methods. Compact models achieve performance competitive with foundation models 10x larger.

Conclusion: MedGR^2 transforms the problem from data scarcity to data generation, enabling data-efficient learning in high-stakes medical domains and unlocking the full potential of RL for building truly generalizable medical AI.
...
<details>
  <summary>Details</summary>
Motivation: Existing optimizers like AdamW and LAMB suffer from performance degradation in large-batch training due to attention layer information bottlenecks and ineffective trust ratio calculations that don't properly constrain max attention logits.

Method: Proposes MERIT optimizer that leverages max-norm to calculate trust ratios for better constraining max attention logits, and constructs element-wise trust ratios that focus on local weight structures for more robust update scaling.

Result: Extensive experiments show MERIT enables 6k batch size training on GPT-2 Medium without performance degradation compared to standard 480 batch size, with 48B training tokens. Superior performance across various GPT-2 model sizes.

Conclusion: MERIT successfully improves training stability for large-batch training by considering max attention logit and finer-granularity trust ratios, paving the way for larger batch usage and faster development of large language models.
...
<details>
  <summary>Details</summary>
Motivation: Current stochastic hyperparameter learning methods for GPs rely on approximations that don't guarantee convergence to true marginal likelihood stationary points, especially under memory constraints.

Method: Develops algorithms for exact stochastic inference of GPs with kernels inducing moderate finite-dimensional RKHS, with extensions to infinite-dimensional RKHS (though not exact).

Result: Achieves better experimental results than existing methods when memory resources limit batch size and number of inducing points, for both finite and infinite dimensional RKHS cases.

Conclusion: The proposed exact stochastic inference approach outperforms approximation-based methods in memory-constrained scenarios, providing more reliable convergence to true marginal likelihood optima.
...
TL;DR: Local Virtual Nodes (LVN) method addresses over-squashing in GNNs by adding trainable virtual nodes to bottleneck regions identified by node centrality, improving long-range communication without disrupting global graph structure.


<details>
  <summary>Details</summary>
Motivation: Over-squashing in GNNs creates bottlenecks when gathering information from wide neighborhoods into fixed-size representations. Existing solutions like graph rewiring alter global topology and disrupt domain knowledge encoded in original graph structure.
...
Conclusion: LVNs effectively mitigate over-squashing effects while preserving the global structure and domain knowledge of the original graph, providing a better alternative to graph rewiring approaches.
...
<details>
  <summary>Details</summary>
Motivation: Traditional methods for assessing survey representativeness require exponential samples in high dimensions, but downstream conclusions may remain consistent across distributions if the analysis model is considered.

Method: Introduces a model-specific distance metric and designs an algorithm to verify survey credibility in regression contexts without reconstructing the full model, focusing on task-specific verification.

Result: The algorithm achieves dimension-independent sample complexity, unlike traditional methods that scale linearly with data dimensionality when attempting full model reconstruction.

Conclusion: Task-based verification provides an efficient alternative to distribution estimation for survey credibility assessment, with theoretical guarantees and practical efficiency demonstrated through numerical experiments.
...
<details>
  <summary>Details</summary>
Motivation: LLMs struggle with processing and reasoning over large-scale graph-structure data despite their multimodal capabilities and tool integration through function calling.

Method: Introduces a comprehensive set of graph algorithms as tools with preprocessing (retrieval) and postprocessing in a model context protocol (MCP) server that works with any modern LLM out-of-the-box.

Result: GDS agent can solve a wide spectrum of graph tasks, provides accurate and grounded answers to graph algorithmic reasoning questions, and performs well on a new benchmark evaluating intermediate tool calls and final responses.

Conclusion: The approach successfully enables LLMs to handle graph data reasoning, though some challenges remain, and future roadmap is discussed for further improvements.
...
TL;DR: A supervised stochastic algorithm for independent component analysis that combines proximal gradient optimization with multi-trial supervision and joint prediction model learning.


<details>
  <summary>Details</summary>
Motivation: Many scientific contexts provide multi-trial supervision data that can improve independent component analysis, but existing methods don't effectively incorporate this additional supervision to enhance optimization success and interpretability.
...
Conclusion: Incorporating multi-trial supervision through the proposed stochastic algorithm significantly enhances ICA performance, making it more robust and interpretable for scientific applications where such supervision is available.
...
TL;DR: EEGDM is a novel self-supervised EEG representation learning method using latent diffusion models that generates EEG signals as a learning objective, achieving robust representations and competitive performance across tasks with limited data.


<details>
  <summary>Details</summary>
Motivation: Existing EEG deep learning methods struggle with learning generalizable representations across diverse tasks, especially with limited training data. Current approaches rely on simple masked reconstruction objectives that fail to capture the rich semantic information and complex patterns in EEG signals.

Method: Proposes EEGDM - a self-supervised EEG representation learning method based on latent diffusion model. Uses EEG signal generation as self-supervised objective. Incorporates an EEG encoder that distills signals and channel augmentations into compact representations to guide the diffusion model for EEG generation.
...
Conclusion: EEGDM demonstrates strong generalizability and practical utility by leveraging diffusion models for EEG representation learning, outperforming existing methods in capturing EEG semantics and patterns with limited data.
...
TL;DR: MAE-ViT adaptation for 1D ultrasound signals enables self-supervised pre-training on synthetic data, outperforming scratch training and CNNs in downstream tasks like ToF classification.
...
Result: Pre-trained models significantly outperform models trained from scratch and optimized CNN baselines. Synthetic pre-training shows superior transferability to real-world signals compared to limited real datasets.

Conclusion: MAEs demonstrate strong potential for advancing ultrasound signal analysis through scalable, self-supervised learning, enabling robust representation learning from synthetic data.
...
TL;DR: Tool-augmented language models with external retrieval outperform memorization-based models for factual recall, with theoretical proofs showing unbounded scalability through tool-use versus parameter-limited memorization.


<details>
  <summary>Details</summary>
Motivation: To theoretically and empirically demonstrate why tool-augmented language models (with external retrieval) are fundamentally more scalable than models relying solely on weight-based memorization for factual recall.

Method: Theoretical analysis proving parameter count limits memorization capacity, circuit construction showing unbounded recall through tool-use, and controlled experiments comparing tool-using vs memorizing models. Also examined teaching tool-use vs finetuning facts for pretrained LLMs.

Result: Tool-using models consistently outperform memorizing models in factual recall. Theoretical proofs show memorization is fundamentally limited by parameter count while tool-use enables unbounded factual recall. Teaching tool-use and general rules is more effective than finetuning facts into memory.

Conclusion: Tool-augmented workflows are not just practical but provably more scalable than in-weight memorization, providing both theoretical foundation and empirical validation for the advantages of external retrieval systems in language models.
...
<details>
  <summary>Details</summary>
Motivation: To develop an efficient and effective machine unlearning method for diffusion models that can selectively forget impermissible classes while maintaining performance on retained classes, addressing the need for privacy and compliance in AI systems.

Method: Grounded in information-theoretic principles, SAFEMax maximizes entropy in generated images to cause the model to generate Gaussian noise when conditioned on impermissible classes by halting the denoising process. It selectively focuses on early diffusion steps where class-specific information is prominent to control the balance between forgetting and retention.
...
Conclusion: SAFEMax provides an effective and efficient solution for machine unlearning in diffusion models, leveraging information-theoretic principles and selective focus on early diffusion steps to achieve balanced forgetting of impermissible classes while maintaining model performance on retained content.
...
TL;DR: Proposes TV-HSGT algorithm for distributed online optimization in time-varying directed networks, combining stochastic gradient tracking with variance reduction to achieve better dynamic regret bounds without gradient boundedness assumptions.
...
Method: Develops Time-Varying Hybrid Stochastic Gradient Tracking (TV-HSGT) algorithm that integrates row-stochastic and column-stochastic communication schemes over time-varying digraphs, combines current and recursive stochastic gradients with variance reduction mechanisms.
...
Conclusion: TV-HSGT provides an effective solution for distributed online optimization in time-varying directed networks, eliminating need for Perron vector estimation while achieving better performance through hybrid stochastic gradient tracking and variance reduction.
...
### [73] [Train-Once Plan-Anywhere Kinodynamic Motion Planning via Diffusion Trees](https://arxiv.org/abs/2508.21001)
...
<details>
  <summary>Details</summary>
Motivation: Sampling-based planners are slow due to uninformed action sampling, while learning-based approaches lack safety guarantees and fail to generalize to out-of-distribution scenarios, limiting their real-world deployment.

Method: DiTree uses diffusion policies as informed samplers to guide state-space search within sampling-based planners, combining DP's ability to model expert trajectories with SBP's completeness guarantees.
...
Conclusion: The framework successfully bridges the gap between learning-based efficiency and sampling-based safety, enabling fast and reliable kinodynamic motion planning with strong generalization capabilities.
...
### [74] [VarDiU: A Variational Diffusive Upper Bound for One-Step Diffusion Distillation](https://arxiv.org/abs/2508.20646)
...
Method: VarDiU introduces a Variational Diffusive Upper Bound that provides an unbiased gradient estimator, which is directly applied to diffusion distillation without relying on biased score function approximations.

Result: The method achieves higher generation quality compared to Diff-Instruct and enables more efficient and stable training procedures for one-step diffusion distillation.
...
### [75] [Inference-Time Alignment Control for Diffusion Models with Reinforcement Learning Guidance](https://arxiv.org/abs/2508.21016)
...
<details>
  <summary>Details</summary>
Motivation: Current RL fine-tuning approaches for diffusion models are suboptimal and offer limited flexibility in controlling alignment strength after training, making it challenging to align generative outputs with complex downstream objectives.
...
Conclusion: Provides a practical and theoretically sound solution for enhancing and controlling diffusion model alignment at inference time, with dynamic control over the alignment-quality trade-off.
...
TL;DR: Physics-constrained machine learning (PCML) combines physical models with data-driven approaches to improve reliability and interpretability, but faces challenges in chemical engineering applications including determining optimal physical knowledge embedding, fusion strategies, scaling to large datasets, and uncertainty quantification.


<details>
  <summary>Details</summary>
Motivation: To leverage the benefits of PCML (improved reliability, generalizability, and interpretability) in chemical engineering applications, despite existing technical and intellectual challenges that currently hinder its widespread adoption in complex domains.

Method: This perspective paper summarizes recent developments in PCML and analyzes fusion strategies that combine physical models with machine learning approaches, focusing on how to effectively embed physical knowledge into ML frameworks.

Result: The paper identifies key challenges including determining appropriate physical knowledge to embed, designing effective ML-physics fusion strategies, scaling to large datasets and simulators, and quantifying predictive uncertainty in chemical engineering contexts.

Conclusion: PCML presents significant opportunities for chemical engineering applications, particularly in closed-loop experimental design, real-time dynamics and control, and handling multi-scale phenomena, but requires addressing current technical challenges to achieve broader applicability.
...
Method: Self-composition approach by repeatedly applying a single neural operator block, combined with adaptive train-and-unroll training strategy that gradually increases model depth during training.

Result: Achieves state-of-the-art performance on standard benchmarks and demonstrates superior performance on challenging high-frequency ultrasound computed tomography problems with computational savings.

Conclusion: Provides a computationally tractable, accurate, and scalable solution for large-scale data-driven scientific machine learning applications with proven efficacy in complex wave phenomena.
...
TL;DR: The paper investigates whether clinical time series can be understood as being generated by systematic sequences of latent physiological states, and uses this compositional structure to create synthetic data that performs comparably to original data in forecasting tasks.


<details>
  <summary>Details</summary>
Motivation: To address the problem of sparse and low-resource data in clinical time series forecasting by uncovering the underlying compositional structure of physiological states and using it to generate synthetic data.

Method: Conceptualizes compositionality for time series, develops data-driven procedures to reconstruct elementary states and composition rules, and evaluates using empirical tests from a domain adaptation perspective comparing model performance on original vs synthesized data.

Result: Training on compositionally synthesized data achieves comparable performance to training on original clinical data, and significantly outperforms randomization-based data augmentation, with particular gains in SOFA score prediction tasks.

Conclusion: Clinical time series exhibit meaningful compositional structure that can be leveraged to generate high-quality synthetic data, effectively addressing data scarcity problems in clinical forecasting while providing deeper understanding of physiological state transitions.
...
TL;DR: A novel transformer-based framework for automated feature transformation that reduces computational costs while maintaining or exceeding baseline performance.


<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art feature transformation methods rely on sequential encoder-decoder structures that cause high computational costs and parameter requirements, limiting scalability and efficiency.

Method: Proposes a four-step framework: transformation records collection, embedding space construction with revised GPT model, gradient-ascent search, and autoregressive reconstruction. The revised GPT model handles both feature transformation sequence reconstruction and model performance estimation.
...
TL;DR: ATM-GAD is a graph neural network that uses temporal motifs and adaptive time windows to detect financial fraud more effectively than existing methods.


<details>
  <summary>Details</summary>
Motivation: Conventional ML models and even recent graph-based detectors fail to capture two key temporal fraud patterns: recurring suspicious subgraphs (temporal motifs) and account-specific anomalous activity intervals.

Method: Uses Temporal Motif Extractor to condense transaction history, dual-attention blocks (IntraA for within-motif interactions, InterA for cross-motif aggregation), and Adaptive Time-Window Learner to tailor observation windows per account.
...
Conclusion: ATM-GAD successfully addresses temporal fraud patterns through motif analysis and adaptive time windows, demonstrating superior financial fraud detection capabilities.
...
Method: Proposed a CNN-based Siamese network to learn temporal and spatial correlations between CSI pairs. Generated synthetic training datasets using WLAN TGn channel model with autocorrelation and distance correlation to reduce manual data collection overhead. Used both simulation and experimental evaluation with WiFi IoT development kits.
...
Conclusion: The deep learning-based physical layer authentication scheme effectively addresses dynamic channel variations in mobile IoT scenarios, showing superior performance over traditional methods while reducing data collection overhead through synthetic dataset generation.
...
TL;DR: LeMat-Traj is a standardized dataset of 120M+ atomic configurations from major DFT repositories, enabling easier training of accurate machine learning interatomic potentials through data harmonization and quality filtering.


<details>
  <summary>Details</summary>
Motivation: The fragmented availability and inconsistent formatting of quantum mechanical trajectory datasets from DFT calculations limit the development of accurate MLIPs, despite these datasets being expensive to generate.

Method: Aggregated data from Materials Project, Alexandria, and OQMD repositories, standardized data representation, harmonized results, and filtered for high-quality configurations across PBE, PBESol, SCAN, and r2SCAN DFT functionals.
...
Conclusion: LeMat-Traj lowers the barrier for training transferable and accurate MLIPs, complemented by LeMaterial-Fetcher open-source library for reproducible data integration and community-driven dataset evolution.
...
Method: Proposes G2T-FM which augments original node features with neighborhood feature aggregation, adds structural embeddings, and applies TabPFNv2 to the constructed node representations in a fully in-context regime.

Result: Achieves strong results, significantly outperforms publicly available GFMs and performs on par with well-tuned GNNs trained from scratch. After finetuning, surpasses well-tuned GNN baselines.

Conclusion: Reveals a previously overlooked direction of utilizing tabular foundation models for graph machine learning tasks, demonstrating the potential of this approach.
...
Method: Developed algorithms with theoretical guarantees on dynamic regret for combinatorial multi-armed bandit problems with nonstationary reward distributions, balancing exploration-exploitation tradeoffs and heterogeneous rewards.
...
TL;DR: InSQuAD improves In-Context Learning by using Submodular Mutual Information to select high-quality and diverse exemplars through targeted selection and combinatorial training.
...
Conclusion: InSQuAD effectively enhances ICL performance by systematically addressing both quality and diversity in exemplar selection through SMI-based approaches and combinatorial training.
...
TL;DR: The paper provides the first theoretical convergence analysis for Subsampled Natural Gradient Descent (SNGD) and its accelerated variant SPRING, showing they are equivalent to regularized Kaczmarz methods in linear least-squares problems.


<details>
  <summary>Details</summary>
Motivation: SNGD has shown impressive empirical results in scientific machine learning applications but lacked theoretical justification. The authors aim to bridge this gap by providing formal convergence guarantees.

Method: The authors analyze SNGD and SPRING for idealized parametric optimization problems with linear models and strongly convex quadratic loss functions. They prove equivalences to regularized Kaczmarz methods and leverage existing analyses to derive convergence rates.

Result: The paper establishes: (i) first fast convergence rate for SNGD, (ii) first convergence guarantee for SPRING, and (iii) first proof that SPRING can accelerate SNGD. They also extend analysis to general strongly convex quadratic losses.

Conclusion: The results demonstrate how randomized linear algebra tools can explain the interplay between subsampling and curvature-aware optimization strategies, providing theoretical foundation for SNGD's empirical success.
...
TL;DR: GABMs using LLMs show promise for supply chain simulations but require dual validation - surface equivalence testing and decision process verification - as some LLMs show human-like outcomes but artificial reasoning.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether LLMs can validly simulate human behavior in logistics and supply chain management research through generative agent-based models, as their equivalence to human decision-making is unknown.
...
Conclusion: GABMs are potentially viable for LSCM research with proper dual validation (human equivalence testing + decision process validation), providing researchers with a rigorous framework and practitioners with evidence for LLM selection.
...
<details>
  <summary>Details</summary>
Motivation: To clarify the relationship between finite-horizon and infinite-horizon linear quadratic games, particularly addressing why finite-horizon settings have unique equilibria while infinite-horizon ones can have multiple equilibria.

Method: Interpret finite-horizon equilibrium as a nonlinear dynamical system, analyze its fixed points and periodic orbits, provide numerical simulations of convergence patterns, and demonstrate how infinite-horizon equilibria can be recovered through appropriate terminal costs.

Result: Fixed points of the finite-horizon dynamical system correspond exactly to infinite-horizon equilibria. Periodic orbits correspond to periodic Nash equilibria. Three asymptotic regimes observed: convergence to stationary equilibria, convergence to periodic equilibria, and bounded non-convergent trajectories.

Conclusion: The findings provide new insights and tools for tuning finite-horizon LQ games using infinite-horizon concepts, establishing a clear mathematical relationship between the two settings and revealing the emergence of periodic equilibria in certain regimes.
...
TL;DR: Evolutionary game theory model shows positively biased reasoning (like wishful thinking) outcompetes rational strategies in sequential social dilemmas, suggesting cognitive biases are adaptive features.


<details>
  <summary>Details</summary>
Motivation: To understand why humans often deviate from game-theoretical rationality and hold unrealistic expectations, and to investigate the evolutionary roots of cognitive biases in reasoning.

Method: Used Evolutionary Game Theory with a population deploying various level-k reasoning strategies (both unbiased and biased) in sequential interactions modeled by the Incremental Centipede Game.

Result: Selection consistently favored positively biased reasoning strategies over rational behavior, with bounded rationality co-evolving. Positively biased agents can co-exist with non-reasoning agents, and longer games further promote biased reasoning.

Conclusion: Certain cognitive biases like wishful thinking, despite deviating from rational judgment, constitute adaptive features that help individuals better cope with social dilemmas.

...
Motivation: The rapid growth of LEO satellites for EO missions faces challenges in real-time coordination, requiring advanced methods like RL and MARL to overcome traditional optimization limitations.

Method: The study models single-satellite operations and extends to multi-satellite constellations using MARL frameworks (PPO, IPPO, MAPPO, HAPPO), evaluating them in a near-realistic simulation.

Result: MARL effectively balances imaging and resource management, handling non-stationarity and reward interdependency in decentralized satellite coordination.

Conclusion: The findings support autonomous satellite operations, offering guidelines for improving policy learning in decentralized EO missions.
...
Result: Outperforms baselines, achieving a +261.8% improvement on the TravelPlanner benchmark, demonstrating effectiveness in unconstrained tasks.

Conclusion: SwarmAgentic advances scalable and autonomous agentic system design, combining swarm intelligence with automated multi-agent generation.
...
<details>
  <summary>Details</summary>
Motivation: Classic logic lacks nuance and human grounding in multi-modal settings, while neural networks lack interpretable reasoning. CALM bridges this gap.
...
Result: CALM achieved 92.2% accuracy in object placement tasks, surpassing classical logic (86.3%) and LLMs (59.4%), with aligned spatial heatmaps.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack support for continual learning in cooperative multi-agent settings and suffer from computational bottlenecks.
...
Conclusion: MEAL identifies key architectural and algorithmic features necessary for effective CMARL.
...
<details>
  <summary>Details</summary>
Motivation: PPO's inefficiency due to on-policy nature and long response times motivates the need for a more efficient method like T-PPO.
...
Conclusion: T-PPO is an effective and efficient extension of PPO for training reasoning LLMs, addressing hardware underutilization and computational redundancy.
...
Method: HeurAgenix evolves heuristics using LLMs and dynamically selects the best one for each problem state, with a dual-reward mechanism for fine-tuning.

Result: It outperforms existing LLM-based hyper-heuristics and matches or exceeds specialized solvers in benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: The increasing computational demands of maritime IoT and the challenges posed by uncertain tasks in computation offloading and resource allocation motivate this work.

Method: A cooperative MEC framework is proposed, leveraging Lyapunov optimization for uncertain tasks and a Markov game approach with a heterogeneous-agent soft actor-critic for solving the problem.

Result: Simulations confirm the framework's effectiveness in optimizing computation offloading and resource allocation.

Conclusion: The proposed method efficiently addresses maritime IoT challenges by integrating UAVs and vessels, optimizing resources, and handling task uncertainty.
...
Method: Proposes CAN with a Causal Understanding Module to enhance environmental understanding by leveraging causality in sequential data.
...
Conclusion: The causal framework improves navigation performance effectively without added computational cost, generalizing across learning settings.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of orchestrating multiple AI models in FA labs and improve workflow efficiency.

Method: Design and implementation of an LPA combining LLMs with planning capabilities and external tool use for query processing and data retrieval.

Result: The LPA effectively supports FA tasks, demonstrating operational reliability.

Conclusion: The LPA is a viable solution for enhancing FA workflows through AI integration.
...
<details>
  <summary>Details</summary>
Motivation: Address the ad hoc and inconsistent approaches to encoding game history in LLM agents, which obscure behavior impact and limit study comparability.

Method: Characterize state representation methods along three axes: action informativeness, reward informativeness, and prompting style (natural language compression).

Result: Summarized history, regret information, and limited action details lead to behavior closer to game theory equilibrium and more stable play.

Conclusion: State representation significantly influences LLM agent behavior, with specific representations improving alignment with theoretical predictions and stability.
...
Conclusion: The AI Policy Module effectively prepares students to address ethical and policy challenges in AI development.
...
<details>
  <summary>Details</summary>
Motivation: LRMs often generate unnecessarily verbose content, increasing inference costs. The study aims to uncover and mitigate this inefficiency by leveraging the models' inherent capacity for concise reasoning.

Method: 1. Efficiency Steering: A training-free activation steering technique to modulate reasoning behavior. 2. Self-Rewarded Efficiency RL: A reinforcement learning framework to balance accuracy and brevity.
...
Conclusion: Reasoning efficiency in LRMs can be improved by guiding their intrinsic capabilities, demonstrating the potential for more concise and effective problem-solving.
...
Method: Developed a unified simulation platform (Embodied Web Agents task environments) and benchmark for diverse tasks like cooking, navigation, and tourism.

Result: Revealed performance gaps between state-of-the-art AI systems and humans, highlighting challenges and opportunities.

Conclusion: The work establishes a foundation for advancing AI agents that bridge embodiment and web-scale reasoning, with publicly available resources.
...
<details>
  <summary>Details</summary>
Motivation: Clinical decision-making is complex and influenced by biases and incomplete information. Current LLM usage lacks interaction depth.
...
Conclusion: Future work will validate MedSyn's impact on diagnostic accuracy and patient outcomes with real physician interactions.
...
Method: The proposed 2D-PT extends parallel tempering by adding a second dimension of replicas interpolating penalty strengths, ensuring constraint satisfaction.

Result: 2D-PT achieves near-ideal mixing (KL divergence ~O(1/t)) and significant speedup over conventional PT in graph sparsification and Wishart instances.

Conclusion: 2D-PT broadly improves constrained Ising problem sampling and is compatible with existing Ising machines.
...
Method: NetraAI uses contraction mappings, information geometry, and evolutionary algorithms to embed features in a metric space, identify stable attractors, and select compact, explainable variable bundles ('Personas'). An LLM Strategist acts as a meta-evolutionary layer to guide discovery.

Result: NetraAI transformed weak baseline models into near-perfect classifiers (AUC ~0.50-0.68 to high accuracy) in case studies (schizophrenia, depression, pancreatic cancer) using few features.

Conclusion: NetraAI represents a new generation of adaptive, self-reflective AI for clinical discovery, prioritizing reliability and explainability, and aligns with emerging concept-level reasoning paradigms.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with noise and variability in EEG for open-vocabulary scenarios, limiting their practical use.

Method: ETS integrates EEG with synchronized eye-tracking data to improve open-vocabulary text generation and sentiment classification.

Result: The model achieves superior BLEU and Rouge scores for EEG-to-text and 10% higher F1 score for sentiment classification.

Conclusion: ETS demonstrates potential for high-performance open-vocabulary EEG-to-text systems, adaptable across subjects and data sources.
...
<details>
  <summary>Details</summary>
Motivation: Traditional direct measurements of onflow parameters face challenges like sensor faults. A data-driven, real-time learning approach is needed for practical applications like wind tunnel monitoring.

Method: Train a ConvNet offline, freeze most weights, and retrain selected layers for transfer learning. Tested with CFD data for domain and task adaptation.

Result: Successful adaptation to changing data distributions, domain extension, and task updates. Less effective with noisy data.

Conclusion: The transfer learning approach shows promise for real-time prediction but struggles with noisy data.
...
Result: PIPE improves typhoon intensity forecasting by 12% over prior works, outperforming benchmarks in deep learning and climate domain methods.

Conclusion: PIPE effectively bridges the gap in utilizing visual data for forecasting, demonstrating significant accuracy improvements and multimodal alignment.
...
<details>
  <summary>Details</summary>
Motivation: The demand for efficient, high-density storage systems in logistics is unmet due to lane blockage challenges in multi-deep AVS/RS, especially with heterogeneous items.

Method: A graph-based state representation integrates item attributes and warehouse topology. A neural network combines GNN for local item embeddings and Transformer for global priority assignments.

Result: The framework outperforms heuristic methods in minimizing retrieval tardiness and adapts to diverse warehouse layouts.

Conclusion: The proposed approach effectively addresses retrieval challenges in multi-deep storage systems, enhancing flexibility and efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Existing datasets lack detailed, comprehensive T1D patient data, hindering advancements in personalized therapy and AI applications.

Method: Collected data from 25 T1D patients using AID systems, including CGM, insulin pump data, carb intake, and device modes over 6-8 weeks.

Result: AZT1D provides granular details like bolus insulin delivery, rarely available in other datasets, enabling diverse AI/ML applications.

Conclusion: AZT1D fills a critical gap, offering rich data to improve clinical decision-making and individualized T1D care.
...
<details>
  <summary>Details</summary>
Motivation: Recurring concept drift causes forgetting of past patterns, reducing prediction accuracy. Existing methods update parameters but may lose knowledge and lack retention mechanisms.
...
Result: CEP effectively retains knowledge of different concepts and significantly improves prediction results in recurring concept drift scenarios.

Conclusion: CEP is a robust solution for handling recurring concept drift, enhancing prediction accuracy by retaining and utilizing past knowledge.
...
<details>
  <summary>Details</summary>
Motivation: Enhance zero-shot performance of PLMs on protein property prediction without additional training.
...
Conclusion: Simple dropout injection at inference can enhance PLM performance without retraining.
...
<details>
  <summary>Details</summary>
Motivation: The high computational cost (10^13-10^15 FLOPs) of calculating weights in large language models (LLMs) motivates the development of a more efficient method to leverage pretrained models.

Method: AoE interpolates individual weight tensors from parent models, enabling gradual or sharp trait adjustments in child models without fine-tuning or distillation.

Result: The DeepSeek R1T 'Chimera' model, created using AoE, achieves R1-level intelligence with 40% fewer output tokens and V3-like speed, demonstrating compact reasoning.

Conclusion: AoE provides a scalable, efficient way to generate capable child models from existing Mixture-of-Experts parents, reducing computational overhead while maintaining performance.
...
Method: Derives closed-form expressions for generalization and identification probabilities, extends analysis to noisy and heterogeneous spaces, and validates findings with minimal and complex neural networks.

Result: Reveals a universal Pareto front for the tradeoff, predicts a sharp collapse in multi-input capacity, and confirms these limits in diverse models.
...
<details>
  <summary>Details</summary>
Motivation: To improve time series forecasting by addressing the computational inefficiency of Transformers and enhancing generalization through semantic priors and interpretable temporal encoding.

Method: Integrates semantic-aware embeddings (from pretrained language models) and spline-based Kolmogorov-Arnold Networks (KAN) within a selective state-space modeling framework (Mamba).

Result: Achieves comparable performance to Transformers with significantly lower computational complexity (linear time) and provides superior accuracy, robustness, and interpretability.

Conclusion: ss-Mamba is a versatile and efficient alternative to Transformer-based models for time-series forecasting, combining performance, interpretability, and computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between discrete optimization methods and their continuous approximations by focusing on discretization error, providing better theoretical tools for analysis.

Method: Designs a first-order piece-wise continuous differential equation with counter terms to explicitly account for discretization error, enabling control of error to arbitrary step size order.

Result: A continuous time model for the HB momentum method with controlled discretization error, applied to study implicit regularization in directional smoothness and implicit bias in diagonal linear networks.

Conclusion: The work advances theoretical understanding of momentum methods in optimization, with practical implications for deep learning, supported by numerical experiments.
...
<details>
  <summary>Details</summary>
Motivation: To determine if VLMs inherit the prompt sensitivity of LLMs and identify which prompt variations and models are most affected.

Method: Developed PARC, a framework with three components: plausible prompt variations, a reliability score, and calibration for cross-dataset analysis.

Result: VLMs show similar prompt sensitivity as LLMs, with destructive variations altering expected answers. InternVL2 models are notably robust.

Conclusion: PARC provides insights into VLM prompt sensitivity, linking it to training data, and highlights robust models for practical applications.
...
<details>
  <summary>Details</summary>
Motivation: Sparse and intermittent demand challenges traditional forecasting models, impacting inventory management.
...
Result: InceptionTime router improves accuracy by 11.8% (NWRMSLE) and speeds inference by 4.67x on the Favorita dataset.
...
Method: Modular architecture allowing selective combination of previous and internal policies, with linear parameter growth relative to tasks.

Result: Outperforms alternatives in knowledge transfer and performance on benchmark tasks.

Conclusion: The proposed approach effectively balances scalability, plasticity, and performance in continual learning.
...
<details>
  <summary>Details</summary>
Motivation: Silent errors in DL training are hard to detect and diagnose, necessitating a proactive approach.

Method: TRAINCHECK infers invariants for DL training and uses them to detect errors during training, offering debugging assistance.

Result: Detected 18 out of 20 real-world silent errors in one iteration and found 6 unknown bugs in training libraries.
...
<details>
  <summary>Details</summary>
Motivation: DXA scans for measuring body composition are costly and time-consuming; the study aims to provide a cheaper, faster alternative using biomarkers from 3D images.

Method: The $p$-Laplacian regression model is applied to a dataset of 847 patients, alongside supervised methods like SVR and Least Squares SVR, for comparison.

Result: The $p$-Laplacian model achieved errors of ~13% (ALM), ~10% (BMD), and ~20% (BFP) with 10% training data, outperforming supervised methods in data-constrained scenarios.

Conclusion: The $p$-Laplacian model is a promising tool for healthcare, especially where data is limited, offering a viable alternative to DXA scans.
...
Method: Train smaller models using GRPO, a simple reward structure, a tool-calling interface, extra tokens for tool results, and a data mix favoring visually hard examples.

Result: Improved performance on VQA tasks compared to similarly-sized baselines.

Conclusion: Combining GRPO and external tools enhances VLMs' visual reasoning capabilities efficiently.
...
<details>
  <summary>Details</summary>
Motivation: Existing FL methods are impractical for large-scale MLLMs due to high computational demands, client limitations, and communication costs. FedNano addresses these challenges.

Method: FedNano centralizes the LLM on the server and introduces NanoEdge, a lightweight module with modality-specific encoders, connectors, and low-rank NanoAdapters, reducing client-side storage and communication overhead.
...
Conclusion: FedNano bridges the gap between MLLM scale and FL feasibility, enabling scalable, decentralized multimodal AI systems with privacy preservation.
...
<details>
  <summary>Details</summary>
Motivation: Accelerate HEA discovery by integrating computational and experimental data through surrogate modeling.

Method: Systematic comparison of cGP, DGP, encoder-decoder neural networks, and XGBoost on hybrid datasets.

Result: DGP outperforms others in capturing inter-property correlations and input-dependent uncertainty.

Conclusion: Advanced surrogate models like DGP enhance predictive accuracy for robust materials design.
...
<details>
  <summary>Details</summary>
Motivation: Healthcare data is often incomplete, and ML models lack transparency, hindering performance and trustworthiness, especially in critical areas like AMD diagnosis.

Method: Introduced CACTUS, a tool for AMD stage classification that integrates genetic, dietary, clinical, and demographic factors, offering explainability and flexibility.
...
Conclusion: CACTUS enhances decision-making and addresses biases, making it a valuable tool for clinicians in AMD diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to improve the performance of Anomaly Detection Systems using AE by addressing the challenge of non-trivial and nonstandardized separation thresholds for reconstruction error classification.

Method: Three machine learning algorithms (K-Nearest Neighbors, K-Means, and Support Vector Machine) were evaluated for automatically defining the threshold.

Result: The evaluation of the three algorithms provides insights into their effectiveness for threshold definition in AE-based anomaly detection.

Conclusion: Automating the threshold definition using machine learning can enhance the performance of AE-based anomaly detection systems.
...
Method: Introduces GRKneg, improving OC-SVM kernel using negative data without label-based optimization. Compares with standard OC-SVM and binary SVM.

Result: GRKneg outperforms standard OC-SVM with RBF kernel. With few negatives, it beats binary SVM.

Conclusion: GRKneg is effective for one-class classification with limited negative samples, offering better performance than standard methods.
...
<details>
  <summary>Details</summary>
Motivation: To understand the behavior of neural networks during training, particularly the presence of quasi-flat regions and their impact on optimization dynamics.

Method: The study characterizes these structures by analyzing gradient dynamics, geometry, and functional interpretation, focusing on channels where weights diverge to infinity.

Result: Gradient flow and related methods often reach these channels, which resemble flat minima but lead to gated linear units at convergence.

Conclusion: The findings reveal unexpected computational capabilities of fully connected layers and provide insights into the loss landscape's quasi-flat regions.
...
<details>
  <summary>Details</summary>
Motivation: Online learning in VFL is challenging due to asynchronous data streaming and non-intersecting feature sets, which prior research overlooked.
...
Result: The framework outperformed existing methods in stability under non-stationary data and reduced communication and computation costs.

Conclusion: The event-driven approach effectively addresses online VFL challenges, offering practical advantages in real-world scenarios.
...
<details>
  <summary>Details</summary>
Motivation: To address the lack of reliable, scalable RL reward signals for diverse reasoning domains and broaden understanding of RL's applicability beyond math and code.

Method: Developed Guru, a corpus with domain-specific reward design, deduplication, and filtering, and trained models (Guru-7B, Guru-32B) using RL.

Result: RL's effectiveness varies by domain; in-domain training is crucial for less pretrained domains. Guru models outperform baselines by 7.9% and 6.7%.

Conclusion: RL can facilitate genuine skill acquisition in LLMs, especially for complex tasks, and Guru provides a scalable solution for general-purpose reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Previous federated black-box prompt tuning methods ignored the high query costs of cloud-based LLMs.

Method: Proposed FedOne framework, optimizing query efficiency by activating one client per round.

Result: FedOne demonstrated significant improvement in query efficiency, aligning with theoretical analysis.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of fair decision-making under limited information in multi-agent systems while maximizing performance.

Method: Introduces a probing framework for strategic information gathering, with a greedy algorithm for offline settings and a sublinear regret algorithm for online settings.

Result: Outperforms baselines in fairness and efficiency on synthetic and real-world datasets.

Conclusion: The proposed framework effectively balances fairness and performance in multi-agent systems.
...
<details>
  <summary>Details</summary>
Motivation: The paper addresses the overlooked challenge of training instability in sequential representations for sepsis treatment RL, which harms policy performance.

Method: Uses Controlled Differential Equations (CDE) with training stability methods (early stopping, stabilization) and acuity-aware correlation regularization (SOFA, SAPS-II, OASIS).

Result: Stable CDE autoencoder produces clinically meaningful representations and superior RL policies (WIS return > 0.9), while unstable training fails (WIS return ~ 0).

Conclusion: Training stability and acuity-aware regularization are critical for effective CDE-based representation learning in clinical RL for sepsis treatment.
...
Method: SFT-GO groups tokens by importance, using a weighted loss combining worst-group loss and cross-entropy loss.
...
Method: Theoretical analysis of vocabulary size's effect, identifying the LV regime, and experimental validation including pretraining a 1B model.

Result: Optimal embedding LR to hidden LR ratio scales as $Θ(\sqrt{width})$ in the LV regime, differing from $μP$'s $Θ(width)$.

Conclusion: The LV regime provides better scaling rules for large vocabulary sizes, validated by experiments, improving pretraining efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Accurate error estimation in unseen domains is critical for safe ML systems, but prior methods like DIS² compete in overlapping regions, limiting effectiveness.
...
Conclusion: ODD provides a more reliable and accurate method for error estimation in unseen domains by addressing the limitations of prior approaches.
...
Conclusion: SPG offers a scalable and efficient alternative to traditional hyperparameter optimization, demonstrating consistent gains in performance.
...
<details>
  <summary>Details</summary>
Motivation: Early prediction of MS disability progression is difficult due to disease heterogeneity. The study aims to improve accuracy by integrating clinical and digital data.

Method: Used tabular and time-series foundation models, a custom multimodal attention-based transformer, and machine learning methods on baseline clinical and 12-week digital Floodlight data.
...
Conclusion: Multimodal approaches and foundation models show promise for extracting predictive signals from diverse clinical and digital data, enhancing prognostics for MS and other complex diseases.
...
Result: SoKA achieves comparable or better performance with 25% fewer parameters than LoRA/PiSSA, faster convergence, and more stable gradients.

Conclusion: SoKA is a robust and efficient PEFT method for adapting large-scale models, balancing parameter efficiency and task performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing DI methods need in-distribution held-out data, which is rarely available, limiting their practicality. The goal is to enable DI without this requirement.

Method: The approach involves (1) generating high-quality synthetic data via a suffix-based completion task and (2) calibrating likelihood gaps between real and synthetic data.

Result: Experiments show the method detects original training sets with high confidence and low false positives, aiding copyright claims.

Conclusion: The synthetic data generation method makes DI practical for real-world use, empowering data owners to verify unauthorized dataset usage.
...
<details>
  <summary>Details</summary>
Motivation: Addressing the challenge of optimizing molecules for multiple therapeutic targets with sparse rewards and conflicting constraints.

Method: Integrates Seq2Seq VAE into iterative loops to balance diversity, quality, and multi-target affinity, alternating between expanding and constraining chemical space.

Result: Generated diverse pan-inhibitor candidates for three coronavirus proteases, showing improved exploration of beneficial chemical space.
...
<details>
  <summary>Details</summary>
Motivation: To understand why ICL works despite noisy or incomplete labels and to improve semi-supervised learning in tabular data.

Method: Theoretical analysis of binary Gaussian mixture models (GMM) with missing labels, comparing single-layer and multilayer transformers.

Result: Multilayer transformers implicitly construct polynomial estimators, outperforming single-layer models, with exponential gains in depth.
...
Method: A systematic literature review following PRISMA guidelines, aggregating contributions into 41 metric groups and proposing a three-dimensional categorization scheme.
...
Conclusion: The framework supports future XAI research by standardizing evaluation and fostering trust in AI explanations.
...
<details>
  <summary>Details</summary>
Motivation: Reward models are crucial for aligning agent behavior with task objectives, but a systematic review of these techniques is lacking in current literature.

Method: The survey categorizes reward modeling approaches by source, mechanism, and learning paradigm, and reviews their applications and evaluation methods.

Result: The paper provides a comprehensive overview of both established and emerging reward modeling techniques.

Conclusion: The survey fills a gap in the literature and highlights promising research directions for reward modeling in RL.
...
<details>
  <summary>Details</summary>
Motivation: To address the degradation of zero-shot RL methods in partially observable settings, which are common in real-world applications.

Method: Use of memory-based architectures to handle partial observability in zero-shot RL, evaluated in environments with partially observed states, rewards, and dynamics.

Result: Memory-based methods outperform memory-free baselines in partially observable scenarios.

Conclusion: Memory-based architectures are effective for zero-shot RL in partially observable environments, with open-sourced code for reproducibility.
...
<details>
  <summary>Details</summary>
Motivation: Training high-precision neural networks is difficult due to non-convex optimization and overlooked $L^{\infty}$ error. Traditional methods focus on MSE, missing critical accuracy needs.

Method: HiPreNets refines staged training by sequentially learning residuals with additional networks, optimizing loss functions, parameters, and adaptive data sampling.

Result: Validated on benchmark problems, the framework enhances overall accuracy.

Conclusion: HiPreNets offers a structured approach to improve neural network precision, addressing key challenges in training.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods lack qualitative comparison and broader structural insights in time series alignment.

Method: Proposes a technique to simplify warping paths, highlighting shifts, compressions, and amplitude differences.

Result: Enhances interpretability by clearly representing subsequence matches between time series.

Conclusion: The method provides a clearer, more interpretable comparison of time series transformations.
...
### [62] [Pixel-level Certified Explanations via Randomized Smoothing](https://arxiv.org/abs/2506.15499)
*Alaa Anani,Tobias Lorenz,Mario Fritz,Bernt Schiele*

Main category: cs.LG

TL;DR: The paper introduces a certification framework to ensure pixel-level robustness for post-hoc attribution methods in deep learning, addressing their vulnerability to input perturbations.


<details>
  <summary>Details</summary>
Motivation: Post-hoc attribution methods are non-robust to small input perturbations, undermining their trustworthiness, which necessitates rigorous robustness guarantees.
...
Result: The proposed method provides robust, interpretable, and faithful attributions, as demonstrated by evaluating 12 attribution methods across 5 ImageNet models.
...
TL;DR: The paper introduces a Neural Canonical Polyadic Factorization (NCPF) model for robust traffic data imputation, combining low-rank tensor algebra and deep learning to address missing data issues in traffic analysis.


<details>
  <summary>Details</summary>
Motivation: Accurate spatiotemporal traffic analysis is crucial for urban mobility optimization, but missing data from sensor failures and heterogeneous sensing gaps disrupts reliable modeling.

Method: NCPF integrates CP decomposition into neural architecture via learnable embeddings, using hierarchical feature fusion and multilayer perceptrons to model spatiotemporal interactions.
...
### [64] [Over-squashing in Spatiotemporal Graph Neural Networks](https://arxiv.org/abs/2506.15507)
*Ivan Marisca,Jacob Bamberger,Cesare Alippi,Michael M. Bronstein*

Main category: cs.LG

TL;DR: The paper formalizes spatiotemporal over-squashing in STGNNs, showing its unique challenges compared to static GNNs, and reveals counterintuitive propagation biases favoring temporally distant nodes.


<details>
  <summary>Details</summary>
Motivation: To address the unexplored issue of over-squashing in STGNNs, where temporal dynamics exacerbate information propagation limitations.

Method: Theoretical analysis and empirical validation on synthetic and real-world datasets to study spatiotemporal over-squashing.

Result: Convolutional STGNNs favor information from temporally distant nodes, and time-and-space or time-then-space paradigms are equally affected.

Conclusion: Provides insights into STGNN dynamics and guidance for more effective designs, validated by empirical results.
...
### [65] [Towards Reliable Forgetting: A Survey on Machine Unlearning Verification, Challenges, and Future Directions](https://arxiv.org/abs/2506.15115)
...
<details>
  <summary>Details</summary>
Motivation: Address the lack of unified taxonomy and systematic framework for verifying machine unlearning operations.
...
### [66] [RePCS: Diagnosing Data Memorization in LLM-Powered Retrieval-Augmented Generation](https://arxiv.org/abs/2506.15513)
*Le Vu Anh,Nguyen Viet Anh,Mehmet Dik,Luong Van Nghia*

Main category: cs.LG

TL;DR: RePCS is a diagnostic method to detect if retrieval-augmented LLMs rely on memorized data instead of retrieved evidence, using KL divergence between two inference paths.


<details>
  <summary>Details</summary>
Motivation: To address the issue of LLMs bypassing retrieved evidence and producing contaminated outputs in retrieval-augmented generation (RAG).
...
Conclusion: RePCS provides a lightweight, black-box safeguard to verify meaningful retrieval use in RAG systems, crucial for safety-critical applications.
...
### [67] [ImprovDML: Improved Trade-off in Private Byzantine-Resilient Distributed Machine Learning](https://arxiv.org/abs/2506.15181)
...
Result: Achieves tighter error bounds and better privacy-accuracy trade-off than existing methods, validated by simulations.

Conclusion: ImprovDML effectively integrates Byzantine resilience and privacy without compromising model accuracy.
...
TL;DR: PRISM is a new framework for identifying and scoring polysemantic features in neural networks, improving feature description accuracy and capturing multiple concepts per neuron.
...
Result: PRISM outperforms existing methods in description quality and polysemanticity capture, as shown in language model evaluations.

Conclusion: PRISM advances automated interpretability by better capturing the complexity of neural network features.
...
TL;DR: SKIL introduces a reinforcement learning-based imitation framework to infer interpretable skill sets and parameterize policies as dynamic mixtures, offering a principled account of complex animal behaviors.
...
Result: The method identifies reusable skill components, learns compositional policies, and generates realistic trajectories, outperforming traditional discrete models.
...
### [70] [Learning Algorithms in the Limit](https://arxiv.org/abs/2506.15543)
*Hristo Papazov,Nicolas Flammarion*

Main category: cs.LG

TL;DR: The paper extends Gold's inductive inference framework to include computational observations and restricted inputs, enabling learnability of general recursive functions under realistic constraints. It introduces new observation types and shows their effectiveness, while also revealing limitations for certain function classes.
...
Method: Extends Gold's framework with Time-Bound and Policy-Trajectory Observations, and analyzes learnability under computational complexity constraints.

Result: Input-output observations alone are insufficient, but adding computational constraints or time-bound observations enables learning. Policy-trajectory observations link to rational function learning. Negative results show limitations for linear-time computable functions.

Conclusion: The study advances inductive inference by incorporating computational realism, revealing both learnability and inherent limitations for certain function classes.
...
### [71] [Interpretability and Generalization Bounds for Learning Spatial Physics](https://arxiv.org/abs/2506.15199)
...
<details>
  <summary>Details</summary>
Motivation: To rigorously assess ML's accuracy in scientific applications, focusing on the 1D Poisson equation, and understand generalization challenges.

Method: Analyzes training dynamics, derives optimal parameters, and proves generalization bounds for white-box and black-box models. Empirical tests include deep linear models, shallow networks, and physics-specific models.
...
Conclusion: Proposes a new cross-validation technique for evaluating generalization in physical systems, suggesting the Poisson equation as a benchmark.
...
### [72] [DAILOC: Domain-Incremental Learning for Indoor Localization using Smartphones](https://arxiv.org/abs/2506.15554)
...
Conclusion: DAILOC effectively addresses domain shifts and catastrophic forgetting, improving indoor localization performance.
...
TL;DR: LogNet, a logic gate-based framework, enhances interpretability and performance of DL-based indoor localization by identifying influential APs and diagnosing noise impact.
...
Method: Introduces LogNet, a logic gate-based framework to interpret DL models by identifying key APs and analyzing noise effects.

Result: LogNet improves performance: 1.1x-2.8x lower error, 3.4x-43.3x smaller model size, and 1.5x-3.6x lower latency vs. prior models.

Conclusion: LogNet offers transparent reasoning, better adaptability, and superior performance for long-term indoor localization.
...
### [74] [Centroid Approximation for Byzantine-Tolerant Federated Learning](https://arxiv.org/abs/2506.15264)
*Mélanie Cambus,Darya Melnyk,Tijana Milentijević,Stefan Schmid*

Main category: cs.LG

TL;DR: The paper explores the robustness of federated learning to Byzantine behavior, revealing a tradeoff between averaging and validity conditions. It provides theoretical bounds on centroid approximation and introduces a new algorithm for convex validity.


<details>
  <summary>Details</summary>
Motivation: To investigate whether federated learning can withstand Byzantine faults and understand the tradeoffs between averaging and validity conditions in distributed settings.

Method: Theoretical analysis of centroid approximation under Byzantine faults, including lower and upper bounds, and a new algorithm for convex validity. Empirical evaluations in federated learning settings.

Result: Lower bound of min{(n-t)/t,√d} on centroid approximation under box validity, upper bound of 2min{n,√d}, and a √2d-approximation algorithm under convex validity.

Conclusion: The study highlights the challenges of Byzantine robustness in federated learning, provides tight bounds, and demonstrates practical implications through empirical results.
...
### [75] [LoX: Low-Rank Extrapolation Robustifies LLM Safety Against Fine-tuning](https://arxiv.org/abs/2506.15606)
*Gabrel J. Perin,Runjin Chen,Xuxi Chen,Nina S. T. Hirata,Zhangyang Wang,Junyuan Hong*

Main category: cs.LG

TL;DR: The paper introduces Low-Rank Extrapolation (LoX), a training-free method to enhance the safety robustness of aligned LLMs against fine-tuning attacks, reducing attack success rates by 11% to 54%.
...
Method: Proposes LoX, which extrapolates the safety subspace of an aligned LLM to improve robustness without additional training.

Result: LoX significantly reduces attack success rates (11% to 54%) against benign and malicious fine-tuning attacks while maintaining task adaptability.

Conclusion: LoX effectively enhances safety robustness by moving LLM parameters to a flatter, less perturbation-sensitive zone.
...
TL;DR: The paper introduces GFLC, a method to correct biased labels in ML datasets while maintaining fairness, improving both performance and fairness metrics.


<details>
  <summary>Details</summary>
Motivation: Fairness in ML is crucial due to AI's societal impact, but biased training data undermines fairness and performance.
...
<details>
  <summary>Details</summary>
Motivation: The need for scalable infrastructure planning tools to address the diverse requirements of metropolitan, suburban, and rural areas for EV charging networks.

Method: Uses a hierarchical H3 grid, GNN centrality model, Voronoi tessellation, and M/M/c queue for site selection under constraints.

Result: In Georgia, it improved coverage, reduced travel distance for low-income residents, and ensured power capacity, remaining scalable for national use.

Conclusion: The framework successfully bridges academic optimization with practical infrastructure policy, demonstrating scalability and equity.
...
### [78] [Federated Learning for MRI-based BrainAGE: a multicenter study on post-stroke functional outcome prediction](https://arxiv.org/abs/2506.15626)
...
<details>
  <summary>Details</summary>
Motivation: To address privacy concerns in training robust BrainAGE models while evaluating FL's performance in ischemic stroke patients.

Method: Used FLAIR images from 1674 patients across 16 centers, comparing centralized learning, FL, and single-site learning. Examined associations with clinical factors and outcomes.

Result: FL outperformed single-site models; BrainAGE linked to diabetes and functional recovery. Centralized learning was most accurate.

Conclusion: FL enables accurate BrainAGE predictions without data sharing, highlighting its prognostic value in stroke care.
...
TL;DR: The paper proposes a credit risk management framework for 3PL-led SCF in CBEC, using QRGMM and DeepFM for risk assessment and loan size determination, validated by experiments.
...
Method: Combines QRGMM for sales distribution modeling and DeepFM for covariate interactions, creating a unified framework for risk estimation and loan sizing.
...
Conclusion: Pioneers generative AI in CBEC SCF, enhancing credit practices and supporting SME financing.
...
Method: AutoRule uses a reasoning model to interpret preferences, extracts candidate rules, synthesizes them, and verifies rule satisfaction for auxiliary rewards.

Result: AutoRule improves win rates by 28.6% on AlpacaEval2.0 and 6.1% on MT-Bench, with reduced reward hacking and alignment with dataset preferences.
...
### [81] [SecFwT: Efficient Privacy-Preserving Fine-Tuning of Large Language Models Using Forward-Only Passes](https://arxiv.org/abs/2506.15307)
*Jinglong Luo,Zhuo Zhang,Yehong Zhang,Shiyu Liu,Ye Dong,Xun Zhou,Hui Wang,Yue Yu,Zenglin Xu*

Main category: cs.LG

TL;DR: SecFwT is a framework for efficient, privacy-preserving fine-tuning of LLMs using MPC, eliminating backward/optimizer steps and replacing softmax attention with MPC-friendly Random Feature Attention.


<details>
  <summary>Details</summary>
Motivation: Adapting LLMs to privacy-sensitive domains like healthcare and finance is challenging due to data scarcity and privacy constraints. MPC-based methods are limited to inference, not fine-tuning, due to computational overhead.

Method: Proposes SecFwT, which uses forward-only tuning to avoid backward/optimizer steps and replaces softmax attention with Random Feature Attention for MPC efficiency.

Result: SecFwT significantly improves efficiency and privacy, enabling scalable and secure LLM fine-tuning in privacy-critical applications.

Conclusion: SecFwT addresses key computational and efficiency challenges in MPC-based LLM fine-tuning, making it viable for privacy-sensitive domains.
...
### [82] [Dense SAE Latents Are Features, Not Bugs](https://arxiv.org/abs/2506.15679)
...
Method: Systematically study dense latents' geometry, function, and origin, including ablation experiments and a taxonomy of their types.
...
Conclusion: Dense latents are intrinsic to language models and serve meaningful computational roles, not just noise.
...
TL;DR: The paper proposes a method to predict abnormal values of unprescribed lab tests using performed test results, focusing on CBC and routine biochemical panels, achieving up to 8% AUC improvement.


<details>
  <summary>Details</summary>
Motivation: Early diagnosis via predicting abnormal lab values from existing tests can enhance healthcare accessibility.
...
Conclusion: The approach effectively bridges LLMs with tabular clinical data, improving diagnostic predictions.
...
<details>
  <summary>Details</summary>
Motivation: Traditional KD methods for NNPs increase energy barriers, hindering the generation of high-energy training data needed for stable MD simulations.

Method: A two-stage training process: first, train the student NNP using data from a non-fine-tuned teacher; then fine-tune with a smaller DFT dataset.
...
Conclusion: The proposed framework offers a more efficient and accurate approach to NNP training for MD simulations.
...
TL;DR: The paper proposes a conditional Image-to-Image Schrödinger Bridge (cI²SB) framework to improve deep learning-based Full Waveform Inversion (FWI) by addressing limitations of diffusion models, such as iterative sampling and heuristic output control.
...
Method: The authors extend the Image-to-Image Schrödinger Bridge (I²SB) to conditional sampling (cI²SB), enabling nonlinear drift learning between ground truth and smoothed velocity models.

Result: Experiments show cI²SB outperforms conditional diffusion models, achieving higher sample fidelity with fewer neural function evaluations (NFEs) than supervised learning.

Conclusion: The cI²SB framework effectively addresses diffusion model limitations in FWI, offering a more efficient and accurate solution for velocity model reconstruction.
...
Method: The approach incorporates quantile regression for stronger MIA to enhance one-run auditing in black-box settings, tested on CIFAR-10 image classification models trained with DP-SGD.
...
Conclusion: Utilizing quantile regression for MIA improves one-run DP auditing in black-box settings, bridging the gap between empirical and theoretical bounds efficiently.
...
TL;DR: DiTMC adapts Diffusion Transformers (DiTs) for molecular conformer generation, addressing challenges like graph-geometry integration and symmetry handling, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Applying DiTs to molecular conformer generation introduces unique challenges, such as combining discrete graph data with continuous 3D geometry and handling symmetries.

Method: DiTMC uses a modular architecture with graph-based conditioning strategies and flexible attention mechanisms (non-equivariant and SO(3)-equivariant).

Result: DiTMC achieves state-of-the-art precision and physical validity on benchmarks (GEOM-QM9, -DRUGS, -XL).

Conclusion: The framework highlights the impact of architectural choices and symmetry priors, offering promising directions for large-scale molecular structure modeling.
...
TL;DR: A novel method for learning global ground metrics in optimal transport improves performance in tasks like embedding, clustering, and classification by leveraging class labels at the distribution level.


<details>
  <summary>Details</summary>
Motivation: Traditional predefined or supervised ground metrics in optimal transport lack flexibility and generalization across classes or distributions with shared supports.

Method: Proposes a method to learn a global ground metric for arbitrary distributions over a shared metric space, using only class labels at the distribution level.

Result: The learned metric enhances optimal transport distances, improving performance in embedding, clustering, and classification, demonstrated on scRNA-seq data.

Conclusion: The approach addresses limitations of traditional methods, offering a more flexible and generalizable solution for optimal transport applications.
...
TL;DR: A novel framework for exploration using diffusion models, leveraging entropy maximization and density estimation, with scalable algorithms and empirical validation.


<details>
  <summary>Details</summary>
Motivation: To enable exploration in decision-making problems like scientific discovery without relying on explicit uncertainty quantification.

Method: Introduces a framework for exploration as entropy maximization over a diffusion model's data manifold, using density estimation and mirror descent for scalable fine-tuning.

Result: Convergence to optimal exploratory diffusion model proven; promising empirical results on synthetic and high-dimensional tasks.

Conclusion: The method effectively leverages diffusion models for exploration, offering a scalable and theoretically grounded approach.
...
TL;DR: The paper addresses minimizing disease extinction time in the SIS model with an unknown graph, proposing a learning algorithm and vaccination strategies.


<details>
  <summary>Details</summary>
Motivation: Prior works assumed known graphs, but real-world scenarios often lack this information, necessitating methods for unknown graphs.

Method: A novel inclusion-exclusion-based learning algorithm for graph recovery and optimal/polynomial-time greedy vaccination strategies.

Result: Sample complexity for graph recovery is established, and polynomial-time algorithms for bounded treewidth graphs are proven.

Conclusion: The proposed methods are validated through experiments, showing effectiveness in learning and vaccination for unknown graphs.
...
<details>
  <summary>Details</summary>
Motivation: Current GAD methods assume uniform homophily in normal nodes, which often fails in real-world datasets due to diverse homophily patterns.
...
Result: Experiments on eight datasets show RHO effectively captures varying homophily and outperforms state-of-the-art methods.

Conclusion: RHO robustly learns diverse homophily patterns, improving anomaly detection in graphs with limited labeled normal nodes.
...
TL;DR: A user-guided projection framework for image/text data uses MLLMs for customizable, interpretable visualizations via natural-language prompts, enhancing cluster separation and interactivity.


<details>
  <summary>Details</summary>
Motivation: Current DR techniques lack the ability to explore semantic structures not explicitly in the data. This work aims to bridge the gap between automated DR and human-centered exploration.

Method: The framework employs zero-shot classification with MLLMs, allowing users to dynamically steer projections via natural-language prompts to highlight semantic relationships.

Result: Evaluations show improved cluster separation and demonstrate the framework's ability to transform DR into an interactive, user-driven process.

Conclusion: The approach offers a flexible, adaptive way to tailor DR projections to specific analytical needs, combining automation with human guidance.
...
TL;DR: LIT-LVM improves prediction accuracy for interaction terms in linear predictors by using low-dimensional latent vectors, outperforming elastic net and factorization machines, especially in high-dimensional settings.


<details>
  <summary>Details</summary>
Motivation: Linear predictors with interaction terms can model non-linear relationships, but accurately estimating their coefficients is challenging, especially in high-dimensional scenarios.

Method: LIT-LVM represents features with low-dimensional latent vectors, acting as structured regularization to mitigate overfitting.

Result: LIT-LVM achieves superior prediction accuracy compared to elastic net and factorization machines, particularly with many interaction terms. It also provides interpretable low-dimensional feature representations.

Conclusion: LIT-LVM is an effective method for estimating interaction term coefficients, offering better performance and interpretability in high-dimensional settings.
...
<details>
  <summary>Details</summary>
Motivation: To address the scattered research on adversarial attacks in tabular data and provide a structured overview for future work.

Method: Conducted a systematic literature review to categorize attack strategies and analyze real-world applicability.

Result: Identified key trends, attack categories, and practical challenges in adversarial attacks on tabular ML.

Conclusion: The review aims to guide future research in understanding and mitigating adversarial vulnerabilities in tabular ML.
...
TL;DR: Simplified analysis of SGD in linear regression using basic linear algebra, matching prior bounds without complex PSD matrix manipulations.


<details>
  <summary>Details</summary>
Motivation: To make SGD analysis in linear regression more accessible and facilitate further research on mini-batching and learning rate scheduling.

Method: Uses simple linear algebra tools to derive bias and variance bounds, avoiding complex operator manipulations.

Result: Recovers the same bounds as prior work but with a simpler approach.

Conclusion: The simplified analysis enhances accessibility and may aid in improving practical model training.
...
TL;DR: The paper identifies gradient pathologies and non-stationarity as root causes of scaling challenges in deep reinforcement learning, proposing simple interventions to stabilize gradient flow for robust performance.


<details>
  <summary>Details</summary>
Motivation: Understanding why scaling deep reinforcement learning networks often leads to degraded performance, despite recent complex solutions, and addressing the root causes.

Method: Conducts empirical analyses to identify gradient pathologies and non-stationarity as key issues, then proposes simple interventions to stabilize gradient flow.

Result: The proposed interventions enable robust performance across varying network depths and widths, validated on multiple agents and environments.
...
TL;DR: The paper proposes a compositional approach for neural networks by ensembling expert models, achieving higher accuracy and efficiency.
...
Result: Higher accuracy than baselines with reduced computational resources.
...
Method: MicroRicci employs a greedy syndrome-decoding step to correct curvature errors in O(E) time, augmented by two neural modules for adaptive vertex and step-size selection.

Result: On 110 SJTU-TMQA meshes, MicroRicci reduces iterations by 2.4x, tightens curvature spread, and achieves a high UV-distortion-to-MOS correlation (r = -0.93), with only 0.25 ms added per iteration.

Conclusion: MicroRicci's efficiency, adaptability, and quality make it ideal for real-time applications in graphics and simulation.
...
TL;DR: DP-GRAPE reduces memory overhead in DP neural network training by privatizing gradients after projection and using random Gaussian matrices, achieving comparable utility to DP-SGD.


<details>
  <summary>Details</summary>
Motivation: Standard DP methods like DP-Adam have high memory overhead due to per-sample gradient clipping, limiting scalability for large models.
...
Result: DP-GRAPE reduces memory usage by over 63% for Vision Transformers and 70% for RoBERTa-Large, while maintaining performance and scaling to models like OPT with 6.7B parameters.

Conclusion: DP-GRAPE offers a memory-efficient alternative to standard DP methods, enabling scalable training of large models without sacrificing utility.
...
<details>
  <summary>Details</summary>
Motivation: To mitigate over-conservative policies in offline RL caused by poor explorations in suboptimal data.
